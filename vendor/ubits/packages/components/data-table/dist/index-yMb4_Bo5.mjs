...
function Y(i) {
	const l = String(i.getDate()).padStart(2, '0'),
...
function I(i, l) {
	const o = new Date(i.getFullYear(), i.getMonth(), i.getDate()),
		p = new Date(l.getFullYear(), l.getMonth(), l.getDate());
  ...
  `),
	...
	const M = async () => {
			console.log('📜 [SCROLLBAR] ========== INICIO initScrollbar =========='),
				...
				console.log('📜 [SCROLLBAR] scrollbarContainerId:', S);
	...
	if (o.parentElement)
		...
		n.observe(document.body, { childList: !0, subtree: !0 }),
			setTimeout(() => {
				o.isConnected &&
					(console.log('📜 [SCROLLBAR] Timeout alcanzado, inicializando'), n.disconnect(), L());
			}, 1e3);
	}
	return (
		setTimeout(() => {
...
function $(i) {
	const {
		...
		f = a.getDay(),
		T = /* @__PURE__ */ new Date().toDateString(),
		k = ['ubits-calendar', l === 'range' ? 'ubits-calendar--range' : 'ubits-calendar--single', M]
			.filter(Boolean)
			.join(' '),
		u = L ? ` style="${L}"` : '',
		c = `
  ...
  `;
	...
	for (let y = 1; y <= h; y++) {
		const v = new Date(e, t, y),
			r = v.toDateString() === T;
		let g = ['ubits-calendar__day'];
		l === 'single' && o && A(v, o) && g.push('ubits-calendar__day--selected'),
			l === 'range' &&
				o &&
				(p
					? A(v, o)
						...
						: A(v, p)
							...
							: j(v, o, p) && g.push('ubits-calendar__day--in-range')
					: A(v, o) && g.push('ubits-calendar__day--range-start')),
			r && g.push('ubits-calendar__day--today');
		let _ = !1;
		d && I(v, d) < 0 && ((_ = !0), g.push('ubits-calendar__day--disabled')),
			S && I(v, S) > 0 && ((_ = !0), g.push('ubits-calendar__day--disabled'));
		const R = _ ? ' disabled' : '',
			D = Y(v);
...
function F(i) {
	...
	const f = () => {
		...
		O = () => {
			const u = e.querySelector('.ubits-calendar__nav-button--prev'),
				H = e.querySelector('.ubits-calendar__nav-button--next'),
				c = e.querySelector('.ubits-calendar__month-input'),
				w = e.querySelector('.ubits-calendar__year-input'),
			...
			u?.addEventListener('click', (b) => {
				b.preventDefault(),
					...
					c && (c.value = E[t.getMonth()]),
					w && (w.value = String(t.getFullYear())),
					f();
			}),
					...
					.forEach((b) => {
						b.addEventListener('click', (r) => {
							...
							const g = b.dataset.date || '',
								[_, R, D] = g.split('/'),
								m = new Date(parseInt(D), parseInt(R) - 1, parseInt(_));
							l === 'single'
								? ((a = m), f(), M && M(m))
								: l === 'range' &&
									(!a || (a && s)
										...
										: a &&
											!s &&
											(I(m, a) < 0 ? ((s = a), (a = m)) : (s = m), f(), L && a && s && L(a, s)));
						});
					});
		};
	return (
		f(),
		{
			element: e,
			update: (u) => {
				u.selectedDate !== void 0 && (a = u.selectedDate ? new Date(u.selectedDate) : null),
					u.endDate !== void 0 && (s = u.endDate ? new Date(u.endDate) : null),
					u.initialDate && (t = new Date(u.initialDate)),
					Object.assign(i, u),
					f();
			},
			destroy: () => {
				const u = e.querySelector('.ubits-calendar__month-dropdown'),
					H = e.querySelector('.ubits-calendar__year-dropdown');
				if (u) {
