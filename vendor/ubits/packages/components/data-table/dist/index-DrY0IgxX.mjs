...
function L(i, s) {
	const n = new Date(i.getFullYear(), i.getMonth(), i.getDate()),
...
function j(i, s) {
  ...
  `;
	i.forEach((r) => {
		const e = r.selected ? 'active' : 'default',
  ...
  `),
	...
	const I = async () => {
			...
			if (!(!r || !e) && !(r.scrollHeight <= r.clientHeight))
				try {
					...
					const { createScrollbar: a } = await import('./index-Ct0Xfho3.mjs').then((g) => g.S),
						l = a({
	...
	return (
		setTimeout(() => {
			const r = document.getElementById(u);
			r &&
				r.querySelectorAll('.ubits-list-item').forEach((e) => {
...
function A(i) {
  ...
  `;
	...
	for (let y = 1; y <= g; y++) {
		const h = new Date(e, t, y),
			o = h.toDateString() === q;
		let f = ['ubits-calendar__day'];
		s === 'single' && n && C(h, n) && f.push('ubits-calendar__day--selected'),
			s === 'range' &&
				n &&
				(p
					? C(h, n)
						? f.push('ubits-calendar__day--range-start')
						: C(h, p)
							? f.push('ubits-calendar__day--range-end')
							: R(h, n, p) && f.push('ubits-calendar__day--in-range')
					: C(h, n) && f.push('ubits-calendar__day--range-start')),
			o && f.push('ubits-calendar__day--today');
		let _ = !1;
		u && L(h, u) < 0 && ((_ = !0), f.push('ubits-calendar__day--disabled')),
			M && L(h, M) > 0 && ((_ = !0), f.push('ubits-calendar__day--disabled'));
		const E = _ ? ' disabled' : '',
			D = N(h);
				...
				e
					...
					.forEach((b) => {
						b.addEventListener('click', (o) => {
							...
							const f = b.dataset.date || '',
								[_, E, D] = f.split('/'),
								m = new Date(parseInt(D), parseInt(E) - 1, parseInt(_));
							s === 'single'
								? ((a = m), v(), I && I(m))
								: s === 'range' &&
									(!a || (a && l)
										...
										: a &&
											!l &&
											(L(m, a) < 0 ? ((l = a), (a = m)) : (l = m), v(), w && a && l && w(a, l)));
						});
					});
		};
	return (
