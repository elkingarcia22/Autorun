	...
	switch (columnType) {
		...
		case 'progreso': {
			...
			if (progressValue === null && cellData) {
				...
				const progressProp =
					cellData.progress !== undefined ? cellData.progress : cellData.progreso;
      ...
      `;
		...
		case 'nombre-avatar-texto': {
			...
			const textoComplementario =
				cellData.area ||
				cellData.areaNombre ||
				cellData.textoComplementario ||
				cellData.complementario ||
				'';
      ...
      `;
		...
		case 'toggle': {
			...
			const labelText =
				typeof column.toggleLabel === 'string'
					? column.toggleLabel
					: showLabel
						? String(row.data[column.id] || row.id)
						: '';
...
function renderCell(column: TableColumn, row: TableRow, pinnedLeft: number = 0): string {
	...
	const isFixedCheckboxColumn =
		column.type !== 'checkbox' && (column.id === 'checkbox' || column.id.startsWith('checkbox-'));
	...
	if (isFixedCheckboxColumn) {
		...
		const pinnedStyle = column.pinned
			? `position: sticky !important; left: ${pinnedLeft}px !important; z-index: 12 !important;`
			: '';
	...
	// CRÍTICO: left: 0px es válido y necesario para la primera columna fijada
	const pinnedStyle = column.pinned
		? ` style="position: sticky !important; left: ${pinnedLeft}px !important; z-index: 12 !important;"`
		: '';
...
function renderColumnHeader(
	...
	if (column.type === 'drag-handle' || column.type === 'expand') {
		...
		const pinnedStyle = column.pinned
			? `position: sticky !important; left: ${pinnedLeft}px !important; z-index: 10 !important;`
			: '';
	...
	const dragHandle =
		columnReorderable && !isFixedCheckboxColumn && !isControlColumn
			? `
  ...
  `
	...
	const sortButton =
		!isFixedCheckboxColumn && !isControlColumn && columnSortable
			? (() => {
	...
	const menuButton =
		!isFixedCheckboxColumn && !isControlColumn && showColumnMenu
			? (() => {
  ...
  `;
	...
	const combinedStyle = [positionStyle, pinnedStyle, zIndexStyle, widthStyle]
		.filter(Boolean)
		.join(' ');
...
function renderRow(
	...
	const rowClasses = ['ubits-data-table__row', isExpanded ? 'ubits-data-table__row--expanded' : '']
		.filter(Boolean)
		.join(' ');
...
function renderDataTableHeader(
  ...
  `
	...
	const columnSelectorButtonHTML =
		showColumnSelectorButton && columnSelectorButton
			? renderButton({
					variant: 'secondary',
					size: 'sm',
					icon: 'columns-3',
...
export function renderDataTable(
	...
	if (columnOrder.length > 0) {
		...
		visibleColumns = filteredColumnOrder
			...
			.concat(
				visibleColumns
					.filter((col) => !filteredColumnOrder.includes(col.id))
					.map((col) => {
						const copy = { ...col };
						// IMPORTANTE: Preservar explícitamente pinned
						if (col.pinned !== undefined) {
	...
	visibleColumns = visibleColumns.map((col) => {
		...
		if (
			colCopy.pinned &&
			!col.id.startsWith('checkbox') &&
			col.type !== 'drag-handle' &&
			col.type !== 'expand'
		) {
			// Log removido
		}
...
export function createDataTable(options: DataTableOptions): {
	...
	const initialLazyLoadItems =
		options.lazyLoad !== false && !options.showPagination
		...
		const checkScroll = () => {
			...
			if (scrollableContainer) {
				...
				if (viewportBottom >= elementBottom - 200) {
					// 200px antes del final del elemento
					// Cargar más items
	...
	const initializeIconFallbacks = () => {
		...
		waIcons.forEach((waIcon, index) => {
			...
			const isDragHandle =
				parent && parent.classList.contains('ubits-data-table__column-drag-handle');
	...
	const render = (preserveScroll: boolean = false) => {
				...
				setTimeout(() => {
					...
					if (activeSearchBtn && prevButton) {
						...
						const inputWrapperStyle = inputWrapper
							? window.getComputedStyle(inputWrapper as HTMLElement)
							: null;
		...
		const shouldRestoreScroll =
			...
			(savedScrollHeight > 0 && savedClientHeight > 0 && savedScrollHeight > savedClientHeight);
		...
		const scrollableContainerForDebug = element.querySelector(
			'.ubits-data-table__scrollable-container',
		) as HTMLElement;
		...
		rows.forEach((row, index) => {
			if (index === 0) {
				// Solo log de la primera fila para no saturar
		...
		checkboxHeaders.forEach((input) => {
			...
			if (columnId) {
				...
				const allChecked =
					currentOptions.rows.length > 0 &&
					currentOptions.rows.every((row) => row.data[columnId] === true);
		...
		const checkPaginationSpacing = () => {
			try {
				...
				const container =
					element.closest('.ubits-data-table__container') ||
					(element.querySelector('.ubits-data-table__container') as HTMLElement);
				...
				if (container) {
					...
					const tableContainer =
						(container.querySelector('.ubits-data-table__scrollable-container') as HTMLElement) ||
						(container.querySelector('.ubits-data-table') as HTMLElement);

					// Buscar la tabla real dentro del contenedor
					const actualTable =
						(tableContainer?.querySelector('.ubits-data-table__table') as HTMLElement) ||
						tableContainer;
	...
	const attachEventListeners = () => {
		...
		try {
							...
							if (header && draggedColumnId) {
								...
								if (columnId && columnId !== draggedColumnId) {
									...
									const isTargetCheckbox =
										columnId === 'checkbox' || columnId.startsWith('checkbox-');
									const isDraggedCheckbox =
										draggedColumnId === 'checkbox' || draggedColumnId.startsWith('checkbox-');
						...
						(e) => {
							...
							if (row) {
								...
								if (droppedRowId && String(targetRowId) !== droppedRowId) {
									const droppedId = isNaN(Number(droppedRowId))
										? droppedRowId
										: Number(droppedRowId);
			...
			columnCheckboxHeaders.forEach((checkbox, index) => {
				...
				const selectAllHandler = (e: Event) => {
					...
					if (currentColumnId === 'checkbox-2') {
						...
						const allChecked =
							currentOptions.rows.length > 0 &&
							currentOptions.rows.every((r) => r.data[currentColumnId] === true);
						...
						if (optionsWithSelectAll.onSelectAll) {
							...
							const scrollHeightBeforeCallback =
								scrollableContainerBeforeCallback?.scrollHeight || 0;
							const clientHeightBeforeCallback =
								scrollableContainerBeforeCallback?.clientHeight || 0;
							...
							const dimensionsChanged =
								Math.abs(scrollHeightAfterCallback - scrollHeightBeforeCallback) > 1 ||
							...
							if (scrollChanged || dimensionsChanged) {
								...
								if (
									scrollChanged &&
									savedScrollBeforeSelectAll > 0 &&
									scrollableContainerAfterCallback
								) {
						...
						if (currentColumnId === 'checkbox-2') {
							...
							if (checkboxContainer) {
										...
										requestAnimationFrame(() => {
											...
											if (verifyCheckmark) {
												...
												if (
													...
													computedStyles.transform.includes('scale(0)')
												) {
								...
								if (allCheckboxes.length > 0) {
									const correctInput =
										(Array.from(allCheckboxes).find((cb) => cb === input) as HTMLInputElement) ||
										(allCheckboxes[0] as HTMLInputElement);
							...
							if (headerCheckbox) {
								const allChecked =
									currentOptions.rows.length > 0 &&
									currentOptions.rows.every((r) => r.data[columnId] === true);
						...
						if (row.expanded) {
							requestAnimationFrame(() => {
								...
								if (rowElement) {
									...
									if (
										expandedRow &&
										expandedRow.classList.contains('ubits-data-table__row-expanded-row')
									) {
										// Buscar el contenedor scrollable
										const scrollableContainer = element.querySelector(
			...
			menuButtons.forEach((button) => {
				...
				const isPinned =
					headerCell.hasAttribute('data-pinned') &&
					headerCell.getAttribute('data-pinned') === 'true';
				...
				// Detectar si estamos en la web (no en Storybook)
				const isWeb =
					typeof window !== 'undefined' && !window.location?.href?.includes('storybook');
				...
				if (isPinned || hasStickyClass) {
					...
					const rootContainer =
						tableElement?.closest('.ubits-data-table__scrollable-container') || element;
          ...
          `;
				...
				const closeDropdown = () => {
					...
					if (
						(isPinned || hasStickyClass) &&
						dropdown.parentElement &&
						dropdown.parentElement !== headerCell
					) {
						dropdown.remove();
					}
				};
			...
			if (showContextMenuValue) {
              ...
              `;
				...
				let contextMenuContainer: HTMLElement | null = document.getElementById(
        ...
        `;
          ...
          `;
			...
			dateEditables.forEach((dateEditableContainer, index) => {
				...
				let calendarInstance: ReturnType<
					typeof import('../../calendar/src/CalendarProvider').createCalendar
				> | null = null;
						...
						if (calendarElement) {
							...
							if (monthDropdown || yearDropdown) {
								...
								if (activeElement) {
									...
									if (
										externalCalendarContainer.contains(activeElement) ||
								...
								if (e && e.target) {
									const target = e.target as HTMLElement;
									if (
										externalCalendarContainer.contains(target) ||
          ...
          `;
			...
			if (currentOptions.header) {
				...
				if (headerElement) {
					// Botón primario
					if (
						currentOptions.header.primaryButton &&
						currentOptions.header.showPrimaryButton !== false
					) {
					...
					// Botones secundarios
					if (
						currentOptions.header.secondaryButtons &&
						currentOptions.header.showSecondaryButtons !== false
					) {
					...
					if (
						...
						currentOptions.header.showSearchButton !== false
					) {
						...
						if (searchBtn) {
							...
							if ((isButton || hasButtonInside) && !isSearchActive) {
								const buttonToUse = isButton
									? (searchBtn as HTMLButtonElement)
									: searchButtonElement;
							...
							if (searchInput) {
								...
								searchInput.addEventListener('blur', (e) => {
									...
									blurTimeout = setTimeout(() => {
										...
										const searchButtonWrapper = element.querySelector(
											'.ubits-data-table__header-search-button',
										);
                ...
                `;
					...
					if (
						currentOptions.header.columnSelectorButton &&
						currentOptions.header.showColumnSelectorButton !== false
					) {
						...
						if (columnSelectorBtn) {
							...
							columnSelectorBtn.addEventListener('click', (e) => {
								...
								if (afterCleanChildren > 0 || afterCleanInnerHTML > 0) {
									...
									requestAnimationFrame(() => {
										if (
											dropdownElement.children.length > 0 ||
											dropdownElement.innerHTML.length > 0
										) {
											console.error(
								...
								const updateDropdownContent = () => {
									const listContainerId = 'ubits-data-table-column-selector-list';
									let listContainer = dropdownElement.querySelector(
										`#${listContainerId}`,
									) as HTMLElement;
	...
	const update = (newOptions: Partial<DataTableOptions>) => {
		...
		if (
			newOptions.showPagination !== undefined &&
			newOptions.showPagination !== previousShowPagination
		) {
