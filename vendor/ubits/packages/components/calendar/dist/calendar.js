...
function j(g) {
	const {
		...
		N = m.getDay(),
		q = /* @__PURE__ */ new Date().toDateString(),
		v = ['ubits-calendar', b === 'range' ? 'ubits-calendar--range' : 'ubits-calendar--single', A]
			.filter(Boolean)
			.join(' '),
		P = $ ? ` style="${$}"` : '',
		Y = `
  ...
  `;
	...
	for (let D = 1; D <= _; D++) {
		...
		b === 'single' && y && k(d, y) && i.push('ubits-calendar__day--selected'),
			b === 'range' &&
				y &&
				(C
					? k(d, y)
					...
					: k(d, y) && i.push('ubits-calendar__day--range-start')),
			E && i.push('ubits-calendar__day--today');
		let l = !1;
		I && M(d, I) < 0 && ((l = !0), i.push('ubits-calendar__day--disabled')),
			H && M(d, H) > 0 && ((l = !0), i.push('ubits-calendar__day--disabled'));
		const r = l ? ' disabled' : '',
			f = z(d);
	...
	const _ = () => {
		...
		N = () => {
			...
			const v = s.querySelector('.ubits-calendar__nav-button--prev'),
				P = s.querySelector('.ubits-calendar__nav-button--next'),
				F = s.querySelector('.ubits-calendar__month-input'),
				Y = s.querySelector('.ubits-calendar__year-input'),
							...
							(o.innerHTML = f),
								...
								requestAnimationFrame(async () => {
									...
									if (
										...
										console.log('📅 [Calendar] Necesita scroll?', t.scrollHeight > t.clientHeight),
							...
							(o.innerHTML = t),
								...
								requestAnimationFrame(async () => {
									...
									if (
										...
										console.log('📅 [Calendar] Necesita scroll?', e.scrollHeight > e.clientHeight),
										...
										try {
													...
													(a = p(e, n, 'vertical')),
														...
														setTimeout(() => {
															const S = n.querySelector('.ubits-scrollbar'),
																R = n.querySelector('.ubits-scrollbar__bar');
															S &&
																R &&
											...
											console.error('❌ [Calendar] Error creando scrollbar para lista de años:', c),
...
export { J as createCalendar, j as renderCalendar };
