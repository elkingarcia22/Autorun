(function (l, d) {
	typeof exports == 'object' && typeof module < 'u'
		...
		: typeof define == 'function' && define.amd
			? define(['exports'], d)
			: ((l = typeof globalThis < 'u' ? globalThis : l || self), d((l.UBITSModal = {})));
})(this, function (l) {
	'use strict';
	const d = { sm: '320px', md: '480px', lg: '640px', xl: '800px', full: '1280px' };
	function u(e) {
		const {
				title: c,
				bodyContent: i = '',
			...
			t = d[b] || d.md,
			y = ['ubits-modal', `ubits-modal--size-${b}`, _ ? 'ubits-modal--full-screen' : '', f]
				.filter(Boolean)
				.join(' '),
			a = `
  ...
  `.trim();
	...
	function p(e) {
		const { containerId: c, onClose: i, closeOnOverlayClick: b = !0, open: _ = !1 } = e;
		let n;
		c ? (n = document.getElementById(c) || document.body) : (n = document.body);
		const f = document.createElement('div');
		f.innerHTML = u(e);
		const t = f.firstElementChild;
		if (!t) throw new Error('No se pudo crear el modal');
		t.querySelector('.ubits-modal');
		const M = t.querySelector('.ubits-modal__close'),
			m = t,
			y = () => {
				t.classList.add('ubits-modal-overlay--open'), (document.body.style.overflow = 'hidden');
			},
			a = () => {
				t.classList.remove('ubits-modal-overlay--open'),
					(document.body.style.overflow = ''),
					i && i();
			},
			C = (o) => {
				const r = t.querySelector('.ubits-modal__body-content');
				if (r) {
		...
		if ((document.addEventListener('keydown', w), e.footerButtons)) {
			const o = t.querySelector('.ubits-modal__footer-left .ubits-modal__footer-button'),
				r = t.querySelector('.ubits-modal__footer-right .ubits-button--secondary'),
				v = t.querySelector('.ubits-modal__footer-right .ubits-button--primary');
			o &&
				e.footerButtons.tertiary?.onClick &&
				o.addEventListener('click', (s) => {
					s.preventDefault(), e.footerButtons.tertiary.onClick(s);
				}),
				r &&
					e.footerButtons.secondary?.onClick &&
					r.addEventListener('click', (s) => {
						s.preventDefault(), e.footerButtons.secondary.onClick(s);
					}),
				v &&
					e.footerButtons.primary?.onClick &&
					v.addEventListener('click', (s) => {
						s.preventDefault(), e.footerButtons.primary.onClick(s);
					});
		}
		return n.appendChild(t), _ && y(), { element: t, open: y, close: a, updateContent: C };
	}
	typeof window < 'u' &&
		((window.createModal = p),
		(window.renderModal = u),
		window.UBITSModal || (window.UBITSModal = {}),
		(window.UBITSModal.createModal = p),
		(window.UBITSModal.renderModal = u)),
		(l.createModal = p),
		(l.renderModal = u),
