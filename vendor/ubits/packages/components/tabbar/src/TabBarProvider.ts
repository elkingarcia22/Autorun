...
function renderTreeNode(
	...
	const hasChildren =
		...
		(item as FloatingMenuSection).subitems?.length > 0;
	...
	const childrenHTML = children
		.map((child) => renderTreeNode(child, level + 1, size, uniqueId))
		.join('');
	const iconHTML =
		level === 0 && item.icon
			? `
 ...
 */
function renderProfileTreeMenuItem(
	...
	// Si tiene hijos, renderizar como nodo expandible
	const childrenHTML = item
		.children!.map((child) => renderProfileTreeMenuItem(child, level + 1, size))
		.join('');
	...
	if (floatingMenuSections && floatingMenuSections.length > 0) {
		...
		if (isPreview) {
			floatingMenuContainer.style.cssText =
				'position: absolute; top: 0; left: 0; right: 0; bottom: 68px; width: 100%; height: 500px; z-index: 2000; overflow: visible; display: none;'; // 60px (TabBar) + 8px (espacio)
		} else {
			floatingMenuContainer.style.cssText = '';
		}
	...
	if (profileMenuItems && profileMenuItems.length > 0) {
		...
		if (isPreview) {
			...
			profileMenuContainer.style.cssText =
				'position: absolute; bottom: 68px; left: 0; right: 0; width: 100%; max-width: 100%; z-index: 2001; overflow: visible; display: none;'; // 60px (TabBar) + 8px (espacio)
		} else {
			profileMenuContainer.style.cssText = '';
		}
				...
				if (profileMenu) {
					if (profileMenu.classList.contains('ubits-profile-menu--show')) {
						...
						if (isPreview && profileMenuContainer) {
							...
							if (profileMenuInner) {
								profileMenuInner.style.cssText =
									'position: absolute; bottom: 0; left: 0; right: 0; width: 100%; max-width: 100%; display: block;';
...
function toggleAccordion(sectionId: string): void {
	...
	const title = document.querySelector(
		`[data-accordion-id="${sectionId}"] .ubits-accordion-title`,
	) as HTMLElement;
