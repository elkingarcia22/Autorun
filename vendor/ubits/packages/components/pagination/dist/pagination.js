...
function _(a, s = 'regular') {
	try {
		const e = s === 'solid' ? 'fas' : 'far',
			t = a.startsWith('fa-') ? a : `fa-${a}`;
		return `<i class="${e} ${t}"></i>`;
	} catch {
		const t = s === 'solid' ? 'fas' : 'far',
			i = a.startsWith('fa-') ? a : `fa-${a}`;
...
function S(a) {
	...
	let g = y,
		f = w;
	o && !i && t
		? ((g = _('chevron-down', n)), (f = 'right'))
		: o && i && w === 'left' && t
			? (g = `${y}${_('chevron-down', n)}`)
			: o && !t && (g = i ? `${y}${_('chevron-down', n)}` : _('chevron-down', n));
	const h =
			{
				xs: 'xs',
				sm: 'sm',
	...
	let $ = '';
	l && p
		? ($ = `${T}<span class="button-text">${p}</span>`)
		: l && !t
			? ($ = T)
			: l && t
				? w === 'right'
					? ($ = `<span class="button-text">${t}</span>${T}`)
					: ($ = `${T}<span class="button-text">${t}</span>`)
				: r && i
					? ($ = y)
					: g && t
						? o && i && w === 'left'
							? ($ = `${_(i, n)}<span>${t}</span>${_('chevron-down', n)}`)
							: f === 'right'
								? ($ = `<span>${t}</span>${g}`)
								: ($ = `${g}<span>${t}</span>`)
						: t
							? ($ = o ? `<span>${t}</span>${_('chevron-down', n)}` : `<span>${t}</span>`)
							: g && ($ = g);
	const E = d ? '<span class="ubits-button__badge"></span>' : '',
		O = r && B && I ? `title="${I}"` : '';
	return `
    <button class="${c}" ${M} ${O}>
      ${$}
...
function q(a, s, e) {
	const t = [];
	if (s <= e) for (let i = 1; i <= s; i++) t.push(i);
	else {
		const i = Math.floor(e / 2);
		let n = Math.max(1, a - i),
			r = Math.min(s, n + e - 1);
		r - n < e - 1 && (n = Math.max(1, r - e + 1));
		for (let u = n; u <= r; u++) t.push(u);
	}
	return t;
}
...
function L(a) {
	const {
		...
		o = Math.max(1, Math.min(s, e)),
		B = ['ubits-pagination', `ubits-pagination--${n}`, `ubits-pagination--${r}`, w]
			.filter(Boolean)
			.join(' '),
		I = [...Object.entries(x).map(([m, h]) => `${m}="${h}"`)].filter(Boolean).join(' '),
		c = {
			first: 'Primera',
			last: 'Última',
			previous: 'Anterior',
	...
	if (b) {
      ...
      <div class="ubits-pagination__items-per-page">
        ...
        <select id="${m}" class="ubits-pagination__select ubits-body-sm">
          ${v
						.map((h) => `<option value="${h}" ${h === i ? 'selected' : ''}>${h}</option>`)
						.join('')}
        </select>
      </div>
    `;
	}
	const g = r === 'sm' ? 'sm' : r === 'lg' ? 'lg' : 'md',
		f = [];
	if (
		(l &&
			n === 'default' &&
...
function D(a) {
	...
	if (!t) return console.error(`❌ [Pagination] Contenedor con ID "${s}" no encontrado`), null;
	const i = L(e);
	t.innerHTML = i;
	const n = t.querySelector('.ubits-pagination');
	if (!n) return console.error('❌ [Pagination] No se pudo crear el elemento de paginación'), null;
	n.querySelectorAll('.ubits-pagination__page-button').forEach((p) => {
		...
		n.querySelectorAll('.ubits-pagination__nav-button').forEach((p) => {
			p.addEventListener('click', () => {
				const d = parseInt(n.getAttribute('data-current-page') || '1'),
					P = parseInt(n.getAttribute('data-total-pages') || '1'),
					b = p.getAttribute('aria-label') || '';
				let v = d;
				b.includes('Primera') || b.includes('First')
					...
					: b.includes('Última') || b.includes('Last')
						...
						: b.includes('Anterior') || b.includes('Previous')
							? (v = Math.max(1, d - 1))
							: (b.includes('Siguiente') || b.includes('Next')) && (v = Math.min(P, d + 1)),
					v !== d && e.onPageChange && e.onPageChange(v);
			});
		});
	const l = n.querySelector('.ubits-pagination__select');
	return (
		l &&
			e.onItemsPerPageChange &&
...
class C extends HTMLElement {
	...
	render() {
		const s = this.getAttribute('variant') || this.options.variant,
			e = this.getAttribute('size') || this.options.size,
			t = parseInt(this.getAttribute('current-page') || String(this.options.currentPage || 1)),
			i = parseInt(this.getAttribute('total-pages') || String(this.options.totalPages || 1)),
			n = {
				...this.options,
				variant: s,
				size: e,
		...
		(this.innerHTML = L(n)),
			this.options.onPageChange &&
				this.querySelectorAll('button').forEach((u) => {
...
typeof window < 'u' &&
	!customElements.get('ubits-pagination') &&
	customElements.define('ubits-pagination', C);
...
class F {
	...
	async initialize(s) {
		customElements.get('ubits-pagination') || customElements.define('ubits-pagination', C),
			typeof window < 'u' &&
...
export { F as PaginationAddon, C as UBITSPagination, D as createPagination, L as renderPagination };
