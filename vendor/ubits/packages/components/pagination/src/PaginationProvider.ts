...
export function createPagination(
	options: PaginationOptions & { containerId?: string },
): HTMLElement | null {
	...
	if (selectButton && listContainer) {
		const listId =
			selectButton.getAttribute('data-list-id') || `ubits-pagination-list-${Date.now()}`;
		const currentValue =
			paginationOptions.itemsPerPage || paginationOptions.itemsPerPageOptions?.[0] || 10;

		// Asegurar que el contenedor tenga el ID antes de usarlo
		listContainer.id = listId;

		// Convertir opciones a ListItems
		const listItems: ListItem[] = (paginationOptions.itemsPerPageOptions || [10, 20, 50, 100]).map(
			(opt) => ({
				label: String(opt),
				value: String(opt),
				state: 'default',
		...
		const toggleDropdown = () => {
			...
			const listSize: 'sm' | 'md' | 'lg' =
				paginationSize === 'sm' ? 'sm' : paginationSize === 'lg' ? 'lg' : 'md';
				...
				const listElement = createList({
					...
					onSelectionChange: (selectedItem, index) => {
						if (
							...
							paginationOptions.itemsPerPageOptions[index] !== undefined
						) {
