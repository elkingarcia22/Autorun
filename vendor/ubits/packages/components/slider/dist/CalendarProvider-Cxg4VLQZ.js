...
function N(i) {
	const s = String(i.getDate()).padStart(2, '0'),
		a = String(i.getMonth() + 1).padStart(2, '0'),
		p = i.getFullYear();
	return `${s}/${a}/${p}`;
}
function I(i, s) {
...
function T(i, s) {
  ...
  `;
	i.forEach((n) => {
		const t = n.selected ? 'active' : 'default',
			e = ['ubits-list-item', 'ubits-list-item--sm', t !== 'default' ? `ubits-list-item--${t}` : '']
				.filter(Boolean)
				.join(' '),
			o = [];
		t === 'active' && o.push('aria-selected="true"'),
			o.push('tabindex="0"'),
			o.push(`data-value="${n.value}"`),
			(x += `
  ...
  `),
	...
	const M = async () => {
			console.log('📜 [SCROLLBAR] ========== INICIO initScrollbar =========='),
				...
				console.log('📜 [SCROLLBAR] scrollbarContainerId:', _);
		...
		S = () => {
			console.log('📜 [SCROLLBAR] setupScrollbar llamado, isConnected:', a.isConnected),
				a.isConnected &&
	...
	if (a.parentElement)
		...
		n.observe(document.body, { childList: !0, subtree: !0 }),
			setTimeout(() => {
				a.isConnected &&
					(console.log('📜 [SCROLLBAR] Timeout alcanzado, inicializando'), n.disconnect(), S());
			}, 1e3);
	}
	return (
		setTimeout(() => {
			const n = document.getElementById(d);
			n &&
				n.querySelectorAll('.ubits-list-item').forEach((t) => {
					t.addEventListener('click', (e) => {
						e.preventDefault(), e.stopPropagation();
						const o = parseInt(e.currentTarget.dataset.value || '0'),
							r = a._scrollbarInstance;
...
function F(i) {
	const {
		...
		O = ['ubits-calendar', s === 'range' ? 'ubits-calendar--range' : 'ubits-calendar--single', M]
			.filter(Boolean)
			.join(' '),
		u = S ? ` style="${S}"` : '',
		c = `
  ...
  `;
	...
	for (let y = 1; y <= C; y++) {
		const h = new Date(t, e, y),
			l = h.toDateString() === k;
		let g = ['ubits-calendar__day'];
		s === 'single' && a && A(h, a) && g.push('ubits-calendar__day--selected'),
			s === 'range' &&
				a &&
				(p
					? A(h, a)
						...
						: A(h, p)
							...
							: Y(h, a, p) && g.push('ubits-calendar__day--in-range')
					: A(h, a) && g.push('ubits-calendar__day--range-start')),
			l && g.push('ubits-calendar__day--today');
		let v = !1;
		d && I(h, d) < 0 && ((v = !0), g.push('ubits-calendar__day--disabled')),
			_ && I(h, _) > 0 && ((v = !0), g.push('ubits-calendar__day--disabled'));
		const R = v ? ' disabled' : '',
			D = N(h);
...
function P(i) {
	...
	if (!t) throw new Error('No se pudo crear el calendario');
	let e = new Date(x),
		o = a ? new Date(a) : null,
		r = p ? new Date(p) : null,
		C = !1;
	const f = () => {
		...
		B = () => {
			const u = t.querySelector('.ubits-calendar__nav-button--prev'),
				H = t.querySelector('.ubits-calendar__nav-button--next'),
				c = t.querySelector('.ubits-calendar__month-input'),
				L = t.querySelector('.ubits-calendar__year-input'),
				w = t.querySelector('.ubits-calendar__month-dropdown'),
				y = t.querySelector('.ubits-calendar__year-dropdown');
			u?.addEventListener('click', (b) => {
				b.preventDefault(),
					b.stopPropagation(),
					e.setMonth(e.getMonth() - 1),
					c && (c.value = E[e.getMonth()]),
					L && (L.value = String(e.getFullYear())),
					f();
			}),
					...
					.forEach((b) => {
						b.addEventListener('click', (l) => {
							...
							const g = b.dataset.date || '',
								[v, R, D] = g.split('/'),
								m = new Date(parseInt(D), parseInt(R) - 1, parseInt(v));
							s === 'single'
								? ((o = m), f(), M && M(m))
								: s === 'range' &&
									(!o || (o && r)
										...
										: o &&
											!r &&
											(I(m, o) < 0 ? ((r = o), (o = m)) : (r = m), f(), S && o && r && S(o, r)));
						});
					});
		};
	return (
		f(),
		{
			element: t,
			update: (u) => {
				u.selectedDate !== void 0 && (o = u.selectedDate ? new Date(u.selectedDate) : null),
					u.endDate !== void 0 && (r = u.endDate ? new Date(u.endDate) : null),
					u.initialDate && (e = new Date(u.initialDate)),
					Object.assign(i, u),
					f();
			},
			destroy: () => {
				const u = t.querySelector('.ubits-calendar__month-dropdown'),
					H = t.querySelector('.ubits-calendar__year-dropdown');
				if (u) {
