...
export function createSlider(options) {
	...
	const sliderElement =
		container.querySelector(`#${containerId}`) || container.querySelector('.ubits-slider');
	if (!sliderElement) {
			...
			if (trackRange) {
				...
				if (isVertical) {
					...
					if (thumbElements.length >= 2 && trackRect.height > 0) {
						const minThumb = Array.from(thumbElements).find((t) =>
							t.classList.contains('ubits-slider-thumb--min'),
						);
						const maxThumb = Array.from(thumbElements).find((t) =>
							t.classList.contains('ubits-slider-thumb--max'),
						);
