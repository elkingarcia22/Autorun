...
export function renderSlider(options) {
    const { containerId, label = '', helperText = '', size = 'md', state = 'default', orientation = 'horizontal', mode = 'single', min = 0, max = 100, step = 1, value = 50, values = [25, 75], showInputs = false, showLabel = true, showHelper = false, showMarks = false, marks = [], showRangeGuide = false, className = '', attributes = {} } = options;
    ...
    if (isVertical)
        sliderClasses.push('ubits-slider--vertical');
    if (size)
        sliderClasses.push(`ubits-slider--${size}`);
    if (isDisabled)
        sliderClasses.push('ubits-slider--disabled');
    if (className)
...
export function createSlider(options) {
    const { containerId, onChange, onRangeChange, min = 0, max = 100, step = 1, mode = 'single', value = 50, values = [25, 75], orientation = 'horizontal', showInputs = false, state = 'default', size = 'md', showRangeGuide = false } = options;
    // Si showRangeGuide está activo, activar automáticamente los inputs
    const effectiveShowInputs = showInputs || showRangeGuide;
    ...
    const createInputs = () => {
        ...
        if (isRange) {
            ...
            let inputMinContainer = sliderElement.querySelector(`#${inputMinContainerId}`);
            if (!inputMinContainer) {
                inputMinContainer = container.querySelector(`#${inputMinContainerId}`);
            }
            if (!inputMinContainer) {
            ...
            if (inputMinContainer) {
                ...
                try {
                    ...
                    if (inputMinInstance) {
                        const inputWrapper = inputMinContainer.querySelector('div[style*="position: relative"]');
                        if (inputWrapper) {
            ...
            let inputMaxContainer = sliderElement.querySelector(`#${inputMaxContainerId}`);
            if (!inputMaxContainer) {
                inputMaxContainer = container.querySelector(`#${inputMaxContainerId}`);
            }
            if (!inputMaxContainer) {
            ...
            if (inputMaxContainer) {
                ...
                try {
                    ...
                    // Asegurar que el wrapper del input no se expanda
                    if (inputMaxInstance) {
                        const inputWrapper = inputMaxContainer.querySelector('div[style*="position: relative"]');
                        if (inputWrapper) {
                            inputWrapper.style.width = '100%';
                            inputWrapper.style.maxWidth = '100%';
                        }
            ...
            let inputValueContainer = sliderElement.querySelector(`#${inputValueContainerId}`);
            if (!inputValueContainer) {
                inputValueContainer = container.querySelector(`#${inputValueContainerId}`);
            }
            if (!inputValueContainer) {
            ...
            if (inputValueContainer) {
                ...
                try {
                    ...
                    if (inputValueInstance) {
                        const inputWrapper = inputValueContainer.querySelector('div[style*="position: relative"]');
                        if (inputWrapper) {
                            inputWrapper.style.width = '100%';
                            inputWrapper.style.maxWidth = '100%';
                        }
        ...
        if (isRange) {
            if (trackRange) {
                ...
                if (isVertical) {
                    ...
                    const thumbElements = container.querySelectorAll('.ubits-slider-thumb');
    ...
    const updateInputs = () => {
        ...
        const rangeGuideCurrent = container.querySelector(`#${containerId}-range-guide-current`);
    ...
    const updateSlider = () => {
        if (isRange) {
            ...
            if (minThumb)
                updateThumbPosition(minThumb, minVal);
            if (maxThumb)
                updateThumbPosition(maxThumb, maxVal);
        }
        else {
            const thumb = thumbs[0];
            if (thumb)
                updateThumbPosition(thumb, currentValue);
        }
        updateTrackFill();
        updateInputs();
    };
    ...
    const handleMouseMove = (e) => {
        if (!isDragging || !activeThumb || isDisabled)
            return;
        e.preventDefault();
    ...
    return {
        ...
        disable: () => {
            ...
            if (inputMinInstance)
                inputMinInstance.disable();
            if (inputMaxInstance)
                inputMaxInstance.disable();
            if (inputValueInstance)
                inputValueInstance.disable();
        },
        enable: () => {
            ...
            if (inputMinInstance)
                inputMinInstance.enable();
            if (inputMaxInstance)
                inputMaxInstance.enable();
            if (inputValueInstance)
                inputValueInstance.enable();
        },
        setState: (newState) => {
