...
export function renderScoreCardMetrics(options: ScoreCardMetricsOptions): string {
	...
	const classes = ['ubits-score-card-metrics', `ubits-score-card-metrics--${size}`, className]
		.filter(Boolean)
		.join(' ');
 ...
 */
export function createScoreCardMetrics(
	options: ScoreCardMetricsOptions & { containerId?: string },
): HTMLElement | null {
