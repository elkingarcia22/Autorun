			...
			requestAnimationFrame(() => {
				...
				if (modalOverlay && modal && button) {
					...
					modalOverlay.style.justifyContent =
						position === 'bottom-right' || position === 'top-right' ? 'flex-end' : 'flex-start';
					modalOverlay.style.paddingBottom = `${buttonRect.height + offset + 16}px`;
					modalOverlay.style.paddingRight =
						position === 'bottom-right' || position === 'top-right' ? `${offset}px` : 'auto';
					modalOverlay.style.paddingLeft =
						position === 'bottom-left' || position === 'top-left' ? `${offset}px` : 'auto';
