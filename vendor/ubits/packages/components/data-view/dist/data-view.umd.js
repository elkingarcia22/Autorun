(function (c, u) {
	typeof exports == 'object' && typeof module < 'u'
		...
		: typeof define == 'function' && define.amd
			? define(['exports'], u)
			: ((c = typeof globalThis < 'u' ? globalThis : c || self), u((c.UBITSDataView = {})));
})(this, function (c) {
	'use strict';
	...
	function u(t, s = 'regular') {
		const a = s === 'solid' ? 'fas' : 'far',
			r = t.startsWith('fa-') ? t : `fa-${t}`;
		return `<i class="${a} ${r}"></i>`;
	}
	function p(t = 'INSTOCK') {
		const s = {
	...
	function C(t) {
		...
		a &&
			(n +=
				'<i class="fas fa-star-half-alt ubits-data-view__star ubits-data-view__star--half"></i>');
		for (let i = 0; i < r; i++)
  ...
  `;
	...
	function O(t, s, a) {
		const {
				...
				showWishlist: w = !0,
				showBuyButton: l = !0,
				buyButtonText: f = 'Buy Now',
				buyButtonIcon: _ = 'shopping-cart',
				wishlistIcon: e = 'heart',
			} = a,
			d = t.id || `product-${s}`,
			b = t.stockStatus || 'INSTOCK',
			o = t.inWishlist || !1;
		return `
  ...
  `;
	...
	function h(t) {
		...
		const {
			...
			l = ['ubits-data-view', `ubits-data-view--${r}`, n].filter(Boolean).join(' '),
			f = Object.entries(i)
				.map(([d, b]) => `${d}="${b}"`)
				.join(' '),
			_ = a ? `id="${a}"` : '';
		let e = `<div class="${l}" ${_} ${f}>`;
		return (
			s.forEach((d, b) => {
				e += O(d, b, t);
			}),
	...
	function T(t) {
		...
		const {
			...
			d = `ubits-data-view--${n}`;
		return (
			(e.className = ['ubits-data-view', d, f].filter(Boolean).join(' ')),
			a && (e.id = a),
			Object.entries(_).forEach(([o, v]) => {
				e.setAttribute(o, v);
			}),
			(e.innerHTML = h(t)),
			e.querySelectorAll('.ubits-data-view__item').forEach((o, v) => {
				const g = r[v];
				if (!g) return;
				i &&
					o.addEventListener('click', (m) => {
						m.target.closest('button') || i(g, v, o);
					});
				const $ = o.querySelector('[data-action="buy"]');
				$ &&
					w &&
					$.addEventListener('click', (m) => {
						m.stopPropagation(), w(g, v, o);
					});
				const y = o.querySelector('[data-action="wishlist"]');
				y &&
					l &&
					y.addEventListener('click', (m) => {
						m.stopPropagation(), l(g, v, o);
					});
			}),
			e
		);
