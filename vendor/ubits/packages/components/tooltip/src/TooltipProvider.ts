...
export function renderTooltip(options: TooltipOptions): string {
	...
	const buttonCount =
		(showPrimaryButton ? 1 : 0) + (showSecondaryButton ? 1 : 0) + (showTertiaryButton ? 1 : 0);
  ...
  `
	...
	if (hasButtons) {
		...
		const primaryButton =
			showPrimaryButton && primaryButtonLabel
		...
		const tertiaryButton =
			showTertiaryButton && tertiaryButtonLabel
		...
		const handleClickOutside = (e: MouseEvent) => {
			...
			if (tooltip.classList.contains('ubits-tooltip--open') && !tooltip.contains(target)) {
				...
				const isTriggerButton =
					clickedElement.closest && clickedElement.closest('[data-tooltip-trigger]');
