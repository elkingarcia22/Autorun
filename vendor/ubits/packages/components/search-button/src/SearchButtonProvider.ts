...
export function renderSearchButton(options: SearchButtonOptions): string {
	...
	const additionalClasses = [state === 'hover' ? 'ubits-search-button--force-hover' : '', className]
		.filter(Boolean)
		.join(' ');
...
export function createSearchButton(options: SearchButtonOptions): {
	...
	const update = (newOptions: Partial<SearchButtonOptions>) => {
		...
		if (isSearchActive && wasSearchActive) {
			...
			const currentClearButton = element.querySelector(
			...
			if (
				currentInput &&
				newOptions.value !== undefined &&
				newOptions.value !== currentInput.value
			) {
			...
			const hasSignificantChange = significantChanges.some(
				(key) =>
					...
					newOptions[key as keyof SearchButtonOptions] !==
						options[key as keyof SearchButtonOptions],
			);
