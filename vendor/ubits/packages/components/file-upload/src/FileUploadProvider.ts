 ...
 */
function renderStatusTagInline(
	label: string,
	status: 'pending' | 'completed' | 'error' | 'uploading' = 'pending',
): string {
...
export function renderFileUpload(options: FileUploadOptions = {}): string {
	...
	const finalConstraintsText =
		constraintsText || `Máx. ${maxFiles} archivos · Hasta ${formatFileSize(maxSize)}`;
	...
	if (actualState === 'files-list' && hasFiles) {
		...
		const filesListHtml = filesToShow
			.map((file, index) => {
	...
	const classes = ['ubits-file-upload', `ubits-file-upload--${actualState}`, className]
		.filter(Boolean)
		.join(' ');
	...
	const update = (newOptions: Partial<FileUploadOptions>) => {
		...
		if (newElement && fileUploadElement.parentNode) {
			...
			if (
				...
				updatedOptions.state !== 'disabled' &&
				updatedOptions.state !== 'filled'
			) {
