...
export function renderCSATMetricCard(options: CSATMetricCardOptions): string {
	...
	const classes = ['ubits-csat-metric-card', `ubits-csat-metric-card--${size}`, className]
		.filter(Boolean)
		.join(' ');
 ...
 */
export function createCSATMetricCard(
	options: CSATMetricCardOptions & { containerId?: string },
): HTMLElement | null {
