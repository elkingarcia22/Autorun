 ...
 */
import { createInput } from './InputProvider';
export class UBITSInput extends HTMLElement {
	...
	updateOptions() {
		...
		const containerId =
			...
			this.id ||
			`ubits-input-${Math.random().toString(36).substr(2, 9)}`;
		...
		this.options = {
			...
			showHelper: this.hasAttribute('show-helper')
				...
				: false,
			showCounter: this.hasAttribute('show-counter')
				? this.getAttribute('show-counter') !== 'false'
				: false,
			maxLength: this.hasAttribute('max-length')
				? parseInt(this.getAttribute('max-length') || '50', 10)
				: 50,
			mandatory: this.hasAttribute('mandatory')
				? this.getAttribute('mandatory') !== 'false'
				: false,
