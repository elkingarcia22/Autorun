...
function extractFigmaTokens(figmaData, mode, primitives) {
	...
	function findModeNodes(obj, currentPath = []) {
		if (typeof obj === 'object' && obj !== null) {
			for (const key in obj) {
				...
				const isLightMode =
					normalizedKey.includes('light') &&
					(normalizedKey.includes('mode') || normalizedKey === 'light');
				const isDarkMode =
					normalizedKey.includes('dark') &&
					(normalizedKey.includes('mode') || normalizedKey === 'dark');

				if (
					key === mode ||
					(normalizedMode === 'light' && isLightMode) ||
					(normalizedMode === 'dark' && isDarkMode)
				) {
	...
	if (figmaData[''] && figmaData['']['s-colors']) {
		...
		const lightModeKey = Object.keys(sColors).find(
			(k) => k.toLowerCase().includes('light') && k.toLowerCase().includes('mode'),
		);
		const darkModeKey = Object.keys(sColors).find(
			(k) => k.toLowerCase().includes('dark') && k.toLowerCase().includes('mode'),
		);
	...
	if (figmaData[''] && figmaData['']['modifiers/Normal']) {
		...
		['chart', 'brand', 'scroll-bar', 'toggle', 'button', 'ai-button', 'focus', 'elevation'].forEach(
			(prop) => {
				if (normal[prop]) {
...
:root{\n${Object.entries(lightFigma)
			.map(([cssVar, val]) => `  ${cssVar}: ${val};`)
			.join('\n')}\n}\n\n[data-theme="dark"]{\n${Object.entries(darkFigma)
			.map(([cssVar, val]) => `  ${cssVar}: ${val};`)
			.join('\n')}\n}\n`;
		...
		console.log(
			`   Total tokens Figma: ${Object.keys(lightFigma).length + Object.keys(darkFigma).length}`,
		);
	} else {
		console.log(
			`⚠️  Archivo ${FIGMA_TOKENS_PATH} no encontrado. Ejecuta scripts/convert-figma-to-css-vars.cjs primero.`,
		);
	}
}
