...
function countPrimitiveColors(tokens, colorName) {
	...
	const colorTokens = tokens.filter((token) => {
		const lower = token.toLowerCase();
		return (
			lower.includes(colorName.toLowerCase()) &&
...
function main() {
	...
	primitiveColors.forEach((color) => {
		...
		console.log(
			`  ${status} ${color}: ${primitives.length} primitivos (${allColorTokens.length} total incluyendo modifiers)`,
		);
	...
	Object.entries(secondaryCategories).forEach(([name, pattern]) => {
		const tokens = figmaTokens.filter(
			(t) => pattern.test(t.toLowerCase()) && !t.toLowerCase().includes('modifiers'),
		);
