...
class ContentManager {
	...
	getSubNavForSection(section) {
		...
		if (
			this.isAdminMode &&
			(normalizedSection === 'inicio' || normalizedSection === 'diagnóstico')
		) {
		...
		if (subNavConfig) {
			console.log(
				'✅ [ContentManager] Configuración de SubNav encontrada para:',
				normalizedSection,
			);
		...
		console.warn(
			`⚠️ [ContentManager] No hay configuración de SubNav para la sección: ${normalizedSection}`,
		);
	...
	updateSubNav(section, activeTabId = null) {
		...
		console.log(
			'🔍 [ContentManager.updateSubNav] SubNav config tabs count:',
			subNavConfig?.tabs?.length,
		);
		...
		if (typeof window.createSubNav === 'function') {
			...
			setTimeout(() => {
				...
				if (tabs && tabs.length > 0) {
					console.log(
						'   - Labels de tabs:',
						Array.from(tabs).map((t) => t.textContent?.trim()),
					);
				}
			}, 100);
		} else {
	...
	updateContent(section, subSection = null) {
		...
		console.log(
			'🔍 [ContentManager.updateContent] updateContent llamado para sección:',
			section,
			'subSection:',
			subSection,
		);
		...
		if (existingHeader && existingContent) {
			...
			if (existingTitle === expectedTitle) {
				console.log(
					'🔍 [ContentManager.updateContent] ⚠️ Contenido ya está actualizado, evitando recarga',
				);
      ...
      `;
			...
			setTimeout(() => {
				...
				if (button) {
					...
					const buttonStylesheets = Array.from(document.styleSheets).filter((sheet) => {
						try {
							return (
								sheet.href && (sheet.href.includes('button.css') || sheet.href.includes('button'))
							);
						} catch (e) {
							return false;
						}
					...
					buttonStylesheets.forEach((sheet, idx) => {
						...
						try {
							...
							const buttonRules = rules.filter(
								(rule) => rule.selectorText && rule.selectorText.includes('.ubits-button'),
							);
					...
					const tokens = {
						'--ubits-button-primary-bg-default': computedRootStyle.getPropertyValue(
							'--ubits-button-primary-bg-default',
						),
						'--ubits-btn-primary-fg': computedRootStyle.getPropertyValue('--ubits-btn-primary-fg'),
						'--ubits-button-primary-hover': computedRootStyle.getPropertyValue(
							'--ubits-button-primary-hover',
						),
						'--ubits-button-primary-pressed': computedRootStyle.getPropertyValue(
						...
						'--ubits-border-radius-md': computedRootStyle.getPropertyValue(
							'--ubits-border-radius-md',
						),
						'--ubits-border-radius-sm': computedRootStyle.getPropertyValue(
					...
					if (buttonStylesheet) {
						...
						try {
							...
							const primaryButtonRule = rules.find(
								(rule) =>
									rule.selectorText &&
									rule.selectorText.includes('.ubits-button--primary') &&
					...
					console.log(
						'🔍 [ContentManager] Estilos inline del botón:',
						button.style.cssText || '(ninguno)',
					);

					// Verificar si el botón tiene el estilo esperado del componente
					const expectedBg = getComputedStyle(root).getPropertyValue(
						'--ubits-button-primary-bg-default',
					);
					...
					console.log(
						'   - ¿Coincide?:',
						expectedBg
							? actualBg.includes('rgb(12, 91, 239)') || actualBg.includes('#0c5bef')
							: false,
					);
				} else {
				...
				console.log(
					'🔍 [ContentManager] CSS de header-section encontrado:',
					headerStylesheets.length,
				);
		...
		console.log(
			'🔍 [ContentManager.updateContent] ✅ Contenido actualizado para sección:',
			section,
			'subSection:',
			subSection,
		);
	...
	handleSectionChange(section, activeTabId = null) {
		...
		this.currentSection = section;
		console.log(
			'🔍 [ContentManager.handleSectionChange] ✅ currentSection actualizado a:',
			this.currentSection,
		);
