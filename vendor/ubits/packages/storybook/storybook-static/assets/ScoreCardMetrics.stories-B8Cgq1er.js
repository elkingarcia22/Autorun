import { r as u } from './ButtonProvider-Dmy-L2BE.js';
import './iframe-BcZai5-G.js';
import './ListProvider-Bx601uSc.js';
import './SpinnerProvider-o6XHV06V.js';
import './preload-helper-PPVm8Dsz.js';
function M(s, t = 'regular', e) {
	const a = t === 'solid' ? 'fas' : 'far',
		o = s.startsWith('fa-') ? s : `fa-${s}`,
		r = e ? `style="color: ${e};"` : '';
	return `<i class="${a} ${o}" ${r}></i>`;
}
function T(s) {
...
function H(s) {
	const {
			title: t,
			totalResponses: e = 0,
			responsesLabel: a = 'respuestas',
			average: o = 0,
			averageLabel: r = 'Promedio:',
			score: i = 0,
			leftLabel: V = '0',
			rightLabel: B = '5',
			chartDescription: E = '0 a 5 del gráfico',
			titleIcon: c,
			titleIconStyle: m = 'regular',
			titleIconColor: p,
			showInfoIcon: y = !1,
		...
		h = ['ubits-score-card-metrics', `ubits-score-card-metrics--${f}`, g].filter(Boolean).join(' '),
		_ = [...Object.entries(v).map(([C, S]) => `${C}="${S}"`)].filter(Boolean).join(' '),
		L = c ? `<div class="ubits-score-card-metrics__title-icon">${M(c, m, p)}</div>` : '',
		$ = y
			? u({
					variant: 'tertiary',
					size: 'sm',
		...
		l = b
			...
			: '',
		x = 'ubits-body-md-bold',
		d = 'ubits-body-sm-regular',
		I = o.toFixed(2),
		w = T(i);
	return `
    <div class="${h}" ${_}>
...
const P = {
		...
		argTypes: {
			title: {
				...
				table: { defaultValue: { summary: 'Califica el producto' }, type: { summary: 'string' } },
			},
			totalResponses: {
			...
			averageLabel: {
				...
				table: { defaultValue: { summary: 'Promedio:' }, type: { summary: 'string' } },
			},
			score: {
			...
			leftLabel: {
				...
				table: { defaultValue: { summary: '0' }, type: { summary: 'string' } },
			},
			rightLabel: {
				control: { type: 'text' },
				description: 'Etiqueta derecha del gráfico',
				table: { defaultValue: { summary: '5' }, type: { summary: 'string' } },
			},
			titleIcon: {
				control: { type: 'text' },
	...
	n = {
		...
		render: (s) => {
			...
			(e.style.maxWidth = '400px'), (e.style.width = '100%');
			const a = H(s);
			return (e.innerHTML = a), t.appendChild(e), t;
		},
	};
n.parameters = {
	...n.parameters,
...
const k = ['Default'];
export { n as Default, k as __namedExportsOrder, P as default };
