import { c as o } from './ButtonAIProvider-CGOC_9a2.js';
...
const c = {
		...
		argTypes: {
			variant: {
				...
				table: { defaultValue: { summary: 'primary' }, type: { summary: 'primary | secondary' } },
			},
			size: {
				control: { type: 'select' },
			...
			text: {
				...
				table: { type: { summary: 'string' }, defaultValue: { summary: 'Button text' } },
			},
			icon: {
				control: { type: 'text' },
				description: 'Nombre del icono FontAwesome (sin prefijo fa-)',
				table: { type: { summary: 'string' }, example: { summary: 'sparkles, check, plus, etc.' } },
			},
			iconStyle: {
				control: { type: 'select' },
    ...
    `),
				requestAnimationFrame(() => {
					try {
						const e = o({ ...n, onClick: (a) => {} });
						e && t.appendChild(e);
					} catch (e) {
						console.error('Error creating ButtonAI:', e),
							(t.innerHTML = `<div style="color: var(--modifiers-normal-color-light-feedback-fg-error-subtle-default); padding: 16px;">Error: ${e instanceof Error ? e.message : String(e)}</div>`);
					}
				}),
				t
			);
...
const d = ['Default'];
export { r as Default, d as __namedExportsOrder, c as default };
