import { r as ae } from './AvatarProvider-CF4x-oFR.js';
import { c as re } from './InputProvider-B1iJaViU.js';
import { r as ne } from './ButtonProvider-Dmy-L2BE.js';
import { createScrollbar as Q } from './ScrollProvider-BVL7eCy8.js';
import { r as se } from './StatusTagProvider-6gx2PeGG.js';
import { c as ie } from './DrawerProvider-CjO9uG7_.js';
import { r as X } from './CheckboxProvider-DIr0OIhT.js';
import { r as Z } from './EmptyStateProvider-B1CNps4j.js';
...
const oe = {
	bajo: { status: 'completed', label: 'Bajo' },
	medio: { status: 'pending', label: 'Medio' },
	alto: { status: 'not-fulfilled', label: 'Alto' },
	'muy-alto': { status: 'denied', label: 'Muy alto' },
};
function j(t) {
...
function ce(t) {
	const c = { size: 'sm', alt: t.name, className: 'ubits-participants-menu__avatar' };
	return t.avatarImage ? (c.imageUrl = t.avatarImage) : (c.initials = t.name), ae(c);
}
function ee(t, c, I = !0, v = !0, A = !0) {
	const y = ['ubits-participants-menu__item', c ? 'ubits-participants-menu__item--selected' : '']
			.filter(Boolean)
			.join(' '),
		b = c
			? 'var(--modifiers-normal-color-light-accent-brand)'
			: 'var(--modifiers-normal-color-light-fg-1-high)',
		o = A && t.status ? le(t.status) : '',
		C = I ? ce(t) : '';
	return `
...
function Y(t) {
	const {
			title: c = 'Participantes',
			searchPlaceholder: I = 'Buscar participan...',
			participants: v = [],
		...
		E = ['ubits-participants-menu', y].filter(Boolean).join(' '),
		R = _ ? v : v.slice(0, 6),
		T = R.map((e) => {
			const s = e.id === A;
			return ee(e, s, b, o, C);
		...
		i = I,
		H = t.activeFilters || { roles: [], statuses: [] },
		l = (H.roles?.length || 0) + (H.statuses?.length || 0),
		N = {
			variant: 'secondary',
			size: 'md',
			icon: 'filter',
	...
	let w = ne(N);
	if (l > 0) {
		const e = `<span class="ubits-badge ubits-badge--sm ubits-badge--number ubits-badge--error ubits-button__badge">${l}</span>`;
		w = w.replace('<span class="ubits-button__badge"></span>', e);
	} else w = w.replace(/<span class="ubits-button__badge"><\/span>/g, '');
	const V = R.length === 0,
		D = t.searchTerm && t.searchTerm.trim() !== '',
		G = l > 0;
	let p = '';
	if (V) {
		let e;
		D
			...
			e &&
				(p = Z({
					title: j(e.title),
					description: e.description ? j(e.description) : void 0,
					icon: e.icon,
				}));
	}
	return `
...
function ue(t) {
	...
	let o = { roles: [], statuses: [] },
		C = '';
	const _ = Array.from(new Set(b.participants.map((e) => e.role))).sort(),
		E = ['bajo', 'medio', 'alto', 'muy-alto'],
		q = c ? `participants-menu-search-${c}` : `participants-menu-search-${Date.now()}`,
		R = () => Y({ ...b, searchInputId: q, activeFilters: o, searchTerm: C }),
	...
	const H = () => {
		let e = null;
		if (c) {
			if (((e = document.getElementById(c)), !e)) {
				console.error('❌ [ParticipantsMenu] No se encontró el contenedor con ID:', c);
				const r = document.querySelectorAll(`[id="${c}"]`);
				r.length > 0
					? (e = r[0])
		...
		const x = () => {
				if (a)
					try {
						...
						if ((console.error('❌ [ParticipantsMenu] Error al actualizar drawer:', r), a)) {
							a.element.remove(), (a = null), x();
							return;
						}
					}
				else
					try {
							...
							footerButtons: {
								...
								primary: {
									...
									onClick: (r) => {
										...
										const n = { roles: [], statuses: [] };
										a &&
											(_.forEach((u, P) => {
												const d = a.element.querySelector(`[data-filter-role="${u}"]`);
												if (d) {
													const S = d.querySelector('.ubits-checkbox__input');
													S && S.checked && n.roles.push(u);
												}
											}),
											E.forEach((u, P) => {
												const d = a.element.querySelector(`[data-filter-status="${u}"]`);
												if (d) {
													const S = d.querySelector('.ubits-checkbox__input');
													S && S.checked && n.statuses.push(u);
												}
											})),
											(o = n),
											w(),
						...
						console.error('❌ [ParticipantsMenu] Error al crear drawer:', r), A && A();
						return;
					}
				a &&
					(a.open(),
			...
			k = () => {
				...
				h.forEach((n) => {
					...
					(h = []),
					_.forEach((n, u) => {
						const P = `filter-role-${u}`,
							d = a.element.querySelector(`#${P}`);
						if (d) {
							d.innerHTML = '';
							const S = o.roles.includes(n),
								$ = X({ label: n, name: 'filter-role', value: n, checked: S, size: 'md' }),
								M = document.createElement('div');
							M.innerHTML = $.trim();
							const m = M.firstElementChild;
							if (m) {
								d.appendChild(m);
								const O = m.querySelector('.ubits-checkbox__input'),
									B = m.querySelector('.ubits-checkbox__square');
								O &&
									B &&
									O.addEventListener('change', (K) => {
										const W = K.target.checked;
										m.classList.toggle('ubits-checkbox--checked', W);
										let f = B.querySelector('.ubits-checkbox__checkmark');
										W
											? (f ||
											...
											: f && ((f.style.opacity = '0'), (f.style.transform = 'scale(0)'));
									}),
									h.push({
										element: m,
										destroy: () => {
				...
				const r = { bajo: 'Bajo', medio: 'Medio', alto: 'Alto', 'muy-alto': 'Muy Alto' };
				E.forEach((n, u) => {
					const P = `filter-status-${u}`,
						d = a.element.querySelector(`#${P}`);
					if (d) {
						d.innerHTML = '';
						const S = o.statuses.includes(n),
							$ = X({ label: r[n], name: 'filter-status', value: n, checked: S, size: 'md' }),
							M = document.createElement('div');
						M.innerHTML = $.trim();
						const m = M.firstElementChild;
						if (m) {
			...
			g = i.querySelector('.ubits-participants-menu__filter-button');
		g &&
			g.addEventListener('click', () => {
				A && A(), x();
			}),
			i.querySelectorAll('[data-participant-id]').forEach((r) => {
				const n = r.getAttribute('data-participant-id');
				r.addEventListener('click', () => {
					if (n && I) {
						i.querySelectorAll('.ubits-participants-menu__item--selected').forEach((S) => {
							S.classList.remove('ubits-participants-menu__item--selected');
							const $ = S.querySelector('.ubits-participants-menu__item-name');
							$ && ($.style.color = 'var(--modifiers-normal-color-light-fg-1-high)'),
								(S.style.backgroundColor = '');
						}),
							r.classList.add('ubits-participants-menu__item--selected');
						const P = r.querySelector('.ubits-participants-menu__item-name');
						P && (P.style.color = 'var(--modifiers-normal-color-light-accent-brand)'),
							(r.style.backgroundColor = 'var(--modifiers-normal-color-light-bg-active)'),
							i.querySelectorAll('.ubits-participants-menu__item--selected').length > 1;
						try {
							I(n);
						} catch (S) {
							console.error('❌ [ParticipantsMenu] Error al ejecutar onParticipantSelect:', S);
						}
					}
				});
			});
		let F = null;
		if (b.enableScrollbar !== !1) {
			const r = i.querySelector('[data-scrollable="true"]');
			if (r && r.id) {
				const n = i.querySelector('.ubits-participants-menu__list-wrapper');
				if (n) {
					const u = `participants-menu-scrollbar-${Date.now()}`;
          ...
          `),
						...
						n.appendChild(F),
						Q({ containerId: u, targetId: r.id, orientation: 'vertical' });
				}
			}
		} else {
			const r = i.querySelector('[data-scrollable="true"]');
			r && r.removeAttribute('data-ubits-scrollbar');
		}
	};
	let l = null,
		N = !1;
	const w = () => {
			const e = i.querySelector('.ubits-participants-menu__filter-button');
			if (!e) return;
			const s = (o.roles?.length || 0) + (o.statuses?.length || 0),
				a = e.querySelector('.ubits-button__badge');
			if (s > 0) {
				if (a)
					if (a.classList.contains('ubits-badge--number')) a.textContent = `${s}`;
					else {
						const h = `<span class="ubits-badge ubits-badge--sm ubits-badge--number ubits-badge--error ubits-button__badge">${s}</span>`,
							x = document.createElement('div');
						x.innerHTML = h;
						const k = x.firstElementChild;
						k && a.parentNode && a.parentNode.replaceChild(k, a);
					}
				else {
				...
				e.classList.add('ubits-button--active');
			} else a && a.remove(), e.classList.remove('ubits-button--active');
		},
		V = () => {
			setTimeout(() => {
				const e = i.querySelector(`#${q}`);
				if (e) {
					const s =
							e.getAttribute('data-search-placeholder') ||
							t.searchPlaceholder ||
							'Buscar participan...',
						a = t.preservedSearchValue || '',
						h = {
							containerId: q,
							type: 'search',
							size: 'md',
								...
								if (!N) {
									if (((C = x || ''), v))
										try {
											...
											console.error('[ParticipantsMenu] Error en onSearchChange:', g);
										}
									w();
								}
							},
					...
					a && (N = !0),
						(l = re(h)),
						l?.inputElement
							? (a && l.inputElement.value !== a && l.setValue(a),
								setTimeout(() => {
									N = !1;
								}, 150))
							: ((N = !1), console.error('[ParticipantsMenu] No se pudo crear input'));
				}
			}, 0);
		};
	c
		? requestAnimationFrame(() => {
				document.getElementById(c) ? H() : requestAnimationFrame(H);
			})
		: H();
	const D = (e, s) => {
		const a = i.querySelector('.ubits-participants-menu__list');
		if (!a) return;
		const x = b.enableScrollbar !== !1 ? e : e.slice(0, 6),
			k = x
				.map((n) => {
					const u = n.id === s;
					return ee(n, u, b.showAvatar !== !1, b.showRole !== !1, b.showStatusTag !== !1);
				})
				.join(''),
			g = x.length === 0,
			L = C && C.trim() !== '',
			F = (o.roles?.length || 0) + (o.statuses?.length || 0) > 0;
		let r = '';
		if (g) {
			let n;
			L
				...
				n &&
					(r = Z({
						title: j(n.title),
						description: n.description ? j(n.description) : void 0,
						icon: n.icon,
					}));
		}
		(a.innerHTML = g ? r : k),
			g ||
				i.querySelectorAll('[data-participant-id]').forEach((u) => {
					const P = u.getAttribute('data-participant-id'),
						d = u.cloneNode(!0);
					u.parentNode?.replaceChild(d, u),
						d.addEventListener('click', () => {
							if (P && I) {
								i.querySelectorAll('.ubits-participants-menu__item--selected').forEach((M) => {
									M.classList.remove('ubits-participants-menu__item--selected');
									const m = M.querySelector('.ubits-participants-menu__item-name');
									m && (m.style.color = 'var(--modifiers-normal-color-light-fg-1-high)'),
										(M.style.backgroundColor = '');
								}),
									d.classList.add('ubits-participants-menu__item--selected');
								const $ = d.querySelector('.ubits-participants-menu__item-name');
								$ && ($.style.color = 'var(--modifiers-normal-color-light-accent-brand)'),
									(d.style.backgroundColor = 'var(--modifiers-normal-color-light-bg-active)');
								try {
									I(P);
								} catch (M) {
									console.error('❌ [ParticipantsMenu] Error al ejecutar onParticipantSelect:', M);
								}
							}
						});
				}),
	...
	return {
		...
		update: (e) => {
			if (
				e.participants &&
				Object.keys(e).every((g) => g === 'participants' || g === 'selectedParticipantId') &&
				l
			) {
				D(e.participants, e.selectedParticipantId);
				return;
			}
			if (e.enableScrollbar !== void 0 && e.enableScrollbar !== b.enableScrollbar) {
				const g = i.querySelector('.ubits-participants-menu__list-wrapper');
				if (g) {
					const L = g.querySelector('[id^="participants-menu-scrollbar-"]');
					L && L.remove();
				}
			}
			const a = { ...b, ...e },
				h = Y({ ...a, searchInputId: q, activeFilters: o, searchTerm: C }),
				x = document.createElement('div');
			x.innerHTML = h;
			const k = x.firstElementChild;
			if (k && i.parentNode)
				if (
					(i.parentNode.replaceChild(k, i), Object.assign(i, k), V(), w(), a.enableScrollbar !== !1)
				) {
					const g = i.querySelector('[data-scrollable="true"]');
					if (g && g.id) {
						const L = i.querySelector('.ubits-participants-menu__list-wrapper');
						if (L) {
							const F = `participants-menu-scrollbar-${Date.now()}`,
            ...
            `),
								...
								L.appendChild(r),
								Q({ containerId: F, targetId: g.id, orientation: 'vertical' });
						}
					}
				} else {
					const g = i.querySelector('[data-scrollable="true"]');
					g && g.removeAttribute('data-ubits-scrollbar');
				}
		},
		updateParticipantsList: D,
		destroy: () => {
...
const Ce = {
		...
		argTypes: {
			...
			selectedParticipantId: {
				control: { type: 'select' },
				options: ['1', '2', '3', '4', '5', '6', '7', '8', '9', void 0],
				description: 'ID del participante seleccionado',
				table: { type: { summary: 'string' }, defaultValue: { summary: '1' }, category: 'Estado' },
			},
		},
	},
	J = [
    ...
    `;
			...
			I.style.cssText = 'max-width: 500px; width: 100%; margin: 0 auto;';
			const v = document.createElement('div'),
				A = `participants-menu-container-${Date.now()}`;
			(v.id = A),
				(v.style.cssText = `
      width: 100%;
      height: 100%;
    `),
			...
			const R = (l) =>
				JSON.stringify({
					...
					selectedParticipantId: l.selectedParticipantId,
					showAvatar: l.showAvatar,
					showRole: l.showRole,
					showStatusTag: l.showStatusTag,
				});
			let T = R(t);
			const z = () => {
				const l = v.querySelector('.ubits-participants-menu'),
					N = l ? l.querySelector('.ubits-input') : v.querySelector('.ubits-input');
				let w = 0,
					V = !1;
				N && ((y = N.value || ''), (w = N.selectionStart || 0), (V = document.activeElement === N)),
					(v.innerHTML = '');
				const D = (t.participants || J).map((p) => ({ ...p, selected: !1 })),
					G = {
						title: t.title || 'Participantes',
						searchPlaceholder: t.searchPlaceholder || 'Buscar participan...',
						participants: D,
						selectedParticipantId: t.selectedParticipantId,
						showAvatar: t.showAvatar !== void 0 ? t.showAvatar : !0,
						showRole: t.showRole !== void 0 ? t.showRole : !0,
						showStatusTag: t.showStatusTag !== void 0 ? t.showStatusTag : !0,
						enableScrollbar: t.enableScrollbar !== void 0 ? t.enableScrollbar : !0,
						preservedSearchValue: y,
						onParticipantSelect: (p) => {
							(b = !0), (t.selectedParticipantId = p), (T = R(t));
						},
						onSearchChange: (p) => {
							(y = p || ''),
								C && clearTimeout(C),
								(C = setTimeout(() => {
									...
									p &&
										p.trim() !== '' &&
										(e = e.filter((s) => {
											const a = s.name.toLowerCase().includes(p.toLowerCase()),
												h = s.role.toLowerCase().includes(p.toLowerCase());
											return a || h;
										})),
										q.roles.length > 0 && (e = e.filter((s) => q.roles.includes(s.role))),
										q.statuses.length > 0 &&
											(e = e.filter((s) => s.status && q.statuses.includes(s.status))),
										(t.participants = e),
										E?.updateParticipantsList
											? ((o = !0),
												E.updateParticipantsList(e, t.selectedParticipantId),
												(T = R(t)),
												setTimeout(() => {
													(_ = !1), (o = !1);
												}, 100))
						...
						onFilterChange: (p) => {
							q = p;
							let e = [...J];
							p.roles.length > 0 && (e = e.filter((s) => p.roles.includes(s.role))),
								p.statuses.length > 0 &&
									(e = e.filter((s) => s.status && p.statuses.includes(s.status))),
								y &&
									y.trim() !== '' &&
									(e = e.filter((s) => {
										const a = s.name.toLowerCase().includes(y.toLowerCase()),
											h = s.role.toLowerCase().includes(y.toLowerCase());
										return a || h;
									})),
								(t.participants = e),
								E?.updateParticipantsList
				...
				setTimeout(() => {
					try {
						const p = v.querySelector('.ubits-participants-menu');
						p && p.remove(),
							(E = ue(G)),
							V &&
								setTimeout(() => {
									const e = v.querySelector('.ubits-input');
									if (e) {
										e.focus();
										const s = e.value || '',
											a = Math.min(w, s.length);
										e.setSelectionRange(a, a);
									}
								}, 100);
					} catch (p) {
						console.error('[Story] Error:', p);
					}
				}, 0);
			};
			z();
...
const xe = ['Default'];
export { U as Default, xe as __namedExportsOrder, Ce as default };
