import { r as v } from './ButtonProvider-Dmy-L2BE.js';
function p(r) {
	const {
			title: l,
			complementaryText: i,
		...
		b = ['ubits-drawer', `ubits-drawer--width-${f}`, u].filter(Boolean).join(' '),
		n = `
...
function C(r) {
	const { containerId: l, onClose: i, closeOnOverlayClick: f = !0, open: d = !1 } = r;
	let o;
	l ? (o = document.getElementById(l) || document.body) : (o = document.body);
	const u = document.createElement('div');
	u.innerHTML = p(r);
	const e = u.firstElementChild;
	if (!e) throw new Error('No se pudo crear el drawer');
	e.querySelector('.ubits-drawer');
	const b = e.querySelector('.ubits-drawer__close'),
		n = e,
		_ = () => {
			e.classList.add('ubits-drawer-overlay--open'), (document.body.style.overflow = 'hidden');
		},
		a = () => {
			e.classList.remove('ubits-drawer-overlay--open'),
				(document.body.style.overflow = ''),
				i && i();
		},
		w = (t) => {
			const c = e.querySelector('.ubits-drawer__body-content');
			if (c) {
				const y = typeof t == 'function' ? t() : t;
				c.innerHTML = y;
			}
		};
	b &&
	...
	const m = (t) => {
		t.key === 'Escape' && e.classList.contains('ubits-drawer-overlay--open') && a();
	};
	if ((document.addEventListener('keydown', m), r.footerButtons)) {
		const t = e.querySelector('.ubits-drawer__footer-left .ubits-drawer__footer-button'),
			c = e.querySelector(
		...
		t &&
			r.footerButtons.tertiary?.onClick &&
			t.addEventListener('click', (s) => {
				s.preventDefault(), r.footerButtons.tertiary.onClick(s);
			}),
			c &&
				r.footerButtons.secondary?.onClick &&
				c.addEventListener('click', (s) => {
					s.preventDefault(), r.footerButtons.secondary.onClick(s);
				}),
			y &&
				r.footerButtons.primary?.onClick &&
				y.addEventListener('click', (s) => {
					s.preventDefault(), r.footerButtons.primary.onClick(s);
				});
	}
	return o.appendChild(e), d && _(), { element: e, open: _, close: a, updateContent: w };
}
export { C as c };
