...
function v(e) {
	return p[e] || p.template;
}
function h(e, o = 'regular') {
	const t = o === 'regular' ? 'far' : 'fas',
		i = e.startsWith('fa-') ? e : `fa-${e}`;
	return `<i class="${t} ${i}"></i>`;
}
function y(e) {
	const { variant: o = 'template', tabs: t, activeTabId: i, showIcons: n = !1 } = e,
		r = v(o),
		s = o === 'template' && t && t.length > 0 ? t : r.tabs,
		l = i || (s.length > 0 ? s[0].id : ''),
		d = s
			.map((a) => {
				const c = a.id === l || a.active ? 'ubits-sub-nav-tab--active' : '',
					m = n ? h(a.icon) : '';
				return `
      <button 
...
function T(e, o) {
	const t = e.querySelectorAll('.ubits-sub-nav-tab'),
		i = (n) => {
			const r = n.getAttribute('data-tab'),
				s = n.getAttribute('data-url');
			if (
				(t.forEach((c) => c.classList.remove('ubits-sub-nav-tab--active')),
				n.classList.add('ubits-sub-nav-tab--active'),
				s)
			) {
				window.location.href = s;
				return;
			}
			const l = v(o.variant || 'template'),
				a = (o.variant === 'template' && o.tabs && o.tabs.length > 0 ? o.tabs : l.tabs).find(
					(c) => c.id === r,
				);
			a && a.onClick && a.onClick(new MouseEvent('click')),
				o.onTabChange && o.onTabChange(r || '', n);
			const u = new CustomEvent('subNavTabClick', { detail: { tabId: r, tabElement: n } });
			document.dispatchEvent(u);
		};
	t.forEach((n) => {
		n.addEventListener('click', (r) => {
			r.preventDefault(), i(n);
		});
	});
}
function I(e) {
	const { containerId: o } = e,
		t = document.getElementById(o);
	if (!t) throw new Error(`Container with id "${o}" not found`);
	const i = y(e);
	t.innerHTML = i;
	const n = t.querySelector('.ubits-sub-nav');
	if (!n) throw new Error('Failed to create sub-nav element');
	return T(n, e), n;
}
const C = {
		...
		argTypes: {
			variant: {
				...
				table: { type: { summary: 'SubNavVariant' }, defaultValue: { summary: 'template' } },
			},
			activeTabId: {
				control: { type: 'text' },
	...
	b = {
		...
		render: (e) => {
			const o = document.getElementById(e.containerId || 'subnav-story-container');
			o && (o.innerHTML = '');
			const t = e.variant || 'template',
				i = v(t);
			let n = e.activeTabId;
			(!n || !i.tabs.find((a) => a.id === n)) && (n = i.tabs.length > 0 ? i.tabs[0].id : '');
    ...
    `;
			...
			(s.id = e.containerId || 'subnav-story-container'),
				(s.style.cssText = `
      width: 100%;
      margin-bottom: 24px;
    `),
				...
				requestAnimationFrame(() => {
					try {
						I({
							...
							variant: t,
							activeTabId: n,
							showIcons: e.showIcons ?? !1,
							onTabChange: (a, u) => {
								const m = v(t).tabs.find((f) => f.id === a),
									g = l.querySelector('div[style*="grid"]');
								if (g && m) {
									const f = g.querySelector('div:nth-child(2)');
									f && (f.innerHTML = `<strong>Tab Activo:</strong> ${m.label}`);
								}
							},
						});
					} catch (a) {
						console.error('Error creando SubNav:', a),
							(s.innerHTML = `<p style="color: var(--modifiers-normal-color-light-feedback-border-error); padding: 16px;">Error: ${a}</p>`);
					}
				}),
				r
			);
...
const w = ['Default'];
export { b as Default, w as __namedExportsOrder, C as default };
