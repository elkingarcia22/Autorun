import { c as v } from './popover-CvtG2qg5.js';
...
const I = {
		...
		argTypes: {
			...
			bodyContent: {
				control: { type: 'text' },
				description:
					'Contenido HTML del cuerpo del popover. Puede ser una cadena HTML o una función que devuelve HTML.',
				table: {
    ...
    `,
			...
			'footerButtons.tertiary.enabled': !0,
			'footerButtons.secondary.label': 'Secondary',
			'footerButtons.secondary.enabled': !0,
			'footerButtons.primary.label': 'Primary',
			'footerButtons.primary.enabled': !0,
			open: !0,
			closeOnOutsideClick: !0,
		},
		render: (e) => {
			const o = document.createElement('div');
			(o.id = 'popover-story-container'),
				(o.style.width = '100vw'),
				(o.style.height = '100vh'),
				(o.style.position = 'relative'),
				(o.style.overflow = 'hidden'),
				(o.style.background = 'var(--modifiers-normal-color-light-bg-2, #f9fafb)'),
				(o.style.display = 'flex'),
				(o.style.alignItems = 'center'),
				(o.style.justifyContent = 'center'),
				(o.style.padding = '40px');
			const r = document.createElement('button');
			(r.className = 'ubits-button ubits-button--primary ubits-button--md'),
				r.setAttribute('data-popover-trigger', 'true'),
				(r.innerHTML = '<span>Abrir Popover</span>'),
				(r.style.width = 'auto'),
				(r.style.zIndex = '1001'),
				o.appendChild(r);
			let t = null;
			const i = () => {
					const n = t && t.element.classList.contains('ubits-popover--open');
					r.querySelector('span').textContent = n ? 'Cerrar Popover' : 'Abrir Popover';
				},
				c = () => {
					if (t) {
						try {
					...
					document.querySelectorAll('.ubits-popover').forEach((a) => {
				...
				s = () => {
					c();
					const n = {
							...(e['footerButtons.tertiary.enabled'] && {
								tertiary: {
									label: e['footerButtons.tertiary.label'] || 'Tertiary',
									onClick: () => alert('Tertiary clicked!'),
								},
							}),
							...(e['footerButtons.secondary.enabled'] && {
								secondary: {
									label: e['footerButtons.secondary.label'] || 'Secondary',
									onClick: () => alert('Secondary clicked!'),
								},
							}),
							...(e['footerButtons.primary.enabled'] && {
								primary: {
									label: e['footerButtons.primary.label'] || 'Primary',
									onClick: () => alert('Primary clicked!'),
								},
							}),
						},
						a = r.getBoundingClientRect(),
						f = { top: a.top - 200 - 9, left: a.left + a.width / 2 };
					(t = v({
						title: e.title,
						width: e.width,
						tailPosition: e.tailPosition,
						tailOffset: e.tailOffset,
						bodyContent: e.bodyContent,
						footerButtons: Object.keys(n).length > 0 ? n : void 0,
						onClose: () => {
							e.onClose && e.onClose(), i();
						},
						open: !1,
						closeOnOutsideClick: e.closeOnOutsideClick !== void 0 ? e.closeOnOutsideClick : !0,
						position: f,
					})),
						t &&
							setTimeout(() => {
								const m = t.element.offsetHeight,
									p = r.getBoundingClientRect();
								t.updatePosition({ top: p.top - m - 9, left: p.left + p.width / 2 });
							}, 0),
						i();
				},
				u = (n) => {
					if (
						(n.preventDefault(), n.stopPropagation(), !t || !t.element || !t.element.parentElement)
					) {
						s(),
							setTimeout(() => {
								t && (t.open(), i());
					...
					t.element.classList.contains('ubits-popover--open') ? t.close() : t.open(), i();
				};
			r.addEventListener('click', u);
			const y = () => {
				const n = t && t.element && t.element.classList.contains('ubits-popover--open');
				c(),
					setTimeout(() => {
						s(),
							n &&
			...
			let d = JSON.stringify({
				...
				tailOffset: e.tailOffset,
				bodyContent: e.bodyContent,
				'footerButtons.tertiary.enabled': e['footerButtons.tertiary.enabled'],
				'footerButtons.tertiary.label': e['footerButtons.tertiary.label'],
				'footerButtons.secondary.enabled': e['footerButtons.secondary.enabled'],
				'footerButtons.secondary.label': e['footerButtons.secondary.label'],
				'footerButtons.primary.enabled': e['footerButtons.primary.enabled'],
				'footerButtons.primary.label': e['footerButtons.primary.label'],
			...
			return (
				o.addEventListener('DOMNodeRemoved', () => {
					clearInterval(b), c();
				}),
				o
			);
...
const x = ['Default'];
export { l as Default, x as __namedExportsOrder, I as default };
