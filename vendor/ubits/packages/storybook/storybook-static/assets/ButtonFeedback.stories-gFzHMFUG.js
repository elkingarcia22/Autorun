import { c as P } from './ModalProvider-Cwkz7B9R.js';
import { c as j } from './InputProvider-B1iJaViU.js';
import { c as H, r as V } from './ButtonProvider-Dmy-L2BE.js';
import { s as E } from './ToastProvider-CtZKLbxG.js';
import './iframe-BcZai5-G.js';
import './preload-helper-PPVm8Dsz.js';
import './ListProvider-Bx601uSc.js';
import './SpinnerProvider-o6XHV06V.js';
function Q(d, i = 'regular') {
	const b = i === 'regular' ? 'far' : 'fas',
		y = d.startsWith('fa-') ? d : `fa-${d}`;
	return `<i class="${b} ${y}"></i>`;
}
function N(d) {
	const {
		containerId: i,
		...
		defaultSection: p = '',
		commentPlaceholder:
			q = '¿Qué funciona bien? ¿Qué falta? ¿Qué mejorarías? ¿Qué necesita tu empresa?',
		n8nWebhookUrl: C,
		onFeedbackSent: B,
		visible: O = !0,
		className: _ = '',
	...
	let x;
	i ? (x = document.getElementById(i) || document.body) : (x = document.body);
	let h = p || (c.length > 0 ? c[0].value : ''),
		f = '',
		u = null,
		s = '';
	const w = () => {
			s = `ubits-button-feedback-form-${Math.random().toString(36).substr(2, 9)}`;
			const g = V({
					variant: 'secondary',
					size: 'sm',
					icon: 'times',
    ...
    `;
		...
		$ = () => {
			...
			document.querySelectorAll('.ubits-button-feedback-modal').forEach((t) => {
				const a = t.closest('.ubits-modal-overlay');
				...
				(u = P({
						...
						primary: {
							...
							onClick: async () => {
								const t = document.getElementById(`${s}-section`),
									a = document.getElementById(`${s}-comment`);
								if (t) {
									const o = t.querySelector('.ubits-input');
									o && (h = o.value);
								}
								if (a) {
									const o = a.querySelector('textarea');
									o && (f = o.value);
								}
								if (!f.trim()) {
									E('warning', 'Por favor, ingresa un comentario');
									return;
								}
								if (C)
									try {
										...
										B && B({ section: h, comment: f }),
											m(),
											E('success', '¡Gracias por tu feedback!');
									} catch (o) {
										console.error('Error enviando feedback:', o),
											E('error', 'Error al enviar el feedback. Por favor, intenta de nuevo.');
									}
								else m();
							},
						},
					...
					onClose: () => {
						e.classList.remove('ubits-button--active'), (u = null);
					},
				})),
				requestAnimationFrame(() => {
					requestAnimationFrame(() => {
						const t = u?.element,
							a = t?.querySelector('.ubits-button-feedback-modal');
						if (t && a && e) {
							(t.style.backgroundColor = 'transparent'), (t.style.pointerEvents = 'none');
							const o = a.querySelector('.ubits-modal__header');
							o && (o.style.display = 'none');
							const l = a.querySelector('.ubits-button-feedback-modal__close');
							l &&
							...
							(t.style.position = 'fixed'),
								...
								(t.style.justifyContent =
									r === 'bottom-right' || r === 'top-right' ? 'flex-end' : 'flex-start'),
								(t.style.paddingBottom = `${M.height + n + 16}px`),
								(t.style.paddingRight =
									r === 'bottom-right' || r === 'top-right' ? `${n}px` : 'auto'),
								(t.style.paddingLeft = r === 'bottom-left' || r === 'top-left' ? `${n}px` : 'auto'),
				...
				requestAnimationFrame(() => {
					requestAnimationFrame(() => {
						...
						if (t)
							try {
								const o = j({
									containerId: `${s}-section`,
									label: 'Sección actual:',
									type: 'select',
									size: 'md',
									value: h,
									selectOptions: c.map((l) => ({ value: l.value, text: l.text })),
									showLabel: !0,
									showHelper: !1,
									onChange: (l) => {
										h = l;
								...
								console.error('Error creando select de sección:', o);
							}
						if (a)
							try {
								const o = j({
									containerId: `${s}-comment`,
									label: 'Tu comentario:',
									type: 'textarea',
									size: 'md',
									value: f,
									...
									showLabel: !0,
									showHelper: !1,
									attributes: { rows: '6', style: 'resize: vertical; min-height: 120px;' },
									onChange: (l) => {
										f = l;
									},
								});
							} catch (o) {
								console.error('Error creando textarea de comentario:', o);
							}
					});
				});
		},
		m = () => {
			if (u) {
				...
				(u = null),
					...
					e.classList.remove('ubits-button--active');
			}
		},
		D = y || 'comment-dots',
		I = b || '',
		T = {
			...
			floating: !0,
			iconOnly: !I && !0,
			className: `ubits-button-feedback--${r} ${_}`.trim(),
			attributes: { 'aria-label': 'Deja tu feedback' },
			onClick: () => {
				$();
			},
		},
		e = H(T);
	T.floating &&
		!e.classList.contains('ubits-button--floating') &&
		e.classList.add('ubits-button--floating'),
			...
			: r === 'bottom-left'
				...
				: r === 'top-right'
					...
					: r === 'top-left' &&
						...
						(e.style.right = 'auto'));
	const L = () => {
			e.classList.remove('ubits-button-feedback--hidden');
		},
		F = () => {
			e.classList.add('ubits-button-feedback--hidden');
		},
		z = () => {
			$();
		},
	...
	return (
		x.appendChild(e), O || F(), { element: e, show: L, hide: F, open: z, close: m, destroy: A }
	);
}
const Z = {
		...
		argTypes: {
			...
			icon: {
				...
				table: { defaultValue: { summary: 'comment-dots' } },
			},
			position: {
				control: { type: 'select' },
			...
			modalTitle: {
				...
				table: { defaultValue: { summary: 'Deja tu Feedback' } },
			},
			sectionOptions: {
				control: { type: 'object' },
	...
	v = {
		args: {
			...
			defaultSection: 'home',
			commentPlaceholder:
				'¿Qué funciona bien? ¿Qué falta? ¿Qué mejorarías? ¿Qué necesita tu empresa?',
			n8nWebhookUrl: '',
			visible: !0,
		},
		render: (d) => {
    ...
    `;
			const b = () => {
				document.documentElement.getAttribute('data-theme') === 'dark'
					? ((i.style.backgroundColor = 'var(--modifiers-normal-color-dark-bg-1)'),
						(i.style.color = 'var(--modifiers-normal-color-dark-fg-1-high)'))
					: ((i.style.backgroundColor = 'var(--modifiers-normal-color-light-bg-1)'),
						(i.style.color = 'var(--modifiers-normal-color-light-fg-1-high)'));
			};
    ...
    `),
			...
			return (
				requestAnimationFrame(() => {
					try {
						k(),
							(n = N({ ...d, onFeedbackSent: (c) => {}, onCancel: () => {}, onClose: () => {} }));
					} catch (c) {
						console.error('Error creando botón de feedback:', c);
					}
				}),
				i
			);
...
const ee = ['Default'];
export { v as Default, ee as __namedExportsOrder, Z as default };
