...
function S(u) {
	const {
		...
		h = [
			...
			c === 'multi-color' ? 'ubits-progress-bar--multi-color' : '',
			b,
		]
			.filter(Boolean)
			.join(' ');
	...
	o !== void 0 &&
		o !== !1 &&
		(d = `<span class="ubits-progress-bar__indicator">${typeof o == 'string' ? o : `${Math.round(n)}%`}</span>`);
	let e = '';
	if (c === 'multi-color' && s.length > 0) {
		const t = s.reduce((r, i) => r + i.value, 0),
			m = Math.max(0, 100 - t),
			a = [...s];
		m > 0 && a.push({ value: m, color: 'gray' }),
			(e = `<div class="ubits-progress-bar__indicator-wrapper">${a
				.map((r, i) => {
					const p = r.value,
						v = g[r.color] || g.gray,
						$ = i === 0,
