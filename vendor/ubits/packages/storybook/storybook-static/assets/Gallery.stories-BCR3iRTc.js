import { r as _ } from './ButtonProvider-Dmy-L2BE.js';
...
function L(o) {
	...
	const v = [
			...
			`ubits-gallery--${a}`,
			`ubits-gallery--${s}`,
			r && 'ubits-gallery--thumbnails',
			l && 'ubits-gallery--lazy',
			e && 'ubits-gallery--lightbox',
			m,
		]
			.filter(Boolean)
			.join(' '),
		p = t
			.map((u, b) => $(u, b, { showThumbnails: r, lazyLoad: l, lightbox: e, aspectRatio: n }))
			.join(''),
		x = [
			`data-layout="${a}"`,
			`data-size="${s}"`,
			`data-columns="${g}"`,
			`data-gap="${c}"`,
			e && 'data-lightbox="true"',
			l && 'data-lazy="true"',
		]
			.filter(Boolean)
			.join(' '),
		i = `--gallery-gap: ${c}px; --gallery-columns: ${g};`;
	return `
...
function $(o, t, a) {
	const { id: s, image: g, thumbnail: c, title: r, description: l, alt: e } = o,
		n = a.showThumbnails && c ? c : g,
		m = e || r || `Imagen ${t + 1}`,
		d = a.lazyLoad ? 'loading="lazy"' : '',
		f = a.lightbox ? 'data-lightbox-item="true"' : '',
		h = a.aspectRatio ? `style="aspect-ratio: ${a.aspectRatio};"` : '';
	return `
...
function z(o, t) {
	const a = o.querySelectorAll('.ubits-gallery-item'),
		s = o.getAttribute('data-lightbox') === 'true',
		g = o.getAttribute('data-lazy') === 'true';
	if (
		a.length !== 0 &&
		(a.forEach((c, r) => {
			const l = c,
				e = l.getAttribute('data-item-id'),
				n = t.items.find((d) => String(d.id) === e);
			if (!n) return;
			l.addEventListener('click', (d) => {
				d.preventDefault(), t.onItemClick && t.onItemClick(n, r), s && A(n, t.items, r);
			});
			const m = l.querySelector('img');
			m &&
				(m.addEventListener('load', () => {
					t.onImageLoad && t.onImageLoad(n, r);
				}),
				m.addEventListener('error', () => {
					t.onImageError && t.onImageError(n, r), (m.style.display = 'none');
					const d = document.createElement('div');
					(d.className = 'ubits-gallery-item__error'),
						(d.innerHTML = '<i class="fas fa-image"></i><span>Error al cargar imagen</span>'),
						l.querySelector('.ubits-gallery-item__image-wrapper')?.appendChild(d);
				}));
		}),
		g && 'IntersectionObserver' in window)
	) {
		const c = new IntersectionObserver(
			(r) => {
				r.forEach((l) => {
					if (l.isIntersecting) {
						const e = l.target,
							n = e.getAttribute('data-full-image');
						n && e.src !== n && (e.src = n), c.unobserve(e);
					}
				});
			},
			{ rootMargin: '50px' },
...
function A(o, t, a) {
	...
	s.className = 'ubits-gallery-lightbox';
	const g = t
			.map((i, u) => {
				const b = u === a ? 'ubits-gallery-lightbox__thumbnail--active' : '',
					w = i.thumbnail || i.image;
				return `
    ...
    `;
		...
		r = _({
			...
			attributes: { 'aria-label': 'Anterior' },
		}),
		l = _({
			variant: 'secondary',
			size: 'md',
			...
			attributes: { 'aria-label': 'Siguiente' },
		});
	(s.innerHTML = `
  ...
  `),
	...
	let e = a;
	const n = s.querySelector('.ubits-gallery-lightbox__close'),
		m = () => {
	...
	const d = s.querySelector('.ubits-gallery-lightbox__prev'),
		f = s.querySelector('.ubits-gallery-lightbox__next'),
		h = s.querySelector('.ubits-gallery-lightbox__image'),
		v = s.querySelectorAll('.ubits-gallery-lightbox__thumbnail'),
		p = () => {
			const i = t[e];
			i &&
				h &&
				((h.src = i.image),
				(h.alt = i.alt || i.title || 'Imagen'),
				v.forEach((u, b) => {
					b === e
						? (u.classList.add('ubits-gallery-lightbox__thumbnail--active'),
							u.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'center' }))
						: u.classList.remove('ubits-gallery-lightbox__thumbnail--active');
				}));
		};
	v.forEach((i, u) => {
		...
		d?.addEventListener('click', (i) => {
			i.stopPropagation(), (e = e > 0 ? e - 1 : t.length - 1), p();
		}),
		f?.addEventListener('click', (i) => {
			i.stopPropagation(), (e = e < t.length - 1 ? e + 1 : 0), p();
		});
	const x = (i) => {
		i.key === 'Escape'
			? (m(), document.removeEventListener('keydown', x))
			: i.key === 'ArrowLeft'
				? ((e = e > 0 ? e - 1 : t.length - 1), p())
				: i.key === 'ArrowRight' && ((e = e < t.length - 1 ? e + 1 : 0), p());
	};
...
const S = [
	...
	j = {
		...
		argTypes: {
			items: {
				...
				table: { type: { summary: 'GalleryItem[]' } },
			},
			layout: {
				control: { type: 'select' },
				options: ['grid', 'masonry', 'list'],
				description: 'Layout de la galería',
				table: { defaultValue: { summary: 'grid' }, type: { summary: 'grid | masonry | list' } },
			},
			size: {
				control: { type: 'select' },
				options: ['xs', 'sm', 'md', 'lg', 'xl'],
				description: 'Tamaño de la galería',
				table: { defaultValue: { summary: 'md' }, type: { summary: 'xs | sm | md | lg | xl' } },
			},
			columns: {
			...
			gap: {
				...
				table: { defaultValue: { summary: '16' }, type: { summary: 'number (px)' } },
			},
			showThumbnails: {
	...
	y = {
		...
		render: (o) => {
			...
			return (a.style.width = '100%'), (a.style.margin = '0 auto'), a.appendChild(t), a;
		},
	};
y.parameters = {
	...y.parameters,
  ...
  render: args => {
    ...
    return container;
		...
		description: {
			story: 'Galería única con todos los controladores y tamaños',
			...y.parameters?.docs?.description,
		},
	},
};
const N = ['Default'];
export { y as Default, N as __namedExportsOrder, j as default };
