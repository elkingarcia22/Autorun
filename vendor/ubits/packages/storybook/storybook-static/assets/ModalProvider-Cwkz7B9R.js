const p = { sm: '320px', md: '480px', lg: '640px', xl: '800px', full: '1280px' };
function C(e) {
	const {
			title: d,
			bodyContent: l = '',
		...
		t = p[r] || p.md,
		b = ['ubits-modal', `ubits-modal--size-${r}`, f ? 'ubits-modal--full-screen' : '', c]
			.filter(Boolean)
			.join(' '),
		a = `
...
function L(e) {
	const { containerId: d, onClose: l, closeOnOverlayClick: r = !0, open: f = !1 } = e;
	let s;
	d ? (s = document.getElementById(d) || document.body) : (s = document.body);
	const c = document.createElement('div');
	c.innerHTML = C(e);
	const t = c.firstElementChild;
	if (!t) throw new Error('No se pudo crear el modal');
	t.querySelector('.ubits-modal');
	const y = t.querySelector('.ubits-modal__close'),
		u = t,
		b = () => {
		...
		_ = (o) => {
			const i = t.querySelector('.ubits-modal__body-content');
			if (i) {
	...
	if ((document.addEventListener('keydown', v), e.footerButtons)) {
		const o = t.querySelector('.ubits-modal__footer-left .ubits-modal__footer-button'),
			i = t.querySelector('.ubits-modal__footer-right .ubits-button--secondary'),
			m = t.querySelector('.ubits-modal__footer-right .ubits-button--primary');
		o &&
	...
	return s.appendChild(t), f && b(), { element: t, open: b, close: a, updateContent: _ };
}
export { L as c };
