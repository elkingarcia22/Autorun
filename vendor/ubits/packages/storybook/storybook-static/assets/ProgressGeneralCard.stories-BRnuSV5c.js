import { r as p } from './ButtonProvider-Dmy-L2BE.js';
import './iframe-BcZai5-G.js';
import './ListProvider-Bx601uSc.js';
import './SpinnerProvider-o6XHV06V.js';
import './preload-helper-PPVm8Dsz.js';
function E(e, r) {
	return r === 0 ? 0 : Math.round((e / r) * 100);
}
function y(e) {
	if (typeof window > 'u' || !window.document || !window.getComputedStyle) return e;
	try {
		const r = document.documentElement,
			a = e.replace(/var\(|\)/g, '').trim(),
			t = getComputedStyle(r).getPropertyValue(a).trim();
		return t ? t.replace(/[()]/g, '').trim() : e;
	} catch {
		return e;
	}
}
function H(
	...
	r = 120,
	a = 12,
	t = 'var(--modifiers-normal-color-light-accent-brand)',
	o = 'var(--modifiers-normal-color-light-bg-3)',
) {
	const n = (r - a) / 2,
		l = 2 * Math.PI * n,
		i = l - (e / 100) * l,
		s = r / 2,
		u = t.startsWith('var(') ? y(t) : t,
		d = o.startsWith('var(') ? y(o) : o;
	return `
    <svg 
...
function j(e, r = 'md') {
	const a = e.percentage ?? E(e.current, e.total),
		t =
			r === 'sm'
		...
		o =
			r === 'sm'
				...
				: r === 'lg'
					? 'ubits-body-md-regular'
					: 'ubits-body-sm-regular',
		n = r === 'sm' ? 'ubits-body-sm-bold' : 'ubits-body-md-bold';
	return `
...
function S(e) {
	const {
			...
			size: l = 'md',
			showTitle: i = !0,
			showCircularProgress: s = !0,
			showCategories: u = !0,
			showInfoIcon: d = !1,
			showActionButton: b = !1,
			progressColor: h = 'var(--ubits-chart-color-bg-neutral-blue-base)',
			circleBackgroundColor: f = 'var(--modifiers-normal-color-light-bg-3)',
			className: C = '',
			attributes: v = {},
		} = e,
		w = [
			'ubits-progress-general-card',
			`ubits-progress-general-card--${n}`,
			`ubits-progress-general-card--${l}`,
			C,
		]
			.filter(Boolean)
			.join(' '),
		$ = [...Object.entries(v).map(([g, k]) => `${g}="${k}"`)].filter(Boolean).join(' '),
		T = 158,
		L = 16,
		P = 'ubits-body-md-bold',
		_ = 'ubits-heading-h2',
		x = 'ubits-body-sm-bold',
		V = d
			? p({
					variant: 'tertiary',
					size: 'sm',
...
const G = {
	...
	argTypes: {
		title: {
			...
			table: { defaultValue: { summary: 'Progreso general' }, type: { summary: 'string' } },
		},
		mainPercentage: {
		...
		mainLabel: {
			...
			table: { defaultValue: { summary: 'Ciclos' }, type: { summary: 'string' } },
		},
		layout: {
			control: { type: 'select' },
		...
		category1Label: {
			...
			table: { defaultValue: { summary: 'Área' }, type: { summary: 'string' } },
		},
		category1Current: {
		...
		category1Total: {
			...
			table: { defaultValue: { summary: '20' }, type: { summary: 'number' } },
		},
		category2Label: {
			control: { type: 'text' },
			description: 'Etiqueta de la categoría 2',
			table: { defaultValue: { summary: 'Equipo' }, type: { summary: 'string' } },
		},
		category2Current: {
			control: { type: 'number', min: 0, step: 1 },
		...
		category3Label: {
			...
			table: { defaultValue: { summary: 'Propio' }, type: { summary: 'string' } },
		},
		category3Current: {
			control: { type: 'number', min: 0, step: 1 },
			description: 'Valor actual de la categoría 3',
			table: { defaultValue: { summary: '5' }, type: { summary: 'number' } },
		},
		category3Total: {
			control: { type: 'number', min: 1, step: 1 },
...
function z(e) {
	...
	return (
		e.category1Label !== void 0 &&
			r.push({
				label: e.category1Label || 'Área',
				current: e.category1Current ?? 3,
				total: e.category1Total ?? 20,
			}),
		e.category2Label !== void 0 &&
			r.push({
				label: e.category2Label || 'Equipo',
				current: e.category2Current ?? 8,
				total: e.category2Total ?? 50,
			}),
		e.category3Label !== void 0 &&
			r.push({
				label: e.category3Label || 'Propio',
				current: e.category3Current ?? 5,
				total: e.category3Total ?? 30,
			}),
		r
	);
}
const c = {
	...
	render: (e) => {
		const r = z(e),
			a = {
				title: e.title,
				mainPercentage: e.mainPercentage ?? 50,
				mainLabel: e.mainLabel,
				categories: r,
				layout: e.layout,
				size: e.size,
				showTitle: e.showTitle,
				showCircularProgress: e.showCircularProgress,
		...
		(t.style.display = 'flex'),
			...
			(t.style.padding = '48px'),
			(t.style.background = 'var(--modifiers-normal-color-light-bg-2)'),
			(t.style.borderRadius = '8px'),
			(t.style.minHeight = '400px');
		const o = document.createElement('div');
		a.layout === 'horizontal'
			? ((o.style.maxWidth = '800px'), (o.style.width = '100%'))
			: ((o.style.maxWidth = '500px'), (o.style.width = '100%'));
		const n = S(a);
		return (o.innerHTML = n), t.appendChild(o), t;
	},
};
c.parameters = {
	...c.parameters,
...
const R = ['Default'];
export { c as Default, R as __namedExportsOrder, G as default };
