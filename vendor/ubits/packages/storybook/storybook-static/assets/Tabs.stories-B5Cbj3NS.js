...
function p(r, a = !1) {
	if (!r) return '';
	let n = r;
	n.startsWith('fa-') || (n = `fa-${n}`);
	const i = a ? 'fas' : 'far';
	if (n.startsWith('far ') || n.startsWith('fas ')) {
		const s = n.replace(/^(far|fas)\s+/, '');
		return `<i class="${i} ${s}"></i>`;
	}
	return `<i class="${i} ${n}"></i>`;
}
function v(r) {
	const { tabs: a, activeTabId: n, className: i = '' } = r;
	if (!a || a.length === 0) return '<div class="ubits-tabs"></div>';
	let s = n;
	if (!s) {
		const o = a.find((t) => t.active);
		s = o ? o.id : a[0].id;
	}
	const e = a
		.map((o) => {
			const t = o.id === s,
				d = t ? 'ubits-tab--active' : '',
				c = o.disabled ? 'ubits-tab--disabled' : '',
				u = ['ubits-tab', d, c].filter(Boolean).join(' '),
				g = o.icon ? p(o.icon, t) : '';
			return `
      <button 
        class="${u}" 
...
function m(r, a) {
	r.querySelectorAll('.ubits-tab[data-listener-attached]').forEach((e) => {
		const l = e.cloneNode(!0);
		e.parentNode?.replaceChild(l, e);
	});
	const i = r.querySelectorAll('.ubits-tab:not(.ubits-tab--disabled)'),
		s = (e) => {
			const l = e.getAttribute('data-tab-id'),
				o = e.getAttribute('data-url');
			if (
				(r.querySelectorAll('.ubits-tab').forEach((c) => {
					c.classList.remove('ubits-tab--active');
				}),
				e.classList.add('ubits-tab--active'),
				o)
			) {
				window.location.href = o;
				return;
			}
			const t = a.tabs.find((c) => c.id === l);
			t && t.onClick && t.onClick(new MouseEvent('click')),
				a.onTabChange && a.onTabChange(l || '', e);
			const d = new CustomEvent('tabsTabClick', { detail: { tabId: l, tabElement: e } });
...
function h(r, a) {
	const n = (a && document.getElementById(a)) || document.createElement('div');
	return (
		a && !n.id && (n.id = a),
		(n.innerHTML = v(r)),
		requestAnimationFrame(() => {
			const i = n.querySelector('.ubits-tabs');
			m(i || n, r);
		}),
		n
	);
...
function f(r = 5, a = !0) {
	...
	return Array.from({ length: r }, (s, e) => ({
		id: `tab-${e + 1}`,
		label: i[e] || `Label ${e + 1}`,
		icon: a ? `far ${n[e] || 'fa-th'}` : void 0,
		active: e === 0,
	}));
}
const b = {
	args: { tabs: f(5, !0), activeTabId: 'tab-1', showIcons: !0, tabCount: 5 },
	render: (r) => {
		const a = r.showIcons !== !1,
			n = f(r.tabCount || 5, a),
			i = r.activeTabId || n[0]?.id;
		n.forEach((t) => {
			t.active = t.id === i;
		});
		...
		return (
			...
			requestAnimationFrame(() => {
				try {
					...
					console.error('Error creando Tabs:', t),
						(e.innerHTML = `<p style="color: var(--modifiers-normal-color-light-feedback-border-error); padding: var(--p-spacing-mode-1-lg, 16px);">Error: ${t}</p>`);
				}
			}),
			s
		);
...
const I = ['Default'];
export { b as Default, I as __namedExportsOrder, x as default };
