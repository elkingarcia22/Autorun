import { c as T, a as A } from './utils-QDDdDBjb.js';
const G = {
		...
		parameters: {
			...
			docs: {
				description: {
					component:
						'Tokens específicos de componentes del sistema UBITS. Incluye tokens para botones (button-tone), scroll bar y toggle. Los tokens de button-tone usan modificadores con valores correctos.',
				},
			},
		},
	},
...
function q() {
	let t = 0;
	return (
		Object.values(o['button-tone']).forEach((e) => {
			Object.values(e).forEach((r) => {
				t += r.light.length + r.dark.length;
			});
		}),
		t
	);
...
const L = {
		render: () => {
			...
			(t.style.padding = '24px'), (t.style.maxWidth = '1400px');
			const e = document.createElement('h2');
			(e.textContent = 'Componentes - Todos los Tokens'),
				(e.style.fontSize = '24px'),
				(e.style.fontWeight = '700'),
				(e.style.marginBottom = '16px'),
				t.appendChild(e);
			const r = q(),
				a = o['ai-button'].light.length + o['ai-button'].dark.length,
				g = r + a + o['scroll-bar'].length + o.toggle.length,
				n = document.createElement('div');
			(n.style.marginBottom = '24px'),
				(n.style.padding = '16px'),
				(n.style.backgroundColor = '#f3f4f6'),
				(n.style.border = '1px solid #d1d5db'),
				(n.style.borderRadius = '8px'),
				(n.style.fontSize = '14px');
			const d = Object.keys(o['button-tone'])
				.map((f) => {
					const C = o['button-tone'][f],
						S = (C.primary?.light.length || 0) + (C.primary?.dark.length || 0),
						b = (C.secondary?.light.length || 0) + (C.secondary?.dark.length || 0),
						P = (C.tertiary?.light.length || 0) + (C.tertiary?.dark.length || 0);
					return `${f}: ${S + b + P} (primary: ${S}, secondary: ${b}, tertiary: ${P})`;
				})
				.join(', ');
			(n.innerHTML = `
      <strong>Resumen:</strong><br>
      • Button Tone: ${r} tokens (${d})<br>
      • AI Button: ${a} tokens (${o['ai-button'].light.length} Light + ${o['ai-button'].dark.length} Dark)<br>
      • Scroll Bar: ${o['scroll-bar'].length} tokens<br>
      • Toggle: ${o.toggle.length} tokens<br>
      <strong>Total: ${g} tokens</strong>
    `),
				t.appendChild(n);
			const i = document.createElement('div');
			i.style.marginBottom = '40px';
			const l = document.createElement('h3');
			(l.textContent = `Button Tone (${r} tokens)`),
				(l.style.fontSize = '20px'),
				(l.style.fontWeight = '600'),
				(l.style.marginBottom = '16px'),
				(l.style.paddingBottom = '8px'),
				(l.style.borderBottom = '2px solid #e5e7eb'),
				i.appendChild(l),
				Object.entries(o['button-tone']).forEach(([f, C]) => {
					const S = document.createElement('div');
					S.style.marginBottom = '32px';
					const b = document.createElement('h4'),
						P = Object.values(C).reduce((R, v) => R + v.light.length + v.dark.length, 0);
					(b.textContent = `${f.charAt(0).toUpperCase() + f.slice(1)} (${P} tokens)`),
						(b.style.fontSize = '18px'),
						(b.style.fontWeight = '600'),
						(b.style.marginBottom = '16px'),
						(b.style.paddingBottom = '8px'),
						(b.style.borderBottom = '1px solid #e5e7eb'),
						S.appendChild(b),
						Object.entries(C).forEach(([R, v]) => {
							const j = document.createElement('div');
							j.style.marginBottom = '24px';
							const D = document.createElement('h5');
							(D.textContent = `${R.charAt(0).toUpperCase() + R.slice(1)} (${v.light.length} Light + ${v.dark.length} Dark)`),
							...
							(N.style.display = 'grid'),
								(N.style.gridTemplateColumns = '1fr 1fr'),
								(N.style.gap = '16px');
							...
							const $ = document.createElement('h6');
							($.textContent = 'Light Mode'),
								...
								O.appendChild($),
								v.light.forEach((I) => {
									O.appendChild(T(I, 'light', { showVariable: !0, showValue: !0, width: '100%' }));
								}),
								N.appendChild(O);
							const k = document.createElement('div');
							(k.style.background = '#0E1825'),
								(k.style.color = '#edeeef'),
								(k.style.border = '1px solid #0E1825'),
								(k.style.borderRadius = '10px'),
								(k.style.padding = '16px');
							const w = document.createElement('h6');
							(w.textContent = 'Dark Mode'),
								(w.style.fontSize = '14px'),
								(w.style.fontWeight = '600'),
								(w.style.marginBottom = '12px'),
								(w.style.color = '#edeeef'),
								k.appendChild(w),
								v.dark.forEach((I) => {
									k.appendChild(T(I, 'dark', { showVariable: !0, showValue: !0, width: '100%' }));
								}),
								N.appendChild(k),
								j.appendChild(N),
								S.appendChild(j);
			...
			(c.textContent = `AI Button (${a} tokens)`),
				(c.style.fontSize = '20px'),
				(c.style.fontWeight = '600'),
				(c.style.marginBottom = '16px'),
				(c.style.paddingBottom = '8px'),
				(c.style.borderBottom = '2px solid #e5e7eb'),
				h.appendChild(c);
			const m = document.createElement('div');
			(m.style.display = 'grid'), (m.style.gridTemplateColumns = '1fr 1fr'), (m.style.gap = '16px');
			...
			(p.textContent = 'Light Mode'),
				...
				u.appendChild(p),
				o['ai-button'].light.forEach((f) => {
					u.appendChild(T(f, 'light', { showVariable: !0, showValue: !0, width: '100%' }));
				}),
				m.appendChild(u);
			const s = document.createElement('div');
			(s.style.background = '#0E1825'),
				(s.style.color = '#edeeef'),
				(s.style.border = '1px solid #0E1825'),
				(s.style.borderRadius = '10px'),
				(s.style.padding = '16px');
			const y = document.createElement('h5');
			(y.textContent = 'Dark Mode'),
				(y.style.fontSize = '14px'),
				(y.style.fontWeight = '600'),
				(y.style.marginBottom = '12px'),
				(y.style.color = '#edeeef'),
				s.appendChild(y),
				o['ai-button'].dark.forEach((f) => {
					s.appendChild(T(f, 'dark', { showVariable: !0, showValue: !0, width: '100%' }));
				}),
				m.appendChild(s),
				h.appendChild(m),
				t.appendChild(h);
			const _ = document.createElement('div');
			_.style.marginBottom = '40px';
			const B = document.createElement('h3');
			(B.textContent = `Scroll Bar (${o['scroll-bar'].length} tokens)`),
				(B.style.fontSize = '20px'),
				(B.style.fontWeight = '600'),
				(B.style.marginBottom = '16px'),
				(B.style.paddingBottom = '8px'),
				(B.style.borderBottom = '2px solid #e5e7eb'),
				_.appendChild(B);
			const U = A(o['scroll-bar'], 'Scroll Bar');
			_.appendChild(U), t.appendChild(_);
			const K = document.createElement('div');
			K.style.marginBottom = '40px';
			const E = document.createElement('h3');
			(E.textContent = `Toggle (${o.toggle.length} tokens)`),
				(E.style.fontSize = '20px'),
				(E.style.fontWeight = '600'),
				(E.style.marginBottom = '16px'),
				(E.style.paddingBottom = '8px'),
				(E.style.borderBottom = '2px solid #e5e7eb'),
				K.appendChild(E);
			const H = A(o.toggle, 'Toggle');
			return K.appendChild(H), t.appendChild(K), t;
		},
	},
	W = {
		render: () => {
			...
			const e = document.createElement('h2');
			return (
				(e.textContent = 'Button Tone'),
				(e.style.fontSize = '24px'),
				(e.style.fontWeight = '700'),
				(e.style.marginBottom = '16px'),
				t.appendChild(e),
				Object.entries(o['button-tone']).forEach(([r, a]) => {
					const g = document.createElement('div');
					g.style.marginBottom = '32px';
					const n = document.createElement('h3'),
						x = Object.values(a).reduce((d, i) => d + i.light.length + i.dark.length, 0);
					(n.textContent = `${r.charAt(0).toUpperCase() + r.slice(1)} (${x} tokens)`),
						(n.style.fontSize = '20px'),
						(n.style.fontWeight = '600'),
						(n.style.marginBottom = '16px'),
						(n.style.paddingBottom = '8px'),
						(n.style.borderBottom = '2px solid #e5e7eb'),
						g.appendChild(n),
						Object.entries(a).forEach(([d, i]) => {
							const l = document.createElement('div');
							l.style.marginBottom = '24px';
							const h = document.createElement('h4');
							(h.textContent = `${d.charAt(0).toUpperCase() + d.slice(1)} (${i.light.length} Light + ${i.dark.length} Dark)`),
							...
							(u.textContent = 'Light Mode'),
								(u.style.fontSize = '16px'),
								(u.style.fontWeight = '600'),
								(u.style.marginBottom = '12px'),
								m.appendChild(u),
								i.light.forEach((y) => {
									m.appendChild(T(y, 'light', { showVariable: !0, showValue: !0, width: '100%' }));
								}),
								c.appendChild(m);
							const p = document.createElement('div');
							(p.style.background = '#0E1825'),
								(p.style.color = '#edeeef'),
								(p.style.border = '1px solid #0E1825'),
							...
							(s.textContent = 'Dark Mode'),
								...
								p.appendChild(s),
								i.dark.forEach((y) => {
									p.appendChild(T(y, 'dark', { showVariable: !0, showValue: !0, width: '100%' }));
								}),
								c.appendChild(p),
								l.appendChild(c),
								g.appendChild(l);
	...
	z = {
		render: () => {
			...
			(t.style.padding = '24px'), (t.style.maxWidth = '1400px');
			const e = document.createElement('h2');
			(e.textContent = 'AI Button'),
				(e.style.fontSize = '24px'),
				(e.style.fontWeight = '700'),
				(e.style.marginBottom = '16px'),
				t.appendChild(e);
			const r = o['ai-button'].light.length + o['ai-button'].dark.length,
				a = document.createElement('div');
			(a.style.marginBottom = '16px'),
				(a.style.fontSize = '16px'),
				(a.style.fontWeight = '600'),
				(a.textContent = `Total: ${r} tokens (${o['ai-button'].light.length} Light + ${o['ai-button'].dark.length} Dark)`),
				t.appendChild(a);
			const g = document.createElement('div');
			...
			(x.textContent = 'Light Mode'),
				...
				n.appendChild(x),
				o['ai-button'].light.forEach((l) => {
					n.appendChild(T(l, 'light', { showVariable: !0, showValue: !0, width: '100%' }));
				}),
				g.appendChild(n);
			...
			return (
				(i.textContent = 'Dark Mode'),
				(i.style.fontSize = '18px'),
				(i.style.fontWeight = '600'),
				(i.style.marginBottom = '12px'),
				(i.style.color = '#edeeef'),
				d.appendChild(i),
				o['ai-button'].dark.forEach((l) => {
					d.appendChild(T(l, 'dark', { showVariable: !0, showValue: !0, width: '100%' }));
				}),
				g.appendChild(d),
				t.appendChild(g),
				t
	...
	M = {
		render: () => {
			...
			(r.style.marginBottom = '16px'),
				...
				(r.textContent = `Total: ${o['scroll-bar'].length} tokens`),
				t.appendChild(r);
			const a = A(o['scroll-bar'], 'Scroll Bar');
	...
	V = {
		render: () => {
			...
			(r.style.marginBottom = '16px'),
				(r.style.fontSize = '16px'),
				(r.style.fontWeight = '600'),
				(r.textContent = `Total: ${o.toggle.length} tokens`),
				t.appendChild(r);
			const a = A(o.toggle, 'Toggle');
			return t.appendChild(a), t;
		},
	};
L.parameters = {
  ...
  render: () => {
    ...
    COMPONENT_TOKENS['ai-button'].light.forEach(token => {
      aiButtonLightCol.appendChild(createColorSwatch(token, 'light', {
        showVariable: true,
        showValue: true,
        width: '100%'
      }));
    ...
    COMPONENT_TOKENS['ai-button'].dark.forEach(token => {
      aiButtonDarkCol.appendChild(createColorSwatch(token, 'dark', {
        showVariable: true,
        showValue: true,
        width: '100%'
      }));
    ...
    return container;
		...
		description: {
			story: 'Story principal que muestra todos los componentes',
...
W.parameters = {
  ...
  render: () => {
    ...
    return container;
		...
		description: { story: 'Story para Button Tone', ...W.parameters?.docs?.description },
	},
};
z.parameters = {
	...z.parameters,
  ...
  render: () => {
    ...
    return container;
		...
		description: { story: 'Story para AI Button', ...z.parameters?.docs?.description },
	},
};
M.parameters = {
	...M.parameters,
  ...
  render: () => {
    ...
    return container;
		...
		description: { story: 'Story para Scroll Bar', ...M.parameters?.docs?.description },
	},
};
V.parameters = {
	...V.parameters,
  ...
  render: () => {
    ...
    return container;
		...
		description: { story: 'Story para Toggle', ...V.parameters?.docs?.description },
	},
};
const J = ['TodosLosComponentes', 'ButtonTone', 'AIButton', 'ScrollBar', 'Toggle'];
export {
	z as AIButton,
	W as ButtonTone,
	M as ScrollBar,
	L as TodosLosComponentes,
	V as Toggle,
