...
const T = {
	...
	parameters: {
		docs: {
			description: {
				component:
					'Página de bienvenida para tests de prototipos UBITS. Incluye múltiples variaciones de diseño con diferentes layouts, posiciones de imagen, alineaciones y estilos.',
			},
		},
		layout: 'fullscreen',
	},
	argTypes: {
		...
		buttonAlignment: {
			...
			table: { defaultValue: { summary: 'left' }, type: { summary: 'left | center | right' } },
		},
		showBanner: {
		...
		selectedImage: {
			control: { type: 'select', labels: w() },
			options: b(),
			description: 'Seleccionar imagen de la galería (50 imágenes disponibles)',
			table: { defaultValue: { summary: '1' }, type: { summary: 'string' }, category: 'Imagen' },
		},
	},
};
function v() {
...
const m = new Map(),
	s = {
		...
		render: (t) => {
			const a = `welcome-instance-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
				i = document.createElement('div');
			(i.id = 'welcome-story-container'),
				i.setAttribute('data-instance-id', a),
				(i.style.cssText = `
      width: 100%;
      height: 100vh;
      position: relative;
    ...
    `),
				...
				m.set(a, { iframe: o, currentConfig: null }),
				(() => {
					if (!m.get(a)) return;
					const d = v();
					(!o.src || !o.src.includes(d)) && (o.src = d);
					const p = () => {
							try {
								const e = o.contentWindow?.document;
								if (e) {
									const r = document.body.getAttribute('data-theme') || 'light';
									e.body.setAttribute('data-theme', r),
										e.documentElement.setAttribute('data-theme', r);
								}
							} catch {}
						},
					...
					u.observe(document.body, { attributes: !0, attributeFilter: ['data-theme'] }),
							...
							p(),
								setTimeout(() => {
									try {
										...
										if (n && n.WelcomePageConfig) {
											const e = n.WelcomePageConfig;
											(e.banner.show = t.showBanner),
												(e.infoBox.show = t.showInfoBox),
												(e.image.size = t.imageSize);
											const r = y(t.selectedImage || '1'),
												f =
													r?.url ||
													'https://images.unsplash.com/photo-1522202176988-66273c2fd55f?w=800&h=1200&fit=crop&auto=format',
												g = r?.name || 'Prototipo UBITS';
											t.layout === 'image-right'
												? ((e.layout.imagePosition = 'right'),
													(e.image.show = !0),
													(e.image.src = f),
													(e.image.alt = g))
												: t.layout === 'image-left'
													? ((e.layout.imagePosition = 'left'),
														(e.image.show = !0),
														(e.image.src = f),
														(e.image.alt = g))
													: ((e.layout.imagePosition = 'none'), (e.image.show = !1)),
												(e.layout.textAlignment = t.textAlignment),
												(e.layout.buttonAlignment = t.buttonAlignment);
											const c = n.document.getElementById('welcome-container');
											c &&
												((c.className = 'welcome-container'),
												t.containerStyle === 'compact'
													? c.classList.add('welcome-container--compact')
													: t.containerStyle === 'wide'
														? c.classList.add('welcome-container--wide')
														: t.containerStyle === 'minimal' &&
															c.classList.add('welcome-container--minimal')),
												n.initializeWelcomePage && n.initializeWelcomePage(e);
										}
									} catch (n) {
										console.error('Error configuring welcome page:', n);
									}
								}, 500);
						}),
						(o.onerror = () => {
        ...
        `);
			...
			return (
				(i.remove = function () {
					themeObserver.disconnect(), m.delete(a), h.call(this);
				}),
				i
			);
		},
...
const A = ['WelcomePage'];
export { s as WelcomePage, A as __namedExportsOrder, T as default };
