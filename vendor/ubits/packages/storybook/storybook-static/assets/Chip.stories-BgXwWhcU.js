function m(e = {}) {
	const {
		...
		p = [
			'ubits-chip',
			`ubits-chip--${c}`,
			t !== 'default' ? `ubits-chip--${t}` : '',
			o ? 'ubits-chip--clickable' : '',
			l ? 'ubits-chip--closable' : '',
			d,
		]
			.filter(Boolean)
			.join(' '),
		u = s
			? `<span class="ubits-chip__left-icon" aria-hidden="true"><i class="far fa-${s}"></i></span>`
			: '',
		b =
			l || a
				? `<button class="ubits-chip__right-icon" type="button" aria-label="Cerrar chip" ${t === 'disabled' ? 'disabled' : ''}>
...
const g = {
	...
	argTypes: {
		...
		rightIcon: {
			control: { type: 'text' },
			description:
				'Icono FontAwesome derecho para el botón de cerrar (ej: "xmark"). Se usa si closable es true.',
			table: {
				type: { summary: 'string | null' },
		...
		closable: {
			...
			table: { defaultValue: { summary: 'false' }, category: 'Comportamiento' },
		},
		clickable: {
			control: { type: 'boolean' },
...
function f(e, n) {
	const c = parseInt(e.slice(1, 3), 16),
		t = parseInt(e.slice(3, 5), 16),
...
const r = {
	...
	render: (e) => {
		...
		const c = m({
			...
			closable: e.closable !== void 0 ? e.closable : !1,
			clickable: e.clickable !== void 0 ? e.clickable : !1,
			className: e.className || '',
		});
		n.innerHTML = c;
		const t = n.querySelector('.ubits-chip');
		if (t && e.state === 'focus') {
			const a = '#5297F4',
				o = f(a, 0.3);
			(t.style.border = `2px solid var(--modifiers-static-inverted-color-light-accent-brand, ${a})`),
				(t.style.boxShadow = `0px 0px 0px 4px ${o}`),
		...
		const s = n.querySelector('.ubits-chip__right-icon');
		return (
			s &&
				e.onClose &&
				s.addEventListener('click', (a) => {
					a.preventDefault(), a.stopPropagation(), e.onClose && e.onClose(a);
				}),
			n
		);
	},
...
const x = ['Default'];
export { r as Default, x as __namedExportsOrder, g as default };
