import { r as A } from './simple-card-CrYdjRkL.js';
...
function P(e) {
	const {
		...
		loop: d = !1,
		gap: v = 16,
		arrowPosition: I = 'outside',
		dotPosition: z = 'bottom',
		className: h = '',
		onItemClick: s,
		onSlideChange: y,
	} = e;
	...
	const S = [
			'ubits-carousel',
			I === 'inside' && 'ubits-carousel--arrows-inside',
			z === 'top' && 'ubits-carousel--dots-top',
			h,
		]
			.filter(Boolean)
			.join(' '),
		V = i.map((C, g) => x(C, g)).join(''),
		b = c ? H(i.length, l) : '',
		f = [
			`data-items-per-view="${l}"`,
			n && 'data-autoplay="true"',
			n && `data-autoplay-interval="${u}"`,
			d && 'data-loop="true"',
			`data-gap="${v}"`,
		]
			.filter(Boolean)
...
function H(e, i) {
	const l = Math.ceil(e / i);
	return `<div class="ubits-carousel__dots">${Array.from(
		{ length: l },
		(c, n) => `
...
function k(e, i) {
	const l = e.querySelector('.ubits-carousel__track'),
		t = e.querySelectorAll('.ubits-carousel-item'),
		c = e.querySelector('.ubits-carousel__arrow--prev'),
		n = e.querySelector('.ubits-carousel__arrow--next'),
		u = e.querySelectorAll('.ubits-carousel__dot');
	if (!l || t.length === 0) return;
	const d = parseInt(e.getAttribute('data-items-per-view') || '3'),
		v = parseInt(e.getAttribute('data-gap') || '16'),
		I = e.getAttribute('data-autoplay') === 'true',
		z = parseInt(e.getAttribute('data-autoplay-interval') || '3000'),
		h = e.getAttribute('data-loop') === 'true';
	let s = 0,
		y = null;
	const S = () => {
		let o = 0;
		t.forEach((a) => {
			const r = a.querySelector('.ubits-simple-card');
			if (r) {
				(a.style.width = 'auto'), (a.style.minWidth = '0'), (a.style.flexShrink = '0');
				const m = r.offsetHeight || r.getBoundingClientRect().height;
				m > o && (o = m);
			}
		}),
			o > 0 &&
	...
	const V = () => {
			...
			const a = t[0].querySelector('.ubits-simple-card');
			if (a && a.offsetWidth > 0) return a.offsetWidth + v;
			if (a) {
				const m = a.getBoundingClientRect();
				if (m.width > 0) return m.width + v;
			}
			const r = e.querySelector('.ubits-carousel__container');
			return r && r.offsetWidth > 0 ? r.offsetWidth / d : 300;
		},
		b = () => {
			const o = V(),
				a = -s * o;
			(l.style.transform = `translateX(${a}px)`),
				u.forEach((r, m) => {
					const p = Math.floor(s / d);
					m === p
						? r.classList.add('ubits-carousel__dot--active')
						: r.classList.remove('ubits-carousel__dot--active');
				}),
				i.onSlideChange && i.onSlideChange(s);
		};
	c &&
		c.addEventListener('click', () => {
			s > 0 ? s-- : h && (s = t.length - d), b(), g();
		}),
		n &&
			n.addEventListener('click', () => {
				const o = Math.max(0, t.length - d);
				s < o ? s++ : h && (s = 0), b(), g();
			}),
		u.forEach((o, a) => {
		...
		t.forEach((o) => {
			o.addEventListener('click', (a) => {
				const r = a.target;
				if (r.closest('button') || r.closest('.ubits-button')) return;
				const m = o.getAttribute('data-item-id'),
					p = i.items.find((_) => String(_.id) === m);
				p && p.onItemClick ? p.onItemClick(p) : p && i.onItemClick && i.onItemClick(p);
			});
		});
	const f = () => {
			I &&
...
const F = [
		...
		},
		{
			...
			content:
				'Resistente al agua y diseñado para atletas. Cronómetro y múltiples funciones deportivas.',
			showHeader: !0,
			headerDecorations: !0,
			variant: 'elevated',
			size: 'md',
	...
	j = {
		...
		argTypes: {
			...
			autoplayInterval: {
				...
				table: { defaultValue: { summary: '3000' }, type: { summary: 'number (ms)' } },
			},
			loop: {
				control: { type: 'boolean' },
			...
			arrowPosition: {
				...
				table: { defaultValue: { summary: 'outside' }, type: { summary: 'inside | outside' } },
			},
			dotPosition: {
				control: { type: 'select' },
				options: ['bottom', 'top'],
				description: 'Posición de los indicadores de paginación',
				table: { defaultValue: { summary: 'bottom' }, type: { summary: 'bottom | top' } },
			},
			cardSize: {
				control: { type: 'select' },
				options: ['sm', 'md', 'lg', 'xl'],
				description: 'Tamaño de las Simple Cards',
				table: { defaultValue: { summary: 'md' }, type: { summary: 'sm | md | lg | xl' } },
			},
			cardVariant: {
				control: { type: 'select' },
		...
		render: (e) => {
			const l = {
					items: F.map((n) => ({
						...n,
						size: e.cardSize || 'md',
						variant: e.cardVariant || 'elevated',
						showHeader: e.showCardHeader !== !1,
						showButtons: e.showCardButtons !== !1,
						onItemClick: (u) => {
							e.onItemClick && e.onItemClick(u);
						},
					})),
					itemsPerView: e.itemsPerView,
			...
			t.innerHTML = P(l);
			const c = t.querySelector('.ubits-carousel');
			return (
				c &&
					requestAnimationFrame(() => {
						requestAnimationFrame(() => {
...
w.parameters = {
  ...
  render: args => {
    ...
    return container;
		...
		description: {
			story: 'Carrusel completo con todos los controles',
			...w.parameters?.docs?.description,
		},
	},
};
const R = ['Default'];
export { w as Default, R as __namedExportsOrder, j as default };
