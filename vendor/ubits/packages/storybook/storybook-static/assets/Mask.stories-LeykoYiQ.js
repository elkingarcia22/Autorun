import { c as B } from './popover-CvtG2qg5.js';
import { c as q } from './ButtonProvider-Dmy-L2BE.js';
import './iframe-BcZai5-G.js';
import './ListProvider-Bx601uSc.js';
import './SpinnerProvider-o6XHV06V.js';
import './preload-helper-PPVm8Dsz.js';
function I(t, d, s, r) {
	const o = r || d.getBoundingClientRect(),
		C = t.__leftCompensation || 0,
		x = t.__topCompensation || 0,
		w = document.body.style.position === 'fixed';
	w || window.pageYOffset || document.documentElement.scrollTop,
		w || window.pageXOffset || document.documentElement.scrollLeft;
	const f = o.top - s + x,
		p = o.left - s + C,
		u = o.width + s * 2,
		m = o.height + s * 2,
		...
		n = t.querySelector('.ubits-mask-layer--top'),
		g = t.querySelector('.ubits-mask-layer--bottom'),
		a = t.querySelector('.ubits-mask-layer--left'),
		b = t.querySelector('.ubits-mask-layer--right'),
		v = t.querySelector('.ubits-mask-highlight');
	if (n) {
		const l = Math.max(0, f);
		n.style.height = `${l}px`;
	}
	if (g) {
		const l = f + m,
			y = Math.max(0, h - l);
		(g.style.top = `${l}px`), (g.style.height = `${y}px`);
	}
	if (a) {
		const l = Math.max(0, p);
		(a.style.top = `${Math.max(0, f)}px`), (a.style.width = `${l}px`), (a.style.height = `${m}px`);
	}
	if (b) {
		const l = p + u,
			y = Math.max(0, e - l);
		(b.style.top = `${Math.max(0, f)}px`),
			(b.style.left = `${l}px`),
			(b.style.width = `${y}px`),
			(b.style.height = `${m}px`);
	}
...
function L(t, d, s, r, o) {
	...
	switch (p) {
		case 'top':
			(e = 'bottom'), (u = t.top - s - o), (m = w);
			break;
		case 'bottom':
			(e = 'top'), (u = t.bottom + o), (m = w);
			break;
		case 'left':
			(e = 'right'), (u = f), (m = t.left - d - o);
			break;
		case 'right':
			(e = 'left'), (u = f), (m = t.right + o);
			break;
	}
	return { top: u, left: m, tailPosition: e };
}
function j(t) {
	const { className: d = '' } = t;
	return `
...
function z(t) {
	const {
		...
		onClose: x,
		open: w = !1,
		popoverPosition: f = 'auto',
		popoverOffset: p = 12,
	} = t;
	let u;
	d ? (u = document.getElementById(d) || document.body) : (u = document.body);
	const m = document.createElement('div');
	m.innerHTML = j(t);
	const e = m.firstElementChild;
	if (!e) throw new Error('No se pudo crear la máscara');
	const h = () => (typeof s == 'string' ? document.querySelector(s) : s);
	let n = h();
	const g = e.querySelector('.ubits-mask-popover-container');
	let a = null,
		b = 0,
		v = 0,
		l = null;
	const y = () => {
			...
			const c = !l ? n.getBoundingClientRect() : l;
			e.__leftCompensation, e.__topCompensation;
			const E = I(e, n, o, c);
			if (a && g) {
				const k = a.element.getBoundingClientRect(),
					P = L(E, k.width || 360, k.height || 200, f, p);
				a.updatePosition({ top: P.top, left: P.left }),
					r.tailPosition !== P.tailPosition &&
						(a && a.destroy(),
						(a = B({
							...r,
							tailPosition: P.tailPosition,
							position: { top: P.top, left: P.left },
							open: !0,
						})),
		...
		T = () => {
			...
			const i = n.getBoundingClientRect(),
				c = L({ top: i.top, left: i.left, width: i.width, height: i.height }, 360, 200, f, p);
			(a = B({
				...r,
				tailPosition: c.tailPosition,
				position: { top: c.top, left: c.left },
				open: !0,
				onClose: () => {
					r.onClose && r.onClose(), M();
				},
			})),
				g.appendChild(a.element),
		...
		_ = () => {
			if (((n = h()), !n)) {
				console.error('❌ [Mask] No se encontró el elemento objetivo al abrir:', s);
				return;
			}
			(b = window.scrollY || window.pageYOffset || 0),
				(v = window.scrollX || window.pageXOffset || 0);
			const i = n.getBoundingClientRect();
			e.classList.add('ubits-mask-overlay--open'),
				(document.body.style.overflow = 'hidden'),
				(document.body.style.position = 'fixed'),
				(document.body.style.top = `-${b}px`),
				(document.body.style.left = `-${v}px`),
				...
				requestAnimationFrame(() => {
					requestAnimationFrame(() => {
						const c = n.getBoundingClientRect(),
							E = c.top - i.top,
							S = c.left - i.left;
						Math.abs(S) > 0.1 || Math.abs(E) > 0.1
							? ((l = c), (e.__leftCompensation = 0), (e.__topCompensation = 0))
							: ((l = i), (e.__leftCompensation = 0), (e.__topCompensation = 0)),
							y(),
							T();
						const k = () => {
							(l = null), y();
		...
		M = () => {
			e.classList.remove('ubits-mask-overlay--open'), (l = null);
			const i = e.__handleResize,
				c = e.__handleScroll;
			i && window.removeEventListener('resize', i),
				c && window.removeEventListener('scroll', c, !0),
			...
			E && (E.style.paddingRight = ''),
				(e.__modifiedParents || []).forEach((k) => {
					k.style.paddingRight = '';
				}),
				(e.__modifiedParents = []),
				(e.__leftCompensation = 0),
				(e.__topCompensation = 0),
				window.scrollTo(v, b),
				a && (a.destroy(), (a = null)),
		...
		A = (i) => {
			if ((typeof i == 'string' ? (n = document.querySelector(i)) : (n = i), !n)) {
				console.error('❌ [Mask] No se encontró el nuevo elemento objetivo:', i);
				return;
			}
			e.classList.contains('ubits-mask-overlay--open') && y();
		},
		$ = () => {
			M(), e.parentElement && e.parentElement.removeChild(e);
		};
	return (
		C &&
			e.addEventListener('click', (i) => {
				const c = i.target;
				(c.classList.contains('ubits-mask-layer') || c.classList.contains('ubits-mask-overlay')) &&
					M();
			}),
		u.appendChild(e),
		w &&
		...
		{ element: e, open: _, close: M, updateTarget: A, destroy: $ }
	);
}
const V = {
		title: 'Feedback/Mask',
		...
		argTypes: {
			targetElement: {
				...
				table: { type: { summary: 'string | HTMLElement' }, category: 'Configuración' },
			},
			'popover.title': {
				control: { type: 'text' },
			...
			return (
				requestAnimationFrame(() => {
					try {
						...
						s.appendChild(o);
						const C = q({ variant: 'primary', size: 'md', text: 'Botón Destacado' });
						(C.id = 'target-button'),
							s.appendChild(C),
							d.appendChild(s),
							requestAnimationFrame(() => {
								r = z({
									...
									popoverPosition: t.popoverPosition || 'auto',
									popoverOffset: t.popoverOffset || 12,
									padding: t.padding || 8,
									closeOnOverlayClick: t.closeOnOverlayClick !== !1,
									open: t.open || !1,
								});
							});
					} catch (o) {
						console.error('❌ Error creando botones:', o);
					}
				}),
				d
			);
...
O.parameters = {
  ...
  args: {
    ...
    open: false
		...
		description: {
			story: 'Ejemplo básico de Mask destacando un botón',
			...O.parameters?.docs?.description,
		},
	},
};
const X = ['Default'];
export { O as Default, X as __namedExportsOrder, V as default };
