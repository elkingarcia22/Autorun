import { r as h } from './BadgeProvider-DQRLkCrk.js';
import './iframe-BcZai5-G.js';
import './preload-helper-PPVm8Dsz.js';
function A(e, t = 'regular') {
	const n = t === 'solid' ? 'fas' : 'far',
		i = e.startsWith('fa-') ? e : `fa-${e}`;
	return `<i class="${n} ${i}"></i>`;
}
function L(e) {
	const t = {
		content: typeof e.content == 'number' ? String(e.content) : e.content,
...
function C(e, t) {
	const n = [
			'ubits-menu-item',
			e.active && 'ubits-menu-item--active',
			e.disabled && 'ubits-menu-item--disabled',
		]
			.filter(Boolean)
			.join(' '),
		i = e.icon ? A(e.icon, e.iconStyle) : '',
		o = e.badge ? L(e.badge) : '',
		s = e.onClick ? 'data-has-click-handler="true"' : '',
		l = e.href ? `data-href="${e.href}"` : '',
...
function T(e) {
	const {
		...
		m = ['ubits-menu', l].filter(Boolean).join(' '),
		f = Object.entries(r)
			.map(([u, v]) => `${u}="${v}"`)
			.join(' '),
		p = s ? `width: ${typeof s == 'number' ? `${s}px` : s};` : '',
		b = t
			? `
...
const D = {
	...
	argTypes: {
		logoImage: {
			...
			table: { category: 'Header', type: { summary: 'string' } },
		},
		appName: {
			control: { type: 'text' },
		...
		width: {
			...
			table: { category: 'Layout', type: { summary: 'number' }, defaultValue: { summary: 280 } },
		},
		section1Title: {
			control: { type: 'text' },
			description: 'Título de la primera sección',
			table: { category: 'Sección 1', type: { summary: 'string' } },
		},
		section1Item1Label: {
			control: { type: 'text' },
			description: 'Label del primer item de la sección 1',
			table: { category: 'Sección 1 - Item 1' },
		},
		section1Item1Icon: {
			control: { type: 'text' },
		...
		section1Item1IconStyle: {
			...
			table: { category: 'Sección 1 - Item 1', defaultValue: { summary: 'regular' } },
		},
		section1Item1Active: {
			control: { type: 'boolean' },
			description: 'Si el item está activo',
			table: { category: 'Sección 1 - Item 1', defaultValue: { summary: !1 } },
		},
		...
		section1Item2Label: {
			...
			table: { category: 'Sección 1 - Item 2' },
		},
		section1Item2Icon: {
			control: { type: 'text' },
			description: 'Icono FontAwesome (sin prefijo fa-)',
		...
		section1Item2IconStyle: {
			...
			options: ['regular', 'solid'],
			description: 'Estilo del icono',
			table: { category: 'Sección 1 - Item 2', defaultValue: { summary: 'regular' } },
		},
		section1Item2Active: {
			control: { type: 'boolean' },
			description: 'Si el item está activo',
			table: { category: 'Sección 1 - Item 2', defaultValue: { summary: !1 } },
		},
		...
		section2Title: {
			...
			table: { category: 'Sección 2', type: { summary: 'string' } },
		},
		section2Item1Label: {
			control: { type: 'text' },
			description: 'Label del primer item de la sección 2',
			table: { category: 'Sección 2 - Item 1' },
		},
		section2Item1Icon: {
			control: { type: 'text' },
			description: 'Icono FontAwesome (sin prefijo fa-)',
		...
		section2Item1IconStyle: {
			...
			options: ['regular', 'solid'],
			description: 'Estilo del icono',
			table: { category: 'Sección 2 - Item 1', defaultValue: { summary: 'regular' } },
		},
		section2Item1Active: {
			control: { type: 'boolean' },
			description: 'Si el item está activo',
			table: { category: 'Sección 2 - Item 1', defaultValue: { summary: !1 } },
		},
		...
		section2Item2Label: {
			...
			table: { category: 'Sección 2 - Item 2' },
		},
		section2Item2Icon: {
			control: { type: 'text' },
			description: 'Icono FontAwesome (sin prefijo fa-)',
		...
		section2Item2IconStyle: {
			...
			options: ['regular', 'solid'],
			description: 'Estilo del icono',
			table: { category: 'Sección 2 - Item 2', defaultValue: { summary: 'regular' } },
		},
		section2Item2Badge: {
			control: { type: 'number' },
			description: 'Número del badge (0 para ocultar)',
			table: { category: 'Sección 2 - Item 2', defaultValue: { summary: 0 } },
		},
		section2Item2BadgeVariant: {
			control: { type: 'select' },
			options: ['success', 'warning', 'error', 'info'],
			description: 'Variante del badge',
			table: { category: 'Sección 2 - Item 2', defaultValue: { summary: 'error' } },
		},
		section2Item2Active: {
			control: { type: 'boolean' },
			description: 'Si el item está activo',
			table: { category: 'Sección 2 - Item 2', defaultValue: { summary: !1 } },
		},
		...
		section2Item3Label: {
			...
			table: { category: 'Sección 2 - Item 3' },
		},
		section2Item3Icon: {
			control: { type: 'text' },
			description: 'Icono FontAwesome (sin prefijo fa-)',
		...
		section2Item3IconStyle: {
			...
			options: ['regular', 'solid'],
			description: 'Estilo del icono',
			table: { category: 'Sección 2 - Item 3', defaultValue: { summary: 'regular' } },
		},
		section2Item3Active: {
			control: { type: 'boolean' },
			description: 'Si el item está activo',
			table: { category: 'Sección 2 - Item 3', defaultValue: { summary: !1 } },
		},
		...
		userAvatarImage: {
			...
			table: { category: 'Usuario', type: { summary: 'string' } },
		},
		userName: {
			control: { type: 'text' },
...
function $(e) {
	...
	if (e.section1Title) {
		const n = [];
		e.section1Item1Label &&
			n.push({
				id: 'section1-item1',
				label: e.section1Item1Label,
				icon: e.section1Item1Icon,
				iconStyle: e.section1Item1IconStyle || 'regular',
				active: e.section1Item1Active || !1,
				disabled: e.section1Item1Disabled || !1,
			}),
			e.section1Item2Label &&
				n.push({
					id: 'section1-item2',
					label: e.section1Item2Label,
					icon: e.section1Item2Icon,
					iconStyle: e.section1Item2IconStyle || 'regular',
					active: e.section1Item2Active || !1,
					disabled: e.section1Item2Disabled || !1,
				}),
			n.length > 0 && t.push({ id: 'section1', title: e.section1Title, items: n });
	}
	if (e.section2Title) {
		const n = [];
		e.section2Item1Label &&
			n.push({
				id: 'section2-item1',
				label: e.section2Item1Label,
				icon: e.section2Item1Icon,
				iconStyle: e.section2Item1IconStyle || 'regular',
				active: e.section2Item1Active || !1,
				disabled: e.section2Item1Disabled || !1,
			}),
			e.section2Item2Label &&
				n.push({
					id: 'section2-item2',
					label: e.section2Item2Label,
					icon: e.section2Item2Icon,
					iconStyle: e.section2Item2IconStyle || 'regular',
					badge:
						e.section2Item2Badge && e.section2Item2Badge > 0
							? { content: e.section2Item2Badge, variant: e.section2Item2BadgeVariant || 'error' }
							: void 0,
					active: e.section2Item2Active || !1,
					disabled: e.section2Item2Disabled || !1,
				}),
			e.section2Item3Label &&
				n.push({
					id: 'section2-item3',
					label: e.section2Item3Label,
					icon: e.section2Item3Icon,
					iconStyle: e.section2Item3IconStyle || 'regular',
					active: e.section2Item3Active || !1,
					disabled: e.section2Item3Disabled || !1,
				}),
			n.length > 0 && t.push({ id: 'section2', title: e.section2Title, items: n });
	}
	return t;
}
const y = {
	args: {
		...
		section2Item2Active: !1,
		section2Item2Disabled: !1,
		section2Item3Label: 'Logout',
		section2Item3Icon: 'sign-out',
		section2Item3IconStyle: 'regular',
		section2Item3Active: !1,
		section2Item3Disabled: !1,
    ...
    `;
		const n = $(e),
			i =
				e.userAvatarImage && e.userName && e.userRole
					? { avatarImage: e.userAvatarImage, name: e.userName, role: e.userRole }
					: void 0,
			a = {
				...
				sections: n,
				userInfo: i,
				onActiveItemChange: (m, f) => {},
				onAvatarClick: () => {},
			},
			o = document.createElement('div');
		(o.id = 'menu-story-container'),
			(o.style.cssText = `
      position: relative;
    ...
    `);
		...
		delete s.containerId,
			...
			requestAnimationFrame(() => {
				const m = o.querySelector('.ubits-menu');
				if (m) {
					const f = m.querySelectorAll('.ubits-menu-item');
					f.forEach((c) => {
						const g = c.getAttribute('data-item-id'),
							u = c.getAttribute('data-section-id');
						if (!g || !u) return;
						const d = a.sections.find((I) => I.id === u)?.items.find((I) => I.id === g);
						d &&
							c.addEventListener('click', (I) => {
								I.preventDefault(),
									!d.disabled &&
										(f.forEach((S) => S.classList.remove('ubits-menu-item--active')),
										c.classList.add('ubits-menu-item--active'),
										d.onClick ? d.onClick(I, d) : d.href && (window.location.href = d.href));
							});
					});
					const p = m.querySelector('.ubits-menu-logo');
					p &&
						a.logoHref &&
						p.addEventListener('click', () => {
							window.location.href = a.logoHref;
						});
					const b = m.querySelector('.ubits-menu-user-avatar');
					b &&
						a.userInfo?.onAvatarClick &&
						b.addEventListener('click', (c) => {
							c.preventDefault(), a.userInfo.onAvatarClick();
						});
				}
			});
...
const M = ['Default'];
export { y as Default, M as __namedExportsOrder, D as default };
