const b = {
		...
		parameters: {
			...
			docs: {
				description: {
					component:
						'Tokens de espaciado del sistema UBITS. Incluye valores básicos (xs, sm, md, lg, xl, 2xl) y valores extendidos (numéricos).',
				},
			},
		},
	},
...
function g(e) {
	const t = document.documentElement,
		n = getComputedStyle(t).getPropertyValue(e).trim() || '0';
	let a, d;
	if (e.startsWith('--p-spacing-mode-1-')) {
		const m = parseFloat(n) || 0;
		(a = m), (d = m > 0 ? `${m}px` : '0');
	} else (a = parseFloat(n) || 0), (d = n);
	...
	(o.style.display = 'grid'),
		...
		(o.style.borderRadius = 'var(--ubits-border-radius-sm, 8px)'),
		(o.style.background = 'var(--modifiers-normal-color-light-bg-1)');
	const c = document.createElement('code');
	(c.textContent = e),
		(c.style.fontSize = '13px'),
		(c.style.color = 'var(--modifiers-normal-color-light-fg-1-high)'),
		(c.style.fontFamily = 'Monaco, Menlo, monospace');
	const i = document.createElement('div');
	(i.style.display = 'flex'), (i.style.alignItems = 'center'), (i.style.gap = '12px');
	const r = document.createElement('div'),
		u = Math.min(1, 200 / Math.max(a, 1)),
		y = a > 0 ? Math.max(a * u, 2) : 0;
	(r.style.width = `${y}px`),
		(r.style.height = '24px'),
		(r.style.background = 'var(--modifiers-normal-color-light-accent-brand)'),
		(r.style.borderRadius = 'var(--ubits-border-radius-sm, 8px)'),
		(r.style.minWidth = a > 0 ? '2px' : '0');
	const s = document.createElement('span');
	return (
		(s.textContent = d),
		(s.style.fontSize = '13px'),
		(s.style.color = 'var(--modifiers-normal-color-light-fg-1-medium)'),
		(s.style.fontFamily = 'Monaco, Menlo, monospace'),
...
const l = {
		render: () => {
			...
			return (
				(t.textContent = 'Spacing Básico (UBITS)'),
	...
	p = {
		render: () => {
			...
			(e.style.display = 'flex'),
				...
				(e.style.borderRadius = 'var(--ubits-border-radius-md, 8px)');
			const t = document.createElement('h2');
			(t.textContent = 'Spacing Extendido (Figma - p-spacing-mode-1)'),
				(t.style.margin = '0 0 16px 0'),
				(t.style.fontSize = '18px'),
				(t.style.fontWeight = '600'),
				(t.style.color = 'var(--modifiers-normal-color-light-fg-1-high)');
			...
			return (
				(n.textContent =
					'Nota: Los tokens de Figma son valores numéricos sin unidades. Se muestran con "px" para referencia visual.'),
				(n.style.margin = '0 0 16px 0'),
				(n.style.fontSize = '13px'),
				(n.style.color = 'var(--modifiers-normal-color-light-fg-1-medium)'),
...
const E = ['Basic', 'Extended'];
export { l as Basic, p as Extended, E as __namedExportsOrder, b as default };
