import { c as d } from './BreadcrumbProvider-BQh42h2n.js';
...
function m(o = 5) {
	...
	return Array.from({ length: o }, (t, r) => ({
		id: `item-${r + 1}`,
		label: e[r] || `Item ${r + 1}`,
		url: r < o - 1 ? `#${e[r]?.toLowerCase() || `item-${r + 1}`}` : void 0,
		active: r === o - 1,
	}));
}
const i = {
	args: { items: m(5), separator: '>', itemCount: 5 },
	render: (o) => {
		const e = m(o.itemCount || 5),
    ...
    `;
		...
		(r.id = 'breadcrumb-story-container'),
			(r.style.cssText = `
      width: 100%;
      margin-bottom: 24px;
    `),
		...
		return (
			...
			requestAnimationFrame(() => {
				try {
					...
					console.error('Error creando Breadcrumb:', a),
						(r.innerHTML = `<p style="color: #ef4444); padding: 16px;">Error: ${a}</p>`);
				}
			}),
			t
		);
...
const h = ['Default'];
export { i as Default, h as __namedExportsOrder, u as default };
