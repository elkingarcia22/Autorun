import { c as S, r as g } from './ButtonProvider-Dmy-L2BE.js';
...
const M = {
		...
		argTypes: {
			...
			text: { control: { type: 'text' }, description: 'Texto del botón' },
			icon: {
				control: { type: 'text' },
				description: 'Nombre del icono FontAwesome (sin prefijo fa-)',
				table: { type: { summary: 'string' }, example: { summary: 'check, plus, times, etc.' } },
			},
			iconStyle: {
				control: { type: 'select' },
				options: ['regular', 'solid'],
				description: 'Estilo del icono FontAwesome',
				table: { defaultValue: { summary: 'regular' }, type: { summary: 'regular | solid' } },
			},
			iconPosition: {
				control: { type: 'select' },
				options: ['left', 'right', 'only'],
				description: 'Posición del icono',
				table: { defaultValue: { summary: 'left' }, type: { summary: 'left | right | only' } },
			},
			iconOnly: {
			...
			active: {
				control: { type: 'boolean' },
				description:
					'Modificador active/outline (fondo transparente + overlay azul). IMPORTANTE: Esto es diferente del estado "active" del controlador de estados (que simula "pressed")',
				table: { defaultValue: { summary: 'false' } },
			},
			floating: {
				control: { type: 'boolean' },
			...
			showTooltip: {
				...
				table: { defaultValue: { summary: 'false' }, category: 'Tooltip' },
			},
			tooltipText: {
				control: { type: 'text' },
	...
	f = {
		...
		render: (n) => {
			...
			(c.style.padding = '20px'),
				(c.style.background = 'var(--modifiers-normal-color-light-bg-1), #ffffff)'),
				(c.style.borderRadius = '8px');
			const r = document.createElement('div');
			(r.style.display = 'flex'),
				(r.style.justifyContent = 'center'),
				(r.style.alignItems = 'flex-start'),
				(r.style.padding = '40px'),
			...
			const t = {
				...n,
				iconOnly: n.iconPosition === 'only' || n.iconOnly,
				iconPosition: n.iconPosition === 'only' ? 'left' : n.iconPosition,
			};
			if (t.dropdown && t.dropdownOptions && t.dropdownOptions.length > 0) {
					...
					requestAnimationFrame(() => {
						try {
							const i = S(t),
								o = i.parentElement,
								d = (l) => {
									if (t.active || n.active) {
										const e = document.documentElement,
											s =
												getComputedStyle(e)
													.getPropertyValue('--modifiers-normal-color-light-bg-active-button')
													.trim() || 'var(--modifiers-normal-color-light-bg-active-button)',
											b =
												getComputedStyle(e)
													.getPropertyValue('--modifiers-normal-color-light-bg-1')
													.trim() || '#ffffff',
											y = `${s}, ${b}`;
										l.style.setProperty('background', y, 'important'),
											...
											l.querySelectorAll('span').forEach((p) => {
												p.style.color = 'var(--modifiers-normal-color-light-accent-brand)';
											}),
											l.querySelectorAll('i').forEach((p) => {
												p.style.color = 'var(--modifiers-normal-color-light-accent-brand)';
											});
									}
								};
							(t.active || n.active) && i.classList.add('ubits-button--active'),
								o
									? (a.appendChild(o),
										d(i),
										t.iconOnly && t.showTooltip && t.tooltipText && v(i, t.tooltipText))
									: (a.appendChild(i),
										d(i),
										t.iconOnly && t.showTooltip && t.tooltipText && v(i, t.tooltipText));
						} catch {
							...
							if (o) {
								if (
									((t.active || n.active) && o.classList.add('ubits-button--active'),
									t.active || n.active)
								) {
									const d = document.documentElement,
										...
										e =
											getComputedStyle(d)
												...
												.trim() || '#ffffff',
										s = `${l}, ${e}`;
									o.style.setProperty('background', s, 'important'),
										o.style.setProperty(
											'color',
											'var(--modifiers-normal-color-light-accent-brand)',
											'important',
										),
										o.style.setProperty('border', 'none', 'important'),
										o.querySelectorAll('span').forEach((u) => {
											u.style.color = 'var(--modifiers-normal-color-light-accent-brand)';
										}),
										o.querySelectorAll('i').forEach((u) => {
											u.style.color = 'var(--modifiers-normal-color-light-accent-brand)';
										});
								}
								t.iconOnly && t.showTooltip && t.tooltipText && v(o, t.tooltipText);
							}
						}
					}),
					r.appendChild(a);
				...
				(i.innerHTML = a),
					...
					requestAnimationFrame(() => {
						...
						if (o) {
							t.floating &&
								!o.classList.contains('ubits-button--floating') &&
								o.classList.add('ubits-button--floating');
							const d = o.classList.contains('ubits-button--active');
							(t.active || n.active) && !d && o.classList.add('ubits-button--active');
							const l = o.classList.contains('ubits-button--active');
							if (t.active || n.active || l) {
								const e = document.documentElement,
									s =
										getComputedStyle(e)
											.getPropertyValue('--modifiers-normal-color-light-bg-active-button')
									...
									b =
										getComputedStyle(e)
											.getPropertyValue('--modifiers-normal-color-light-bg-1')
											.trim() || 'var(--modifiers-normal-color-light-bg-1)',
									y = `${s}, ${b}`;
								o.style.setProperty('background', y, 'important'),
									o.style.setProperty('border', 'none', 'important'),
									o.style.setProperty(
										'color',
										'var(--modifiers-normal-color-light-accent-brand)',
									...
									o.querySelectorAll('span').forEach((p) => {
										p.style.color = 'var(--modifiers-normal-color-light-accent-brand)';
									}),
									o.querySelectorAll('i').forEach((p) => {
										p.style.color = 'var(--modifiers-normal-color-light-accent-brand)';
									});
							}
							t.iconOnly && t.showTooltip && t.tooltipText && v(o, t.tooltipText);
...
function v(n, c) {
	const r = n.dataset.tooltipInstance;
	if (r) {
		const s = document.getElementById(r);
		s && s.remove(), delete n.dataset.tooltipInstance;
	}
	const t = n._tooltipMouseEnter,
		a = n._tooltipMouseLeave;
	t && (n.removeEventListener('mouseenter', t), delete n._tooltipMouseEnter),
		a && (n.removeEventListener('mouseleave', a), delete n._tooltipMouseLeave),
		n.hasAttribute('title') && n.removeAttribute('title');
	const i = `button-tooltip-${Math.random().toString(36).substr(2, 9)}`,
		o = c
			.replace(/&/g, '&amp;')
			.replace(/</g, '&lt;')
			.replace(/>/g, '&gt;')
	...
	if (e) {
		...
		const s = () => {
				...
				(e.style.top = '-9999px'),
					...
					e.classList.add('ubits-tooltip--open'),
					e.offsetHeight;
			...
			b = () => {
				e.classList.remove('ubits-tooltip--open'),
					(e.style.opacity = '0'),
		...
		n.addEventListener('mouseenter', y, !1),
			n.addEventListener('mouseleave', u, !1),
			(n._tooltipMouseEnter = y),
			(n._tooltipMouseLeave = u),
			(n.dataset.tooltipInstance = i);
	}
}
const h = {
	args: {
	...
	render: (n) => {
		...
		(r.textContent = 'Estado Active - Con nuevo fondo bg-active-button y texto accent-brand'),
			(r.style.marginBottom = '24px'),
			(r.style.color = 'var(--modifiers-normal-color-light-fg-1-high, #303a47)'),
			c.appendChild(r);
		const t = document.createElement('div');
		(t.style.display = 'grid'),
			(t.style.gridTemplateColumns = 'repeat(auto-fit, minmax(200px, 1fr))'),
			(t.style.gap = '16px'),
			(t.style.marginBottom = '32px');
		const a = document.createElement('div');
		(a.style.display = 'flex'), (a.style.flexDirection = 'column'), (a.style.gap = '8px');
		const i = document.createElement('label');
		(i.textContent = 'Primary Active'),
			(i.style.fontSize = '14px'),
			(i.style.color = 'var(--modifiers-normal-color-light-fg-1-medium, #5c646f)'),
			(i.style.fontWeight = '500'),
			a.appendChild(i),
			(a.innerHTML += g({ ...n, variant: 'primary', active: !0, text: 'Primary Active' })),
			t.appendChild(a);
		const o = document.createElement('div');
		(o.style.display = 'flex'), (o.style.flexDirection = 'column'), (o.style.gap = '8px');
		const d = document.createElement('label');
		(d.textContent = 'Secondary Active'),
			(d.style.fontSize = '14px'),
			(d.style.color = 'var(--modifiers-normal-color-light-fg-1-medium, #5c646f)'),
			(d.style.fontWeight = '500'),
			o.appendChild(d),
			(o.innerHTML += g({ ...n, variant: 'secondary', active: !0, text: 'Secondary Active' })),
			t.appendChild(o);
		const l = document.createElement('div');
		(l.style.display = 'flex'), (l.style.flexDirection = 'column'), (l.style.gap = '8px');
		const e = document.createElement('label');
		(e.textContent = 'Tertiary Active'),
			(e.style.fontSize = '14px'),
			(e.style.color = 'var(--modifiers-normal-color-light-fg-1-medium, #5c646f)'),
			(e.style.fontWeight = '500'),
			l.appendChild(e),
			(l.innerHTML += g({ ...n, variant: 'tertiary', active: !0, text: 'Tertiary Active' })),
			t.appendChild(l),
			c.appendChild(t);
		const s = document.createElement('div');
		return (
			(s.style.padding = '16px'),
			(s.style.background = 'var(--modifiers-normal-color-light-bg-2), #f3f3f4)'),
...
const V = ['Default', 'ActiveState'];
export { h as ActiveState, f as Default, V as __namedExportsOrder, M as default };
