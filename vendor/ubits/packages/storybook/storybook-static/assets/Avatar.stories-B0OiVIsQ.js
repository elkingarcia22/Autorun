import { r as l } from './AvatarProvider-CF4x-oFR.js';
...
const p = {
		...
		argTypes: {
			imageUrl: {
				control: { type: 'text' },
				description:
					'URL de la imagen del avatar (para variante Photo). Si se proporciona, se usa la variante Photo.',
				table: {
					type: { summary: 'string' },
			...
			initials: {
				control: { type: 'text' },
				description:
					'Texto para mostrar como iniciales (para variante Initials). Ej: "John Doe" genera "JD". Si se proporciona sin imageUrl, se usa la variante Initials.',
				table: {
					type: { summary: 'string' },
			...
			icon: {
				control: { type: 'text' },
				description:
					'Nombre del icono FontAwesome (para variante Icon). Ej: "user", "robot". Se usa si no hay imageUrl ni initials.',
				table: {
					type: { summary: 'string' },
			...
			badgeColor: {
				...
				description:
					'Color del badge. Si se proporciona, se muestra el badge. Dejar vacío para ocultar el badge.',
				table: {
			...
			badgeContent: {
				control: { type: 'text' },
				description:
					'Contenido del badge (número o texto). Si no se proporciona o está vacío, se muestra solo el punto (dot). Ej: "5", "99+", "Nuevo"',
				table: {
			...
			className: {
				...
				table: { type: { summary: 'string' }, defaultValue: { summary: '' }, category: 'Avanzado' },
			},
		},
	},
	i = {
		...
		render: (e) => {
			...
			(a.style.display = 'flex'),
				...
				(a.style.padding = '48px'),
				(a.style.minHeight = '120px'),
				(a.style.background = 'var(--modifiers-normal-color-light-bg-2, #f9fafb)'),
				(a.style.borderRadius = '8px'),
				(a.style.marginBottom = '20px');
			const o = document.createElement('div'),
				r = {
					size: e.size || 'md',
					badgeColor: e.badgeColor && e.badgeColor.trim() !== '' ? e.badgeColor : void 0,
					badgeContent:
						e.badgeContent && e.badgeContent.toString().trim() !== '' ? e.badgeContent : void 0,
					alt: e.alt || 'Avatar',
			...
			if (
				(e.imageUrl && e.imageUrl.trim() !== ''
					...
					: e.initials && e.initials.trim() !== ''
						? (r.initials = e.initials)
						: (r.icon = e.icon || 'user'),
				(o.innerHTML = l(r)),
				e.onClick)
			) {
				const s = o.querySelector('.ubits-avatar');
				s &&
					(s.addEventListener('click', (t) => {
						t.preventDefault(), e.onClick && e.onClick(t);
					}),
					s.addEventListener('keydown', (t) => {
						(t.key === 'Enter' || t.key === ' ') && (t.preventDefault(), e.onClick && e.onClick(t));
					}));
			}
...
const g = ['Default'];
export { i as Default, g as __namedExportsOrder, p as default };
