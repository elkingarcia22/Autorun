import { r as $ } from './ButtonProvider-Dmy-L2BE.js';
const g = { sm: '240px', md: '360px', lg: '400px', xl: '480px' };
function h(s) {
	const {
			title: a,
			bodyContent: n = '',
		...
		v = g[f] || g.md,
		t = `ubits-popover--width-${f}`,
		u = `ubits-popover--tail-${c}`,
		y = ['ubits-popover', t, u, d].filter(Boolean).join(' '),
		m = `
...
function L(s) {
	...
	let d;
	a ? (d = document.getElementById(a) || document.body) : (d = document.body);
	...
	const u = () => {
		...
		y = () => {
			t.classList.remove('ubits-popover--open'), n && n();
		},
		m = (o) => {
	...
	if (f) {
		const o = (p) => {
			...
			if (t.classList.contains('ubits-popover--open') && !t.contains(r)) {
				const x = r;
				(x.closest && x.closest('[data-popover-trigger]')) || y();
			}
		};
	...
	return (
		d.appendChild(t),
		c && u(),
		{ element: t, open: u, close: y, updateContent: m, updatePosition: _, destroy: b }
	);
}
export { L as c };
