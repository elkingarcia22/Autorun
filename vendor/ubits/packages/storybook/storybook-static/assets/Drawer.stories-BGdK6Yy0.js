import { c as s } from './DrawerProvider-CjO9uG7_.js';
import { r as i } from './ButtonProvider-Dmy-L2BE.js';
...
const y = {
		...
		argTypes: {
			...
			width: {
				...
				description:
					'Ancho del drawer como porcentaje del viewport (100, 80, 60, 50, 40, 30). En móvil siempre es 100%.',
				table: {
					type: { summary: 'number' },
			...
			bodyContent: {
				control: { type: 'text' },
				description:
					'Contenido HTML del cuerpo del drawer. Puede ser una cadena HTML o una función que devuelve HTML.',
				table: {
    ...
    `,
		...
		render: (e) => {
			...
			(t.className = 'ubits-button ubits-button--primary ubits-button--md'),
				(t.innerHTML = '<span>Abrir Drawer</span>'),
				(t.style.width = 'auto'),
			...
			const l = () => {
				if (!r) {
					...
					e['footerButtons.tertiary.enabled'] &&
						(a.tertiary = {
							label: e['footerButtons.tertiary.label'] || 'Cancelar',
							onClick: () => {},
						}),
						e['footerButtons.secondary.enabled'] &&
							(a.secondary = {
								label: e['footerButtons.secondary.label'] || 'Guardar',
								onClick: () => {},
							}),
						e['footerButtons.primary.enabled'] &&
							(a.primary = {
								label: e['footerButtons.primary.label'] || 'Crear',
								onClick: () => {},
							}),
						(r = s({
							title: e.title,
							complementaryText: e.complementaryText,
							width: e.width,
							bodyContent: e.bodyContent,
							footerButtons: Object.keys(a).length > 0 ? a : void 0,
							containerId: void 0,
							closeOnOverlayClick: e.closeOnOverlayClick,
							onClose: () => {
								e.onClose && e.onClose(),
									r && r.element && r.element.remove(),
									(r = null),
			...
			return t.addEventListener('click', l), o.appendChild(t), o;
		},
	};
n.parameters = {
	...n.parameters,
...
const b = ['Default'];
export { n as Default, b as __namedExportsOrder, y as default };
