...
function v(i = {}) {
	const {
			...
			status: r = 'pending',
			leftIcon: l,
			rightIcon: e = 'chevron-down',
			clickable: d = !1,
			className: c = '',
		} = i,
		o = a[r] || a.pending,
		g = l ? `<span class="ubits-status-tag-left-icon"><i class="far fa-${l}"></i></span>` : '',
		f =
			e != null
				? `<span class="ubits-status-tag-right-icon"><i class="far fa-${e}"></i></span>`
				: '',
		b = ['ubits-status-tag', `ubits-status-tag--${s}`, d ? 'ubits-status-tag--clickable' : '', c]
			.filter(Boolean)
			.join(' '),
		n = `
    ${r === 'draft' || r === 'in-progress' || r === 'syncing' ? `background: linear-gradient(90deg, var(--modifiers-normal-color-light-bg-active) 0%, var(--modifiers-normal-color-light-bg-active) 100%), linear-gradient(90deg, var(--modifiers-normal-color-light-bg-1) 0%, var(--modifiers-normal-color-light-bg-1) 100%); background-color: ${o.bg};` : `background-color: ${o.bg};`}
    color: ${o.text};
    border-color: ${o.border};
  `.trim();
	return `
    <span class="${b}" style="${n}" data-status="${r}">
      ${g}
      <span class="ubits-status-tag-label">${t}</span>
      ${f}
    </span>
  `.trim();
}
