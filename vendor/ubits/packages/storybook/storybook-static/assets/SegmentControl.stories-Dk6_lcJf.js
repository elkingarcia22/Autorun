...
function b(r, t = !1) {
	if (!r) return '';
	let e = r;
	e.startsWith('fa-') || (e = `fa-${e}`);
	const a = t ? 'fas' : 'far';
	if (e.startsWith('far ') || e.startsWith('fas ')) {
		const i = e.replace(/^(far|fas)\s+/, '');
		return `<i class="${a} ${i}"></i>`;
	}
	return `<i class="${a} ${e}"></i>`;
}
function S(r) {
	const { segments: t, activeSegmentId: e, className: a = '' } = r;
	if (!t || t.length === 0) return '<div class="ubits-segment-control"></div>';
	let i = e;
	if (!i) {
		const s = t.find((o) => o.active);
		i = s ? s.id : t[0].id;
	}
	const n = t
		.map((s) => {
			const o = s.id === i,
				d = o ? 'ubits-segment--active' : '',
				l = s.disabled ? 'ubits-segment--disabled' : '',
				f = ['ubits-segment', d, l].filter(Boolean).join(' '),
				m = s.icon ? b(s.icon, o) : '';
			return `
      <button 
        class="${f}" 
...
function u(r, t) {
	r.querySelectorAll('.ubits-segment[data-listener-attached]').forEach((n) => {
		const c = n.cloneNode(!0);
		n.parentNode?.replaceChild(c, n);
	});
	const a = r.querySelectorAll('.ubits-segment:not(.ubits-segment--disabled)'),
		i = (n) => {
			const c = n.getAttribute('data-segment-id'),
				s = n.getAttribute('data-url');
			if (
				(r.querySelectorAll('.ubits-segment').forEach((l) => {
					l.classList.remove('ubits-segment--active');
				}),
				n.classList.add('ubits-segment--active'),
				r.querySelectorAll('.ubits-segment').forEach((l) => {
					const f = l.classList.contains('ubits-segment--active'),
						m = l.querySelector('i');
					if (m) {
						const h = m.className.replace(/^(far|fas)\s+/, '').replace(/^fa-/, ''),
							v = f ? 'fas' : 'far';
						m.className = `${v} fa-${h}`;
					}
				}),
			...
			const o = t.segments.find((l) => l.id === c);
			o && o.onClick && o.onClick(new MouseEvent('click')),
				t.onSegmentChange && t.onSegmentChange(c || '', n);
			const d = new CustomEvent('segmentControlSegmentClick', {
				detail: { segmentId: c, segmentElement: n },
			});
			document.dispatchEvent(d);
		};
	a.forEach((n) => {
		n.setAttribute('data-listener-attached', 'true'),
			n.addEventListener('click', (c) => {
				c.preventDefault(), i(n);
			});
	});
}
function y(r, t) {
	const e = (t && document.getElementById(t)) || document.createElement('div');
	return (
		t && !e.id && (e.id = t),
		(e.innerHTML = S(r)),
		requestAnimationFrame(() => {
			const a = e.querySelector('.ubits-segment-control');
			u(a || e, r);
		}),
		e
	);
...
const x = {
	...
	argTypes: {
		segments: {
			...
			table: { type: { summary: 'SegmentItem[]' } },
		},
		activeSegmentId: {
			control: { type: 'text' },
...
function p(r = 5, t = !0) {
	...
	return Array.from({ length: r }, (i, n) => ({
		id: `segment-${n + 1}`,
		label: a[n] || `Label ${n + 1}`,
		icon: t ? `far ${e[n] || 'fa-th'}` : void 0,
		active: n === 0,
	}));
}
const g = {
	args: { segments: p(5, !0), activeSegmentId: 'segment-1', showIcons: !0, segmentCount: 5 },
	render: (r) => {
		const t = r.showIcons !== !1,
			e = p(r.segmentCount || 5, t),
			a = r.activeSegmentId || e[0]?.id;
		e.forEach((o) => {
			o.active = o.id === a;
		});
    ...
    `;
		...
		(n.id = 'segment-control-story-container'),
			(n.style.cssText = `
      width: 100%;
    `),
			i.appendChild(n);
		const c = document.createElement('div');
		(c.id = 'segment-control-info-panel'),
			(c.style.cssText = `
      margin-top: 20px;
      padding: 16px;
		...
		return (
			...
			requestAnimationFrame(() => {
				try {
					...
					console.error('Error creando Segment Control:', o);
				}
			}),
			i
		);
...
const w = ['Default'];
export { g as Default, w as __namedExportsOrder, x as default };
