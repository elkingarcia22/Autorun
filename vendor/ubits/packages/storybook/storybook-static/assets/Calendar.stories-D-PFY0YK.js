...
import { c } from './index-BoEJW2yg.js';
...
const m = {
		...
		parameters: {
			layout: 'centered',
			backgrounds: { default: 'transparent', disable: !0 },
			docs: {
				description: {
					component:
	...
	r = {
		...
		render: (a) => {
			const n = (e) => {
				...
				o = {
					mode: a.mode || 'single',
					selectedDate: n(a.selectedDate),
					endDate: n(a.endDate),
					minDate: n(a.minDate),
					maxDate: n(a.maxDate),
					initialDate: n(a.initialDate) || new Date(),
			...
			let l = null;
			try {
				return (l = c(o)), l.element;
			} catch (e) {
				console.error('Error al crear calendario:', e);
				const t = document.createElement('div');
				return (
					(t.style.cssText =
						'color: var(--modifiers-normal-color-light-fg-1-high); padding: 20px; text-align: center;'),
					(t.textContent = `Error al cargar el calendario: ${e}`),
					t
				);
			}
		},
...
const D = ['Default'];
export { r as Default, D as __namedExportsOrder, m as default };
