...
function T(t = 'INSTOCK') {
	const o = {
		...
		e = o[t] || o.INSTOCK;
	return `<span class="ubits-data-view__stock-badge ${e.class}">${e.text}</span>`;
}
function I(t) {
	...
	let c = '';
	for (let a = 0; a < o; a++)
		c += '<i class="fas fa-star ubits-data-view__star ubits-data-view__star--filled"></i>';
	e &&
		(c += '<i class="fas fa-star-half-alt ubits-data-view__star ubits-data-view__star--half"></i>');
	for (let a = 0; a < u; a++)
		c += '<i class="far fa-star ubits-data-view__star ubits-data-view__star--empty"></i>';
	return `
...
function k(t, o, e) {
	const {
			...
			showWishlist: w = !0,
			showBuyButton: n = !0,
			buyButtonText: g = 'Buy Now',
			buyButtonIcon: p = 'shopping-cart',
			wishlistIcon: s = 'heart',
		} = e,
		l = t.id || `product-${o}`,
		d = t.stockStatus || 'INSTOCK',
		i = t.inWishlist || !1;
	return `
...
function x(t) {
	const {
		...
		n = ['ubits-data-view', `ubits-data-view--${u}`, c].filter(Boolean).join(' '),
		g = Object.entries(a)
			.map(([l, d]) => `${l}="${d}"`)
			.join(' '),
		p = e ? `id="${e}"` : '';
	let s = `<div class="${n}" ${p} ${g}>`;
	return (
		o.forEach((l, d) => {
			s += k(l, d, t);
		}),
...
function B(t) {
	...
	const {
		...
		l = `ubits-data-view--${c}`;
	return (
		(s.className = ['ubits-data-view', l, g].filter(Boolean).join(' ')),
		e && (s.id = e),
		Object.entries(p).forEach(([i, m]) => {
			s.setAttribute(i, m);
		}),
		(s.innerHTML = x(t)),
		s.querySelectorAll('.ubits-data-view__item').forEach((i, m) => {
			const r = u[m];
			if (!r) return;
			a &&
				i.addEventListener('click', (b) => {
					b.target.closest('button') || a(r, m, i);
				});
			const h = i.querySelector('[data-action="buy"]');
			h &&
				w &&
				h.addEventListener('click', (b) => {
					b.stopPropagation(), w(r, m, i);
				});
			const f = i.querySelector('[data-action="wishlist"]');
			f &&
				n &&
				f.addEventListener('click', (b) => {
					b.stopPropagation(), n(r, m, i);
				});
		}),
		s
	);
...
const j = {
	...
	argTypes: {
		...
		defaultRating: {
			...
			table: { defaultValue: { summary: '4' }, type: { summary: 'number (0-5)' } },
		},
		defaultPrice: {
...
function $(t, o = 'INSTOCK', e = 4, u = 50) {
	const c = ['Accessories', 'Fitness', 'Clothing', 'Electronics', 'Home'],
		a = [
			'Bamboo Watch',
			'Black Watch',
			'Blue Band',
	...
	return Array.from({ length: t }, (w, n) => {
		const g = n % c.length,
			p = n % a.length,
			s = ['INSTOCK', 'LOWSTOCK', 'OUTOFSTOCK'],
			l = n % s.length,
			d = [3, 3.5, 4, 4.5, 5],
			i = n % d.length,
			m = [15, 29, 50, 65, 72, 79, 100],
			r = n % m.length,
			h = [
...
const v = {
	...
	render: (t) => {
		const {
				...
				showWishlist: p = !0,
				showBuyButton: s = !0,
				buyButtonText: l = 'Buy Now',
				buyButtonIcon: d = 'shopping-cart',
				wishlistIcon: i = 'heart',
			} = t,
			m = $(o, u, c, a),
			r = document.createElement('div');
		(r.id = 'data-view-container'),
			(r.style.width = '100%'),
			(r.style.maxWidth = '800px'),
			(r.style.margin = '0 auto'),
			(r.style.background = 'var(--modifiers-normal-color-light-bg-2)'),
		...
		const f = (() => {
			const b = {
				...
				onProductClick: (y, C) => {},
				onBuyClick: (y, C) => {
					alert(`Comprar: ${y.name} - $${y.price}`);
				},
				onWishlistClick: (y, C) => {
					y.inWishlist = !y.inWishlist;
					const S = r.querySelector('.ubits-data-view');
					S && S.remove();
					const O = B(b);
					r.appendChild(O);
				},
...
const z = ['Default'];
export { v as Default, z as __namedExportsOrder, j as default };
