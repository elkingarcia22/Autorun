function f(o) {
	const {
		...
		c = [
			'ubits-radio-button',
			`ubits-radio-button--${l}`,
			a !== 'default' ? `ubits-radio-button--${a}` : '',
			t ? 'ubits-radio-button--checked' : '',
			n ? 'ubits-radio-button--disabled' : '',
			b,
		]
			.filter(Boolean)
			.join(' '),
  ...
  `,
		_ = s ? `<span class="ubits-radio-button__complementary-text">${s}</span>` : '',
		m = `
