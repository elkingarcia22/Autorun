import { r as k } from './ButtonProvider-Dmy-L2BE.js';
import { r as j } from './ButtonAIProvider-CGOC_9a2.js';
import { r as F } from './StatusTagProvider-6gx2PeGG.js';
import { c as R } from './TooltipProvider-C3qYI4Ua.js';
import { c as W } from './ListProvider-Bx601uSc.js';
import { c as Y, r as D } from './BreadcrumbProvider-BQh42h2n.js';
...
function U(t) {
	const {
		...
		z = ['ubits-header-section', V].filter(Boolean).join(' '),
		L = b
			? k({
					variant: 'secondary',
					size: 'md',
					...
					iconStyle: 'regular',
					iconOnly: !0,
					attributes: { 'data-back-button': 'true', 'aria-label': 'Volver' },
				})
			: '',
		E = y
			? k({
					...
					iconStyle: 'regular',
					iconOnly: !0,
					attributes: { 'data-info-button': 'true', 'aria-label': 'Información' },
				})
			: '',
		f = S && u ? F({ ...u, size: u.size || 'sm', rightIcon: null }) : '';
	let x = '';
	g && i
		? (x = `
    ...
    `);
	const I = t.showSecondaryButton || !1,
		C = I
			? k({
					variant: 'secondary',
					size: 'md',
					text: t.secondaryButtonText || '',
					icon: t.secondaryButtonIcon,
					iconStyle: 'regular',
					attributes: {
						'data-secondary-button': 'true',
						'aria-label': t.secondaryButtonText || 'Botón secundario',
					},
				})
			: '',
		M = t.showOptionsButton || !1,
		h = M
			? k({
					variant: 'secondary',
					size: 'md',
					...
					iconStyle: 'regular',
					iconOnly: !0,
					attributes: { 'data-options-button': 'true', 'aria-label': 'Más opciones' },
				})
			: '',
		O = B.filter((s) => s.id !== 'ai-button'),
		c = B.find((s) => s.id === 'ai-button'),
		e = O.filter((s) => s.variant === 'secondary' || !s.variant),
		o = O.filter((s) => s.variant === 'primary');
	let n = '';
	if (N && B.length > 0) {
		const s = e
			.map((a) => {
				const v = { ...a, size: 'md', text: a.text, onClick: a.onClick };
				return k(v);
			})
			.join('');
		let r = '';
		c &&
			(r = j({
				variant: 'secondary',
				size: 'md',
				text: c.text || 'AI button',
				icon: c.icon || 'sparkles',
				iconStyle: c.iconStyle || 'regular',
				iconOnly: !1,
				disabled: !1,
				badge: !1,
				active: !1,
...
function G(t) {
	const {
		...
		actions: T = [],
		showActions: V = !0,
		showSecondaryButton: z = !1,
		showOptionsButton: L = !1,
		showBreadcrumb: E = !1,
		breadcrumb: f,
		onBackClick: x,
		onInfoClick: I,
	...
	let h = null;
	if ((g ? (h = g) : i && (h = document.getElementById(i)), !h))
		return console.error('HeaderSection: Contenedor no encontrado'), null;
	const O = U({ ...t });
	h.innerHTML = O;
	const c = h.querySelector('.ubits-header-section');
	if (!c) return console.error('HeaderSection: Elemento no encontrado después de renderizar'), null;
	if (S) {
		const e = c.querySelector('[data-back-button="true"]');
		if (e) {
			const o = e.closest('button') || e;
			o && x && o.addEventListener('click', x);
		}
	}
	if (u && B) {
		const e = c.querySelector('[data-info-button="true"]');
		if (e) {
			const o = e.closest('button') || e;
			let n = null,
				m = !1;
			const w = () => {
					...
					const d = o.getBoundingClientRect(),
						H = n.element.getBoundingClientRect(),
						l = d.left + d.width / 2,
						_ =
							parseFloat(
								getComputedStyle(document.documentElement)
									.getPropertyValue('--ubits-spacing-sm')
									.replace('px', ''),
							) || 8,
						P = d.top - H.height - _;
					n.updatePosition({ top: P, left: l });
				},
				s = () => {
					if (!n) {
						const d = o.getBoundingClientRect();
						(n = R({
							...
							showTertiaryButton: !1,
							width: 'sm',
							tailPosition: 'bottom',
							position: { left: d.left + d.width / 2, top: d.top - 200 },
							closeOnOutsideClick: !0,
							onClose: () => {
								(m = !1), o.classList.remove('ubits-button--active');
							},
						})),
					...
					n.open(), (m = !0), o.classList.add('ubits-button--active'), w();
				},
				r = () => {
					n && m && (n.close(), (m = !1), o.classList.remove('ubits-button--active'));
				};
			o.addEventListener('click', (d) => {
				d.stopPropagation(), m ? r() : s(), I && I(d);
			}),
				document.addEventListener('click', (d) => {
					m && n && !o.contains(d.target) && !n.element.contains(d.target) && r();
				});
			let p = null;
			const a = () => {
			...
			window.addEventListener('scroll', a, !0), window.addEventListener('resize', a);
			const v = () => {
				window.removeEventListener('scroll', a, !0),
					window.removeEventListener('resize', a),
					n && (n.destroy(), (n = null));
			};
			c.__headerSectionCleanup = v;
		}
	}
	if (N && A) {
		const e = c.querySelector('.ubits-status-tag');
		e && A.onClick && (e.addEventListener('click', A.onClick), (e.style.cursor = 'pointer'));
	}
	if (V && T.length > 0) {
		const e = T.filter((r) => r.id !== 'ai-button'),
			o = T.find((r) => r.id === 'ai-button'),
			n = e.filter((r) => r.variant === 'secondary' || !r.variant),
			m = e.filter((r) => r.variant === 'primary');
		if (
			(c
				.querySelectorAll(
				...
				.forEach((r, p) => {
					const a = n[p];
					a && a.onClick && r.addEventListener('click', a.onClick);
				}),
			o)
		) {
			const r = c.querySelector('.ubits-header-section__actions .ubits-button-ai');
			r && o.onClick && r.addEventListener('click', o.onClick);
		}
		c.querySelectorAll('.ubits-header-section__actions .ubits-button--primary').forEach((r, p) => {
			const a = m[p];
			a && a.onClick && r.addEventListener('click', a.onClick);
		});
	}
	if (z) {
		const e = c.querySelector('[data-secondary-button="true"]');
		if (e) {
			const o = e.closest('button') || e;
			o && C && o.addEventListener('click', C);
		}
	}
	if (L) {
		const e = c.querySelector('[data-options-button="true"]');
		if (e) {
			const o = e.closest('button') || e,
				n = document.createElement('div');
			n.className = 'ubits-header-section-options-dropdown';
			const w =
				(parseFloat(
					getComputedStyle(document.documentElement)
						.getPropertyValue('--ubits-spacing-12')
						.replace('px', ''),
				) || 48) * 2.5;
			(n.style.cssText = `
        position: fixed;
      ...
      `),
			...
			const r = (a) => {
				if ((a.stopPropagation(), s)) {
					(n.style.display = 'none'), (s = !1), o.classList.remove('ubits-button--active');
					return;
				}
				...
				if (
					...
					(n.style.top = `${v.bottom + window.scrollY + d}px`),
					(n.style.right = `${window.innerWidth - v.right}px`),
					t.optionsMenuItems && t.optionsMenuItems.length > 0)
				) {
					const q = `header-section-options-menu-${Date.now()}`;
					(n.id = q), (n.innerHTML = '');
					const H = t.optionsMenuItems.map((l) => ({
						label: l.label,
					...
					try {
						W({
							...
							size: 'md',
							maxHeight: 'none',
							onSelectionChange: (l, _) => {
								if (l && t.optionsMenuItems && t.optionsMenuItems[_]) {
									const P = t.optionsMenuItems[_];
									P.onClick &&
										P.onClick(new MouseEvent('click'), { label: l.label, value: l.value }),
										(n.style.display = 'none'),
										(s = !1),
										o.classList.remove('ubits-button--active');
								}
							},
						}),
							setTimeout(() => {
								const l = n.querySelector('.ubits-list');
								l &&
									H.length > 5 &&
									((l.style.maxHeight = 'calc(var(--ubits-spacing-12) * 6)'),
									(l.style.overflowY = 'auto'));
							}, 0);
					} catch (l) {
						console.error('Error creating options menu:', l);
					}
				}
				(s = !0), o.classList.add('ubits-button--active'), M && M(a);
			};
			...
			document.addEventListener('click', p),
				(c.__optionsDropdownCleanup = () => {
					document.removeEventListener('click', p);
				});
		}
	}
	if (E && f) {
		const e = c.querySelector('.ubits-header-section__breadcrumb-wrapper');
		if (e) {
			const o = `header-section-breadcrumb-${Date.now()}`;
			(e.id = o), (e.innerHTML = '');
			try {
				Y(f, o);
			} catch (n) {
				console.error('Error creating breadcrumb:', n), (e.innerHTML = D(f));
			}
		}
	}
	return c;
...
const at = {
		...
		argTypes: {
			...
			statusTagStatus: {
				...
				if: { arg: 'showStatusTag' },
				table: { type: { summary: 'string' }, defaultValue: { summary: 'pending' } },
			},
			showActions: {
				control: { type: 'boolean' },
			...
			containerId: { control: !1, description: 'ID del contenedor (asignado automáticamente)' },
		},
	},
	$ = {
		args: {
			...
			showOptionsButton: !0,
			optionsMenuItems: [
				{ label: 'Opción 1', value: 'option-1', onClick: (t, i) => {} },
				{ label: 'Opción 2', value: 'option-2', onClick: (t, i) => {} },
				{ label: 'Opción 3', value: 'option-3', onClick: (t, i) => {} },
			],
			showBreadcrumb: !0,
			breadcrumbItems: [
				{ id: 'home', label: 'Home', url: '#', onClick: (t) => {} },
				{ id: 'products', label: 'Products', url: '#', onClick: (t) => {} },
				{ id: 'current', label: 'Current Product' },
			],
		},
		render: (t) => {
			const i = document.createElement('div');
			(i.id = t.containerId || 'header-section-story-container'),
				(i.style.width = '100%'),
				(i.style.maxWidth = '1220px'),
    ...
    `),
				...
				requestAnimationFrame(() => {
					try {
						const b =
								t.showStatusTag && t.statusTagLabel
									? { label: t.statusTagLabel, status: t.statusTagStatus || 'pending', size: 'sm' }
									: void 0,
							y = [];
						t.showAIAction &&
							y.push({
						...
						const S =
							t.showBreadcrumb && t.breadcrumbItems
								? { items: t.breadcrumbItems, separator: '>' }
								: void 0;
						G({
							...t,
							container: i,
						...
						console.error('Error creating HeaderSection:', b),
							(i.innerHTML = `<div style="color: var(--modifiers-normal-color-light-feedback-fg-error-subtle-default); padding: 16px;">Error: ${b instanceof Error ? b.message : String(b)}</div>`);
					}
				}),
				i
			);
...
const it = ['Default'];
export { $ as Default, it as __namedExportsOrder, at as default };
