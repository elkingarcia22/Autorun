...
const w = {
	...
	argTypes: {
		...
		maxHeight: {
			...
			table: { defaultValue: { summary: '400px' }, type: { summary: 'string' } },
		},
		showScrollbar: {
...
function x(a) {
	const {
			size: s = 'md',
			item1State: o = 'default',
			item2State: r = 'active',
			item3State: d = 'disabled',
			item4State: b = 'default',
			maxHeight: u = '400px',
			showScrollbar: c = !1,
		} = a,
		y = [
			{ label: 'Item 1', state: o },
			{ label: 'Item 2', state: r },
			{ label: 'Item 3', state: d },
			{ label: 'Item 4', state: b },
		...
		h = `list-container-${Date.now()}`,
		p = `list-${Date.now()}`,
		v = `list-scrollbar-${Date.now()}`;
	if (c) {
		let n = `
			...
			y.forEach((i) => {
				const e = i.state || 'default',
					l = [
						...
						.join(' '),
					t = [];
				e === 'active' && t.push('aria-selected="true"'),
					e === 'disabled' ? t.push('aria-disabled="true"') : t.push('tabindex="0"'),
					(n += `
				...
				if (i && e)
					if (typeof window.createScrollbarLocal == 'function') {
						if (i.scrollHeight > i.clientHeight) {
							const l = window.createScrollbarLocal(i, e, 'vertical');
							l &&
								((e.style.pointerEvents = 'auto'),
								(i._scrollbarInstance = l),
								setTimeout(() => {
									const t = e.querySelector('.ubits-scrollbar'),
										m = e.querySelector('.ubits-scrollbar__bar');
									t &&
										m &&
						...
						S(
							async () => {
								const { createScrollbar: l } = await import('./ScrollProvider-BVL7eCy8.js');
								return { createScrollbar: l };
							},
							[],
							import.meta.url,
						)
							.then(({ createScrollbar: l }) => {
								if (i.scrollHeight > i.clientHeight) {
									const t = l({ orientation: 'vertical', targetId: p, containerId: v });
									t &&
										((e.style.pointerEvents = 'auto'),
										(i._scrollbarInstance = t),
										setTimeout(() => {
											const m = e.querySelector('.ubits-scrollbar'),
												g = e.querySelector('.ubits-scrollbar__bar');
											m &&
												g &&
							...
							.catch((l) => {
								console.error('Error al importar ScrollProvider:', l);
							});
			}, 100),
			n
		);
	} else {
		let n = `<div class="ubits-list" role="list" style="max-height: ${u}; overflow-y: auto; -ms-overflow-style: none; scrollbar-width: none;">`;
		return (
			y.forEach((i) => {
				const e = i.state || 'default',
					l = [
...
const f = {
	...
	render: (a) => {
		...
		(s.style.padding = '20px'),
			(s.style.background = 'var(--modifiers-normal-color-light-bg-1, #ffffff)'),
			(s.style.borderRadius = '8px');
		const o = document.createElement('div');
		(o.style.display = 'flex'), (o.style.flexDirection = 'column'), (o.style.gap = '24px');
		const r = document.createElement('div');
		(r.style.padding = '16px'),
			(r.style.background = 'var(--modifiers-normal-color-light-bg-2, #f9fafb)'),
			(r.style.borderRadius = '8px'),
			(r.style.border = '1px solid var(--modifiers-normal-color-light-border-1, #e5e7eb)'),
			(r.innerHTML = `
    ...
    `);
		...
		(d.id = `list-preview-${Date.now()}`), (d.style.width = '100%'), (d.style.maxWidth = '400px');
		const b = x(a);
		d.innerHTML = b;
		const u = 'ubits-scrollbar-css';
		if (!document.getElementById(u)) {
			const c = document.createElement('link');
			(c.id = u),
				(c.rel = 'stylesheet'),
				(c.href = '../../components/scroll/src/styles/scroll.css'),
				document.head.appendChild(c);
		}
		return o.appendChild(r), o.appendChild(d), s.appendChild(o), s;
	},
};
f.parameters = {
	...f.parameters,
...
const $ = ['Default'];
export { f as Default, $ as __namedExportsOrder, w as default };
