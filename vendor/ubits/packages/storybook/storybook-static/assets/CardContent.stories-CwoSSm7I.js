const l = [
	...
	d = { Básico: 'far fa-gauge-min', Intermedio: 'far fa-gauge', Avanzado: 'far fa-gauge-max' },
...
function o(e, t = 'regular') {
	const s = t === 'solid' ? 'fas' : 'far',
		a = e.startsWith('fa-') ? e : `fa-${e}`;
	return `<i class="${s} ${a}"></i>`;
}
function u(e) {
	const t = m[e.status],
		s = t.class,
		a = t.text,
		i = d[e.level] || d.Intermedio;
	return `
...
function g(e) {
	...
	if (!a) throw new Error('Failed to create card element');
	return a;
}
const f = {
	...
	argTypes: {
		type: {
			...
			table: { defaultValue: { summary: 'Curso' }, type: { summary: l.join(' | ') } },
		},
		title: {
			control: { type: 'text' },
			description: 'Título del contenido',
			table: { defaultValue: { summary: 'Segmenta la experiencia del cliente' } },
		},
		provider: {
			control: { type: 'select' },
			options: Object.keys(r),
			description: 'Proveedor/Aliado del contenido',
			table: { defaultValue: { summary: 'UBITS' }, type: { summary: Object.keys(r).join(' | ') } },
		},
		providerLogo: {
			control: { type: 'text' },
			description:
		...
		duration: {
			...
			table: { defaultValue: { summary: '60 min' } },
		},
		level: {
			control: { type: 'select' },
		...
		progress: {
			...
			table: { defaultValue: { summary: '0' }, type: { summary: 'number (0-100)' } },
		},
		image: {
			control: { type: 'text' },
		...
		competency: {
			...
			table: { defaultValue: { summary: 'Product design' }, type: { summary: c.join(' | ') } },
		},
		language: {
			control: { type: 'select' },
			options: p,
			description: 'Idioma del contenido',
			table: { defaultValue: { summary: 'Español' }, type: { summary: p.join(' | ') } },
		},
	},
};
function v(e) {
	const t = r[e];
	return t ? t.replace('assets/images/', '/images/') : r.UBITS;
}
function y(e) {
	const t = e.provider || 'UBITS',
		s = e.providerLogo || v(t);
	return {
		type: e.type || 'Curso',
		title: e.title || 'Segmenta la experiencia del cliente',
		provider: t,
		providerLogo: s,
		duration: e.duration || '60 min',
		level: e.level || 'Básico',
		...
		competency: e.competency || 'Product design',
...
const n = {
	...
	render: (e) => {
		...
		(s.style.display = 'flex'),
			...
			(s.style.padding = '48px'),
			(s.style.background = 'var(--modifiers-normal-color-light-bg-1)'),
			(s.style.border = '1px solid var(--modifiers-normal-color-light-border-1)'),
			(s.style.borderRadius = '8px'),
			(s.style.minHeight = '400px');
		const a = document.createElement('div');
		(a.style.maxWidth = '360px'), (a.style.width = '100%');
		const i = g(t);
...
const b = ['Default'];
export { n as Default, b as __namedExportsOrder, f as default };
