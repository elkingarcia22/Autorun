import { c as m, r as u } from './InputProvider-B1iJaViU.js';
...
function g(e = 20) {
	return Array.from({ length: e }, (o, t) => ({ value: `opt-${t + 1}`, text: `Opción ${t + 1}` }));
}
function f() {
	return [
...
const n = {
	args: {
		...
		rightIcon: '',
		value: '',
		showRichTextToolbar: !1,
	},
	render: (e) => {
		const o = document.createElement('div');
		o.style.cssText = 'max-width: 600px; margin: 20px auto; padding: 20px;';
		const t = `input-storybook-${Math.random().toString(36).substr(2, 9)}`,
			a = e.type === 'textarea' ? '' : e.leftIcon,
			i = e.type === 'textarea' ? '' : e.rightIcon,
			s = {
				...e,
				containerId: t,
				leftIcon: a,
				rightIcon: i,
				selectOptions: e.type === 'select' ? g(20) : void 0,
				autocompleteOptions: e.type === 'autocomplete' ? f() : void 0,
			};
		try {
					...
					requestAnimationFrame(() => {
						...
						if (c)
							try {
								...
								console.error('Error creating input:', d);
								const p = u(s);
								c.innerHTML = p;
							}
      ...
      `),
			...
			console.error('Error rendering input:', r),
				(o.innerHTML = `<div style="color: red; padding: 16px;">Error: ${r}</div>`);
		}
		return o;
	},
};
...
const w = ['Default'];
export { n as Default, w as __namedExportsOrder, I as default };
