import { r as c } from './StatusTagProvider-6gx2PeGG.js';
...
const m = {
		...
		argTypes: {
			...
			leftIcon: {
				control: { type: 'text' },
				description:
					'Icono FontAwesome izquierdo (ej: "grid-2"). Dejar vacío para ocultar el icono izquierdo.',
				table: {
			...
			rightIcon: {
				control: { type: 'text' },
				description:
					'Icono FontAwesome derecho (ej: "chevron-down"). Dejar vacío para ocultar el icono derecho.',
				table: {
					type: { summary: 'string | null' },
	...
	r = {
		...
		render: (e) => {
			...
			(o.style.padding = '20px'),
				(o.style.background = 'var(--modifiers-normal-color-light-bg-1, #ffffff)'),
			...
			(t.style.display = 'flex'),
				...
				(t.style.background = 'var(--modifiers-normal-color-light-bg-2, #f9fafb)'),
				(t.style.borderRadius = '8px'),
				(t.style.marginBottom = '20px');
			const i = document.createElement('div'),
				l = e.leftIcon && e.leftIcon.trim() !== '' ? e.leftIcon : void 0,
				s = e.rightIcon && e.rightIcon.trim() !== '' ? e.rightIcon : void 0;
			if (((i.innerHTML = c({ ...e, leftIcon: l, rightIcon: s })), e.clickable)) {
				const a = i.querySelector('.ubits-status-tag');
				a &&
					(a.setAttribute('role', 'button'),
					a.setAttribute('tabindex', '0'),
					a.addEventListener('click', (n) => {
						n.preventDefault(), e.onClick && e.onClick(n);
					}),
					a.addEventListener('keydown', (n) => {
						(n.key === 'Enter' || n.key === ' ') && (n.preventDefault(), e.onClick && e.onClick(n));
					}));
			}
			return t.appendChild(i), o.appendChild(t), o;
		},
	};
r.parameters = {
	...r.parameters,
...
const g = ['Default'];
export { r as Default, g as __namedExportsOrder, m as default };
