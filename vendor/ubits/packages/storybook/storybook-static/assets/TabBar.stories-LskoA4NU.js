...
function w(e, o = 'regular') {
	const t = o === 'regular' ? 'far' : 'fas',
		r = e.startsWith('fa-') ? e : `fa-${e}`;
	return `<i class="${t} ${r}"></i>`;
}
function B(e) {
	const { items: o, activeTabId: t, visible: r = !1, className: c = '' } = e,
		g = ['ubits-tabbar', r ? 'ubits-tabbar--visible' : '', c].filter(Boolean).join(' '),
		d = o
			.map((i) => {
				const n = ['ubits-tabbar-item', i.id === t ? 'ubits-tabbar-item--active' : '']
					.filter(Boolean)
					.join(' ');
				let m = '';
				return (
					i.avatar
						? (m = `<img src="${i.avatar}" alt="${i.avatarAlt || i.label}" class="ubits-tabbar-avatar">`)
						: i.icon && (m = `<span class="ubits-tabbar-icon">${w(i.icon)}</span>`),
					`
      <div 
        class="${n}" 
        data-tab-id="${i.id}"
...
function q(e) {
	...
	let n = null;
	if ((t ? (n = t) : o && (n = document.getElementById(o)), !n))
		return console.error('TabBar: Contenedor no encontrado'), null;
	const m = n.classList.contains('ubits-tabbar-preview-container'),
		b = B({ ...e, visible: d || m, className: m ? 'ubits-tabbar--preview' : '' });
	(n.innerHTML = b),
		m && window.getComputedStyle(n).position === 'static' && (n.style.position = 'relative');
	const a = n.querySelector('.ubits-tabbar');
	if (!a) return console.error('TabBar: Elemento no encontrado después de renderizar'), null;
	const p = e.treeMenuSize || 'md';
	return (
		D(
			a,
			r,
			...
			i,
			u,
			e.floatingMenuSections,
			e.profileMenuItems,
			e.onFloatingMenuItemClick,
			e.onProfileMenuItemClick,
			n,
			p,
		),
		a
...
function E(e, o = 0, t = 'md', r = 'floating-menu') {
	const c = (e.children && e.children.length > 0) || e.subitems?.length > 0,
		g = e.isLink || (!c && e.url),
		d = `${r}-node-${o}-${e.id}`,
		i = t === 'xs' ? '8px 12px' : t === 'sm' ? '10px 14px' : t === 'lg' ? '16px 20px' : '12px 16px',
		u = t === 'xs' ? '28px' : t === 'sm' ? '32px' : t === 'lg' ? '48px' : '40px',
		n =
			t === 'xs'
		...
		a = t === 'xs' ? '10px' : t === 'sm' ? '12px' : t === 'lg' ? '16px' : '14px',
		p =
			t === 'xs'
...
function A(e, o = 'md') {
	const t = `floating-menu-${Date.now()}`,
		r = e.map((c) => E(c, 0, o, t)).join('');
	return `
...
function $(e, o = 0, t = 'md') {
	const r = e.children && e.children.length > 0,
		c = o * 24,
		g = `ubits-profile-tree-${r ? 'header' : 'link'}--${t}`,
		d = o === 0 ? `<i class="far fa-${e.icon} ubits-profile-tree-icon"></i>` : '';
	if (!r)
		return `
    ...
    `;
	const i = e.children.map((u) => $(u, o + 1, t)).join('');
	return `
    <div class="ubits-profile-tree-item" data-profile-item-id="${e.id}" data-tree-level="${o}" style="padding-left: ${c}px;">
...
function D(e, o, t, r = !1, c, g, d, i, u, n, m = 'md') {
	const b = e.querySelectorAll('.ubits-tabbar-item'),
		a = n || e.parentElement,
		p = a?.classList.contains('ubits-tabbar-preview-container');
	let s = null,
		l = null;
	if (g && g.length > 0) {
		(s = document.getElementById('ubits-floating-menu-container') || document.createElement('div')),
			(s.id = 'ubits-floating-menu-container'),
			p
				? (s.style.cssText =
						'position: absolute; top: 0; left: 0; right: 0; bottom: 68px; width: 100%; height: 500px; z-index: 2000; overflow: visible; display: none;')
				: (s.style.cssText = ''),
			document.getElementById('ubits-floating-menu-container') ||
				(a ? a.appendChild(s) : document.body.appendChild(s));
		const M = A(g, m);
		(s.innerHTML = M), _(s, i);
	}
	if (d && d.length > 0) {
		(l = document.getElementById('ubits-profile-menu-container') || document.createElement('div')),
			(l.id = 'ubits-profile-menu-container'),
			p
				? (l.style.cssText =
						'position: absolute; bottom: 68px; left: 0; right: 0; width: 100%; max-width: 100%; z-index: 2001; overflow: visible; display: none;')
				: (l.style.cssText = ''),
			document.getElementById('ubits-profile-menu-container') ||
				(a ? a.appendChild(l) : document.body.appendChild(l));
		const M = P(d, m);
		if (((l.innerHTML = M), p)) {
			const f = l.querySelector('.ubits-profile-menu');
			f &&
				(f.classList.add('ubits-profile-menu--preview'),
				(f.style.cssText =
					'position: absolute; bottom: 0; left: 0; right: 0; width: 100%; max-width: 100%; display: block;'));
		}
		H(l, d, u);
	}
	b.forEach((M) => {
		const f = M,
			h = f.getAttribute('data-tab-id');
		if (!h) return;
		const x = o.find((k) => k.id === h);
		x &&
			f.addEventListener('click', (k) => {
				if ((k.preventDefault(), k.stopPropagation(), h === 'modo-oscuro' && r)) {
					if ((N(e, c), s)) {
						s.style.display = 'none';
						const v = s.querySelector('.ubits-floating-menu');
						v && v.classList.remove('ubits-floating-menu--show');
					}
					if (l) {
						l.style.display = 'none';
						const v = l.querySelector('.ubits-profile-menu');
						v && v.classList.remove('ubits-profile-menu--show');
					}
					return;
				}
				if (h === 'modulos' && s) {
					const v = s.querySelector('.ubits-floating-menu');
					if (v)
						if (v.classList.contains('ubits-floating-menu--show'))
							v.classList.remove('ubits-floating-menu--show');
						else {
							if ((v.classList.add('ubits-floating-menu--show'), p && s)) {
								v.classList.add('ubits-floating-menu--preview');
								const y = e.getBoundingClientRect(),
									I = n ? n.getBoundingClientRect() : { top: 0 },
									C = 60,
									S = 8;
								y.top - I.top,
									...
									(v.style.cssText =
										'position: absolute; top: 0; left: 0; right: 0; bottom: 0; width: 100%; max-width: 100%; display: block; box-sizing: border-box;');
							}
							if (l) {
								const y = l.querySelector('.ubits-profile-menu');
								y && y.classList.remove('ubits-profile-menu--show'), (l.style.display = 'none');
							}
						}
					return;
				}
				if (h === 'perfil' && l) {
					const v = l.querySelector('.ubits-profile-menu');
					if (v)
						if (v.classList.contains('ubits-profile-menu--show'))
							v.classList.remove('ubits-profile-menu--show'), (l.style.display = 'none');
						else {
							if ((v.classList.add('ubits-profile-menu--show'), p && l)) {
								...
								y &&
									((y.style.cssText =
										'position: absolute; bottom: 0; left: 0; right: 0; width: 100%; max-width: 100%; display: block;'),
									y.classList.add('ubits-profile-menu--preview'));
							}
							if (s) {
								const y = s.querySelector('.ubits-floating-menu');
								y && y.classList.remove('ubits-floating-menu--show');
							}
						}
					return;
				}
				x.onClick && x.onClick(x, k), O(e, h), t && t(h, x, f);
			});
	});
}
function _(e, o) {
	const t = e.querySelector('.ubits-floating-menu');
	if (!t) return;
	const r = t.querySelector('#ubits-floating-menu-close');
	r &&
		r.addEventListener('click', (i) => {
			i.preventDefault(),
				i.stopPropagation(),
				e && (e.style.display = 'none'),
				t && t.classList.remove('ubits-floating-menu--show');
		});
	const c = t.querySelector('.ubits-tree-menu');
	c &&
		(c.addEventListener('click', (i) => {
			const n = i.target.closest('.ubits-tree-node__content');
			if (!n) return;
			if (n.classList.contains('ubits-tree-node__content--expandable')) {
				const b = n.getAttribute('data-node-id'),
					a = c.querySelector(`[data-children-id="${b}"]`),
					p = n.querySelector('.ubits-tree-node__chevron i'),
					s = n.getAttribute('data-expanded') === 'true';
				a &&
					(s
						? ((a.style.display = 'none'),
							n.setAttribute('data-expanded', 'false'),
							n.setAttribute('aria-expanded', 'false'),
							p && (p.className = 'far fa-chevron-right'))
						: ((a.style.display = 'block'),
							n.setAttribute('data-expanded', 'true'),
							n.setAttribute('aria-expanded', 'true'),
							p && (p.className = 'far fa-chevron-down')));
			}
			if (
				(c.querySelectorAll('.ubits-tree-node__content').forEach((b) => {
					b.classList.remove('ubits-tree-node__content--active'),
						b.removeAttribute('aria-selected');
				}),
				n.classList.add('ubits-tree-node__content--active'),
				n.setAttribute('aria-selected', 'true'),
				!n.classList.contains('ubits-tree-node__content--expandable'))
			) {
				const b = n.getAttribute('data-section-id'),
					a = n.getAttribute('href');
				o && b && o(b, void 0, a || void 0);
			}
		}),
		c.addEventListener('keydown', (i) => {
			const n = i.target.closest('.ubits-tree-node__content');
			n && (i.key === 'Enter' || i.key === ' ') && (i.preventDefault(), n.click());
		}));
	const g = (i) => {
		i.key === 'Escape' &&
			t.classList.contains('ubits-floating-menu--show') &&
			((e.style.display = 'none'), t.classList.remove('ubits-floating-menu--show'));
	};
	document.addEventListener('keydown', g);
	const d = (i) => {
		if (t.classList.contains('ubits-floating-menu--show')) {
			const u = i.target;
			!t.contains(u) &&
				!u.closest('[data-tab-id="modulos"]') &&
				((e.style.display = 'none'), t.classList.remove('ubits-floating-menu--show'));
		}
	};
...
function H(e, o, t) {
	const r = e.querySelector('.ubits-profile-menu');
	if (!r) return;
	r.querySelectorAll('.ubits-profile-tree-node').forEach((d) => {
		const i = d.querySelector('.ubits-profile-tree-header');
		i &&
			i.addEventListener('click', (u) => {
				u.preventDefault(), u.stopPropagation();
				const n = d.getAttribute('data-tree-node-id');
				n && F(r, n);
			});
	}),
		r.querySelectorAll('.ubits-profile-tree-link').forEach((d) => {
			d.addEventListener('click', (i) => {
				i.preventDefault(), i.stopPropagation();
				const u = d.closest('[data-profile-item-id]')?.getAttribute('data-profile-item-id');
				if (
					(r.querySelectorAll('.ubits-profile-tree-link').forEach((n) => {
						n.classList.remove('ubits-profile-tree-link--active');
					}),
					d.classList.add('ubits-profile-tree-link--active'),
					u)
				) {
					const n = o.find((m) => m.id === u);
					n &&
						(n.onClick ? n.onClick() : n.url && (window.location.href = n.url), t && t(u, n), L());
				}
			});
		}),
		document.addEventListener('keydown', (d) => {
			d.key === 'Escape' && r.classList.contains('ubits-profile-menu--show') && L();
		}),
		document.addEventListener('click', (d) => {
			if (r.classList.contains('ubits-profile-menu--show')) {
				const i = d.target;
				!r.contains(i) && !i.closest('[data-tab-id="perfil"]') && L();
			}
		});
}
function F(e, o) {
	const t = e.querySelector(`[data-tree-children-id="${o}"]`),
		r = e.querySelector(`[data-chevron-id="${o}"]`),
		c = e.querySelector(`[data-tree-node-id="${o}"] .ubits-profile-tree-header`);
	if (!t || !r) {
		console.warn(`Profile tree menu node not found: ${o}`, { children: !!t, chevron: !!r });
		return;
	}
	window.getComputedStyle(t).display !== 'none'
		? ((t.style.display = 'none'),
			(r.style.transform = 'rotate(0deg)'),
			c && c.classList.remove('ubits-profile-tree-header--active'))
		: ((t.style.display = 'block'),
			(r.style.transform = 'rotate(180deg)'),
			c && c.classList.add('ubits-profile-tree-header--active'));
}
function L() {
	const e = document.getElementById('ubits-profile-menu');
	e && e.classList.remove('ubits-profile-menu--show');
}
function O(e, o) {
	e.querySelectorAll('.ubits-tabbar-item').forEach((r) => {
		r.getAttribute('data-tab-id') === o
			? r.classList.add('ubits-tabbar-item--active')
			: r.classList.remove('ubits-tabbar-item--active');
	});
}
function N(e, o) {
	let t = e.closest('[data-theme]');
	t || (t = document.body);
	const c = (t.getAttribute('data-theme') || 'light') === 'dark' ? 'light' : 'dark';
	t.setAttribute('data-theme', c), o && o(c === 'dark');
}
const j = [
		{
			id: 'aprendizaje',
	...
	z = [
		{ id: 'inicio', title: 'Inicio', icon: 'house', url: null, isLink: !1 },
		{
			id: 'empresa',
			title: 'Empresa',
			icon: 'building',
		...
		{ id: 'diagnóstico', title: 'Diagnóstico', icon: 'chart-mixed', url: null, isLink: !1 },
		{
			id: 'desempeño',
			title: 'Desempeño',
			icon: 'bars-progress',
		...
		{ id: 'encuestas', title: 'Encuestas', icon: 'clipboard-list-check', url: null, isLink: !1 },
	],
	R = [
	...
	Z = {
		...
		parameters: {
			...
			a11y: {
				config: {
					rules: [
						{ id: 'focus-order-semantics', enabled: !1 },
						{ id: 'focusable-content', enabled: !1 },
					],
				},
				manual: !0,
			},
			interactions: { disable: !0 },
			actions: { disable: !0 },
			docs: {
				description: {
					component:
		...
		argTypes: {
			...
			activeTabId: {
				...
				table: { type: { summary: 'string' }, defaultValue: { summary: 'modulos' } },
			},
			darkModeEnabled: {
...
function U(e) {
	return { floatingMenuSections: e === 'admin' ? z : j, profileMenuItems: e === 'admin' ? V : R };
}
const W = [
		{ id: 'modulos', label: 'Módulos', icon: 'th-large', onClick: (e, o) => {} },
		{
			id: 'perfil',
			label: 'Mi perfil',
		...
		{ id: 'modo-oscuro', label: 'Modo oscuro', icon: 'moon', onClick: (e, o) => {} },
	],
	T = {
		args: {
			...
			onTabChange: (e, o, t) => {},
			onDarkModeToggle: (e) => {},
			onFloatingMenuItemClick: (e, o, t) => {},
			onProfileMenuItemClick: (e, o) => {},
		},
		render: (e) => {
			const o = window.console.error;
			(window.console.error = (...a) => {
				const p = a.join(' ');
				p.includes('Could not determine window of node') ||
					p.includes('HTMLBodyElement') ||
					p.includes('Minified React error') ||
					p.includes('deferred DOM Node') ||
					o.apply(window.console, a);
			}),
				setTimeout(() => {
					window.console.error = o;
				}, 5e3);
			const t = document.getElementById(e.containerId || 'tabbar-story-container');
			t && t.parentElement && (t.innerHTML = '');
			const r = e.variant || 'colaborador',
				c = U(r),
				g = c.floatingMenuSections,
				d = c.profileMenuItems,
    ...
    `;
      ...
      order: 1;
    `;
			const n = e.items?.find((a) => a.id === e.activeTabId) || e.items?.[0];
			(u.innerHTML = `
    ...
    `),
			...
			(m.id = e.containerId || 'tabbar-story-container'),
				m.classList.add('ubits-tabbar-preview-container'),
				(m.style.cssText = `
    ...
    `),
			...
			const b = (a) => {
				const p = a.target;
				if (p === document.body || (!i.contains(p) && p.tagName === 'BODY'))
					return a.stopImmediatePropagation(), a.preventDefault(), !1;
			};
			return (
				document.addEventListener('focusin', b, { capture: !0 }),
				document.addEventListener('focus', b, { capture: !0 }),
				document.body.addEventListener('focusin', b, { capture: !0 }),
				document.body.addEventListener('focus', b, { capture: !0 }),
				requestAnimationFrame(() => {
					requestAnimationFrame(() => {
						setTimeout(() => {
							try {
								const a = Object.getOwnPropertyDescriptor(HTMLElement.prototype, 'focus')?.value;
								a &&
									...
									q({
										...
										container: m,
										visible: e.visible !== !1,
										darkModeEnabled: e.darkModeEnabled !== !1,
										floatingMenuSections: c.floatingMenuSections,
										profileMenuItems: c.profileMenuItems,
										onTabChange: (s, l, M) => {
											try {
												const f = u.querySelector('div[style*="grid"]');
												if (f) {
													const h = f.querySelector('div:nth-child(3)');
													h && (h.innerHTML = `<strong>Tab Activo:</strong> ${l.label}`);
												}
											} catch {}
											e.onTabChange && e.onTabChange(s, l, M);
										},
										onDarkModeToggle: e.onDarkModeToggle,
										onFloatingMenuItemClick: e.onFloatingMenuItemClick,
										onProfileMenuItemClick: e.onProfileMenuItemClick,
									}) ||
										(m.innerHTML =
											'<p style="color: var(--modifiers-normal-color-light-feedback-border-error); padding: 16px; font-size: 12px;">Error: No se pudo crear el TabBar</p>'),
									setTimeout(() => {
										if (a)
											try {
												Object.defineProperty(document.body, 'focus', {
								...
								m.innerHTML = `<p style="color: var(--modifiers-normal-color-light-feedback-border-error); padding: 16px; font-size: 12px;">Error: ${a}</p>`;
							}
						}, 500);
					});
				}),
				setTimeout(() => {
					document.removeEventListener('focusin', b, { capture: !0 }),
						document.removeEventListener('focus', b, { capture: !0 }),
						document.body.removeEventListener('focusin', b, { capture: !0 }),
						document.body.removeEventListener('focus', b, { capture: !0 });
				}, 3e3),
				i
			);
		},
...
T.parameters = {
  ...
  render: args => {
    ...
    return wrapper;
		...
		description: {
			story: 'Story por defecto con todos los controles',
			...T.parameters?.docs?.description,
		},
	},
};
const J = ['Default'];
export { T as Default, J as __namedExportsOrder, Z as default };
