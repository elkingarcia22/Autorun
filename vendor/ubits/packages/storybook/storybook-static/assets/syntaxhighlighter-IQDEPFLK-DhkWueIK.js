import {
	...
	S as xt,
	a as vt,
} from './DocsRenderer-GHJI37HO-HaPcB3v9.js';
import { b as Z, c as F } from './iframe-BcZai5-G.js';
...
function wt(e, r) {
	...
	if (Object.getOwnPropertySymbols) {
		...
		for (a = 0; a < o.length; a++)
			(t = o[a]), r.indexOf(t) === -1 && {}.propertyIsEnumerable.call(e, t) && (n[t] = e[t]);
	}
	return n;
}
var At = F({
						...
						for (; u < o.length; ) {
							...
							if (C.type === 'keyword' && C.content === 'mutation') {
								...
								if (
									...
									for (var E = u; E < S; E++) {
										var _ = o[E];
										_.type === 'variable' && x.indexOf(_.content) >= 0 && h(_, 'variable-input');
									}
								}
							}
						}
			...
			function t(a) {
				(function (n) {
					...
					for (
						...
						n.util.type(f) === 'RegExp' && (f = n.languages.javascript[l] = { pattern: f });
						var h = f.inside || {};
				...
				(function (n) {
					...
					(l = f(l).source),
						...
						n.languages.insertBefore(
							'inside',
							'attr-name',
							{ spread: { pattern: f(/<SPREAD>/.source), inside: n.languages.jsx } },
							n.languages.jsx.tag,
						),
						n.languages.insertBefore(
							'inside',
									...
									(typeof S != 'string' &&
										(S.type === 'tag' && S.content[0] && S.content[0].type === 'tag'
											? S.content[0].content[0].content === '</'
												? b.length > 0 &&
													b[b.length - 1].tagName === h(S.content[0].content[1]) &&
													b.pop()
												: S.content[S.content.length - 1].content === '/>' ||
													b.push({ tagName: h(S.content[0].content[1]), openedBraces: 0 })
											: b.length > 0 && S.type === 'punctuation' && S.content === '{'
												...
												: b.length > 0 &&
													...
													? b[b.length - 1].openedBraces--
													: (E = !0)),
									(E || typeof S == 'string') && b.length > 0 && b[b.length - 1].openedBraces === 0)
								) {
									var _ = h(S);
									y < x.length - 1 &&
										(typeof x[y + 1] == 'string' || x[y + 1].type === 'plain-text') &&
					...
					(n.languages.markdown = n.languages.extend('markup', {})),
							...
							table: {
								...
								inside: {
									'table-data-rows': {
										...
										lookbehind: !0,
										inside: {
											'table-data': { pattern: RegExp(d), inside: n.languages.markdown },
											punctuation: /\|/,
										},
									},
									'table-line': {
						...
						['url', 'bold', 'italic', 'strike'].forEach(function (y) {
							['url', 'bold', 'italic', 'strike', 'code-snippet'].forEach(function (S) {
								y !== S &&
									(n.languages.markdown[y].inside.content.inside[S] = n.languages.markdown[S]);
							});
						}),
						...
						n.hooks.add('wrap', function (y) {
							if (y.type === 'code-block') {
								...
								if (s) y.content = n.highlight(b(y.content.value), s, S);
									...
									var p = 'md-' + new Date().valueOf() + '-' + Math.floor(Math.random() * 1e16);
									(y.attributes.id = p),
										n.plugins.autoloader.loadLanguages(S, function () {
											var g = document.getElementById(p);
											g && (g.innerHTML = n.highlight(g.textContent, n.languages[S], S));
										});
								}
							}
						});
						...
						return (
							(S = S.replace(/&(\w{1,8}|#x?[\da-f]{1,8});/gi, function (E, _) {
								if (((_ = _.toLowerCase()), _[0] === '#')) {
									var A;
									return (
										_[1] === 'x' ? (A = parseInt(_.slice(2), 16)) : (A = Number(_.slice(1))), x(A)
									);
								} else {
									var i = C[_];
									return i || E;
					...
					Object.defineProperty(a.languages.markup.tag, 'addInlined', {
						value: function (n, o) {
							...
							(u['language-' + o] = {
								...
								(u.cdata = /^<!\[CDATA\[|\]\]>$/i);
							var d = { 'included-cdata': { pattern: /<!\[CDATA\[[\s\S]*?\]\]>/i, inside: u } };
							d['language-' + o] = { pattern: /[\s\S]+/, inside: a.languages[o] };
							var l = {};
							(l[n] = {
								pattern: RegExp(
			...
			function t(a) {
				(function (n) {
					...
					(n.languages.yaml = {
						...
						boolean: { pattern: h(/false|true/.source, 'i'), lookbehind: !0, alias: 'important' },
						null: { pattern: h(/null|~/.source, 'i'), lookbehind: !0, alias: 'important' },
	...
	Pt = F({
		'../node_modules/hastscript/factory.js'(e, r) {
			...
			function h(E, _) {
				...
				return E === 'input' || !A || typeof A != 'string'
					...
					: typeof _.children == 'object' && 'length' in _.children
						...
						: ((A = A.toLowerCase()),
							E === 'button'
								? A !== 'menu' && A !== 'submit' && A !== 'reset' && A !== 'button'
								: 'value' in _);
			}
			function C(E, _) {
				var A, i;
			...
			function S(E) {
				for (var _ = E.length, A = -1, i = {}, s; ++A < _; ) (s = E[A]), (i[s.toLowerCase()] = s);
				return i;
			}
		},
	}),
	...
	Qt = F({
		'../node_modules/refractor/node_modules/parse-entities/index.js'(e, r) {
			...
			var $ = 1,
				...
				O = 7,
				L = {};
			(L[$] = 'Named character references must be terminated by a semicolon'),
				(L[H] = 'Numeric character references must be terminated by a semicolon'),
				(L[z] = 'Named character references cannot be empty'),
				(L[X] = 'Numeric character references cannot be empty'),
				(L[J] = 'Named character references must be known'),
				(L[W] = 'Numeric character references cannot be disallowed'),
				(L[O] = 'Numeric character references cannot be outside the permissible Unicode range');
			function B(j, I) {
				var T = {},
					K,
					U;
				I || (I = {});
				for (U in C) (K = I[U]), (T[U] = K ?? C[U]);
				return (
			...
			function Q(j, I) {
				var T = I.additional,
					...
					q;
				for (
					typeof T == 'string' && (T = T.charCodeAt(0)), ye = be(), Y = pe ? ft : h, te--, Fe++;
					++te < Fe;
				)
					if ((ne === b && (oe = gt[He] || 1), (ne = j.charCodeAt(te)), ne === E)) {
						if (
							((D = j.charCodeAt(te + 1)),
							D === x ||
								D === b ||
						...
						(ie = j.charCodeAt(q) === _),
							ie && (q++, (Ne = ee === c ? d(P) : !1), Ne && ((ge = P), (se = Ne))),
							(V = 1 + q - ue),
							(!ie && !K) ||
								(P
									? ee === c
										? (ie && !se
												? Y(J, 1)
												: (ge !== P && ((q = he + ge.length), (V = 1 + q - he), (ie = !1)),
													ie ||
														((Se = ge ? $ : z),
														I.attribute
															? ((D = j.charCodeAt(q)),
																D === i ? (Y(Se, V), (se = null)) : u(D) ? (se = null) : Y(Se, V))
															: Y(Se, V))),
											(R = se))
										: (ie || Y(H, V),
											(R = parseInt(P, k[ee])),
							...
							R
								? (Ge(),
									...
									(_e = be()),
									_e.offset++,
									re && re.call(ke, R, { start: ye, end: _e }, j.slice(ue - 1, q)),
									(ye = _e))
								: ((P = j.slice(ue - 1, q)), (le += P), (oe += P.length), (te = q - 1));
					} else ne === 10 && (Ue++, He++, (oe = 0)), ne === ne ? ((le += f(ne)), oe++) : Ge();
				return je.join('');
				function be() {
				...
				function ft(Ze, We) {
					var Oe = be();
					(Oe.column += We), (Oe.offset += We), pe.call(me, L[Ze], Oe, Ze);
				}
				function Ge() {
					le && (je.push(le), U && U.call(de, le, { start: ye, end: be() }), (le = ''));
				}
			}
			function M(j) {
				...
				a = (function (n) {
					var o = /(?:^|\s)lang(?:uage)?-([\w-]+)(?=\s|$)/i,
						...
						l = {
								...
								clone: function i(s, p) {
									...
									switch (l.util.type(s)) {
										case 'Object':
											...
											(g = {}), (p[m] = g);
											for (var c in s) s.hasOwnProperty(c) && (g[c] = i(s[c], p));
											return g;
										case 'Array':
											return (
												(m = l.util.objId(s)),
								...
								insertBefore: function (i, s, p, g) {
									...
									for (var w in m)
										if (m.hasOwnProperty(w)) {
											if (w == s) for (var v in p) p.hasOwnProperty(v) && (c[v] = p[v]);
											p.hasOwnProperty(w) || (c[w] = m[w]);
										}
									var k = g[i];
									return (
										(g[i] = c),
										l.languages.DFS(l.languages, function (N, $) {
											$ === k && N != i && (this[N] = c);
										}),
										c
									);
									...
									for (var w in s)
										if (s.hasOwnProperty(w)) {
											...
											k === 'Object' && !m[c(v)]
												...
												: k === 'Array' && !m[c(v)] && ((m[c(v)] = !0), i(v, p, w, m));
										}
								},
							},
							plugins: {},
							...
							highlightElement: function (i, s, p) {
								...
								if (s && n.Worker) {
									...
									(N.onmessage = function ($) {
										...
										N.postMessage(
											JSON.stringify({ language: v.language, code: v.code, immediateClose: !0 }),
										);
							...
							hooks: {
								...
								run: function (i, s) {
									var p = l.hooks.all[i];
									if (!(!p || !p.length)) for (var g = 0, m; (m = p[g++]); ) m(s);
								},
							},
							Token: f,
						};
					...
					f.stringify = function i(s, p) {
						...
						var m = {
							...
							c = s.alias;
						c && (Array.isArray(c) ? Array.prototype.push.apply(m.classes, c) : m.classes.push(c)),
							l.hooks.run('wrap', m);
						var w = '';
						for (var v in m.attributes)
							w += ' ' + v + '="' + (m.attributes[v] || '').replace(/"/g, '&quot;') + '"';
						return (
							'<' +
							m.tag +
							' class="' +
						...
						for (var w in p)
							if (!(!p.hasOwnProperty(w) || !p[w])) {
								...
								for (var k = 0; k < v.length; ++k) {
									...
									for (
										var W = N.pattern || N, O = g.next, L = m;
										O !== s.tail && !(c && L >= c.reach);
										L += O.value.length, O = O.next
									) {
										var B = O.value;
										...
										if (!(B instanceof f)) {
											var Q = 1,
												M;
											if (z) {
												if (((M = h(W, L, i, H)), !M || M.index >= i.length)) break;
												var T = M.index,
													ce = M.index + M[0].length,
													j = L;
												for (j += O.value.length; T >= j; ) (O = O.next), (j += O.value.length);
												if (((j -= O.value.length), (L = j), O.value instanceof f)) continue;
												for (
													var I = O;
													I !== s.tail && (j < ce || typeof I.value == 'string');
													I = I.next
												)
													Q++, (j += I.value.length);
											...
											var de = O.prev;
											U && ((de = b(s, de, U)), (L += U.length)), y(s, de, Q);
											var ke = new f(w, $ ? l.tokenize(K, $) : K, X, K);
											if (((O = b(s, de, ke)), re && b(s, O, re), Q > 1)) {
					...
					function S(i) {
						for (var s = [], p = i.head.next; p !== i.tail; ) s.push(p.value), (p = p.next);
						return s;
					}
					if (!n.document)
						return (
		...
		'../node_modules/refractor/core.js'(e, r) {
			...
			function E(c, w) {
				...
				if (b.util.type(w) === 'Object') (k = w), (w = null);
					...
					else throw new Error('Unknown language: `' + w + '` is not registered');
				}
			...
			function s(c, w) {
				...
				for (N = -1, k = v.length; ++N < k; ) ($ = v[N]), (v[N] = b.Token.stringify($, w, v));
				return v;
			}
			function p(c) {
				return c;
	...
	if (f) {
		...
		if (!o) x = fe(fe({}, d), {}, { className: Xe(d.className) });
			...
			x = fe(
				fe({}, d),
				{},
				{ className: Xe(S) || void 0, style: cn(d.className, Object.assign({}, d.style, n), t) },
			);
		}
		var E = C(r.children);
...
function Ae(e) {
	...
	if (
		((y.className = y.className ? [].concat(Te(y.className.trim().split(/\s+/)), Te(f)) : f),
		t && o)
	) {
		var S = st(a, t, n);
		r.unshift(it(t, S));
...
function ut(e) {
	for (
		...
		var n = e[a];
		if (n.type === 'text') t.push(Ae({ children: [n], className: Te(new Set(r)) }));
		else if (n.children) {
...
`,
					...
					if (v === 0) {
						var $ = h
								.slice(x + 1, b)
								.concat(Ae({ children: [N], className: p.properties.className })),
							H = E($, k);
						C.push(H);
						...
						if (z) {
							...
							var W = [X],
								O = E(W, k, p.properties.className);
							C.push(O);
						}
					} else {
						var L = [N],
							B = E(L, k, p.properties.className);
						C.push(B);
					}
				}),
					(x = b);
...
function Zn(e) {
	var r = e.rows,
		...
		a = e.useInlineStyles;
	return r.map(function (n, o) {
		return qe({ node: n, stylesheet: t, useInlineStyles: a, key: 'code-segment-'.concat(o) });
	});
}
function ct(e) {
...
function Kn(e, r) {
	return function (t) {
		var a,
			...
			J = X === void 0 ? 'code' : X,
			W = t.code,
			O = W === void 0 ? (Array.isArray(u) ? u[0] : u) || '' : W,
			L = t.astGenerator,
			B = wt(t, Bn);
		L = L || e;
		var Q = E
		...
		((c === void 0 && $) || v) && (c = !0), ($ = $ || Zn);
		var I = [{ type: 'text', value: O }],
			T = Wn({ astGenerator: L, language: o, code: O, defaultCodeValue: I });
		T.language === null && (T.value = I);
		var K =
				(a = (n = O.match(/\n/g)) === null || n === void 0 ? void 0 : n.length) !== null &&
				a !== void 0
					? a
					: 0,
			U = s + K,
		...
		return G.createElement(
			z,
			j,
			G.createElement(J, x, !A && Q, $({ rows: re, stylesheet: l, useInlineStyles: y })),
		);
	};
}
var Be = Z(an()),
...
var Vn = (0, rn.default)(2)((e) =>
		Object.entries(e.code || {}).reduce((r, [t, a]) => ({ ...r, [`* .${t}`]: a }), {}),
	),
	Jn = ht(),
	Qn = Ee.div(
	...
	ra = ({ rows: e, stylesheet: r, useInlineStyles: t }) =>
		e.map((a, n) =>
			qe({ node: pt(a), stylesheet: r, useInlineStyles: t, key: `code-segement${n}` }),
		),
	oa = (e, r) => (r ? (e ? ({ rows: t, ...a }) => e({ rows: t.map((n) => pt(n)), ...a }) : ra) : e),
	dt = ({
		children: e,
		language: r = 'jsx',
		copyable: t = !1,
		...
		return G.createElement(
			...
			t
				? G.createElement(yt, { actionItems: [{ title: x ? 'Copied' : 'Copy', onClick: y }] })
				: null,
		);
	};
dt.registerLanguage = (...e) => ze.registerLanguage(...e);
var ua = dt;
export { dt as SyntaxHighlighter, ua as default, Yn as supportedLanguages };
