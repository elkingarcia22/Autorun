import { r as X } from './ButtonProvider-Dmy-L2BE.js';
...
function re(e) {
	if (e.length === 0) return { max: 100, min: 0 };
	const o = Math.max(...e),
		s = Math.min(...e),
		c = Math.ceil(o / 20) * 20,
		i = Math.floor(s / 20) * 20;
	return { max: c || 100, min: i || 0 };
}
const ee = { xs: 4, sm: 8, md: 16, lg: 20 };
function ae(
	e,
	o = [],
	s,
	c,
	i = 'var(--ubits-chart-color-bg-neutral-blue-base)',
	$ = 'var(--modifiers-normal-color-light-bg-1)',
	v = 'var(--modifiers-normal-color-light-border-1)',
	d = 360,
	b = 158,
	f = !0,
	C = !0,
	...
	const D = f ? e : e.filter((t) => t >= 0),
		U = s !== void 0 && c !== void 0 ? { max: s, min: f ? c : 0 } : re(D),
		{ max: w, min: x } = U,
		z = w - x;
	e.length;
	const k = Math.max(30, Math.min(35, d * 0.08)),
		B = 4,
		h = 8,
		R = 25,
		V = b - h - R,
		...
		j = e
			.map((t, r) => {
				const l = f ? t : Math.max(0, t),
					u = l >= 0;
				let g;
				if (u) g = (l / w) * (y - h);
				else {
					const p = Math.abs(x);
					g = (Math.abs(l) / p) * (h + V - y);
				}
				const n = g >= 0.5 && Math.abs(l) >= 0.01;
				return { index: r, value: l, label: o[r] || `${r + 1}`, isPositive: u, shouldRender: n };
			})
	...
	const G = H - 1,
		...
		A = j.map((t, r) => {
			const l = T + r * (S + Q);
			let u, g;
			if (t.isPositive) (g = (t.value / w) * (y - h)), (u = y - g);
			else {
				const n = Math.abs(x);
				(g = (Math.abs(t.value) / n) * (h + V - y)), (u = y);
			}
			return {
				x: l,
				y: u,
    ...
    >
      ...
      ${A.map((t) => {
				...
				if (l) {
					...
					t.x,
						t.y + t.height,
						(u = `M ${n} ${p + r} Q ${n} ${p} ${n + r} ${p} L ${m - r} ${p} Q ${m} ${p} ${m} ${p + r} L ${m} ${N} L ${n} ${N} Z`);
				} else {
					const n = t.x,
						p = t.y,
						m = t.x + t.width,
					...
					t.x,
						t.y + t.height,
						(u = `M ${n} ${p} L ${m} ${N} L ${m} ${I - r} Q ${m} ${I} ${m - r} ${I} L ${n + r} ${I} Q ${n} ${I} ${n} ${I - r} Z`);
				}
				return `
...
function ne(e, o = 'md') {
	const s = e.percentage ?? te(e.current, e.total),
		c =
			o === 'sm'
...
function se(e, o = 'md', s = 'var(--ubits-chart-color-bg-neutral-blue-base)') {
	const c = e.percentage ?? te(e.current, e.total),
		i =
			o === 'sm'
				? 'ubits-body-sm-regular'
				: o === 'lg'
		...
		$ = 'ubits-body-md-bold',
		d = oe({
			size: o === 'sm' ? 'sm' : o === 'lg' ? 'lg' : 'md',
			value: c,
			variant: 'default',
			indicator: !1,
			className: 'ubits-bar-metric-card__progress-bar',
		}),
		b = `<span class="ubits-progress-bar__indicator">${e.current}/${e.total} <span class="${$}">${c}%</span></span>`,
		f = d.replace(/(<\/div>\s*)(<\/div>\s*)$/, `$1${b}$2`);
	return `
...
function ie(e) {
	const {
			...
			showCategories: U = !0,
			showInfoIcon: w = !0,
			showActionButton: x = !0,
			showNegativeValues: z = !0,
			showGridLines: k = !0,
			barColor: B = 'var(--ubits-chart-color-bg-neutral-blue-base)',
			chartBackgroundColor: h = 'var(--modifiers-normal-color-light-bg-1)',
			gridLineColor: R = 'var(--modifiers-normal-color-light-border-1)',
			className: V = '',
			attributes: y = {},
		} = e,
		F = ['ubits-bar-metric-card', `ubits-bar-metric-card--${f}`, `ubits-bar-metric-card--${C}`, V]
			.filter(Boolean)
			.join(' '),
		j = [...Object.entries(y).map(([a, L]) => `${a}="${L}"`)].filter(Boolean).join(' '),
		S = 392 - 12 * 2,
		P = 158,
		q = 'ubits-body-md-bold',
		O = w
			? X({
					variant: 'tertiary',
					size: 'sm',
					...
					iconStyle: 'regular',
					iconOnly: !0,
					attributes: { 'aria-label': 'Información', type: 'button' },
				})
			: '',
		E = x
			? X({
					...
					iconStyle: 'regular',
					iconOnly: !0,
					attributes: { 'aria-label': 'Ver más', type: 'button' },
				})
			: '',
		J =
			c && s !== void 0
    ...
    `
			: '',
		K = (a) => {
			if (typeof window < 'u' && window.document && window.getComputedStyle)
				try {
					const M = document.documentElement,
						Z = a.replace(/var\(|\)/g, '').trim(),
						t = getComputedStyle(M).getPropertyValue(Z).trim();
					if (t) return t.replace(/\)+$/, '').trim();
					{
						const r = Array.from(document.styleSheets);
						let l = null;
						for (const u of r)
							try {
								const g = Array.from(u.cssRules || []);
								for (const n of g)
									if (n instanceof CSSStyleRule && n.selectorText === ':root') {
										const m = n.style.getPropertyValue(Z);
										if (m) {
											l = m.trim().replace(/\)+$/, '').trim();
											break;
										}
									}
								if (l) break;
			...
			if (a === 'var(--ubits-chart-color-bg-neutral-blue-base)') return '#557593';
			const L = a.replace(/\)+$/, '').trim();
			return L !== a ? L : a;
		};
	let _ = B.startsWith('var(') ? K(B) : B;
	_ = _.replace(/\)+$/, '').trim();
	const Q =
			D && i.length > 0 && f !== 'horizontal'
				? (() => {
						const a = ae(
							i,
							$,
							...
							S,
							P,
							e.showNegativeValues !== void 0 ? e.showNegativeValues : !0,
							e.showGridLines !== void 0 ? e.showGridLines : !0,
							C,
						);
						return `
...
const be = {
		...
		argTypes: {
			...
			barData: {
				...
				table: { type: { summary: 'number[]' }, category: 'Gráfico' },
			},
			barLabels: {
				control: { type: 'object' },
				description: 'Etiquetas para las barras (opcional)',
				table: { type: { summary: 'string[]' }, category: 'Gráfico' },
			},
			maxValue: {
				control: { type: 'number' },
				description:
			...
			barColor: {
				...
				table: {
					type: { summary: 'string' },
					defaultValue: {
						summary: 'var(--modifiers-normal-color-light-chart-bg-neutral-blue-base)',
					},
					category: 'Estilo',
				},
			},
    ...
    `;
			const s = document.createElement('div'),
				c = `bar-metric-card-container-${Date.now()}`;
			(s.id = c),
				(s.style.cssText = `
      width: 100%;
      overflow: visible !important;
    ...
    `),
			...
			const i = [];
			e.category1Label &&
				e.category1Current !== void 0 &&
				e.category1Total !== void 0 &&
				i.push({ label: e.category1Label, current: e.category1Current, total: e.category1Total }),
				e.category2Label &&
					e.category2Current !== void 0 &&
					e.category2Total !== void 0 &&
					i.push({ label: e.category2Label, current: e.category2Current, total: e.category2Total }),
				e.category3Label &&
					e.category3Current !== void 0 &&
					e.category3Total !== void 0 &&
					i.push({ label: e.category3Label, current: e.category3Current, total: e.category3Total });
			const $ = i.length > 0 ? i : e.categories || [],
				v = {
					title: e.title || 'Métricas',
					responseCount: e.responseCount !== void 0 ? e.responseCount : 0,
					showResponseCount: e.showResponseCount !== void 0 ? e.showResponseCount : !1,
					barData: e.barData || [],
					barLabels: e.barLabels,
					maxValue: e.maxValue,
					minValue: e.minValue,
					categories: $,
					layout: e.layout || 'vertical',
					size: e.size || 'md',
					showTitle: e.showTitle !== void 0 ? e.showTitle : !0,
					showBarChart: e.showBarChart !== void 0 ? e.showBarChart : !0,
					showCategories: e.showCategories !== void 0 ? e.showCategories : !0,
					showInfoIcon: e.showInfoIcon !== void 0 ? e.showInfoIcon : !0,
					showActionButton: e.showActionButton !== void 0 ? e.showActionButton : !0,
					showNegativeValues: e.showNegativeValues !== void 0 ? e.showNegativeValues : !0,
					showGridLines: e.showGridLines !== void 0 ? e.showGridLines : !0,
					barColor: e.barColor || '#557593)',
					chartBackgroundColor:
						e.chartBackgroundColor || 'var(--modifiers-normal-color-light-bg-1)',
					gridLineColor: e.gridLineColor || 'var(--modifiers-normal-color-light-border-1)',
					onClick: e.onClick,
					onAction: e.onAction,
				},
				d = ie(v);
			return (s.innerHTML = d), o;
		},
	};
W.parameters = {
	...W.parameters,
...
const he = ['Default'];
export { W as Default, he as __namedExportsOrder, be as default };
