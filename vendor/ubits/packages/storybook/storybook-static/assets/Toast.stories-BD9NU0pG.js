import { r as y, s as v } from './ToastProvider-CtZKLbxG.js';
import { c as u } from './ButtonProvider-Dmy-L2BE.js';
...
function g() {
	m()
		.querySelectorAll('.ubits-toast')
		.forEach((n) => {
			const i = n;
			i.classList.add('ubits-toast--exit'),
				setTimeout(() => {
...
const d = {
	...
	render: (t) => {
		...
		(n.style.display = 'flex'),
			(n.style.gap = 'var(--p-spacing-mode-1-md, 12px)'),
					...
					g(),
						setTimeout(() => {
							try {
								...
								v(c, t.message || '', a),
									setTimeout(() => {
										o.querySelectorAll('.ubits-toast').length === 0 &&
											(console.error('❌ El toast no se agregó al contenedor'),
											alert('Error: El toast no se mostró. Revisa la consola para más detalles.'));
									}, 100);
							} catch (o) {
								console.error('❌ Error al mostrar toast:', o),
									alert(`Error: ${o instanceof Error ? o.message : 'Error desconocido'}`);
							}
						}, 200);
				},
			},
		...
		requestAnimationFrame(() => {
			try {
				...
				(a.className = 'ubits-button ubits-button--primary ubits-button--md'),
					(a.textContent = 'Mostrar Toast'),
					a.addEventListener('click', i.onClick),
					n.appendChild(a);
				const c = document.createElement('button');
				(c.className = 'ubits-button ubits-button--secondary ubits-button--md'),
					(c.textContent = 'Limpiar Toasts'),
		...
		(l.style.width = '100%'), (l.style.marginBottom = '20px'), (l.style.position = 'relative');
		const f = y({
			type: t.type || 'info',
			title: t.title,
			message: t.message,
			noClose: t.noClose,
			action: t.action ? { label: 'Action', onClick: () => {} } : void 0,
		});
		l.innerHTML = f;
		const r = l.querySelector('.ubits-toast');
		if (r) {
			(r.style.position = 'relative'), (r.style.width = '100%'), (r.style.maxWidth = '560px');
			const o = r.querySelector('.ubits-toast__close');
			o &&
				o.addEventListener('click', () => {
					r.classList.add('ubits-toast--exit'),
						setTimeout(() => {
							r.parentNode && r.parentNode.removeChild(r);
						}, 180);
				});
			const a = r.querySelector('[data-toast-action]');
			a &&
				t.action &&
		...
		return (
			...
			(s.style.background = 'var(--modifiers-normal-color-light-bg-2)'),
			(s.style.borderRadius = '8px'),
			(s.style.fontSize = '14px'),
			(s.style.color = 'var(--modifiers-normal-color-light-fg-1-medium)'),
			(s.style.border = '1px solid var(--modifiers-normal-color-light-border-1)'),
			(s.style.lineHeight = '1.6'),
			(s.style.fontFamily = 'var(--font-family-noto-sans-font-family, "Noto Sans", sans-serif)'),
			(s.innerHTML = `
...
const k = ['Default'];
export { d as Default, k as __namedExportsOrder, w as default };
