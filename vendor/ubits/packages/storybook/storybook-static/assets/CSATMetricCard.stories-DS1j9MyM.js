...
function S(a, e = 'regular', t) {
	const s = e === 'solid' ? 'fas' : 'far',
		o = a.startsWith('fa-') ? a : `fa-${a}`,
		r = t ? `style="color: ${t};"` : '';
	return `<i class="${s} ${o}" ${r}></i>`;
}
const L = [
...
function H(a) {
	...
	for (let r = 0; r < 5; r++) {
		const c = L[r],
			...
			d = n ? c.color : 'var(--modifiers-normal-color-light-border-1)';
		o += `
...
function A(a) {
	const {
			title: e,
			totalResponses: t = 0,
			responsesLabel: s = 'respuestas',
			average: o = 0,
			averageLabel: r = 'Promedio:',
			score: c = 0,
			titleIcon: n,
			titleIconStyle: l = 'regular',
			titleIconColor: d,
			showInfoIcon: b = !1,
			showActionButton: y = !1,
			size: f = 'md',
			className: g = '',
			attributes: v = {},
		} = a,
		h = ['ubits-csat-metric-card', `ubits-csat-metric-card--${f}`, g].filter(Boolean).join(' '),
		_ = [...Object.entries(v).map(([w, T]) => `${w}="${T}"`)].filter(Boolean).join(' '),
		x = n ? `<div class="ubits-csat-metric-card__title-icon">${S(n, l, d)}</div>` : '',
		C = b
			? p({
					variant: 'tertiary',
					size: 'sm',
...
const j = {
	...
	i = {
		...
		render: (a) => {
			...
			(t.style.maxWidth = '400px'), (t.style.width = '100%');
			const s = A(a);
			return (t.innerHTML = s), e.appendChild(t), e;
		},
	};
i.parameters = {
	...i.parameters,
...
const z = ['Default'];
export { i as Default, z as __namedExportsOrder, j as default };
