...
function S(e) {
	const {
		...
		g = [
			...
			`ubits-selection-card--${b}`,
			l !== 'default' ? `ubits-selection-card--${l}` : '',
			u ? 'ubits-selection-card--selected' : '',
		]
			.filter(Boolean)
			.join(' ');
	let m = '';
	...
	const f = E({
			label: '',
			value: t,
			name: `selection-card-group-${t}`,
			checked: u,
			size: 'md',
			state: d ? 'disabled' : 'default',
			disabled: d,
  ...
  `.trim();
	return typeof window < 'u' && getComputedStyle(document.documentElement), C;
}
function N(e) {
	const t = S(e),
	...
	if (!a)
		throw (
			(console.error('❌ [SelectionCard] Error al parsear HTML. HTML generado:', t),
			new Error('No se pudo crear el elemento selection-card. Verifica que el HTML sea válido.'))
		);
	return (
		a.classList.contains('ubits-selection-card') ||
		...
		setTimeout(() => {
			...
			o.getPropertyValue('--modifiers-normal-color-light-border-1').trim(),
				o.getPropertyValue('--modifiers-static-inverted-color-light-accent-brand').trim(),
				o.getPropertyValue('--modifiers-normal-color-light-bg-1').trim(),
				o.getPropertyValue('--ubits-spacing-lg').trim(),
				o.getPropertyValue('--ubits-spacing-sm').trim(),
				o.getPropertyValue('--ubits-border-radius-md').trim(),
				o.getPropertyValue('--font-body-md-size').trim(),
				o.getPropertyValue('--font-body-md-line').trim(),
				o.getPropertyValue('--weight-semibold').trim(),
				o.getPropertyValue('--font-sans').trim(),
				r.border,
				r.borderWidth,
				r.borderStyle,
				r.borderColor,
			...
			const s = a.querySelector('.ubits-selection-card__title');
			if (s) {
				const i = window.getComputedStyle(s);
				i.fontSize, i.fontWeight, i.lineHeight, i.fontFamily;
			}
			a.classList.contains('ubits-selection-card--selected');
		}, 100),
		a
	);
}
const T = {
		...
		argTypes: {
			id: {
				...
				table: { defaultValue: { summary: 'card-1' } },
			},
			title: {
				control: { type: 'text' },
				description: 'Título de la card',
				table: { defaultValue: { summary: 'Asignar toda la empresa' } },
			},
			description: {
				control: { type: 'text' },
			...
			'selectionCount.current': {
				control: { type: 'number', min: 0, max: 1e3, step: 1 },
			...
			'selectionCount.total': {
				...
				table: { type: { summary: 'number' }, defaultValue: { summary: '290' } },
			},
			state: {
				control: { type: 'select' },
	...
	c = {
		...
		render: (e) => {
			...
			(t.style.display = 'flex'),
				...
				(t.style.background = 'var(--modifiers-normal-color-light-bg-1)'),
				(t.style.border = '1px solid var(--modifiers-normal-color-light-border-1)'),
				(t.style.minHeight = '200px');
			const n = document.createElement('div');
			(n.style.maxWidth = '500px'), (n.style.width = '100%');
			const a = {
				id: e.id,
				title: e.title,
				description: e.showDescription ? e.description : void 0,
			...
			try {
				...
				console.error('❌ [SelectionCard Story] Error al crear card:', r);
				const o = document.createElement('div');
				(o.textContent = `Error: ${r instanceof Error ? r.message : 'Error desconocido'}`),
					(o.style.color = 'red'),
...
const _ = ['Default'];
export { c as Default, _ as __namedExportsOrder, T as default };
