import { r as D } from './ProgressProvider-DCMY_PMl.js';
import { r as y } from './ButtonProvider-Dmy-L2BE.js';
...
function U(e) {
	...
	return Math.round((e / Math.pow(t, r)) * 100) / 100 + ' ' + o[r];
}
function H(e = {}) {
	const {
			state: t = 'default',
			...
			showProgress: d = !0,
			showIcon: s = !1,
			dropText: h = 'Arrastra tus archivos aquí',
			constraintsText: g,
			selectButtonText: v = 'Seleccionar archivos',
			fileName: x,
			fileExtension: S,
			fileSize: u,
			uploadText: O = 'Haz clic para subir archivo',
			fileStatus: J = 'pending',
			className: w = '',
		} = e,
		z = o && o.length > 0,
		i = z && t !== 'files-list' ? 'files-list' : t,
		$ = g || `Máx. ${r} archivos · Hasta ${U(l)}`;
	if (i === 'files-list' && z) {
		const b = r === 1,
			_ = b ? o.slice(0, 1) : o,
			P = _.map((a, k) => {
				const C = a.id || `file-${k}`,
					c = a.progress !== void 0 ? a.progress : 0;
				a.status;
				const M = d && a.status === 'uploading' && c > 0,
					j = n === !0,
					V = M === !0;
				return `
          ...
          <div class="ubits-file-upload__file-info">
            ...
            ${
              ...
              <div class="ubits-file-upload__progress-container">
                ${(() => {
									try {
										return D({ size: 'xs', value: c, variant: 'default', indicator: `${c}%` });
									} catch (N) {
										return (
											console.error('Error rendering progress bar:', N),
	...
	const B = ['ubits-file-upload', `ubits-file-upload--${i}`, w].filter(Boolean).join(' ');
	let p = 'var(--modifiers-normal-color-light-border-1)',
		F = 'var(--modifiers-normal-color-light-bg-1)';
	i === 'dragging'
		? (p = 'var(--modifiers-normal-color-light-accent-brand)')
		: i === 'error'
			? (p = 'var(--modifiers-normal-color-light-feedback-accent-error)')
			: i === 'disabled' &&
				((F = 'var(--modifiers-normal-color-light-bg-disabled)'),
				(p = 'var(--modifiers-normal-color-light-border-disabled)'));
	const I = s
			? `
  ...
  `
		...
		A = y({
			...
			attributes: T ? { 'aria-disabled': 'true' } : {},
		});
	return `
    <div class="${B}" 
...
const R = {
    ...
    `);
			...
			(l.textContent =
				'Componente File Upload con diseño moderno. Soporta dos vistas: Drop Zone (área de arrastrar y soltar) y Files List (lista de archivos con progreso). Usa componentes UBITS y tokens UBITS exclusivamente.'),
				(l.style.cssText = `
    ...
    `);
			...
			n.id = `file-upload-container-${Date.now()}`;
			const f = () => {
				n.innerHTML = '';
				let s = e.files || [];
				e.state === 'files-list' &&
				...
				const g = s && s.length > 0 ? 'files-list' : e.state || 'default',
					v = e.showFileSize !== void 0 ? e.showFileSize : !0,
					x = e.showProgress !== void 0 ? e.showProgress : !0,
					S = {
						state: g,
						files: s,
						maxFiles: e.maxFiles || 6,
						maxSize: e.maxSize || 5242880,
						dropText: e.dropText || 'Arrastra tus archivos aquí',
						selectButtonText: e.selectButtonText || 'Seleccionar archivos',
						showIcon: e.showIcon !== void 0 ? e.showIcon : !1,
						showFileSize: v,
						showProgress: x,
					};
				try {
					...
					console.error('[FileUpload Storybook] Error rendering file upload:', u);
				}
			};
			f();
			let d = null;
			return (
				setInterval(() => {
					...
					(!d || JSON.stringify(s) !== JSON.stringify(d)) &&
						((d = JSON.parse(JSON.stringify(s))), f());
				}, 100),
				o.appendChild(r),
				o.appendChild(l),
				o.appendChild(n),
...
const W = ['Default'];
export { m as Default, W as __namedExportsOrder, R as default };
