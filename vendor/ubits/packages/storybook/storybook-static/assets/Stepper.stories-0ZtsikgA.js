...
function f(e) {
	...
	const l = ['ubits-stepper', `ubits-stepper--${n}`, `ubits-stepper--${r}`, d]
			.filter(Boolean)
			.join(' '),
		i = s
			.map((a, p) => {
				const t = p === s.length - 1,
					m = a.state || (p === 0 ? 'active' : 'default');
				return v(a, m, t, n, o, c, 'default');
			})
			.join('');
	return `
    <div class="${l}">
...
function v(e, n, r, o, c, s, d = 'default') {
	const l = [
			'ubits-stepper__step',
			`ubits-stepper__step--${n}`,
			r ? 'ubits-stepper__step--last' : '',
		]
			.filter(Boolean)
			.join(' '),
		i = b(e.number, n),
		a = r ? '' : S(d, o),
		p = T(c ? e.title : void 0, s ? e.description : void 0);
	return o === 'vertical'
		? `
      <div class="${l}">
...
function b(e, n) {
	const r = ['ubits-stepper__indicator', `ubits-stepper__indicator--${n}`]
		.filter(Boolean)
		.join(' ');
	let o = '';
	return (
...
function S(e, n) {
	return `<div class="${['ubits-stepper__connector', `ubits-stepper__connector--${n}`].filter(Boolean).join(' ')}"></div>`;
}
function T(e, n) {
	const r = e ? `<h3 class="ubits-stepper__title ubits-body-md-semibold">${y(e)}</h3>` : '',
		o = n ? `<p class="ubits-stepper__description ubits-body-sm-regular">${y(n)}</p>` : '';
	return !r && !o
		? '<div class="ubits-stepper__step-content ubits-stepper__step-content--empty"></div>'
		: `
    <div class="ubits-stepper__step-content">
...
const _ = {
    ...
    `;
			...
			(r.id = `stepper-container-${Date.now()}`),
				(r.style.cssText = `
      width: 100%;
      max-width: 800px;
      margin: 0 auto;
    `);
			const o = () => {
				r.innerHTML = '';
				const i = e.numSteps || 3,
					a = [];
				for (let t = 1; t <= i; t++) {
					const m = e[`step${t}State`] || (t === 1 ? 'completed' : t === 2 ? 'active' : 'default'),
						g =
							e[`step${t}Title`] ||
							`Step ${t === 1 ? 'One' : t === 2 ? 'Two' : t === 3 ? 'Three' : t === 4 ? 'Four' : 'Five'}`,
						h =
							e[`step${t}Description`] ||
							`Desc for step ${t === 1 ? 'one' : t === 2 ? 'two' : t === 3 ? 'three' : t === 4 ? 'four' : 'five'}`;
					a.push({ number: t, title: g, description: h, state: m });
				}
				const p = {
					orientation: e.orientation || 'horizontal',
					size: e.size || 'md',
					showTitle: e.showTitle !== void 0 ? e.showTitle : !0,
					showDescription: e.showDescription !== void 0 ? e.showDescription : !0,
					steps: a,
				};
				try {
					const t = f(p);
					...
					console.error('Error al renderizar stepper:', t),
						(r.innerHTML =
							'<p style="color: var(--modifiers-normal-color-light-feedback-accent-error);">Error al renderizar el stepper</p>');
				}
			};
			o();
			...
			const l = () => {
				s && (clearInterval(s), (s = null)), (r.innerHTML = '');
			};
			return n.addEventListener('DOMNodeRemoved', l), n.appendChild(r), n;
		},
	};
u.parameters = {
	...u.parameters,
...
const $ = ['Default'];
export { u as Default, $ as __namedExportsOrder, _ as default };
