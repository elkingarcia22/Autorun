function g(e = {}) {
	const {
		...
		o = [
			'ubits-skeleton',
			`ubits-skeleton--${i}`,
			t !== 'md' ? `ubits-skeleton--${t}` : '',
			m ? 'ubits-skeleton--animated' : '',
			p,
		]
			.filter(Boolean)
			.join(' '),
	...
	l &&
		(l === 'full'
			...
			: typeof l == 'number'
				? r.push(`width: ${l}px`)
				: r.push(`width: ${l}`)),
		s && (typeof s == 'number' ? r.push(`height: ${s}px`) : r.push(`height: ${s}`));
	const c = [...r, u].filter(Boolean).join('; '),
		a = c ? ` style="${c}"` : '';
	if (i === 'text') {
		const h = Array.from(
			{ length: n },
			(f, y) =>
				`<span class="ubits-skeleton__line" style="width: ${y === n - 1 && n > 1 ? '60%' : '100%'}"></span>`,
		).join('');
		return `<div class="${o}"${a}>${h}</div>`;
	}
	return i === 'circle'
		? `<div class="${o}"${a}></div>`
		: i === 'rectangle'
...
const w = {
	...
	d = {
		args: { variant: 'text', size: 'md', width: 'full', height: '', lines: 3, animated: !0 },
		render: (e) => {
			const i = document.createElement('div');
			(i.style.padding = '40px'),
				(i.style.background = 'var(--modifiers-normal-color-light-bg-1)'),
				(i.style.width = '100%'),
			...
			(l.style.width = '100%'), (l.style.boxSizing = 'border-box');
			const s = g(e);
			l.innerHTML = s;
			const n = l.querySelector('.ubits-skeleton');
			return (
				n &&
					(e.variant === 'text'
						? ((n.style.width = '100%'), (n.style.maxWidth = '100%'))
						: e.variant === 'circle'
							? ((n.style.width = ''),
								(n.style.height = ''),
								(n.style.margin = '0 auto'),
								(n.style.display = 'inline-block'))
							: (e.variant === 'rectangle' || e.variant === 'custom') &&
								(e.width === 'full' || !e.width
									? (n.style.width = '100%')
									: e.width &&
										(n.style.width = typeof e.width == 'number' ? `${e.width}px` : e.width),
								e.height &&
									(n.style.height = typeof e.height == 'number' ? `${e.height}px` : e.height))),
				t.appendChild(l),
				i.appendChild(t),
				i
			);
...
const v = ['Default'];
export { d as Default, v as __namedExportsOrder, w as default };
