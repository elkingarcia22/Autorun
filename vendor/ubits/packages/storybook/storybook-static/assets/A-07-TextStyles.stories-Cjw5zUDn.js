const B = {
	...
	parameters: {
		...
		docs: {
			description: {
				component:
					'Estilos completos de texto del sistema UBITS. Cada estilo incluye todas las propiedades tipograficas: font-family, font-weight, letter-spacing, paragraph-indent, text-case y text-decoration. Incluye Display (D1-D4), Heading (H1-H2) y Body (lg, md, sm, xs).',
			},
		},
	},
};
function T(t) {
	...
	return getComputedStyle(n).getPropertyValue(t).trim() || 'N/A';
}
function v() {
	const t = document.styleSheets,
		n = [];
	try {
		const e = Array.from(t).find((i) => {
			try {
				return i.href?.includes('figma-tokens.css') || !1;
			} catch {
				return !1;
			}
		});
		e &&
			Array.from(e.cssRules || []).forEach((l) => {
				l instanceof CSSStyleRule &&
					l.style.cssText.split(';').forEach((r) => {
						const a = r.match(/--[^:]+:/);
						a && n.push(a[0].replace(':', ''));
					});
			});
	} catch {}
	const o = {},
		...
		s = {};
	return (
		['d1', 'd2', 'd3', 'd4'].forEach((e) => {
			['regular', 'semibold', 'bold'].forEach((i) => {
				const l = `${e}-${i}`;
				o[l] = {
					fontfamily: `--modifiers-normal-display-${e}-${i}-fontfamily`,
					fontweight: `--modifiers-normal-display-${e}-${i}-fontweight`,
					letterspacing: `--modifiers-normal-display-${e}-${i}-letterspacing`,
					paragraphindent: `--modifiers-normal-display-${e}-${i}-paragraphindent`,
					textcase: `--modifiers-normal-display-${e}-${i}-textcase`,
					textdecoration: `--modifiers-normal-display-${e}-${i}-textdecoration`,
				};
			});
		}),
		['h1', 'h2'].forEach((e) => {
			const i = e;
			m[i] = {
				fontfamily: `--modifiers-normal-heading-${e}-fontfamily`,
				fontweight: `--modifiers-normal-heading-${e}-fontweight`,
				lineheight: `--modifiers-normal-heading-${e}-lineheight`,
				letterspacing: `--modifiers-normal-heading-${e}-letterspacing`,
				paragraphindent: `--modifiers-normal-heading-${e}-paragraphindent`,
				textcase: `--modifiers-normal-heading-${e}-textcase`,
				textdecoration: `--modifiers-normal-heading-${e}-textdecoration`,
			};
		}),
		['lg', 'md', 'sm', 'xs'].forEach((e) => {
			['regular', 'semibold', 'bold'].forEach((i) => {
				const l = `${e}-${i}`;
				s[l] = {
					fontfamily: `--modifiers-normal-body-${e}-${i}-fontfamily`,
					fontweight: `--modifiers-normal-body-${e}-${i}-fontweight`,
					letterspacing: `--modifiers-normal-body-${e}-${i}-letterspacing`,
					paragraphindent: `--modifiers-normal-body-${e}-${i}-paragraphindent`,
					textcase: `--modifiers-normal-body-${e}-${i}-textcase`,
					textdecoration: `--modifiers-normal-body-${e}-${i}-textdecoration`,
				};
			});
		}),
		{ display: o, heading: m, body: s }
	);
}
function h(t, n) {
		...
		[
			...
			.forEach((i) => {
				...
				(a.style.padding = '8px 12px'),
					...
					(a.textContent = n[i.key] || 'N/A'),
					l.appendChild(a);
				const c = document.createElement('td');
				if (
					((c.style.padding = '8px 12px'),
					(c.style.fontSize = '14px'),
					(c.style.color = '#303a47'),
					n[i.key])
				) {
					const p = T(n[i.key]);
					c.textContent = p;
				} else c.textContent = 'N/A';
				l.appendChild(c), s.appendChild(l);
			}),
		o.appendChild(s),
		o
	);
...
const x = {
		render: () => {
			...
			(n.textContent = 'Text Styles - Estilos Completos de Texto'),
			...
			(o.textContent =
				'Estilos completos de texto que incluyen todas las propiedades tipograficas: font-family, font-weight, letter-spacing, paragraph-indent, text-case y text-decoration.'),
				(o.style.fontSize = '16px'),
				(o.style.color = '#5c646f'),
				(o.style.marginBottom = '32px'),
				(o.style.lineHeight = '1.6'),
				t.appendChild(o);
			const { display: m, heading: s, body: e } = v(),
				i = document.createElement('div');
			i.style.marginBottom = '40px';
			const l = document.createElement('h3');
			(l.textContent = `Display (${Object.keys(m).length} estilos)`),
				...
				Object.entries(m).forEach(([d, y]) => {
					i.appendChild(h(d.toUpperCase(), y));
				}),
				t.appendChild(i);
			const r = document.createElement('div');
			r.style.marginBottom = '40px';
			const a = document.createElement('h3');
			(a.textContent = `Heading (${Object.keys(s).length} estilos)`),
				(a.style.fontSize = '20px'),
				(a.style.fontWeight = '600'),
				(a.style.marginBottom = '16px'),
				(a.style.paddingBottom = '8px'),
				(a.style.borderBottom = '2px solid #e5e7eb'),
				r.appendChild(a),
				Object.entries(s).forEach(([d, y]) => {
					r.appendChild(h(d.toUpperCase(), y));
				}),
				t.appendChild(r);
			...
			return (
				(p.textContent = `Body (${Object.keys(e).length} estilos)`),
				(p.style.fontSize = '20px'),
				(p.style.fontWeight = '600'),
				(p.style.marginBottom = '16px'),
				(p.style.paddingBottom = '8px'),
				(p.style.borderBottom = '2px solid #e5e7eb'),
				c.appendChild(p),
				Object.entries(e).forEach(([d, y]) => {
					c.appendChild(h(d.toUpperCase(), y));
				}),
				t.appendChild(c),
				t
			);
	...
	b = {
		render: () => {
			...
			const { display: o } = v();
			return (
				Object.entries(o).forEach(([m, s]) => {
					t.appendChild(h(m.toUpperCase(), s));
				}),
				t
			);
		},
	...
	C = {
		render: () => {
			...
			(t.style.padding = '24px'), (t.style.maxWidth = '1400px');
			const n = document.createElement('h2');
			(n.textContent = 'Uso Práctico: Text Styles Combinados'),
				(n.style.fontSize = '24px'),
				(n.style.fontWeight = '700'),
				(n.style.marginBottom = '16px'),
				t.appendChild(n);
...
line-height: var(--modifiers-normal-body-xs-semibold-lineheight);
        `.trim(),
					...
					(d.textContent = e.preview),
						(d.style.fontFamily = 'var(--font-family-noto-sans-font-family)'),
						(d.style.fontSize = 'var(--modifiers-normal-body-md-semibold-fontsize)'),
						(d.style.fontWeight = 'var(--weight-semibold, 600)'),
						(d.style.lineHeight = 'var(--modifiers-normal-body-md-semibold-lineheight)'),
						(d.style.color = '#303a47'),
						e.name.includes('SM')
							? ((d.style.fontSize = 'var(--modifiers-normal-body-sm-semibold-fontsize)'),
								(d.style.lineHeight = 'var(--modifiers-normal-body-sm-semibold-lineheight)'),
								e.name.includes('Regular') && (d.style.fontWeight = 'var(--weight-regular, 400)'))
							: e.name.includes('XS')
								? ((d.style.fontSize = 'var(--modifiers-normal-body-xs-semibold-fontsize)'),
									(d.style.lineHeight = 'var(--modifiers-normal-body-xs-semibold-lineheight)'))
								: e.name.includes('Regular') &&
									((d.style.fontSize = 'var(--modifiers-normal-body-md-regular-fontsize)'),
									(d.style.fontWeight = 'var(--weight-regular, 400)'),
									(d.style.lineHeight = 'var(--modifiers-normal-body-md-regular-lineheight)')),
						c.appendChild(d),
						l.appendChild(c);
			...
			return (
				(s.style.marginTop = '32px'),
				(s.style.padding = '16px'),
				(s.style.backgroundColor = '#fef3c7'),
				(s.style.border = '1px solid #fbbf24'),
...
x.parameters = {
  ...
  render: () => {
    ...
    return container;
		...
		description: {
			story: 'Story principal - Todos los Text Styles',
...
b.parameters = {
  ...
  render: () => {
    ...
    return container;
		...
		description: { story: 'Story - Display', ...b.parameters?.docs?.description },
	},
};
u.parameters = {
	...u.parameters,
  ...
  render: () => {
    ...
    return container;
		...
		description: { story: 'Story - Heading', ...u.parameters?.docs?.description },
	},
};
S.parameters = {
	...S.parameters,
  ...
  render: () => {
    ...
    return container;
		...
		description: { story: 'Story - Body', ...S.parameters?.docs?.description },
	},
};
C.parameters = {
	...C.parameters,
...
line-height: var(--modifiers-normal-body-xs-semibold-lineheight);
    ...
    return container;
		...
		description: {
			story: `Story - Uso Práctico: Text Styles Combinados
...
const w = ['TodosLosTextStyles', 'Display', 'Heading', 'Body', 'UsoPractico'];
export {
	S as Body,
	b as Display,
	u as Heading,
	x as TodosLosTextStyles,
	C as UsoPractico,
