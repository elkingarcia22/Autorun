...
function w(e, t = 'regular') {
	const o = t === 'regular' ? 'far' : 'fas',
		n = e.startsWith('fa-') ? e : `fa-${e}`;
	return `<i class="${o} ${n}"></i>`;
}
function E(e) {
	const t = window.innerHeight,
		...
		r = t - o - 16,
		l = Math.max(578, r);
	(e.style.height = `${l}px`), (e.style.top = `${o}px`);
}
function M(e) {
	const {
			variant: t = 'colaborador',
			bodyButtons: o,
			footerButtons: n = [],
			logoHref: r,
			logoImage: d = 'images/Ubits-logo.svg',
			profileMenuItems: l = [],
			avatarImage: i = 'images/Profile-image.jpg',
			darkModeEnabled: s = !0,
			className: f = '',
			attributes: p = {},
		} = e,
		c = r || (t === 'admin' ? 'admin.html' : 'index.html'),
		u = ['ubits-sidebar', f].filter(Boolean).join(' '),
		g = Object.entries(p)
			.map(([a, v]) => `${a}="${v}"`)
			.join(' '),
		m = o
			.map((a) => {
				const v = [
						...
						.filter(Boolean)
						.join(' '),
					y = a.onClick ? 'data-has-click-handler="true"' : '',
					B = a.href ? `data-href="${a.href}"` : '';
				return `
      <button 
        class="${v}" 
...
`),
  ...
  `
    ...
    <div class="ubits-sidebar-profile-menu" id="ubits-sidebar-profile-menu">
      ${l
				.map((a) => {
					if (a.divider) return '<div class="ubits-sidebar-profile-menu-divider"></div>';
					const v = a.onClick ? 'data-has-click-handler="true"' : '',
						y = a.href ? `data-href="${a.href}"` : '';
					return `
...
function I(e) {
	const t = document.getElementById('ubits-sidebar-tooltip');
	if (!t) return;
	const o = e.parentElement;
	if (!o) return;
	e.querySelectorAll('[data-tooltip]').forEach((r) => {
		const d = r.getAttribute('data-tooltip');
		d &&
					...
					requestAnimationFrame(() => {
						requestAnimationFrame(() => {
							const p = s.getBoundingClientRect().height,
								b = l.right - i.left + 12,
								c = l.top - i.top + l.height / 2 - p / 2;
							(s.style.left = `${b}px`), (s.style.top = `${c}px`), (s.style.visibility = 'visible');
						});
					});
			}),
			r.addEventListener('mouseleave', () => {
				t.classList.remove('show'), (t.style.visibility = 'hidden');
			}));
	});
}
function $(e, t) {
	const o = e.querySelector('.ubits-sidebar-user-avatar'),
		n = document.getElementById('ubits-sidebar-profile-menu');
	if (!o || !n) return;
	const r = t.containerId,
		d = r ? document.getElementById(r) : e.parentElement,
		l = () => {
			if (!d || d === document.body) return;
			const c = e.getBoundingClientRect(),
				u = d.getBoundingClientRect(),
				g = c.left - u.left + 96,
				m = 27;
			(n.style.position = 'absolute'), (n.style.left = `${g}px`), (n.style.bottom = `${m}px`);
		};
	d && d !== document.body
		? (window.getComputedStyle(d).position === 'static' && (d.style.position = 'relative'),
			l(),
			window.addEventListener('resize', l))
		: ((n.style.position = 'fixed'), (n.style.left = '96px'), (n.style.bottom = '27px'));
	let i = null,
		s = null;
	const f = () => {
	...
	if (
		...
		o.addEventListener('mouseleave', p),
		n.addEventListener('mouseenter', f),
		n.addEventListener('mouseleave', p),
		t.onAvatarClick)
	)
		o.addEventListener('click', (c) => {
			c.preventDefault(), t.onAvatarClick?.();
		});
	else {
		const c = o.getAttribute('data-href');
		c &&
			o.addEventListener('click', () => {
				window.location.href = c;
			});
	}
	n.querySelectorAll('.ubits-sidebar-profile-menu-item').forEach((c, u) => {
		const g = t.profileMenuItems?.[u];
		!g ||
			g.divider ||
			c.addEventListener('click', (m) => {
				m.preventDefault(),
					g.onClick ? g.onClick() : g.href && (window.location.href = g.href),
					p();
			});
	});
}
function A(e, t) {
	const o = e.querySelector('#ubits-darkmode-toggle');
	if (!o) return;
	const n = t.containerId;
	let r = null;
	n && (r = document.getElementById(n)), r || (r = e.parentElement);
	const d = (l) => {
		...
		i &&
			...
			i.classList.add('ubits-icon-transition'),
			requestAnimationFrame(() => {
				l === 'dark'
					? i.classList.add('fa-solid', 'fa-sun-bright')
					: i.classList.add('far', 'fa-moon');
			}),
			setTimeout(() => {
				i.classList.remove('ubits-icon-transition');
			}, 400));
	};
	o.addEventListener('click', (l) => {
		l.preventDefault(), l.stopPropagation();
		const s = (o.getAttribute('data-theme') || 'light') === 'light' ? 'dark' : 'light';
		o.setAttribute('data-theme', s),
			d(s),
			r && r.setAttribute('data-theme', s),
			t.onDarkModeToggle && t.onDarkModeToggle(s === 'dark');
	});
}
function L(e) {
	const { containerId: t, bodyButtons: o, height: n } = e,
		r = document.getElementById(t);
	if (!r) throw new Error(`Container with id "${t}" not found`);
	window.getComputedStyle(r).position === 'static' && (r.style.position = 'relative');
	const l = M(e);
	r.innerHTML = l;
	const i = r.querySelector('.ubits-sidebar'),
		s = document.getElementById('ubits-sidebar-profile-menu');
	s && !r.contains(s) && r.appendChild(s);
	const f = document.getElementById('ubits-sidebar-tooltip');
	if ((f && !r.contains(f) && r.appendChild(f), !i))
		throw new Error('Failed to create sidebar element');
	n
		? (i.style.height = typeof n == 'number' ? `${n}px` : n)
		: (E(i), window.addEventListener('resize', () => E(i))),
		I(i),
		$(i, e),
		e.darkModeEnabled !== !1 && A(i, e);
	const p = i.querySelectorAll('.ubits-sidebar-body .ubits-sidebar-nav-button');
	p.forEach((u, g) => {
		const m = o[g];
		m &&
			u.addEventListener('click', (h) => {
				h.preventDefault(),
					m.state !== 'disabled' &&
						(p.forEach((k) => k.classList.remove('active')),
						u.classList.add('active'),
						e.onActiveButtonChange && e.onActiveButtonChange(m.section),
						m.onClick ? m.onClick(h) : m.href && (window.location.href = m.href));
			});
	}),
		i.querySelectorAll('.ubits-sidebar-footer .ubits-sidebar-nav-button').forEach((u, g) => {
			const m = e.footerButtons?.[g];
			m &&
				u.id !== 'ubits-darkmode-toggle' &&
				u.addEventListener('click', (h) => {
					h.preventDefault(),
						m.state !== 'disabled' &&
							(m.onClick ? m.onClick(h) : m.href && (window.location.href = m.href));
	...
	const c = i.querySelector('.ubits-sidebar-logo');
	if (c) {
		const u = c.getAttribute('data-href');
		u &&
...
const S = [
	...
	H = [
		{ section: 'inicio', icon: 'fa-house', tooltip: 'Inicio', href: 'admin.html' },
		{ section: 'empresa', icon: 'fa-building', tooltip: 'Empresa', href: 'admin-empresa.html' },
		{
			section: 'aprendizaje',
			icon: 'fa-graduation-cap',
			tooltip: 'Aprendizaje',
	...
	D = [
		{ section: 'api', icon: 'fa-code', tooltip: 'API', href: 'admin-api.html', id: 'api-button' },
		{
	...
	j = [
		{ icon: 'fa-user', label: 'Ver mi perfil', href: 'profile.html' },
		{ divider: !0 },
		{ icon: 'fa-laptop', label: 'Modo Administrador', href: 'admin.html' },
		{ divider: !0 },
		{ icon: 'fa-key', label: 'Cambio de contraseña', onClick: () => {} },
		{ icon: 'fa-sign-out', label: 'Cerrar sesión', onClick: () => {} },
	],
	z = [
		{ icon: 'fa-user', label: 'Ver mi perfil', href: 'profile.html' },
		{ divider: !0 },
		{ icon: 'fa-user-gear', label: 'Modo colaborador', href: 'profile.html' },
		{ divider: !0 },
		{ icon: 'fa-key', label: 'Cambio de contraseña', onClick: () => {} },
		{ icon: 'fa-sign-out', label: 'Cerrar sesión', onClick: () => {} },
	];
function O(e) {
	return e === 'admin'
		? { bodyButtons: H, footerButtons: D, profileMenuItems: z }
		: { bodyButtons: S, footerButtons: [], profileMenuItems: j };
}
const q = {
	...
	argTypes: {
		variant: {
			...
			table: { defaultValue: { summary: 'colaborador' }, type: { summary: 'colaborador | admin' } },
		},
		activeButton: {
			control: { type: 'select' },
			options: [
...
function U(e) {
	const t = O(e);
	return {
		bodyButtons: t.bodyButtons,
		footerButtons: t.footerButtons,
		profileMenuItems: t.profileMenuItems,
	};
}
function C(e, t) {
	return e.map((o) => ({ ...o, state: o.section === t ? 'active' : 'default' }));
}
const x = {
	args: {
	...
	render: (e) => {
		let t = document.getElementById('sidebar-story-wrapper');
		t
			? ((t.innerHTML = ''),
				(t.style.cssText = `
        display: flex;
      ...
      `),
		...
		(o.id = e.containerId || 'sidebar-story-container'),
			(o.style.cssText = `
      position: relative;
      width: 96px;
      height: 650px;
    ...
    `),
			t.appendChild(o);
		const n = e.variant || 'colaborador',
			r = e.activeButton || '',
			d = U(n),
			l = C(d.bodyButtons, r),
			i = r ? C(d.footerButtons || [], r) : d.footerButtons || [],
			s = {
				containerId: o.id,
				variant: n,
				bodyButtons: l,
				footerButtons: i,
				profileMenuItems: d.profileMenuItems,
				logoHref: n === 'admin' ? 'admin.html' : 'index.html',
				logoImage: e.logoImage || '/images/Ubits-logo.svg',
				avatarImage: e.avatarImage || '/images/Profile-image.jpg',
				darkModeEnabled: e.darkModeEnabled !== !1,
				height: 650,
				onActiveButtonChange: (c) => {},
				onDarkModeToggle: (c) => {},
				onAvatarClick: () => {},
			};
		try {
			...
			console.error('Error creating sidebar:', c);
			const u = M(s);
			o.innerHTML = u;
		}
...
const N = ['Default'];
export { x as Default, N as __namedExportsOrder, q as default };
