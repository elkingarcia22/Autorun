...
function _(e, t = 'regular', r) {
	const o = t === 'solid' ? 'fas' : 'far',
		a = e.startsWith('fa-') ? e : `fa-${e}`,
		s = r ? `style="color: ${r};"` : '';
	return `<i class="${o} ${a}" ${s}></i>`;
}
function S(e) {
	return typeof e == 'number' ? e.toLocaleString('es-ES') : String(e);
}
function T(e) {
	const {
			title: t,
		...
		b = ['ubits-metric-card', `ubits-metric-card--${i}`, p].filter(Boolean).join(' '),
		f = [...Object.entries(y).map(([C, $]) => `${C}="${$}"`)].filter(Boolean).join(' '),
		g = a ? `<div class="ubits-metric-card__title-icon">${_(a, s, d)}</div>` : '',
		v = u
			? c({
					variant: 'tertiary',
					size: 'sm',
		...
		l = m
			...
			: '',
		h = 'ubits-body-md-regular',
		I = i === 'sm' ? 'ubits-body-sm-regular' : 'ubits-body-md-regular',
		w = 'ubits-heading-h2',
		x = S(r);
	return `
    <div class="${b}" ${f}>
...
const H = {
		...
		argTypes: {
			title: {
				...
				table: { defaultValue: { summary: 'Net confidence score' }, type: { summary: 'string' } },
			},
			value: {
				control: { type: 'text' },
				description: 'Valor principal (puede ser número o string, ej: "200 / 204")',
				table: { defaultValue: { summary: '200 / 204' }, type: { summary: 'string | number' } },
			},
			label: {
				control: { type: 'text' },
				description: 'Texto descriptivo debajo del valor',
				table: { defaultValue: { summary: 'Colaboradores' }, type: { summary: 'string' } },
			},
			titleIcon: {
				control: { type: 'text' },
	...
	n = {
		...
		render: (e) => {
			...
			const o = T(e);
			return (r.innerHTML = o), t.appendChild(r), t;
		},
	};
n.parameters = {
	...n.parameters,
...
const L = ['Default'];
export { n as Default, L as __namedExportsOrder, H as default };
