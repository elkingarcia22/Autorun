import { c as _ } from './utils-QDDdDBjb.js';
const J = {
	...
	parameters: {
		...
		docs: {
			description: {
				component:
					'Tokens para efectos visuales: elevation (sombras), focus (anillos de enfoque) y floating (elevacion flotante). Los efectos de elevation tienen estados Default y Hover, focus tiene propiedades configurables (position, blur, spread, color), y floating usa tokens de Figma con dos sombras combinadas.',
			},
		},
	},
};
function R(e, o, t = 0, p = 1, n = 2, c = 0, i = 'rgba(0, 0, 0, 0.12)') {
	...
	(f.textContent = 'Preview'),
		...
		(f.style.color = '#303a47'),
		m.appendChild(f);
	const l = document.createElement('div');
	(l.textContent = e),
		(l.style.width = '120px'),
		(l.style.height = '120px'),
		(l.style.backgroundColor = '#f3f3f4'),
		(l.style.border = '1px solid #d0d2d5'),
		...
		(l.style.color = '#303a47');
	const I = `${t}px ${p}px ${n}px ${c}px ${i}`;
	(l.style.boxShadow = I), m.appendChild(l);
	const v = document.createElement('div');
	v.style.flex = '1';
	const $ = document.createElement('h4');
	($.textContent = 'Controles'),
		($.style.fontSize = '16px'),
	...
	(S.style.display = 'flex'), (S.style.gap = '12px');
	const x = document.createElement('input');
	(x.type = 'number'),
		(x.value = t.toString()),
		(x.style.width = '80px'),
		(x.style.padding = '6px 8px'),
		(x.style.border = '1px solid #d0d2d5'),
		(x.style.borderRadius = '6px'),
		(x.style.fontSize = '14px'),
		x.addEventListener('input', () => {
			const b = parseFloat(x.value) || 0,
				E = parseFloat(s.value) || 0,
				w = parseFloat(r.value) || 0,
				T = parseFloat(d.value) || 0,
				B = u.value,
				F = `${b}px ${E}px ${w}px ${T}px ${B}`;
			l.style.boxShadow = F;
		});
	const h = document.createElement('label');
	(h.textContent = 'X'),
		(h.style.fontSize = '12px'),
		(h.style.color = '#5c646f'),
	...
	(s.type = 'number'),
		...
		(s.style.border = '1px solid #d0d2d5'),
	...
	(z.textContent = 'Y'),
		...
		(z.style.color = '#5c646f'),
		(z.style.marginLeft = '12px'),
		(z.style.marginRight = '4px'),
		S.appendChild(z),
		S.appendChild(s),
		V.appendChild(S);
	...
	(r.type = 'number'),
		...
		(r.style.border = '1px solid #d0d2d5'),
	...
	(d.type = 'number'),
		...
		(d.style.border = '1px solid #d0d2d5'),
	...
	(P.style.display = 'flex'), (P.style.alignItems = 'center'), (P.style.gap = '8px');
	const H = document.createElement('div');
	(H.style.width = '32px'),
		(H.style.height = '32px'),
		(H.style.borderRadius = '4px'),
		(H.style.border = '1px solid #d0d2d5'),
		(H.style.backgroundColor = i),
		P.appendChild(H);
	const u = document.createElement('input');
	(u.type = 'text'),
		(u.value = i),
		(u.style.flex = '1'),
		(u.style.padding = '6px 8px'),
		(u.style.border = '1px solid #d0d2d5'),
		(u.style.borderRadius = '6px'),
		(u.style.fontSize = '14px'),
		(u.style.fontFamily = 'monospace'),
		u.addEventListener('input', () => {
			H.style.backgroundColor = u.value;
			const b = parseFloat(x.value) || 0,
				E = parseFloat(s.value) || 0,
				w = parseFloat(r.value) || 0,
				T = parseFloat(d.value) || 0,
	...
	(k.type = 'number'),
		(k.value = '30'),
		(k.style.width = '60px'),
		(k.style.padding = '6px 8px'),
		(k.style.border = '1px solid #d0d2d5'),
...
const Y = {
		render: () => {
			...
			(e.style.padding = '24px'), (e.style.maxWidth = '1400px');
			const o = document.createElement('h2');
			(o.textContent = 'Effects - Elevation y Focus'),
			...
			return (
				(t.textContent =
					'Tokens para efectos visuales: elevation (sombras) y focus (anillos de enfoque). Los efectos de elevation tienen estados Default y Hover, mientras que focus tiene propiedades configurables.'),
				(t.style.fontSize = '16px'),
				(t.style.color = '#5c646f'),
	...
	q = {
		render: () => {
			...
			(e.style.padding = '24px'), (e.style.maxWidth = '1400px');
			const o = document.createElement('h2');
			(o.textContent = 'Elevation - Sombras'),
				(o.style.fontSize = '24px'),
				(o.style.fontWeight = '700'),
			...
			(f.textContent = 'Tokens Existentes'),
				...
				m.appendChild(f);
			const l = document.createElement('div');
			(l.style.display = 'flex'), (l.style.flexDirection = 'column'), (l.style.gap = '12px');
			const I = _('--modifiers-normal-effects-elevation-overlay', 'light', {
				showVariable: !0,
				showValue: !0,
				width: '100%',
			});
			return l.appendChild(I), m.appendChild(l), e.appendChild(m), e;
		},
	},
	G = {
		render: () => {
			...
			(e.style.padding = '24px'), (e.style.maxWidth = '1400px');
			const o = document.createElement('h2');
			(o.textContent = 'Focus - Anillos de Enfoque'),
				(o.style.fontSize = '24px'),
				(o.style.fontWeight = '700'),
				(o.style.marginBottom = '24px'),
				e.appendChild(o);
			...
			const p = R(
				'Focus',
				'0px 0px 0px 4px rgba(82, 151, 244, 0.3)',
				0,
				0,
				0,
				4,
			...
			(c.textContent = 'Tokens Existentes'),
			...
			(i.style.display = 'flex'), (i.style.flexDirection = 'column'), (i.style.gap = '12px');
			const a = _('--modifiers-normal-focus-color', 'light', {
				showVariable: !0,
				showValue: !0,
				width: '100%',
			});
			return i.appendChild(a), n.appendChild(i), e.appendChild(n), e;
		},
	},
	U = {
		render: () => {
			...
			(e.style.padding = '24px'), (e.style.maxWidth = '1400px');
			const o = document.createElement('h2');
			(o.textContent = 'Interactive - Elevation States'),
				(o.style.fontSize = '24px'),
				(o.style.fontWeight = '700'),
				(o.style.marginBottom = '24px'),
				e.appendChild(o);
			const t = document.createElement('p');
			(t.textContent =
				'Estados interactivos de elevation: Default y Hover. Cada estado tiene sus propias propiedades de sombra configurables.'),
				(t.style.fontSize = '16px'),
				(t.style.color = '#5c646f'),
				(t.style.marginBottom = '32px'),
				(t.style.lineHeight = '1.6'),
			...
			p.appendChild(c), e.appendChild(p);
			const i = document.createElement('div');
			...
			return i.appendChild(m), e.appendChild(i), e;
		},
	},
	A = {
		render: () => {
			...
			(e.style.padding = '24px'), (e.style.maxWidth = '1400px');
			const o = document.createElement('h2');
			(o.textContent = 'Floating - Elevacion Flotante'),
				(o.style.fontSize = '24px'),
				(o.style.fontWeight = '700'),
				(o.style.marginBottom = '24px'),
				e.appendChild(o);
			const t = document.createElement('p');
			(t.textContent =
				'Elevacion flotante construida con tokens de Figma. Usa dos sombras combinadas (floating-0 y floating-1) para crear un efecto de profundidad.'),
				(t.style.fontSize = '16px'),
				(t.style.color = '#5c646f'),
				(t.style.marginBottom = '32px'),
				(t.style.lineHeight = '1.6'),
			...
			const p = document.documentElement,
				n = (d) => getComputedStyle(p).getPropertyValue(d).trim(),
				c = parseFloat(n('--modifiers-normal-elevation-floating-0-x')) || 0,
				i = parseFloat(n('--modifiers-normal-elevation-floating-0-y')) || 14,
				a = parseFloat(n('--modifiers-normal-elevation-floating-0-blur')) || 28.8,
				m = parseFloat(n('--modifiers-normal-elevation-floating-0-spread')) || 0,
				f = n('--modifiers-normal-elevation-floating-0-color') || '#0000003d',
				l = parseFloat(n('--modifiers-normal-elevation-floating-1-x')) || 0,
				I = parseFloat(n('--modifiers-normal-elevation-floating-1-y')) || 0,
				v = parseFloat(n('--modifiers-normal-elevation-floating-1-blur')) || 8,
				$ = parseFloat(n('--modifiers-normal-elevation-floating-1-spread')) || 0,
				V = n('--modifiers-normal-elevation-floating-1-color') || '#00000033',
				C = `${c}px ${i}px ${a}px ${m}px ${f}`,
				S = `${l}px ${I}px ${v}px ${$}px ${V}`,
				x = `${C}, ${S}`,
				h = document.createElement('div');
			h.style.marginBottom = '40px';
			const s = document.createElement('h3');
			(s.textContent = 'Floating'),
				(s.style.fontSize = '20px'),
				(s.style.fontWeight = '600'),
				(s.style.marginBottom = '16px'),
				(s.style.color = '#303a47'),
				h.appendChild(s);
			const z = R('Floating', x, c, i, a, m, f);
			h.appendChild(z), e.appendChild(h);
			const g = document.createElement('div');
			(g.style.marginTop = '40px'),
				(g.style.padding = '20px'),
				(g.style.backgroundColor = '#f9fafb'),
				(g.style.border = '1px solid #e5e7eb'),
				(g.style.borderRadius = '12px');
			const y = document.createElement('h3');
			(y.textContent = 'Tokens de Figma'),
			...
			return (
				...
				[
					...
					const L = _(d, 'light', { showVariable: !0, showValue: !0, width: '100%' });
					r.appendChild(L);
				}),
				[
...
Y.parameters = {
  ...
  render: () => {
    ...
    return container;
		...
		description: {
			story: 'Story principal - Todos los efectos',
...
q.parameters = {
  ...
  render: () => {
    ...
    return container;
		...
		description: { story: 'Story - Elevation', ...q.parameters?.docs?.description },
	},
};
G.parameters = {
	...G.parameters,
  ...
  render: () => {
    ...
    return container;
		...
		description: { story: 'Story - Focus', ...G.parameters?.docs?.description },
	},
};
U.parameters = {
	...U.parameters,
  ...
  render: () => {
    ...
    return container;
		...
		description: {
			story: 'Story - Interactive (Elevation con estados Default y Hover)',
...
A.parameters = {
  ...
  render: () => {
    ...
    return container;
		...
		description: {
			story: 'Story - Floating (Elevation flotante con tokens de Figma)',
			...A.parameters?.docs?.description,
		},
	},
};
const K = ['Docs', 'Elevation', 'Focus', 'Interactive', 'Floating'];
export {
	Y as Docs,
	q as Elevation,
	A as Floating,
