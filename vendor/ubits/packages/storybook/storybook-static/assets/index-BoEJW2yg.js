import { _ as z } from './preload-helper-PPVm8Dsz.js';
const k = [
		'Enero',
		'Febrero',
		'Marzo',
...
function P(r, s) {
	...
	const p = `calendar-list-container-${Date.now()}`,
		u = `calendar-list-${Date.now()}`,
		M = `calendar-scrollbar-${Date.now()}`;
	let $ = `
  ...
  `;
	r.forEach((i) => {
		const e = i.selected ? 'active' : 'default',
			t = ['ubits-list-item', 'ubits-list-item--sm', e !== 'default' ? `ubits-list-item--${e}` : '']
				.filter(Boolean)
				.join(' '),
			a = [];
		e === 'active' && a.push('aria-selected="true"'),
			a.push('tabindex="0"'),
			a.push(`data-value="${i.value}"`),
			($ += `
  ...
  `),
	...
	const I = async () => {
			...
			if (!(!i || !e) && !(i.scrollHeight <= i.clientHeight))
				try {
					const t = window.createScrollbarLocal;
					if (typeof t == 'function') {
						const g = t(i, e, 'vertical');
						if (g) {
							n._scrollbarInstance = g;
							return;
						}
					}
					const { createScrollbar: a } = await z(
							async () => {
								const { createScrollbar: g } = await import('./ScrollProvider-BVL7eCy8.js');
								return { createScrollbar: g };
							},
							[],
							import.meta.url,
						),
						l = a({ orientation: 'vertical', targetId: u, containerId: M });
	...
	if (n.parentElement) w();
	else {
		const i = new MutationObserver(() => {
			n.isConnected && (i.disconnect(), w());
		});
	...
	return (
		setTimeout(() => {
			const i = document.getElementById(u);
			i &&
				i.querySelectorAll('.ubits-list-item').forEach((e) => {
					e.addEventListener('click', (t) => {
						t.preventDefault(), t.stopPropagation();
						const a = parseInt(t.currentTarget.dataset.value || '0'),
							l = n._scrollbarInstance;
...
function A(r) {
	const {
		...
		t = i.getMonth(),
		a = new Date(e, t, 1),
		g = new Date(e, t + 1, 0).getDate(),
		v = a.getDay(),
		q = new Date().toDateString(),
		F = ['ubits-calendar', s === 'range' ? 'ubits-calendar--range' : 'ubits-calendar--single', I]
			.filter(Boolean)
			.join(' '),
		d = w ? ` style="${w}"` : '',
		c = `
  ...
  `;
	...
	for (let y = 1; y <= g; y++) {
		const _ = new Date(e, t, y),
			o = _.toDateString() === q;
		let m = ['ubits-calendar__day'];
		s === 'single' && n && C(_, n) && m.push('ubits-calendar__day--selected'),
			s === 'range' &&
				n &&
				(p
					? C(_, n)
						? m.push('ubits-calendar__day--range-start')
						: C(_, p)
							? m.push('ubits-calendar__day--range-end')
							: V(_, n, p) && m.push('ubits-calendar__day--in-range')
					: C(_, n) && m.push('ubits-calendar__day--range-start')),
			o && m.push('ubits-calendar__day--today');
		let h = !1;
		u && L(_, u) < 0 && ((h = !0), m.push('ubits-calendar__day--disabled')),
			M && L(_, M) > 0 && ((h = !0), m.push('ubits-calendar__day--disabled'));
		const E = h ? ' disabled' : '',
			D = R(_);
		x += `<button type="button" class="${m.join(' ')}" data-date="${D}"${E}>${y}</button>`;
	}
	return (
		(x += '</div>'),
		`
...
function j(r) {
	...
	const v = () => {
		...
		Y = () => {
			const d = e.querySelector('.ubits-calendar__nav-button--prev'),
				T = e.querySelector('.ubits-calendar__nav-button--next'),
				c = e.querySelector('.ubits-calendar__month-input'),
				S = e.querySelector('.ubits-calendar__year-input'),
				x = e.querySelector('.ubits-calendar__month-dropdown'),
				y = e.querySelector('.ubits-calendar__year-dropdown');
			d?.addEventListener('click', (b) => {
				b.preventDefault(),
					b.stopPropagation(),
					t.setMonth(t.getMonth() - 1),
					c && (c.value = k[t.getMonth()]),
					S && (S.value = String(t.getFullYear())),
					v();
			}),
				...
				c?.addEventListener('click', (b) => {
					if ((b.preventDefault(), b.stopPropagation(), x)) {
						...
						if (o.style.display === 'block') o.style.display = 'none';
							...
							const h = k.map((D, f) => ({ label: D, value: f, selected: f === t.getMonth() }));
							o.innerHTML = '';
							const E = P(h, (D) => {
				...
				S?.addEventListener('click', (b) => {
					if ((b.preventDefault(), b.stopPropagation(), y)) {
						...
						if (o.style.display === 'block') o.style.display = 'none';
							...
							const h = t.getFullYear(),
								E = Array.from({ length: 100 }, (f, B) => {
									const H = h - 50 + B;
									return { label: String(H), value: H, selected: H === h };
								});
							o.innerHTML = '';
							const D = P(E, (f) => {
								t.setFullYear(f), (o.style.display = 'none'), S && (S.value = String(f)), v();
							});
				...
				e
					...
					.forEach((b) => {
						b.addEventListener('click', (o) => {
							...
							const m = b.dataset.date || '',
								[h, E, D] = m.split('/'),
								f = new Date(parseInt(D), parseInt(E) - 1, parseInt(h));
							s === 'single'
								? ((a = f), v(), I && I(f))
								: s === 'range' &&
									(!a || (a && l)
										...
										: a &&
											!l &&
											(L(f, a) < 0 ? ((l = a), (a = f)) : (l = f), v(), w && a && l && w(a, l)));
						});
					});
		};
	return (
		v(),
		{
			element: e,
			update: (d) => {
				d.selectedDate !== void 0 && (a = d.selectedDate ? new Date(d.selectedDate) : null),
					d.endDate !== void 0 && (l = d.endDate ? new Date(d.endDate) : null),
					d.initialDate && (t = new Date(d.initialDate)),
					Object.assign(r, d),
					v();
			},
			destroy: () => {
				const d = e.querySelector('.ubits-calendar__month-dropdown'),
...
const J = Object.freeze(
		Object.defineProperty(
			{ __proto__: null, createCalendar: j, renderCalendar: A },
			Symbol.toStringTag,
			{ value: 'Module' },
		),
	),
