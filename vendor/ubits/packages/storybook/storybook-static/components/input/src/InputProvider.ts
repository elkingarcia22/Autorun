...
export function renderInput(options: InputOptions): string {
	...
	if (showLabel && label) {
		const mandatoryText = mandatory
			? ` <span class="ubits-input-mandatory">(${mandatoryType})</span>`
			: '';
	...
	const leftIconClass =
		hasLeftIcon && leftIcon.startsWith('fa-')
			? `far ${leftIcon}`
			: hasLeftIcon
				? `far fa-${leftIcon}`
				: '';
	const rightIconClass =
		hasRightIcon && rightIcon.startsWith('fa-')
			? `far ${rightIcon}`
			: hasRightIcon
				? `far fa-${rightIcon}`
				: '';
	...
	if (type === 'select') {
		...
		const selectValue = value
			? selectOptions.find((opt) => opt.value === value)?.text || placeholder
			: placeholder;
	...
	if (finalHasLeftIcon) {
		const leftIconClass = finalLeftIcon.startsWith('fa-')
			? `far ${finalLeftIcon}`
			: `far fa-${finalLeftIcon}`;
	...
	if (finalHasRightIcon) {
		const rightIconClass = finalRightIcon.startsWith('fa-')
			? `far ${finalRightIcon}`
			: `far fa-${finalRightIcon}`;
...
export function createInput(options: InputOptions): {
	...
	const inputElement = container.querySelector('.ubits-input') as
		| HTMLInputElement
		| HTMLTextAreaElement;
	...
	return {
		...
		setState: (newState: string) => {
			...
			if (type === 'textarea' && options.showRichTextToolbar) {
				const richTextWrapper = inputElement.closest(
					'.ubits-input-rich-text-wrapper',
				) as HTMLElement;
				const toolbar = richTextWrapper?.querySelector(
					'.ubits-input-rich-text-toolbar',
				) as HTMLElement;
...
function createAutocompleteDropdown(
	...
	const listSize: ListSize =
		inputSize === 'xs' ? 'xs' : inputSize === 'sm' ? 'sm' : inputSize === 'md' ? 'md' : 'lg';
	...
	if (clearIcon) {
		...
		clearIcon.addEventListener('click', (e) => {
			...
			const listContainer = container.querySelector(
				'.ubits-autocomplete-list-container',
			) as HTMLElement;
    ...
    `,
		...
		onClose: () => {
			...
			const modal = document
				...
				?.closest('.ubits-modal-overlay') as HTMLElement;
			if (modal) {
...
function setupRichTextToolbar(
	...
	if (!leftIconElement && container.parentElement) {
		leftIconElement = container.parentElement.querySelector(
	...
	if (!leftIconElement && richTextWrapper?.parentElement) {
		leftIconElement = richTextWrapper.parentElement.querySelector(
			'.ubits-input-icon-left',
		) as HTMLElement;
	}
	...
	if (hasLeftIcon && leftIconElement) {
		...
		requestAnimationFrame(() => {
			requestAnimationFrame(() => {
				...
				if (inputWrapper) {
					iconAfterInsert =
						(inputWrapper.querySelector('.ubits-input-icon-left') as HTMLElement) ||
						leftIconElement;
				}
				if (!iconAfterInsert && container.parentElement) {
					iconAfterInsert =
						(container.parentElement.querySelector('.ubits-input-icon-left') as HTMLElement) ||
						leftIconElement;
				}
 ...
 */
function setupTextareaPlaceholderAlignment(
	container: HTMLElement,
	textareaElement: HTMLTextAreaElement,
): void {
	// Buscar el wrapper de múltiples formas
	...
	requestAnimationFrame(() => {
		requestAnimationFrame(() => {
			const iconAfterRender =
				(inputWrapper?.querySelector('.ubits-input-icon-left') as HTMLElement) || leftIconElement;
