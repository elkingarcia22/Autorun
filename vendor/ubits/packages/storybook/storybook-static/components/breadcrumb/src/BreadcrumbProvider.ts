...
export function renderBreadcrumb(options: BreadcrumbOptions): string {
	...
	// Renderizar items
	const itemsHTML = itemsWithActive
		.map((item, index) => {
			...
			const classes = ['ubits-breadcrumb__item', activeClass, disabledClass]
				.filter(Boolean)
				.join(' ');
...
function initBreadcrumbListeners(breadcrumbElement: HTMLElement, options: BreadcrumbOptions): void {
	...
	const existingItems = breadcrumbElement.querySelectorAll<HTMLElement>(
	...
	const items = breadcrumbElement.querySelectorAll<HTMLElement>(
