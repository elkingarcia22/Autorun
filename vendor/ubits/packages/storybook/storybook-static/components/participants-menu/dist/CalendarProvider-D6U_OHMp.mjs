...
function N(i) {
	const s = String(i.getDate()).padStart(2, '0'),
		o = String(i.getMonth() + 1).padStart(2, '0'),
		p = i.getFullYear();
	return `${s}/${o}/${p}`;
}
function E(i, s) {
	const o = new Date(i.getFullYear(), i.getMonth(), i.getDate()),
...
function T(i, s) {
  ...
  `;
	i.forEach((n) => {
		const e = n.selected ? 'active' : 'default',
  ...
  `),
	...
	const H = async () => {
			console.log('📜 [SCROLLBAR] ========== INICIO initScrollbar =========='),
				console.log('📜 [SCROLLBAR] listId:', u),
				console.log('📜 [SCROLLBAR] scrollbarContainerId:', L);
		...
		w = () => {
			console.log('📜 [SCROLLBAR] setupScrollbar llamado, isConnected:', o.isConnected),
				o.isConnected &&
	...
	if (o.parentElement)
		...
		n.observe(document.body, { childList: !0, subtree: !0 }),
			setTimeout(() => {
				o.isConnected &&
					(console.log('📜 [SCROLLBAR] Timeout alcanzado, inicializando'), n.disconnect(), w());
			}, 1e3);
	}
	return (
		setTimeout(() => {
			const n = document.getElementById(u);
			n &&
				n.querySelectorAll('.ubits-list-item').forEach((e) => {
					e.addEventListener('click', (t) => {
						t.preventDefault(), t.stopPropagation();
						const a = parseInt(t.currentTarget.dataset.value || '0'),
							d = o._scrollbarInstance;
...
function F(i) {
	const {
		...
		t = n.getMonth(),
		a = new Date(e, t, 1),
		v = new Date(e, t + 1, 0).getDate(),
		f = a.getDay(),
		k = /* @__PURE__ */ new Date().toDateString(),
		O = ['ubits-calendar', s === 'range' ? 'ubits-calendar--range' : 'ubits-calendar--single', H]
			.filter(Boolean)
			.join(' '),
		r = w ? ` style="${w}"` : '',
		l = `
  ...
  `;
	...
	for (let y = 1; y <= v; y++) {
		const D = new Date(e, t, y),
			c = D.toDateString() === k;
		let g = ['ubits-calendar__day'];
		s === 'single' && o && I(D, o) && g.push('ubits-calendar__day--selected'),
			s === 'range' &&
				o &&
				(p
					? I(D, o)
						? g.push('ubits-calendar__day--range-start')
						: I(D, p)
							? g.push('ubits-calendar__day--range-end')
							: Y(D, o, p) && g.push('ubits-calendar__day--in-range')
					: I(D, o) && g.push('ubits-calendar__day--range-start')),
			c && g.push('ubits-calendar__day--today');
		let _ = !1;
		u && E(D, u) < 0 && ((_ = !0), g.push('ubits-calendar__day--disabled')),
			L && E(D, L) > 0 && ((_ = !0), g.push('ubits-calendar__day--disabled'));
		const M = _ ? ' disabled' : '',
			S = N(D);
...
function P(i) {
	...
	const f = () => {
		...
		B = () => {
			const r = e.querySelector('.ubits-calendar__nav-button--prev'),
				x = e.querySelector('.ubits-calendar__nav-button--next'),
				l = e.querySelector('.ubits-calendar__month-input'),
				h = e.querySelector('.ubits-calendar__year-input'),
				C = e.querySelector('.ubits-calendar__month-dropdown'),
			...
			r == null ||
				r.addEventListener('click', (b) => {
					b.preventDefault(),
						...
						l && (l.value = A[t.getMonth()]),
						h && (h.value = String(t.getFullYear())),
						f();
				}),
				x == null ||
				...
				e
					...
					.forEach((b) => {
						b.addEventListener('click', (c) => {
							...
							const g = b.dataset.date || '',
								[_, M, S] = g.split('/'),
								m = new Date(parseInt(S), parseInt(M) - 1, parseInt(_));
							s === 'single'
								? ((a = m), f(), H && H(m))
								: s === 'range' &&
									(!a || (a && d)
										...
										: a &&
											!d &&
											(E(m, a) < 0 ? ((d = a), (a = m)) : (d = m), f(), w && a && d && w(a, d)));
						});
					});
		};
	return (
		f(),
		{
			element: e,
			update: (r) => {
				r.selectedDate !== void 0 && (a = r.selectedDate ? new Date(r.selectedDate) : null),
					r.endDate !== void 0 && (d = r.endDate ? new Date(r.endDate) : null),
					r.initialDate && (t = new Date(r.initialDate)),
					Object.assign(i, r),
					f();
			},
			destroy: () => {
				const r = e.querySelector('.ubits-calendar__month-dropdown'),
					x = e.querySelector('.ubits-calendar__year-dropdown');
				if (r) {
...
export { P as createCalendar, F as renderCalendar };
