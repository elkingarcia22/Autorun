 ...
 */
const STATUS_TAG_MAPPING: Record<
	ParticipantStatus,
	{
...
function renderParticipant(
	...
	const statusTag =
		showStatusTag && participant.status ? renderParticipantStatusTag(participant.status) : '';
...
export function renderParticipantsMenu(options: ParticipantsMenuOptions): string {
	...
	const participantsHtml = participantsToShow
		.map((participant) => {
	...
	const searchInputId =
		(options as any).searchInputId ||
		(options.containerId
			? `participants-menu-search-${options.containerId}`
		...
		if (emptyStateConfig) {
			emptyStateHTML = renderEmptyState({
				...
				description: emptyStateConfig.description
					? escapeHtml(emptyStateConfig.description)
					: undefined,
...
export function createParticipantsMenu(options: ParticipantsMenuOptions): {
	...
	const insertMenu = () => {
        ...
        `;
			...
			const statusesHTML = allStatuses
				.map((status, index) => {
			...
			item.addEventListener('click', () => {
				if (participantId && onParticipantSelect) {
					...
					(item as HTMLElement).style.backgroundColor =
						'var(--modifiers-normal-color-light-bg-active)';
	...
	const initializeInput = () => {
		setTimeout(() => {
			...
			if (searchInputContainer) {
				const placeholder =
					searchInputContainer.getAttribute('data-search-placeholder') ||
	...
	const updateParticipantsList = (
		...
		newSelectedParticipantId?: string,
	) => {
		...
		const participantsToShow =
			restOptions.enableScrollbar !== false
				? newParticipants
		...
		const hasActiveFilters =
			(activeFilters.roles?.length || 0) + (activeFilters.statuses?.length || 0) > 0;
	...
	const update = (newOptions: Partial<ParticipantsMenuOptions>) => {
		const onlyParticipantsChanged =
			newOptions.participants &&
			Object.keys(newOptions).every(
		...
		if (
			newOptions.enableScrollbar !== undefined &&
			newOptions.enableScrollbar !== restOptions.enableScrollbar
		) {
			const listWrapper = menuElement.querySelector(
				'.ubits-participants-menu__list-wrapper',
			) as HTMLElement;
