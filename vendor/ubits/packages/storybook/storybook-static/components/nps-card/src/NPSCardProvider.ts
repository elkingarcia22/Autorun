...
function renderSemicircularGauge(
	...
	const angleDiff =
		scoreAngle > startAngle ? scoreAngle - startAngle : 360 - startAngle + scoreAngle;
...
export function renderNPSCard(options: NPSCardOptions): string {
	...
	const classes = ['ubits-nps-card', `ubits-nps-card--${size}`, className]
		.filter(Boolean)
		.join(' ');
