...
export function renderBadge(options: BadgeOptions = {}): string {
	...
	const badgeType =
		type || (content !== undefined && content !== null && content !== '' ? 'number' : 'dot');
	...
	const badgeContent =
		badgeType === 'number' && content !== undefined && content !== null ? String(content) : '';
	...
	if (needsDot) {
		...
		const normalizedVariant = String(variant || 'primary')
			.toLowerCase()
			.trim();
		...
		if (badgeType === 'number' && badgeContent) {
			...
			const numberDotSize =
				size === 'xs' ? '18px' : size === 'sm' ? '20px' : size === 'md' ? '22px' : '24px';
			...
			if (style === 'bold') {
				...
				const fontSize =
					size === 'xs' ? '10px' : size === 'sm' ? '11px' : size === 'md' ? '12px' : '13px';
			...
			const dotSize =
				size === 'xs' ? '6px' : size === 'sm' ? '7px' : size === 'md' ? '8px' : '10px';
			...
			const finalDotBgColor =
				style === 'bold' ? 'var(--modifiers-normal-color-light-bg-1)' : dotBgColor;

			// Para bold, forzar blanco explícitamente; para otros, usar el color calculado
			const finalColor =
				style === 'bold' ? 'var(--modifiers-normal-color-light-bg-1)' : finalDotBgColor;
	...
	if (style && ['light', 'neutral', 'bold'].includes(style)) {
		if (showLabel) {
			...
			if (labelText) {
				...
				const labelColor =
					style === 'bold' ? 'style="color: var(--ubits-fg-on-accent, #ffffff) !important;"' : '';
