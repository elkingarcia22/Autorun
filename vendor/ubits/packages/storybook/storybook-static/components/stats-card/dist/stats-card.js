...
function T(a, s = 'md') {
	const {
		...
		h = w(e),
		p = s === 'sm' ? 'ubits-body-sm' : s === 'lg' ? 'ubits-body-lg' : 'ubits-body-md',
		d = s === 'sm' ? 'ubits-body-xs' : 'ubits-body-sm',
		u = r ? _(r, n, o) : '';
	let b = '';
	if (i) {
		const c = i.type,
			m =
				c === 'increase'
					? 'ubits-stats-card__change--increase'
					: c === 'decrease'
						? 'ubits-stats-card__change--decrease'
						: 'ubits-stats-card__change--neutral',
			f = c === 'increase' ? 'arrow-up' : c === 'decrease' ? 'arrow-down' : 'minus',
			$ = i.label || `${Math.abs(i.value)}%`;
		b = `
...
function C(a) {
	const {
		...
		b = [
			...
			.filter(Boolean)
			.join(' '),
		g = [...Object.entries(p).map(([v, y]) => `${v}="${y}"`)].filter(Boolean).join(' '),
		c = s
         ...
         `
			...
			: '',
		m = r.map((v) => T(v, e)).join(''),
		f = n === 'grid' ? `ubits-stats-card__grid ubits-stats-card__grid--${o}` : '';
	return `
    <div class="${b}" ${g}>
      ${c}
...
function L(a) {
	...
	if (!e) return console.error(`❌ [StatsCard] Contenedor con ID "${s}" no encontrado`), null;
	const r = C(t);
	e.innerHTML = r;
	const n = e.querySelector('.ubits-stats-card');
	if (!n) return console.error('❌ [StatsCard] No se pudo crear el elemento de la tarjeta'), null;
...
class S extends HTMLElement {
	...
	render() {
		const s = this.getAttribute('variant') || this.options.variant,
			t = this.getAttribute('size') || this.options.size,
			e = this.getAttribute('layout') || this.options.layout,
			r = parseInt(this.getAttribute('columns') || '2'),
			n = this.hasAttribute('bordered') || this.options.bordered,
			o = this.hasAttribute('elevated') || this.options.elevated,
			i = {
				...this.options,
				variant: s,
				size: t,
		...
		(this.innerHTML = C(i)),
			this.options.onClick && this.addEventListener('click', this.options.onClick);
	}
}
typeof window < 'u' &&
	!customElements.get('ubits-stats-card') &&
	customElements.define('ubits-stats-card', S);
...
class A {
	...
	async initialize(s) {
		customElements.get('ubits-stats-card') || customElements.define('ubits-stats-card', S),
			typeof window < 'u' &&
...
export { A as StatsCardAddon, S as UBITSStatsCard, L as createStatsCard, C as renderStatsCard };
