...
function renderIconHelper(
	iconName: string,
	iconStyle: 'regular' | 'solid' = 'regular',
	iconColor?: string,
): string {
...
function renderStatItem(item: StatItem, size: StatsCardOptions['size'] = 'md'): string {
	...
	const valueSizeClass =
		size === 'sm' ? 'ubits-body-sm' : size === 'lg' ? 'ubits-body-lg' : 'ubits-body-md';
	...
	if (change) {
		...
		const changeIcon =
			changeType === 'increase' ? 'arrow-up' : changeType === 'decrease' ? 'arrow-down' : 'minus';
...
export function renderStatsCard(options: StatsCardOptions): string {
	...
	const gridClass =
		layout === 'grid' ? `ubits-stats-card__grid ubits-stats-card__grid--${columns}` : '';
...
export function createStatsCard(
	options: StatsCardOptions & { containerId?: string },
): HTMLElement | null {
