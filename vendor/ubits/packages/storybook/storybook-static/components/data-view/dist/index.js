...
function y(t) {
	...
	a &&
		(n += '<i class="fas fa-star-half-alt ubits-data-view__star ubits-data-view__star--half"></i>');
	for (let i = 0; i < r; i++)
...
function O(t) {
	...
	const {
		...
		d = ['ubits-data-view', `ubits-data-view--${r}`, n].filter(Boolean).join(' '),
		v = Object.entries(i)
			.map(([c, u]) => `${c}="${u}"`)
			.join(' '),
		w = a ? `id="${a}"` : '';
...
function I(t) {
	...
	const {
		...
		c = `ubits-data-view--${n}`;
	return (
		(e.className = ['ubits-data-view', c, v].filter(Boolean).join(' ')),
		a && (e.id = a),
		Object.entries(w).forEach(([o, l]) => {
			e.setAttribute(o, l);
		}),
		(e.innerHTML = O(t)),
		e.querySelectorAll('.ubits-data-view__item').forEach((o, l) => {
			const f = r[l];
			if (!f) return;
			i &&
...
export { I as createDataView, O as renderDataView };
