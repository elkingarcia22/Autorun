...
function renderStepIndicator(number: number, state: StepperStepState): string {
	const indicatorClasses = ['ubits-stepper__indicator', `ubits-stepper__indicator--${state}`]
		.filter(Boolean)
		.join(' ');
...
function renderConnector(state: StepperStepState, orientation: 'horizontal' | 'vertical'): string {
	...
	const connectorClasses = ['ubits-stepper__connector', `ubits-stepper__connector--${orientation}`]
		.filter(Boolean)
		.join(' ');
