...
function Z(u) {
	const {
		...
		e = ['ubits-list', a].filter(Boolean).join(' '),
		o = Object.entries(r)
			.map(([i, g]) => `${i}="${g}"`)
			.join(' ');
	...
	return (
		n.forEach((i, g) => {
			const b = i.value || `list-item-${g}`,
				y = i.state || (i.selected ? 'active' : 'default'),
				m = [
					'ubits-list-item',
					`ubits-list-item--${s}`,
					...
					.join(' '),
				t = [];
			i.selected && t.push('aria-selected="true"'),
				y === 'disabled' ? t.push('aria-disabled="true"') : t.push('tabindex="0"'),
				t.push(`data-value="${b}"`),
				t.push(`data-index="${g}"`),
				i.attributes &&
					Object.entries(i.attributes).forEach(([h, d]) => {
						t.push(`${h}="${d}"`);
					}),
				(p += `
...
function ot(u) {
	const { containerId: n, items: s, size: c = 'md', onSelectionChange: a, multiple: r = !1 } = u,
		e = document.getElementById(n);
	if (!e) throw new Error(`Container with id "${n}" not found`);
	const o = Z(u);
	e.innerHTML = o;
	const p = e.querySelector('.ubits-list');
	if (!p) throw new Error('Failed to create list element');
			...
			m &&
						...
						if ((m.onClick && m.onClick(m, y), r)) {
							if (
								(b.classList.contains('ubits-list-item--active')
									? (b.classList.remove('ubits-list-item--active'),
										b.removeAttribute('aria-selected'))
									: (b.classList.add('ubits-list-item--active'),
										b.setAttribute('aria-selected', 'true')),
								a)
							) {
								const h = Array.from(i)
									.map((d, T) =>
										d.classList.contains('ubits-list-item--active')
											? { item: s[T], index: T }
											: null,
									)
									.filter(Boolean);
				...
				m.state !== 'disabled' &&
					b.addEventListener('keydown', (t) => {
						const h = y;
						let d = null;
						if (t.key === 'ArrowDown') t.preventDefault(), (d = h < s.length - 1 ? h + 1 : 0);
						else if (t.key === 'ArrowUp') t.preventDefault(), (d = h > 0 ? h - 1 : s.length - 1);
							...
							t.key === 'Home'
								...
								: t.key === 'End' && (t.preventDefault(), (d = s.length - 1));
						if (d !== null) {
							const T = i[d];
							T &&
								s[d]?.state !== 'disabled' &&
								(T.focus(), T.scrollIntoView({ block: 'nearest', behavior: 'smooth' }));
						}
					}));
		}),
		p
...
function ut(u) {
	const {
		...
		o = st[c] || st.md,
		g = ['ubits-modal', `ubits-modal--size-${c}`, a ? 'ubits-modal--full-screen' : '', e]
			.filter(Boolean)
			.join(' '),
		b = `
...
function J(u) {
	...
	if (!o) throw new Error('No se pudo crear el modal');
	o.querySelector('.ubits-modal');
	const p = o.querySelector('.ubits-modal__close'),
		i = o,
		g = () => {
			o.classList.add('ubits-modal-overlay--open'), (document.body.style.overflow = 'hidden');
		},
		b = () => {
			o.classList.remove('ubits-modal-overlay--open'),
	...
	if ((document.addEventListener('keydown', m), u.footerButtons)) {
		const t = o.querySelector('.ubits-modal__footer-left .ubits-modal__footer-button'),
			h = o.querySelector('.ubits-modal__footer-right .ubits-button--secondary'),
			d = o.querySelector('.ubits-modal__footer-right .ubits-button--primary');
		t &&
			u.footerButtons.tertiary?.onClick &&
			t.addEventListener('click', (T) => {
				T.preventDefault(), u.footerButtons.tertiary.onClick(T);
			}),
			h &&
				u.footerButtons.secondary?.onClick &&
				h.addEventListener('click', (T) => {
					T.preventDefault(), u.footerButtons.secondary.onClick(T);
				}),
			d &&
				u.footerButtons.primary?.onClick &&
...
function pt(u) {
	...
	const q = t && t.trim() !== '',
		I = h && h.trim() !== '';
	q && t.startsWith('fa-') ? `${t}` : q && `${t}`,
		I && h.startsWith('fa-') ? `${h}` : I && `${h}`,
		(w += '<div style="position: relative; display: inline-block; width: 100%;">');
	let R = h,
		B = I,
		D = t,
		V = q;
	...
	const j = e === 'disabled' ? ' disabled' : '',
		E = g ? ` maxlength="${b}"` : '',
		P = q ? 'padding-left: 40px;' : 'padding-left: 12px;',
		$ = I ? 'padding-right: 40px;' : 'padding-right: 12px;';
	if (o === 'select') {
		const M = u.selectOptions || [],
			H = (d && M.find((_) => _.value === d)?.text) || c;
		(w += `<input type="text" class="${v.join(' ')}" style="width: 100%; ${P} ${$}" value="${H}" readonly>`),
			I ||
				((R = 'fa-chevron-down'),
				(B = !0),
				$ === 'padding-right: 12px;' &&
					(w = w.replace(
						`style="width: 100%; ${P} ${$}"`,
						`style="width: 100%; ${P} padding-right: 40px;"`,
					)));
	} else if (o === 'textarea')
		if (k) {
			(w += '<div class="ubits-input-rich-text-wrapper">'),
				(w += `
      ...
      `);
		...
		e === 'disabled' &&
			(_ +=
				'; background: var(--ubits-bg-3) !important; color: var(--ubits-fg-1-low) !important; border-color: var(--ubits-border-2) !important;'),
			(w += `<input type="text" class="${v.join(' ')}" style="${_}" placeholder="${c}" value="${d}" autocomplete="off"${j}${E}>`);
		...
		e === 'disabled' &&
			(_ +=
				'; background: var(--ubits-bg-3) !important; color: var(--ubits-fg-1-low) !important; border-color: var(--ubits-border-2) !important;'),
			(w += `<input type="text" class="${v.join(' ')}" style="${_}" placeholder="${c}" value="${d}" readonly${j}>`);
	} else if (o === 'password') {
		let M = P,
			H = $;
		I ||
		...
		e === 'disabled' &&
			(_ +=
				'; background: var(--ubits-bg-3) !important; color: var(--ubits-fg-1-low) !important; border-color: var(--ubits-border-2) !important;'),
			(w += `<input type="password" class="${v.join(' ')}" style="${_}" placeholder="${c}" value="${d}"${j}${E}>`);
	} else
	...
	(w += '</div>'),
		(i || g) &&
			((w += '<div class="ubits-input-helper">'),
			i && a && (w += `<span>${a}</span>`),
			g && (w += `<span class="ubits-input-counter">0/${b}</span>`),
			(w += '</div>'));
...
function X(u) {
	...
	if (!b) return console.error(`UBITS Input: No se encontró el contenedor con ID "${n}"`), null;
	const y = pt(u);
	b.innerHTML = y;
	const m = b.querySelector('div[style*="position: relative"]'),
		t = b.querySelector('.ubits-input'),
		h = b.querySelector('.ubits-input-counter');
	if (!t || !m) return console.error('UBITS Input: No se pudo crear el elemento input'), null;
	if (
		(getComputedStyle(b).position === 'static' && (b.style.position = 'relative'),
		o === 'select' && gt(b, t, p, g, u.placeholder || '', s, u.size || 'md'),
		o === 'search' && ft(b, t, s),
		o === 'autocomplete' && mt(b, t, i, s, u.size || 'md'),
		o === 'calendar' && ht(b, t, s),
		o === 'password' && bt(b, t),
		o === 'textarea' && u.showRichTextToolbar
			? wt(b, t, u.onChange)
			: o === 'textarea' && !u.showRichTextToolbar && St(b, t),
		r && h && yt(t, h, e),
		s && typeof s == 'function')
	) {
			...
			}),
		{
			...
			setState: (d) => {
				if (
					...
					].forEach((S) => t.classList.remove(S)),
					d !== 'default' && t.classList.add(`ubits-input--${d}`),
					d === 'disabled' ? (t.disabled = !0) : (t.disabled = !1),
					o === 'textarea' && u.showRichTextToolbar)
				) {
					const k = t
					...
					if (k) {
						...
						window.getComputedStyle(k).borderTop,
							w &&
								w !== 'none' &&
								w !== '0px' &&
								(console.warn(
									`[Rich Text] ⚠️ Línea divisoria detectada en setState("${d}"), removiendo...`,
								),
	...
	if (s) {
		...
		s.addEventListener('click', (e) => {
			e.preventDefault(),
				...
				(c = !c),
				c
					? ((n.type = 'text'), r || (s.className = 'far fa-eye-slash ubits-input-icon-right'))
					: ((n.type = 'password'), r || (s.className = 'far fa-eye ubits-input-icon-right'));
		});
	}
}
function ft(u, n, s) {
...
function mt(u, n, s, c, a = 'md') {
	const r = a === 'xs' ? 'xs' : a === 'sm' ? 'sm' : a === 'md' ? 'md' : 'lg',
		e = u.querySelector('.ubits-input-icon-right');
	if (e) {
	...
	(o.className = 'ubits-autocomplete-list-container'),
		(o.style.cssText = `
    position: absolute;
    top: 100%;
    left: 0;
  ...
  `),
	...
	const p = (i = !1) => {
		...
		if (
			(i || g.length < 1
				? (b = s.slice(0, 8))
				: (b = s.filter((t) => t.text.toLowerCase().includes(g)).slice(0, 8)),
			b.length === 0)
		) {
		...
		try {
			ot({
				...
				g.length > 0 &&
					o.querySelectorAll('.ubits-list-item').forEach((h) => {
						const d = h.textContent || '';
						if (d.toLowerCase().includes(g)) {
							const T = new RegExp(`(${g})`, 'gi'),
								S = d.replace(T, '<strong>$1</strong>');
							h.innerHTML = S;
						}
					});
		} catch (t) {
			...
			(o.innerHTML = h),
				g.length > 0 &&
					o.querySelectorAll('.ubits-list-item').forEach((S) => {
						const k = S.textContent || '';
						if (k.toLowerCase().includes(g)) {
							const w = new RegExp(`(${g})`, 'gi'),
								q = k.replace(w, '<strong>$1</strong>');
							S.innerHTML = q;
						}
					}),
...
function gt(u, n, s, c, a, r, e = 'md') {
	...
	const o = e === 'xs' ? 'xs' : e === 'sm' ? 'sm' : e === 'md' ? 'md' : 'lg',
		p = document.createElement('div');
	(p.className = 'ubits-select-list-container'),
		(p.style.cssText = `
    position: absolute;
    top: 100%;
    left: 0;
  ...
  `),
		...
		y ||
			...
			setTimeout(() => {
				...
				try {
					...
					(p.innerHTML = q),
						p.querySelectorAll('.ubits-list-item').forEach((R, B) => {
							const D = b[B];
							D &&
								D.state !== 'disabled' &&
				...
				if (d < s.length) {
					...
					if (w) {
						const q = new IntersectionObserver(
								...
								{ root: w, rootMargin: '50px' },
							),
							I = p.querySelector('.ubits-list-item:last-child');
						I && q.observe(I);
					}
				}
				y = !1;
	...
	n.addEventListener('click', () => {
		p.style.display === 'block'
			...
			: ((g = 0), (b = []), m(0), (p.style.display = 'block'));
	}),
...
function ht(u, n, s) {
	let c = null,
		a = null;
	const r = (i) => {
			const g = String(i.getDate()).padStart(2, '0'),
				b = String(i.getMonth() + 1).padStart(2, '0'),
				y = i.getFullYear();
			return `${g}/${b}/${y}`;
		},
		e = (i) => {
		...
		o = async () => {
			...
			if (
				(a ||
					...
					(a.className = 'ubits-calendar-picker-container'),
					(a.style.cssText =
			...
			try {
				const i = await import('./CalendarProvider-Cxg4VLQZ.js'),
					{ createCalendar: g } = i,
					b = n.value,
					y = e(b) || /* @__PURE__ */ new Date();
				(c = g({
					mode: 'single',
					selectedDate: e(b),
					initialDate: y,
...
function yt(u, n, s) {
	const c = () => {
		Y(n, u.value.length, s),
			u.value.length > s && ((u.value = u.value.substring(0, s)), Y(n, s, s));
	};
...
function Y(u, n, s) {
	(u.textContent = `${n}/${s}`),
		n >= s
			? u.classList.add('ubits-input-counter--limit')
			: u.classList.remove('ubits-input-counter--limit');
}
function vt(u, n) {
    ...
    `,
		...
		e = r.element;
	e.id = s;
	const o = document.getElementById(`${s}-insert-btn`),
...
function xt(u, n) {
    ...
    `,
		...
		o = e.element;
	o.id = s;
	const p = o.querySelector('.ubits-button--primary'),
	...
	p &&
		...
		p.addEventListener('click', () => {
			const y = parseInt(i.value) || 2,
				m = parseInt(g.value) || 2;
			if (y > 0 && m > 0) {
				const t = document.createElement('table');
				(t.style.borderCollapse = 'collapse'),
					(t.style.width = '100%'),
					(t.style.margin = 'var(--ubits-spacing-sm, 8px) 0'),
					(t.style.border = '1px solid var(--ubits-border-1)');
				for (let d = 0; d < y; d++) {
					const T = document.createElement('tr');
					for (let S = 0; S < m; S++) {
						const k = document.createElement('td');
						(k.style.border = '1px solid var(--ubits-border-1)'),
							(k.style.padding = 'var(--ubits-spacing-sm, 8px)'),
							(k.style.minWidth = '50px'),
							(k.textContent = ' '),
							T.appendChild(k);
					}
					t.appendChild(T);
...
function $t(u, n) {
    ...
    `,
		...
		e = r.element;
	e.id = s;
	const o = e.querySelector('.ubits-button--primary'),
		p = document.getElementById(c);
	o &&
		p &&
...
function wt(u, n, s) {
	...
	(e.style.cssText = n.style.cssText),
		...
		(e.style.padding = o.padding || '12px 12px'),
		(e.style.margin = '0'),
		(e.style.outline = 'none'),
		(e.style.overflow = 'auto'),
		(e.style.minHeight = o.minHeight || '80px'),
		(e.style.resize = 'vertical'),
		(e.contentEditable = 'true'),
		e.setAttribute('data-placeholder', r);
	let p = u.closest('.ubits-input-wrapper');
	p || (p = u.parentElement?.closest('.ubits-input-wrapper')),
		p || (p = document.getElementById(u.id)?.parentElement?.closest('.ubits-input-wrapper')),
		console.log('[Rich Text Placeholder] ===== DEBUG ALINEAMIENTO ====='),
		console.log('[Rich Text Placeholder] inputWrapper:', p),
		console.log('[Rich Text Placeholder] container:', u),
		console.log('[Rich Text Placeholder] container.parentElement:', u.parentElement),
		console.log('[Rich Text Placeholder] richTextWrapper:', a),
		console.log('[Rich Text Placeholder] richTextWrapper.parentElement:', a?.parentElement);
	let i = null;
	if (
		(p && (i = p.querySelector('.ubits-input-icon-left')),
		!i && u.parentElement && (i = u.parentElement.querySelector('.ubits-input-icon-left')),
		!i && a?.parentElement && (i = a.parentElement.querySelector('.ubits-input-icon-left')),
		!i)
	) {
	...
	if (
		...
		console.log('[Rich Text Placeholder] hasLeftIcon:', g),
		g && i)
	) {
		...
		console.log('[Rich Text Placeholder] Icono encontrado:', i),
			...
			console.log('[Rich Text Placeholder] Icono left (computed):', h),
			console.log('[Rich Text Placeholder] Icono top (computed):', d),
			console.log('[Rich Text Placeholder] Icono transform:', T);
		const S = o.paddingLeft || '12px',
			k = o.paddingTop || '12px',
			w = o.paddingRight || '12px',
			q = o.paddingBottom || '12px';
		...
		console.log('[Rich Text Placeholder] Texto:', {
			...
			e.style.setProperty('--placeholder-left', S),
			e.style.setProperty('--placeholder-top', k),
		...
		g &&
			...
			requestAnimationFrame(() => {
				requestAnimationFrame(() => {
					let m = i;
					if (
						(p && (m = p.querySelector('.ubits-input-icon-left') || i),
						!m &&
							u.parentElement &&
							(m = u.parentElement.querySelector('.ubits-input-icon-left') || i),
						m)
					) {
						...
						if (
							...
							console.log('[Rich Text Placeholder] Icono rect:', t),
							console.log('[Rich Text Placeholder] EditableDiv rect:', h),
							...
							const k = d + t.height / 2,
								w = parseFloat(o.fontSize || '16px'),
								q = o.lineHeight;
							let I;
							q === 'normal'
								? (I = w * 1.2)
								: q.includes('px')
									? (I = parseFloat(q))
									: (I = w * parseFloat(q));
							const R = parseFloat(o.paddingTop || '12px'),
								B = R + w * 0.75,
								D = k - B,
								V = R + D;
							...
							const v = Math.max(0, V),
								E = (e.style.padding || o.padding || '12px 12px').split(' '),
								P = E[1] || E[0] || '12px',
								$ = E[2] || E[0] || '12px',
								z = E[3] || E[1] || E[0] || '40px';
							(e.style.padding = `${v}px ${P} ${$} ${z}`),
								e.style.setProperty('--placeholder-top', `${v}px`),
								e.style.setProperty('--placeholder-left', z),
	...
	e.addEventListener('input', b),
		...
		c.querySelectorAll('.ubits-rich-text-btn').forEach((m) => {
			m.addEventListener('click', (t) => {
				...
				if (h) {
					if (h === 'insertImage') vt(e, b);
					else if (h === 'insertTable') xt(e, b);
					else if (h === 'createLink') $t(e, b);
					else if (h === 'code') {
						const d = window.getSelection();
						if (d && d.rangeCount > 0) {
							const T = d.getRangeAt(0),
								S = document.createElement('code');
							(S.style.background = 'var(--ubits-bg-2)'),
								(S.style.padding = 'var(--ubits-spacing-xs, 2px) var(--ubits-spacing-sm, 4px)'),
								(S.style.borderRadius = 'var(--ubits-border-radius-sm, 4px)'),
								(S.style.fontFamily = 'var(--font-mono, monospace)');
							try {
					...
					} else document.execCommand(h, !1, void 0);
					b();
				}
			});
		});
...
function St(u, n) {
	...
	s || (s = u.parentElement?.closest('.ubits-input-wrapper')),
		s || (s = document.getElementById(u.id)?.parentElement?.closest('.ubits-input-wrapper'));
	let c = null;
	if (
		(s && (c = s.querySelector('.ubits-input-icon-left')),
		!c && u.parentElement && (c = u.parentElement.querySelector('.ubits-input-icon-left')),
		!c)
	) {
			...
			requestAnimationFrame(() => {
				...
				if (r && n) {
					...
					if (o.width > 0 && o.height > 0) {
						...
						const i = p + e.height / 2,
							g = window.getComputedStyle(n),
							b = parseFloat(g.fontSize || '16px'),
							y = parseFloat(g.paddingTop || '12px'),
							m = y + b * 0.75,
							t = i - m,
							h = y + t,
							d = Math.max(0, h),
							S = (g.padding || '12px 12px').split(' '),
							k = S[1] || S[0] || '12px',
							w = S[2] || S[0] || '12px',
							q = S[3] || S[1] || S[0] || '40px';
...
function It(u) {
	...
	R && V.push('ubits-slider--vertical'),
		a && V.push(`ubits-slider--${a}`),
		I && V.push('ubits-slider--disabled'),
		k && V.push(k);
	let v = `<div class="${V.join(' ')}" id="${n}">`;
	if (
		(t && s && (v += `<label class="ubits-slider-label">${s}</label>`),
		(v += '<div class="ubits-slider-main-wrapper">'),
		q && B && (v += `<div class="ubits-slider-input" id="${n}-input-min"></div>`),
		(v += '<div class="ubits-slider-wrapper">'),
		(v += '<div class="ubits-slider-track-container" style="position: relative; flex: 1;">'),
		(v += '<div class="ubits-slider-track">'),
		B)
	) {
		const E = ((y[0] - p) / (i - p)) * 100,
			$ = ((y[1] - p) / (i - p)) * 100 - E;
		R
			? (v += `<div class="ubits-slider-track-range" style="bottom: ${E}%; height: ${$}%;"></div>`)
			: (v += `<div class="ubits-slider-track-range" style="left: ${E}%; width: ${$}%;"></div>`);
	} else {
		const E = ((D - p) / (i - p)) * 100;
		R
			? (v += `<div class="ubits-slider-track-fill" style="height: ${E}%; bottom: 0;"></div>`)
			: (v += `<div class="ubits-slider-track-fill" style="width: ${E}%;"></div>`);
	}
	if (
		(d &&
			T.length > 0 &&
			((v += '<div class="ubits-slider-marks">'),
			T.forEach((E) => {
				const P = ((E - p) / (i - p)) * 100;
				R
					? (v += `<div class="ubits-slider-mark" style="top: ${100 - P}%; left: 50%;"></div>`)
					: (v += `<div class="ubits-slider-mark" style="left: ${P}%; top: 50%;"></div>`);
			}),
			(v += '</div>')),
		B)
	) {
		const E = ((y[0] - p) / (i - p)) * 100,
			P = ((y[1] - p) / (i - p)) * 100;
		R
			? ((v += `<div class="ubits-slider-thumb ubits-slider-thumb--min" style="top: ${100 - E}%; left: 50%;" data-value="${y[0]}" tabindex="0" ${I ? 'disabled' : ''}></div>`),
				(v += `<div class="ubits-slider-thumb ubits-slider-thumb--max" style="top: ${100 - P}%; left: 50%;" data-value="${y[1]}" tabindex="0" ${I ? 'disabled' : ''}></div>`))
			: ((v += `<div class="ubits-slider-thumb ubits-slider-thumb--min" style="left: ${E}%; top: 50%;" data-value="${y[0]}" tabindex="0" ${I ? 'disabled' : ''}></div>`),
				(v += `<div class="ubits-slider-thumb ubits-slider-thumb--max" style="left: ${P}%; top: 50%;" data-value="${y[1]}" tabindex="0" ${I ? 'disabled' : ''}></div>`));
	} else {
		const E = ((D - p) / (i - p)) * 100;
		R
			? (v += `<div class="ubits-slider-thumb" style="top: ${100 - E}%; left: 50%;" data-value="${D}" tabindex="0" ${I ? 'disabled' : ''}></div>`)
			: (v += `<div class="ubits-slider-thumb" style="left: ${E}%; top: 50%;" data-value="${D}" tabindex="0" ${I ? 'disabled' : ''}></div>`);
	}
	if (
		((v += '</div>'),
		(v += '</div>'),
		...
		q &&
			(B
				? (v += `<div class="ubits-slider-input" id="${n}-input-max"></div>`)
				: (v += `<div class="ubits-slider-input" id="${n}-input-value"></div>`)),
		(v += '</div>'),
		!R)
	) {
		if (
			((v += '<div class="ubits-slider-range-guide-wrapper">'),
			(v += `<div class="ubits-slider-range-guide" id="${n}-range-guide">`),
			S)
		) {
			const E = i - p;
			let $ = Math.ceil(E / 10);
			const z = Math.pow(10, Math.floor(Math.log10($))),
				M = $ / z;
			let H = z;
			...
			for (; _ <= i; ) {
				const G = ((_ - p) / (i - p)) * 100;
				(v += `<span class="ubits-slider-range-guide-value" style="left: ${G}%">${Math.round(_)}</span>`),
					(_ += H);
			}
		} else {
	...
	h &&
		c &&
		((v += '<div class="ubits-input-helper">'), (v += `<span>${c}</span>`), (v += '</div>')),
		(v += '</div>');
...
function nt(u) {
	...
	if (!d) return console.error(`UBITS Slider: No se encontró el contenedor con ID "${n}"`), null;
	const T = It(u);
	d.innerHTML = T;
	const S = d.querySelector(`#${n}`) || d.querySelector('.ubits-slider');
	if (!S) return console.error('UBITS Slider: No se encontró el elemento slider'), null;
	const k = d.querySelector('.ubits-slider-track'),
		w = d.querySelectorAll('.ubits-slider-thumb'),
		q = d.querySelector(`#${n}-value-display`),
		I = o === 'range',
		R = g === 'vertical',
		B = y === 'disabled';
	let D = null,
				...
				if (I) {
					...
					if ((l || (l = d.querySelector(`#${f}`)), l || (l = document.getElementById(f)), l)) {
						...
						try {
							...
							if (
								(C &&
									(C.setAttribute('data-slider-input', 'min'),
									C.setAttribute('min', a.toString()),
									C.setAttribute('max', r.toString()),
									C.setAttribute('step', e.toString())),
								D)
							) {
					...
					} else console.error('UBITS Slider: No se encontró el contenedor del input min:', f);
					const x = `${n}-input-max`;
					...
					if ((L || (L = d.querySelector(`#${x}`)), L || (L = document.getElementById(x)), L)) {
						...
						try {
							...
							if (
								(C &&
									(C.setAttribute('data-slider-input', 'max'),
									C.setAttribute('min', a.toString()),
									C.setAttribute('max', r.toString()),
									C.setAttribute('step', e.toString())),
								V)
					...
					} else console.error('UBITS Slider: No se encontró el contenedor del input max:', x);
				} else {
					...
					if ((l || (l = d.querySelector(`#${f}`)), l || (l = document.getElementById(f)), l)) {
						...
						try {
							...
							if (
								(x &&
									(x.setAttribute('data-slider-input', 'value'),
									x.setAttribute('min', a.toString()),
									x.setAttribute('max', r.toString()),
									x.setAttribute('step', e.toString())),
								v)
							) {
					...
					} else console.error('UBITS Slider: No se encontró el contenedor del input value:', f);
				}
				(P = E()), ct();
			}
		},
	...
	const H = (f, l) => {
			...
			let L;
			R ? (L = 1 - (l - x.top) / x.height) : (L = (f - x.left) / x.width),
		...
		_ = (f, l) => {
			const x = ((l - a) / (r - a)) * 100;
			R
				? ((f.style.top = `${100 - x}%`), (f.style.left = '50%'))
				: ((f.style.left = `${x}%`), (f.style.top = '50%')),
				f.setAttribute('data-value', l.toString());
		},
		G = () => {
			const f = d.querySelector('.ubits-slider-track-fill'),
				l = d.querySelector('.ubits-slider-track-range');
			if (I) {
				if (l) {
					const x = $[0],
						L = $[1],
					...
					if (R) {
						...
						if (W.length >= 2 && N.height > 0) {
							const F = Array.from(W).find((O) => O.classList.contains('ubits-slider-thumb--min')),
								tt = Array.from(W).find((O) => O.classList.contains('ubits-slider-thumb--max'));
							if (F && tt) {
								const O = F.getBoundingClientRect();
								tt.getBoundingClientRect();
								const et = (O.height / N.height) * 100,
									it = Math.max(0, C - et / 2),
									dt = Math.max(0, A - et / 2) - it;
								(l.style.bottom = `${it}%`), (l.style.height = `${dt}%`);
							} else (l.style.bottom = `${C}%`), (l.style.height = `${A - C}%`);
						} else (l.style.bottom = `${C}%`), (l.style.height = `${A - C}%`);
					} else {
						const N = A - C;
				...
				if (R) {
					const L = k.getBoundingClientRect(),
						C = d.querySelector('.ubits-slider-thumb');
					if (C && L.height > 0) {
						const N = (C.getBoundingClientRect().height / L.height) * 100,
							W = Math.max(0, x - N / 2);
						(f.style.height = `${W}%`), (f.style.bottom = '0');
					} else (f.style.height = `${x}%`), (f.style.bottom = '0');
				} else f.style.width = `${x}%`;
			}
		},
		lt = () => {
			if (
				...
				P.forEach((l) => {
					...
					x === 'value' && !I
						...
						: x === 'min' && I
							? (l.value = $[0].toString())
							: x === 'max' && I && (l.value = $[1].toString());
				}),
				v && !I && v.setValue($.toString()),
				D && I && D.setValue($[0].toString()),
				V && I && V.setValue($[1].toString()),
				q)
			)
				if (I) {
					const [l, x] = $;
					q.textContent = `${l} - ${x}`;
				} else q.textContent = $.toString();
			const f = d.querySelector(`#${n}-range-guide-current`);
			if (f && !t)
				if (I) {
					const l = $[1];
					f.textContent = l.toString();
				} else f.textContent = $.toString();
		},
		U = () => {
			if (I) {
				const [f, l] = $,
					x = d.querySelector('.ubits-slider-thumb--min'),
					L = d.querySelector('.ubits-slider-thumb--max');
				x && _(x, f), L && _(L, l);
			} else {
				const f = w[0];
				f && _(f, $);
		...
		K = (f) => {
			...
			if (I) {
				const [x, L] = $;
				if (M.classList.contains('ubits-slider-thumb--min')) $ = [Math.min(l, L - e), L];
				else {
	...
	w.length === 0
		...
		: w.forEach((f) => {
				f.addEventListener('mousedown', (l) => at(l, f)),
					f.addEventListener(
						'touchstart',
						(l) => {
			...
			P.forEach((f) => {
				...
				f.parentNode?.replaceChild(l, f),
					l.addEventListener('input', (x) => {
						...
						if (isNaN(L)) return;
						const C = Math.max(a, Math.min(r, L)),
							A = l.getAttribute('data-slider-input');
						if (I) {
							const [N, W] = $;
							if (A === 'min') ($ = [Math.min(C, W - e), W]), c && c($, x);
							else if (A === 'max') {
	...
	return (
		w.forEach((f) => {
			f.addEventListener('keydown', (l) => {
				...
				if (I) {
					const [L, C] = $,
						A = f.classList.contains('ubits-slider-thumb--min'),
						N = A ? L : C;
					switch (l.key) {
						case 'ArrowRight':
						case 'ArrowUp':
		...
		U(),
		{
			...
			disable: () => {
				S.classList.add('ubits-slider--disabled'),
					w.forEach((f) => {
			...
			enable: () => {
				S.classList.remove('ubits-slider--disabled'),
					w.forEach((f) => {
...
export { Tt as SliderAddon, nt as createSlider, It as renderSlider };
