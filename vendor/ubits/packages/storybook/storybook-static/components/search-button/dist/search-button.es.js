...
function _(t) {
	...
	if (!a) throw new Error('Failed to create search button element');
	if ((u.appendChild(a), t.active || t.state === 'active')) {
		const n = a.querySelector('.ubits-search-button__input'),
			e = a.querySelector('.ubits-search-button__clear');
		n &&
			(t.onChange &&
			...
			e &&
				e.addEventListener('click', function (s) {
					if (
						(s.preventDefault(), s.stopPropagation(), n && ((n.value = ''), n.focus(), t.onChange))
					) {
		...
		update: (n) => {
			...
			if (o && a.parentNode)
				if ((a.parentNode.replaceChild(o, a), e.active || e.state === 'active')) {
					const i = o.querySelector('.ubits-search-button__input'),
						b = o.querySelector('.ubits-search-button__clear');
					i &&
						(e.onChange &&
							(i.addEventListener('input', e.onChange), i.addEventListener('change', e.onChange)),
						e.onFocus && i.addEventListener('focus', e.onFocus),
						e.onBlur && i.addEventListener('blur', e.onBlur)),
						b &&
							b.addEventListener('click', function (p) {
								if (
									(p.preventDefault(),
									p.stopPropagation(),
									i && ((i.value = ''), i.focus(), e.onChange))
								) {
...
export { _ as createSearchButton, m as renderSearchButton };
