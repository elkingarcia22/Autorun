(function (r, l) {
	typeof exports == 'object' && typeof module < 'u'
		...
		: typeof define == 'function' && define.amd
			? define(['exports'], l)
			: ((r = typeof globalThis < 'u' ? globalThis : r || self), l((r.UbitsSearchButton = {})));
})(this, function (r) {
	'use strict';
	function l() {
		return `
  ...
  `.trim();
	...
	function g(t) {
		...
		if ((d.appendChild(i), t.active || t.state === 'active')) {
			const n = i.querySelector('.ubits-search-button__input'),
				e = i.querySelector('.ubits-search-button__clear');
			n &&
				(t.onChange &&
		...
		return {
			...
			update: (n) => {
				...
				if (u && i.parentNode)
					if ((i.parentNode.replaceChild(u, i), e.active || e.state === 'active')) {
						const a = u.querySelector('.ubits-search-button__input'),
							v = u.querySelector('.ubits-search-button__clear');
						a &&
							(e.onChange &&
								(a.addEventListener('input', e.onChange), a.addEventListener('change', e.onChange)),
							e.onFocus && a.addEventListener('focus', e.onFocus),
							e.onBlur && a.addEventListener('blur', e.onBlur)),
							v &&
								v.addEventListener('click', function (C) {
									if (
										(C.preventDefault(),
										C.stopPropagation(),
										a && ((a.value = ''), a.focus(), e.onChange))
									) {
						...
						const a = u;
						a && e.onClick && a.addEventListener('click', e.onClick);
					}
			},
		};
	}
	(r.createSearchButton = g),
		(r.renderSearchButton = h),
