	...
	return {
		...
		buildStart() {
			...
			console.error = (...args: any[]) => {
				...
				if (
					message.includes('tsconfig.node.json') &&
					(message.includes('ENOENT') || message.includes('failed'))
				) {
			...
			if (this.error) {
				...
				this.error = (err: Error | string) => {
					...
					if (
						errorMessage.includes('tsconfig.node.json') &&
						(errorMessage.includes('ENOENT') || errorMessage.includes('failed'))
					) {
						// Suprimir el error silenciosamente
						return null;
					}
		...
		configureServer(server) {
			...
			if (server.config.logger) {
				...
				server.config.logger.error = (msg: string, options?: any) => {
					if (
						msg.includes('tsconfig.node.json') &&
						(msg.includes('ENOENT') || msg.includes('failed'))
					) {
						// Suprimir el error silenciosamente
						return;
					}
			...
			server.ws.on('error', (err: Error) => {
				if (
					err.message.includes('tsconfig.node.json') &&
					(err.message.includes('ENOENT') || err.message.includes('failed'))
				) {
					// Suprimir el error silenciosamente
					return;
				}
