...
} from '../../addons/tabbar/src/configs/defaultFloatingMenu';
import type {
	TabBarOptions,
	TabBarItem,
	FloatingMenuSection,
	ProfileMenuItem,
} from '../../addons/tabbar/src/types/TabBarOptions';
...
function getTabBarConfig(variant: 'colaborador' | 'admin') {
	return {
		floatingMenuSections:
			variant === 'admin' ? adminFloatingMenuSections : colaboradorFloatingMenuSections,
			...
			requestAnimationFrame(() => {
				setTimeout(() => {
					try {
						...
						const originalBodyFocus = Object.getOwnPropertyDescriptor(
