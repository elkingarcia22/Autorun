...
import type {
	...
	TableColumn,
	TableRow,
} from '../../addons/data-table/src/types/DataTableOptions';
...
export const Default: Story = {
    ...
    `;
		...
		existingContainers.forEach((oldContainer) => {
			...
			const oldScrollableContainer = oldContainer.querySelector(
		...
		const column3CheckboxLabel =
			args.column3CheckboxLabel !== undefined ? args.column3CheckboxLabel : true;
		...
		const buildColumn = (
			...
			if (columnType === 'correo') {
				column.emailClickable =
					options.emailClickable !== undefined ? options.emailClickable : true;
			}
		...
		const showHeaderTitle =
			(args as any).showHeaderTitle !== undefined ? (args as any).showHeaderTitle : true;
		const headerCounter =
			(args as any).headerCounter !== undefined ? (args as any).headerCounter : true;
		const headerDisplayedItems = (args as any).headerDisplayedItems ?? 32;
		const headerTotalItems = (args as any).headerTotalItems ?? 206;
		const showHeaderPrimaryButton =
			(args as any).showHeaderPrimaryButton !== undefined
				? (args as any).showHeaderPrimaryButton
				: true;
		const headerPrimaryButtonText = (args as any).headerPrimaryButtonText ?? 'Nuevo';
		const showHeaderSecondaryButtons =
			(args as any).showHeaderSecondaryButtons !== undefined
				? (args as any).showHeaderSecondaryButtons
				: true;
		const showHeaderSearchButton =
			(args as any).showHeaderSearchButton !== undefined
				? (args as any).showHeaderSearchButton
				: true;
		const showHeaderFilterButton =
			(args as any).showHeaderFilterButton !== undefined
				? (args as any).showHeaderFilterButton
				: true;
		const showHeaderColumnSelectorButton =
			(args as any).showHeaderColumnSelectorButton !== undefined
				? (args as any).showHeaderColumnSelectorButton
				: true;

		// Controles de la barra de acciones
		const showActionButtonViewSelected =
			(args as any).showActionButtonViewSelected !== undefined
				? (args as any).showActionButtonViewSelected
				: true;
		const showActionButtonNotifications =
			(args as any).showActionButtonNotifications !== undefined
				? (args as any).showActionButtonNotifications
				: true;
		const showActionButtonCopy =
			(args as any).showActionButtonCopy !== undefined ? (args as any).showActionButtonCopy : true;
		const showActionButtonView =
			(args as any).showActionButtonView !== undefined ? (args as any).showActionButtonView : true;
		const showActionButtonEdit =
			(args as any).showActionButtonEdit !== undefined ? (args as any).showActionButtonEdit : true;
		const showActionButtonDownload =
			(args as any).showActionButtonDownload !== undefined
				? (args as any).showActionButtonDownload
				: true;
		const showActionButtonDelete =
			(args as any).showActionButtonDelete !== undefined
				? (args as any).showActionButtonDelete
				: true;
		...
		const options: DataTableOptions = {
			...
			emptyState: {
				noData: {
					...
					description:
						(args as any).emptyStateNoDataDescription ||
				...
				noSearchResults: {
					...
					description:
						(args as any).emptyStateNoSearchResultsDescription ||
				...
				noFilterResults: {
					title:
						(args as any).emptyStateNoFilterResultsTitle ||
						'No hay resultados con los filtros aplicados',
					description:
						(args as any).emptyStateNoFilterResultsDescription ||
					...
					showPrimaryButton:
						(args as any).emptyStateNoFilterResultsShowPrimaryButton !== undefined
							? (args as any).emptyStateNoFilterResultsShowPrimaryButton
							: true,
					onAction: () => {
		...
		const checkAndCreateTable = () => {
			...
			const existingScrollable = containerElement.querySelector(
				'.ubits-data-table__scrollable-container',
			);
