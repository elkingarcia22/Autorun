...
export const TodosLosComponentes: Story = {
	render: () => {
		...
		const aiButtonCount =
			COMPONENT_TOKENS['ai-button'].light.length + COMPONENT_TOKENS['ai-button'].dark.length;
		const totalCount =
			buttonToneCount +
			aiButtonCount +
			COMPONENT_TOKENS['scroll-bar'].length +
			COMPONENT_TOKENS['toggle'].length;
		...
		const toneCounts = tones
			.map((tone) => {
				const toneData =
					COMPONENT_TOKENS['button-tone'][tone as keyof (typeof COMPONENT_TOKENS)['button-tone']];
				const primaryCount =
					(toneData.primary?.light.length || 0) + (toneData.primary?.dark.length || 0);
				const secondaryCount =
					(toneData.secondary?.light.length || 0) + (toneData.secondary?.dark.length || 0);
				const tertiaryCount =
					(toneData.tertiary?.light.length || 0) + (toneData.tertiary?.dark.length || 0);
    ...
    `;
		...
		Object.entries(COMPONENT_TOKENS['button-tone']).forEach(([tone, variants]) => {
			...
			const toneCount = Object.values(variants).reduce(
				(sum, variant) => sum + variant.light.length + variant.dark.length,
				0,
			);
			...
			Object.entries(variants).forEach(([variant, modes]) => {
				...
				modes.light.forEach((token) => {
					lightCol.appendChild(
						createColorSwatch(token, 'light', {
							showVariable: true,
							showValue: true,
							width: '100%',
						}),
		...
		COMPONENT_TOKENS['ai-button'].light.forEach((token) => {
			aiButtonLightCol.appendChild(
				createColorSwatch(token, 'light', { showVariable: true, showValue: true, width: '100%' }),
			);
		});
		...
		COMPONENT_TOKENS['ai-button'].dark.forEach((token) => {
			aiButtonDarkCol.appendChild(
				createColorSwatch(token, 'dark', { showVariable: true, showValue: true, width: '100%' }),
			);
		});
		...
		const scrollBarComparison = createLightDarkComparison(
			COMPONENT_TOKENS['scroll-bar'],
			'Scroll Bar',
		);
