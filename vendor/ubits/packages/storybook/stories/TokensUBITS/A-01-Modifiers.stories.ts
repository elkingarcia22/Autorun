...
import {
	extractColorTokens,
	organizeTokensByCategory,
	organizeTokensByCategoryAndState,
...
function getModifierTokens(modifier: 'normal' | 'inverted' | 'static' | 'static-inverted'): {
	...
	const lightTokens = allTokens.filter((t) => t.includes(`modifiers-${modifier}-color-light`));

	const darkTokens = allTokens.filter((t) => t.includes(`modifiers-${modifier}-color-dark`));
		...
		modifiers.forEach((modifier) => {
			...
			if (totalCount > 0) {
				...
				allCategories.forEach((categoryName) => {
					...
					const lightTotal = Object.values(lightCategory).reduce(
						(sum, tokens) => sum + tokens.length,
						0,
					);
					const darkTotal = Object.values(darkCategory).reduce(
						(sum, tokens) => sum + tokens.length,
						0,
					);
		...
		if (totalCount > 0) {
			...
			allCategories.forEach((categoryName) => {
				...
				sortedStates.forEach((stateName) => {
					...
					const stateLabel =
						stateName === 'base' ? 'Base' : stateName.charAt(0).toUpperCase() + stateName.slice(1);
					stateTitle.textContent = `${stateLabel} (${lightStateTokens.length} Light + ${darkStateTokens.length} Dark)`;
