...
export const TodosLosSemanticos: Story = {
	render: () => {
		...
		const feedbackCount =
			SEMANTIC_TOKENS.feedback.accent.light.length +
    ...
    `;
		...
		SEMANTIC_TOKENS.feedback.chart.light.forEach((token) => {
			chartFeedbackLightCol.appendChild(
				createColorSwatch(token, 'light', { showVariable: true, showValue: true, width: '100%' }),
			);
		});
		...
		SEMANTIC_TOKENS.feedback.chart.dark.forEach((token) => {
			chartFeedbackDarkCol.appendChild(
				createColorSwatch(token, 'dark', { showVariable: true, showValue: true, width: '100%' }),
			);
		});
