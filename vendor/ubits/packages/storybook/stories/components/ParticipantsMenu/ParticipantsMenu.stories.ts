...
// - participantsMenu.destroy(): Método para destruir el menú`,
    ...
    `;
		...
		menuContainer.id = args.containerId || 'participants-menu-implementation-container';
    ...
    `;
		...
		// Crear el menú
		try {
			const participantsMenu = createParticipantsMenu({
				...
				showRole: args.showRole ?? true,
				showStatusTag: args.showStatusTag ?? true,
				enableScrollbar: args.enableScrollbar ?? true,
				onParticipantSelect: args.onParticipantSelect,
				onSearchChange: args.onSearchChange,
				onFilterClick: args.onFilterClick,
				onFilterChange: args.onFilterChange,
			});
		} catch (error) {
			console.error('Error creando ParticipantsMenu:', error);
