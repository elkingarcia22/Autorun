...
export const Default: Story = {
	...
	render: (args) => {
		...
		const updateButtonText = () => {
			const isOpen =
				tooltipInstance && tooltipInstance.element.classList.contains('ubits-tooltip--open');
		...
		const createAndOpenTooltip = () => {
			...
			tooltipInstance = createTooltip({
				...
				closeOnOutsideClick:
					args.closeOnOutsideClick !== undefined ? args.closeOnOutsideClick : true,
				position: position,
			});
		...
		const recreateTooltip = () => {
			const wasOpen =
				tooltipInstance &&
				tooltipInstance.element &&
				tooltipInstance.element.classList.contains('ubits-tooltip--open');
			destroyTooltip();
			setTimeout(() => {
				createAndOpenTooltip();
