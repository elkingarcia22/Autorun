...
export class InitializationWizard {
	...
	async start(options?: { autoSelect?: ProjectType }): Promise<WizardResult> {
		...
		if (autoAnswers && autoAnswers.template && autoAnswers.module && !hasAutoAnswers) {
			...
			openBrowser =
				openBrowserEnv === undefined ? true : openBrowserEnv === 'true' || openBrowserEnv === '1';
		} else {
	...
	private async askOpenBrowser(): Promise<boolean> {
		...
		console.log('   Por defecto, el wizard abre el template en el navegador automáticamente.');
		console.log('   Si desactivas esta opción, puedes abrirlo manualmente después.\n');
	...
	private async openTemplateInBrowser(filePath: string): Promise<string | null> {
		...
		try {
			...
			try {
				...
				console.warn(`   ⚠️  No se pudo abrir navegador externo: ${error.message}`);
			...
			console.log(`      2. Abrirá el template en el browser interno de Cursor`);
			console.log(`   ✅ Template abierto en navegador externo (Chrome)`);
			console.log(`   📋 El agente abrirá automáticamente en browser interno de Cursor`);
