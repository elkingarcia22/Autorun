...
export class InitializationWizard {
  ...
  async start(options?: { autoSelect?: ProjectType }): Promise<WizardResult> {
    ...
    if (
      ...
      // ⚠️ SIEMPRE abrir el browser automáticamente (sin preguntar)
      openBrowser = true;
  ...
  /**
   * ⚠️ DEPRECATED: Ya no se pregunta, siempre se abre automáticamente
   * El browser se abre automáticamente sin preguntar al usuario
   */
  private async askOpenBrowser(): Promise<boolean> {
    // Siempre retornar true - el browser se abre automáticamente
    return true;
  }
  ...
  private async askAddons(): Promise<string[]> {
    ...
    // DEBUG: Log para entender qué está pasando (solo en modo DEBUG)
    ...
    for (const addonId of defaultAddons) {
      ...
      if (addon) {
        console.log(
          `      ✅ ${addon.name}${addon.description ? ' - ' + addon.description : ''}`
        );
      }
    }
  ...
  private async setupUBITSFromAnswers(answers: {
    ...
    // ⚠️ FIX: Agregar 'autorun' a la lista para instalar su MCP automáticamente
  ...
  private async openTemplateInBrowser(
    ...
    try {
      ...
      // Paso 1: NO abrir en navegador externo - solo browser interno de Antigravity
      console.log(`\n   🔌 URL del template: ${httpUrl}`);
      console.log(`   🌐 Abriendo en navegador de Antigravity...`);

      // El browser de Antigravity se abrirá automáticamente detectando los mensajes de consola
      console.log(`   ✅ Navegador de Antigravity abrirá automáticamente`);
  ...
  private async loadComponentsFromStorybook(): Promise<void> {
    ...
    try {
      const { runQuickTest } =
        await import('../helpers/storybookImplementationTester');
      console.log(
...
`;
  ...
  private async askAndConfigureGitHub(): Promise<string | null> {
    ...
    // ⚠️ FIX: Agregar timeout para evitar bloqueos
    let wantsToConfigure = false;
    try {
      // Verificar si readline está disponible
      const rl = (this.prompt as any)?.rl;
      const rlAvailable =
        rl && !rl.closed && process.stdin.isTTY && process.stdout.isTTY;

      if (rlAvailable) {
        // Usar confirm() con timeout
        ...
        const timeoutPromise = new Promise<boolean>((resolve) => {
          setTimeout(() => {
            console.log(
              '   ⚠️  Timeout esperando respuesta, omitiendo configuración de GitHub...'
            );
            resolve(false);
          }, 10000);
        });
        ...
        // Si readline no está disponible, omitir automáticamente
        console.log(
          '   ℹ️  Omitiendo configuración de GitHub (modo automático)\n'
        );
        return null;
      }
    } catch (error: any) {
      // Si hay error, omitir automáticamente
      if (
        error.code === 'ERR_USE_AFTER_CLOSE' ||
        error.message?.includes('readline')
      ) {
        console.log(
          '   ℹ️  Configuración de GitHub omitida (readline cerrado)\n'
        );
      } else {
        console.log(
          `   ⚠️  Error preguntando sobre GitHub: ${error.message || error}\n`
        );
      }
      return null;
    }
    ...
    // Si quiere configurar, preguntar por la URL (con timeout también)
  ...
  private async configureGitHub(githubUrlEnv?: string): Promise<string | null> {
    try {
      ...
      // Modo interactivo: preguntar al usuario (con timeout)
      ...
      try {
        ...
        if (
          ...
          console.log(
            `   ⚠️  Error preguntando URL de GitHub: ${error.message || error}`
          );
        }
        return null;
      }
  ...
  private async configureMCPForAddons(selectedAddons: string[]): Promise<void> {
    ...
    > = {
      ...
      'google-sheets': {
        name: 'Google Sheets',
        // ⚠️ FIX: Solo instalar 'google-sheets', 'mcp-gsheets' es un alias que causa duplicados
        // Ambos apuntan al mismo paquete 'mcp-gsheets@latest' en getServerConfig
        ...
        getCredentials: async () => {
          ...
          // ⚠️ FIX: Retornar null si no hay credenciales (en lugar de strings vacíos)
          // Esto evita que se agreguen variables de entorno vacías al MCP
          const hasCredentials =
            (googleProjectId && googleProjectId.trim() !== '') ||
            (googleApplicationCredentials &&
              googleApplicationCredentials.trim() !== '') ||
            (googleServiceAccountKey &&
              googleServiceAccountKey.trim() !== '') ||
            (googlePrivateKey &&
              googlePrivateKey.trim() !== '' &&
              googleClientEmail &&
              googleClientEmail.trim() !== '');

          if (!hasCredentials) {
            // No hay credenciales, retornar objeto vacío (no null para evitar errores)
            // El MCPInstaller manejará esto correctamente
            return {};
          }

          // Retornar solo las credenciales que tienen valores válidos
          const result: Record<string, string> = {};
          if (googleProjectId && googleProjectId.trim() !== '') {
            result.googleProjectId = googleProjectId.trim();
          }
          if (
            googleApplicationCredentials &&
            googleApplicationCredentials.trim() !== ''
          ) {
            result.googleApplicationCredentials =
              googleApplicationCredentials.trim();
          }
          if (
            googleServiceAccountKey &&
            ...
            result.googleServiceAccountKey = googleServiceAccountKey.trim();
          }
          if (
            googlePrivateKey &&
            googlePrivateKey.trim() !== '' &&
            ...
            result.googlePrivateKey = googlePrivateKey.trim();
            result.googleClientEmail = googleClientEmail.trim();
          }

          return result;
        },
      },
      // ⚠️ FIX: Agregar MCP de Autorun a la lista
      autorun: {
        name: 'Autorun',
        mcpNames: ['autorun'],
        getCredentials: async () => {
          // Autorun MCP no requiere credenciales, usa configuración del proyecto
          return {};
        },
      },
    };
    ...
    if (addonsWithMCP.length === 0) {
      ...
      console.log(
        `   💡 Para configurar MCP, instala uno de estos add-ons: github, vercel, clarity, figma-sync, storybook, supabase, n8n, google-sheets, autorun`
      );
      return;
    }
    ...
    try {
      // ⚠️ FIX: Verificar si readline está disponible antes de preguntar
      // Algunos add-ons (n8n, google-sheets) ya preguntaron individualmente sobre MCP
      // Si readline está cerrado, instalar automáticamente sin preguntar
      let configureMCP = false;

      try {
        // ⚠️ MCP DEPRECATED: Saltar instalación automáticamente
        // El MCP Server de Autorun está deprecated, ver MIGRATION.md
        console.log(
          '\n   ⚠️  MCP Server está DEPRECATED - usando Workflows/Skills nativos de Antigravity'
        );
        console.log(
          '   💡 Ver MIGRATION.md para más información sobre la nueva estructura'
        );
        console.log(
          '   ℹ️  Los add-ons funcionales externos (GitHub, Vercel, Clarity) siguen disponibles\n'
        );

        // Saltar completamente la configuración de MCP
        configureMCP = false;

        // Simular el flujo de error para salir limpiamente
        throw new Error('MCP_DEPRECATED_SKIP');
      } catch (error: any) {
        // Si es nuestro error de skip, salir limpiamente
        if (error.message === 'MCP_DEPRECATED_SKIP') {
          console.log(
            '   ✅ Continuando sin configuración de MCP (deprecated)'
          );
          return;
        }
        // Si hay error al preguntar (readline cerrado), instalar automáticamente
        if (
          error.code === 'ERR_USE_AFTER_CLOSE' ||
          error.message?.includes('readline')
        ) {
          console.log(
            '   ℹ️  Instalando MCPs automáticamente (algunos add-ons ya los configuraron individualmente)...'
          );
          configureMCP = true;
        } else {
          // Otro error, instalar automáticamente para no bloquear el wizard
          console.warn(
            '   ⚠️  Error preguntando sobre MCP:',
            error.message || error
          );
          console.log(
            '   ℹ️  Instalando MCPs automáticamente para evitar bloqueos...'
          );
          configureMCP = true;
        }
      }
      ...
      console.log(
        `   🔍 Procesando ${addonsWithMCP.length} add-on(s) con soporte MCP: ${addonsWithMCP.join(', ')}\n`
      );
      ...
      for (const addonId of addonsWithMCP) {
        ...
        if (!addonInfo) {
          console.warn(
            `   ⚠️  Add-on '${addonId}' no tiene configuración MCP definida`
          );
          continue;
        }
          ...
          try {
            ...
            if (!mcpInfo.available) {
              if (addonInfo.mcpNames.length > 1) {
                console.log(
                  `      ⚠️  MCP '${mcpName}' no está disponible, intentando siguiente...`
                );
                continue;
              }
              console.log(
                `      ⚠️  MCP no está disponible para ${addonInfo.name}`
              );
              skippedCount++;
              continue;
            }
            ...
            if (result.success) {
              ...
              console.log(
                `      ❌ Error instalando MCP '${mcpDisplayName}' para ${addonInfo.name}: ${result.message}`
              );
              errorCount++;
            }
          } catch (error: any) {
            console.error(
              `      ❌ Error procesando MCP '${mcpName}' para ${addonInfo.name}:`,
              error.message || error
            );
            errorCount++;
          }
      ...
      if (errorCount > 0) {
        console.log(
          `   ❌ ${errorCount} MCP(s) con error(es) durante la instalación`
        );
      }
