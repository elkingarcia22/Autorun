...
export async function autoDetectWizardState(): Promise<WizardDetectionResult> {
  ...
  try {
    ...
    if (!(await hasWizardState())) {
      console.log(
        '✅ [Auto Wizard Detection] No hay archivo de estado del wizard'
      );
      return {
        detected: false,
        processed: false,
    ...
    if (!wizardState || !wizardState.url) {
      console.log(
        '✅ [Auto Wizard Detection] Archivo de estado no tiene información válida'
      );
      return {
        detected: false,
        processed: false,
    ...
    console.log(
      `✅ [Auto Wizard Detection] Wizard state detectado: ${wizardState.url}`
    );
    console.log(
      `🚀 [Auto Wizard Detection] Emitiendo mensajes para inicialización automática...`
    );
    ...
    console.log(
      `[AUTORUN_BROWSER_URL]${wizardState.url}[/AUTORUN_BROWSER_URL]`
    );
    ...
    console.error(
      `❌ [Auto Wizard Detection] Error detectando wizard state: ${error.message}`
    );
    return {
      detected: false,
      processed: false,
 ...
 */
export async function processWizardState(
  wizardState: { url: string; initHub?: boolean }
  ...
  try {
    ...
    console.error(
      `❌ [Auto Wizard Detection] Error procesando wizard state: ${error.message}`
    );
    throw error;
  }
}
