...
import { extractExactCodeFromStorybook, getSourceCode } from './storybookExactCodeExtractor.js';
 ...
 */
export async function extractInteractionInfo(componentId: string): Promise<InteractionInfo | null> {
	console.log(
	...
	try {
		...
		const exactCode = await extractExactCodeFromStorybook(componentId, 'default').catch(() => null);
		...
		if (componentId.includes('modal') || componentId === 'modal') {
			const openPattern = extractOpenPattern(exactCode?.html || undefined, sourceCode || undefined);
			const closePattern = extractClosePattern(exactCode?.html || undefined, sourceCode || undefined);

			if (openPattern) {
				interactionInfo.openMethod = openPattern;
				console.log(`✅ [Interaction Extractor] Método de apertura encontrado: ${openPattern}`);
			}

			if (closePattern) {
				interactionInfo.closeMethod = closePattern;
				console.log(`✅ [Interaction Extractor] Método de cierre encontrado: ${closePattern}`);
			}

			// Validar que NO se use data-open
			if (
				openPattern &&
				(openPattern.includes('data-open') || openPattern.includes('setAttribute'))
			) {
		...
		console.warn(`⚠️ [Interaction Extractor] Error extrayendo información: ${error.message}`);
		return null;
	}
}
...
function extractOpenPattern(html?: string, sourceCode?: string): string | null {
	...
	if (sourceCode) {
		...
		const openMatch = sourceCode.match(/(?:open|show|display).*?modal.*?overlay.*?open/gi);
		if (openMatch) {
			return openMatch[0];
		}
	}
	...
	if (html) {
		...
		const onclickMatch = html.match(/onclick=["']([^"']*?ubits-modal-overlay--open[^"']*?)["']/i);
		if (onclickMatch) {
 ...
 */
function extractClosePattern(html?: string, sourceCode?: string): string | null {
	// Buscar en código fuente primero (más confiable)
	if (sourceCode) {
		...
		const closeMatch = sourceCode.match(/(?:close|hide|remove).*?modal.*?overlay.*?open/gi);
		if (closeMatch) {
			return closeMatch[0];
		}
	}
