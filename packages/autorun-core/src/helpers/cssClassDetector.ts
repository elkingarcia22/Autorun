...
export function detectButtonClasses(templateContent: string): {
	...
	const hasGenericButton =
		templateContent.includes('class="button') || templateContent.includes("class='button");
 ...
 */
export function isClassAvailable(className: string, templateContent: string): boolean {
	...
	if (className.startsWith('ubits-')) {
		return (
			...
			(templateContent.includes('components/') && templateContent.includes(className.split('-')[1]))
		);
	}
