...
import { buildSafeStorybookUrl, verifyAvailableStories } from './verifyStorybookStories';
...
export async function getComponentStories(
	...
	try {
		...
		const baseUrlResult = await getStorybookUrlWithFallback('', { checkAvailability: false });
		...
		try {
			...
			console.warn(`⚠️ [Storybook Stories] No se pudo obtener index.json, usando método alternativo:`, error);
		}

		// Si no se obtuvieron historias, usar método alternativo (navegación en navegador)
		if (stories.length === 0) {
			console.log(`📚 [Storybook Stories] Usando método alternativo para obtener historias de ${componentName}`);
		...
		console.error(`❌ [Storybook Stories] Error obteniendo historias de ${componentName}:`, error);
		throw error;
	}
}
 ...
 */
async function extractStoriesFromIndex(indexData: any, componentId: string): Promise<StorybookStory[]> {
 ...
 */
async function createFunctionalStories(componentId: string): Promise<StorybookStory[]> {
	const functionalStories: Record<string, { name: string; description: string; urlParams: string }> = {
		'data-data-table': {
			// Historias específicas del DataTable (una funcionalidad por historia)
			'columnas-reordenables': {
				name: 'Columnas Reordenables',
				description: 'Implementar funcionalidad de reordenamiento de columnas mediante drag & drop',
				urlParams: 'columnReorderable=true&columnSortable=false&showCheckbox=false&rowReorderable=false&rowExpandable=false',
			},
			'filas-reordenables': {
				name: 'Filas Reordenables',
				description: 'Implementar funcionalidad de reordenamiento de filas mediante drag & drop',
				urlParams: 'rowReorderable=true&columnReorderable=false&columnSortable=false&showCheckbox=false&rowExpandable=false',
			},
			'filas-expandibles': {
				name: 'Filas Expandibles',
				description: 'Implementar funcionalidad de expandir/colapsar filas para mostrar contenido adicional',
				urlParams: 'rowExpandable=true&columnReorderable=false&columnSortable=false&showCheckbox=false&rowReorderable=false',
			},
			'ordenamiento-columnas': {
				name: 'Ordenamiento de Columnas',
				description: 'Implementar funcionalidad de ordenamiento (ascendente/descendente) en columnas',
				urlParams: 'columnSortable=true&columnReorderable=false&showCheckbox=false&rowReorderable=false&rowExpandable=false',
			},
			'seleccion-multiple': {
				name: 'Selección Múltiple',
				description: 'Implementar funcionalidad de selección múltiple con checkboxes y Action Bar',
				urlParams: 'showCheckbox=true&columnReorderable=false&columnSortable=false&rowReorderable=false&rowExpandable=false',
			},
			'paginacion': {
				name: 'Paginación',
				description: 'Implementar funcionalidad de paginación con controles de página e items por página',
				urlParams: 'showPagination=true&columnReorderable=false&columnSortable=false&showCheckbox=false&rowReorderable=false&rowExpandable=false',
			},
			'busqueda-filtros': {
				name: 'Búsqueda y Filtros',
				description: 'Implementar funcionalidad de búsqueda y filtros en el header del DataTable',
				urlParams: 'showHeaderSearchButton=true&showHeaderFilterButton=true&columnReorderable=false&columnSortable=false&showCheckbox=false',
			},
			'columnas-sticky': {
				name: 'Columnas Sticky (Fijas)',
				description: 'Implementar funcionalidad de columnas fijas (sticky) al hacer scroll horizontal',
				urlParams: 'checkboxSticky=true&dragHandleSticky=false&expandSticky=false&showCheckbox=true&columnReorderable=false',
			},
		},
	};
	...
	if (!componentStories) {
		console.warn(`⚠️ [Storybook Stories] No hay historias funcionales definidas para ${componentId}`);
		return [];
	}
	...
	const defaultExists = verifiedStories?.availableStories.some(s => s.name === 'default') ?? false;
	
	const baseUrl = 'https://ubits-storybook10.vercel.app';
	const storyName = defaultExists ? 'default' : (verifiedStories?.availableStories[0]?.kebabName || 'default');
 ...
 */
async function getStoriesFromBrowser(componentId: string, baseUrl: string): Promise<StorybookStory[]> {
	// Este método requiere que el agente navegue manualmente
	// Por ahora, retornamos historias funcionales específicas
	console.log(`💡 [Storybook Stories] Usando historias funcionales para ${componentId}`);
 ...
 */
function mapComponentNameToStorybookId(componentName: string): string {
...
export function generateImplementationPlanFromStories(
	...
	const steps = componentStories.stories.map((story, index) => {
		...
		const previousStepId = index > 0 ? `${componentStories.componentId}-story-${index}` : undefined;
		...
		return {
			...
			estimatedTime: story.name === 'default' ? '10-15 minutos' : '5-10 minutos',
		};
	});
