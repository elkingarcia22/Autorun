...
import { buildSafeStorybookUrl, verifyAvailableStories } from './verifyStorybookStories.js';
...
async function createFunctionalStories(componentId: string): Promise<StorybookStory[]> {
	...
	const defaultExists =
		verifiedStories?.availableStories.some((s) => s.name === 'default') ?? false;
...
export async function mapComponentNameToStorybookId(componentName: string): Promise<string> {
	try {
		...
		if (mappedId) {
			...
			try {
				const { validateAndCorrectStorybookId, getCorrectStorybookIdWithRetry } = await import(
					'./storybookIdValidator'
				);
				...
				console.warn(`⚠️ [Storybook Stories] Validación falló, intentando con retry...`);
				const retryResult = await getCorrectStorybookIdWithRetry(componentName, mappedId);

				if (retryResult.found) {
					console.log(`✅ [Storybook Stories] ID encontrado con retry: ${retryResult.componentId}`);
					return retryResult.componentId;
				}

				console.warn(`⚠️ [Storybook Stories] ID no válido después de retry: ${mappedId}`);
	...
	try {
		...
		const discoveryResult = await getCorrectStorybookId(componentName, undefined);
...
export async function mapAndValidateComponentNameToStorybookId(
	...
	try {
		...
		const { getCorrectStorybookId } = await import('./storybookIdDiscovery');
		const discoveryResult = await getCorrectStorybookId(componentName, mappedId);
		...
		if (discoveryResult.found) {
			...
			if (discoveryResult.availableStories) {
				console.log(`  📚 Historias disponibles: ${discoveryResult.availableStories.join(', ')}`);
			}
			return discoveryResult.componentId;
		}
 ...
 */
export function generateImplementationPlanFromStories(componentStories: ComponentStories): {
	componentName: string;
	steps: Array<{
		id: string;
		name: string;
