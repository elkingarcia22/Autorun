...
export async function validateStructureBeforeWrite(
  ...
  try {
    ...
    if (cached) {
      ...
      console.log(
        `📚 [Structure Validator] Consultando Storybook (no hay caché)...`
      );
    ...
    try {
      ...
      const sourceComparison = compareWithSourceCode(
        sourceCode,
        generatedCode,
        componentId
      );
    ...
    if (valid) {
      console.log(
        `✅ [Structure Validator] Validación pasó: estructura correcta`
      );
    } else {
      console.error(
    ...
    if (warnings.length > 0) {
      console.warn(
        `⚠️ [Structure Validator] ${warnings.length} advertencia(s) encontrada(s)`
      );
...
function validateInteraction(
  ...
  if (interactionInfo.openMethod) {
    ...
    if (componentId.includes('modal') || componentId === 'modal') {
      if (
        ...
        errors.push(
          `❌ Modal NO debe usar 'data-open'. Debe usar: ${expectedMethod}`
        );
      }
      ...
      if (
        ...
        errors.push(
          `❌ Modal debe usar '${expectedMethod}' para abrir, NO 'data-open'`
        );
      }
    }
  }
  ...
  if (interactionInfo.closeMethod) {
    ...
    if (
      !generatedCode.includes(expectedMethod) &&
      !generatedCode.includes('classList.remove')
    ) {
      warnings.push(
        `⚠️ Método de cierre puede no coincidir con el esperado: ${expectedMethod}`
      );
    }
  }
...
function compareWithSourceCode(
  ...
  if (componentId.includes('modal') || componentId === 'modal') {
    ...
    if (
      ...
      errors.push(
        `❌ Contenedor principal incorrecto. Debe usar 'ubits-modal-overlay'`
      );
    }
 ...
 */
function validateModalSpecific(
  generatedCode: string
): StructureValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];
  ...
  if (!generatedCode.includes('ubits-modal-overlay')) {
    errors.push(
      `❌ Modal debe usar 'ubits-modal-overlay' como contenedor principal`
    );
  }
