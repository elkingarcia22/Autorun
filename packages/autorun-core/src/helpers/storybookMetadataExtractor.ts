...
export async function extractMetadataFromStory(
	...
	try {
		...
		if (!sourceCode) {
			console.warn(`   ⚠️ No se pudo obtener código fuente para ${componentId}`);
			return null;
		}
...
function parseUbitsBlock(ubitsBlock: string): StorybookMetadata | null {
	...
	// Extraer tokensUsed
	const tokensUsedMatch = ubitsBlock.match(/tokensUsed\s*:\s*\[([^\]]+)\]/);
	if (tokensUsedMatch) {
		metadata.tokensUsed = extractArrayFromBlock(tokensUsedMatch[1], '');
	}

	// Extraer slots
	const slotsMatch = ubitsBlock.match(/slots\s*:\s*\{([^}]+)\}/s);
	if (slotsMatch) {
		const slotsBlock = slotsMatch[1];
		metadata.slots = parseSlotsBlock(slotsBlock);
	}

	// Si no hay nada, retornar null
	if (!metadata.componentId && !metadata.dependsOn && !metadata.internals && !metadata.tokensUsed && !metadata.slots) {
		return null;
	}
...
/**
 * Parsea bloque de slots
 */
function parseSlotsBlock(slotsBlock: string): { [key: string]: string[] } {
	const slots: { [key: string]: string[] } = {};
	
	// Buscar cada slot: "slotName": [...]
	const slotPattern = /["']([^"']+)["']\s*:\s*\[([^\]]+)\]/g;
	let match: RegExpExecArray | null;
	
	while ((match = slotPattern.exec(slotsBlock)) !== null) {
		const slotName = match[1];
		const slotComponents = extractArrayFromBlock(match[2], '');
		slots[slotName] = slotComponents;
	}
 ...
 */
function extractArrayFromBlock(block: string, key: string): string[] {
	let arrayBlock = block;

	// Si hay una key, buscar el array dentro de esa key
	if (key) {
		const keyMatch = block.match(new RegExp(`${key}\\s*:\\s*\\[([^\\]]+)\\]`, 's'));
		if (keyMatch) {
