	...
	try {
		...
		for (const cssUrl of ubitsPaths) {
			try {
				...
				if (response.ok) {
					const css = await response.text();
					console.log(`⚠️ [CSS Validator] CSS encontrado en UBITS (fallback): ${cssUrl}`);
					return css;
				}
			} catch (error) {
				// Continuar
...
export async function validateCSSClasses(
	...
	const suggestions = await suggestCorrectClasses(missingClasses, componentId, componentCSS);

	const result: CSSValidationResult = {
		valid: missingClasses.length === 0,
		errors:
			missingClasses.length > 0 ? [`Clases CSS no encontradas: ${missingClasses.join(', ')}`] : [],
		warnings:
			suggestions.length > 0
				? [`Se encontraron ${suggestions.length} clase(s) incorrecta(s) con sugerencias`]
				: [],
		suggestions: suggestions.map((s) => `${s.incorrect} → ${s.correct} (${s.confidence})`),
		missingClasses,
		validClasses,
	};
	...
	if (result.valid) {
		...
		console.error(`❌ [CSS Validator] ${missingClasses.length} clase(s) no encontrada(s)`);
 ...
 */
function tryCorrectClassPattern(incorrect: string, componentId: string): string | null {
	...
	if (incorrect.startsWith(prefix) && !incorrect.includes('--') && !incorrect.includes('__')) {
