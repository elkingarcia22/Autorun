 ...
 */
export async function getComponentPropsWithFallback(componentId: string): Promise<MCPCallResult> {
	...
	try {
		...
		const visualProps = await extractPropsVisually(componentId, activeConfig.url);
...
function getMCPServerForStorybook(config: any): string {
	...
	const urlToServer: Record<string, string> = {
		...
		'https://libraries-ui.ubitslearning.com': 'storybook-libraries-ui-mcp',
	};
 ...
 */
export async function extractPropsFromDocsHTML(html: string): Promise<ComponentProps[]> {
	const props: ComponentProps[] = [];

	// Buscar tabla de props en el HTML de Docs
	// Storybook muestra props en una tabla con estructura específica
	const propsTableRegex = /<table[^>]*class="[^"]*props[^"]*"[^>]*>([\s\S]*?)<\/table>/i;
...
function extractPropsFromTableBody(tbodyHtml: string): ComponentProps[] {
	...
	for (const rowMatch of rows) {
		...
		const propType =
			typeMatch && typeMatch[1] ? typeMatch[1].replace(/<[^>]+>/g, '').trim() : 'unknown';

		// Extraer si es requerida (buscar "required" o "optional")
		const isRequired = rowHtml.includes('required') || rowHtml.includes('Required');
...
export async function validateStructureAgainstProps(
	...
	if (missingProps.length > 0) {
		errors.push(`Props requeridas faltantes: ${missingProps.map((p) => p.name).join(', ')}`);
	}

	// Validar valores por defecto (advertencias)
	props.forEach((prop) => {
		if (prop.defaultValue && !html.includes(prop.defaultValue.toString())) {
			warnings.push(`Valor por defecto recomendado para "${prop.name}": ${prop.defaultValue}`);
		}
	});
