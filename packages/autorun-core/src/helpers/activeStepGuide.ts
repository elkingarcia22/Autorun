...
} from '../validation/PhaseValidator';
...
export class ActiveStepGuide {
  ...
  static async getCurrentStep(
    ...
    console.log(
      `📋 [Active Step Guide] Siguiente fase requerida: ${nextPhase}`
    );

    // 2. Crear paso activo basado en la fase
    const activeStep = await this.createActiveStepForPhase(
      componentName,
      nextPhase
    );

    // 3. Verificar si el paso está completado
    const isCompleted = activeStep.verification
      ? await activeStep.verification()
      : false;

    if (isCompleted) {
      console.log(
        `✅ [Active Step Guide] Paso ya está completado: ${activeStep.step}`
      );
    ...
    switch (phase) {
      case 'FASE_0_VERIFICACION_SCRIPTS':
        return {
          ...
          guides: [
            'docs/guias/implementacion/GUIA-ERROR-SCRIPT-UMD-DATATABLE-NO-CARGA.md',
          ],
        };
  ...
  static async blockUntilStepCompleted(
    ...
    if (step.verification) {
      ...
      if (!isCompleted) {
        console.error(
          `❌ [Active Step Guide] Paso no completado: ${step.description}`
        );
        return {
          completed: false,
  ...
  private static async getStorybookId(componentName: string): Promise<string> {
    try {
      ...
      if (hub) {
        ...
        if (preCheckAddon) {
          return (
            (preCheckAddon as any).getStorybookId?.(componentName) ||
