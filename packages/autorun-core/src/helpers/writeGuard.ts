...
export async function guardWrite(
  ...
  // ⚠️ CRÍTICO: Si se detectó un componente, SIEMPRE forzar uso de autorun.apply() o interceptedWrite()
  if (!interceptResult.shouldIntercept) {
    if (interceptResult.componentName) {
      console.error(
        `❌ [Write Guard] Componente ${interceptResult.componentName} detectado.`
      );
      console.error(
        `❌ [Write Guard] ⚠️ OBLIGATORIO: Usar autorun.apply() o interceptedWrite() para implementar componentes UBITS.`
      );
      console.error(
        `❌ [Write Guard] NO puedes usar write() o search_replace() directos cuando se detecta un componente.`
      );
      ...
      // Esto fuerza el uso de autorun.apply() o interceptedWrite()
      return {
        allowed: false,
        reason: `Componente ${interceptResult.componentName} detectado. Debe usar autorun.apply() o interceptedWrite() para implementar componentes UBITS.`,
