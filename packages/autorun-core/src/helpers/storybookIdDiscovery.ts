...
async function discoverFromLocalStories(): Promise<DiscoveryResult> {
	console.log('📚 [Storybook ID Discovery] Descubriendo desde archivos locales...');
...
export async function discoverStorybookComponents(): Promise<DiscoveryResult> {
	...
	try {
		...
		console.log(`🔍 [Storybook ID Discovery] Procesando ${totalEntries} entradas...`);
		...
		for (const [storyId, entry] of Object.entries(indexData.entries)) {
			if (typeof entry === 'object' && entry !== null) {
				...
				const isStory = entryObj.type === 'story' || entryObj.subtype === 'story';
				...
				if (!componentIdMatch) {
					...
					if (processedCount < 5) {
						console.log(`  ⚠️ [Storybook ID Discovery] No match para: ${storyId}`);
					}
					continue;
				}
		...
		console.log(`✅ [Storybook ID Discovery] ${result.totalComponents} componentes descubiertos`);
		console.log(
			`📊 [Storybook ID Discovery] Estadísticas: ${processedCount} historias procesadas, ${skippedDocs} docs omitidos`,
		);
		console.log('🔍 [Storybook ID Discovery] ========================================\n');
...
export async function getCorrectStorybookId(
	...
	console.log(`🔍 [Storybook ID Discovery] Buscando ID correcto para: ${componentName}`);
	...
	const finalId = fallbackId || componentName.toLowerCase().replace(/\s+/g, '-');
	console.warn(`⚠️ [Storybook ID Discovery] Componente no encontrado, usando fallback: ${finalId}`);
 ...
 */
export async function getAvailableStoriesForComponent(componentId: string): Promise<string[]> {
	const component = await findComponentByIdOrName(componentId);
...
export async function generateComponentInventory(): Promise<string> {
	...
	for (const component of discovery.components) {
		...
		inventory.push(`- **Historias:** ${component.stories.map((s) => `\`${s}\``).join(', ')}`);
