...
import { getStorybookInfoCached, setStorybookInfoCached, StorybookInfo } from './storybookCache';
...
export async function consultStorybookCompleto(
	...
	if (useCache) {
		...
		if (cached) {
			console.log(`✅ [Storybook Parallel Consult] Usando información en caché`);
			return {
				success: true,
				info: cached,
				errors: [],
	...
	console.log(`📚 [Storybook Parallel Consult] Consultando Storybook en paralelo...`);
	...
	try {
		const [
			...
			getComponentPropsWithFallback(componentId).catch((error) => {
				console.warn(`⚠️ [Storybook Parallel Consult] MCP falló: ${error.message}`);
				return null;
			}),
			...
			(async () => {
				try {
					...
					return {
						url: urlResult.url,
						source: urlResult.url.includes('github') ? 'GitHub (fallback)' : 'Vercel',
						storyUsed: urlResult.storyUsed,
					};
				} catch (error: any) {
					console.warn(`⚠️ [Storybook Parallel Consult] Vercel falló: ${error.message}`);
					return null;
				}
			})(),
			...
			extractExactCodeFromStorybook(componentId, 'default').catch((error) => {
				console.warn(`⚠️ [Storybook Parallel Consult] ExactCode falló: ${error.message}`);
				return null;
			}),

			// API
			extractAPIFromStorybook(componentId).catch((error) => {
				console.warn(`⚠️ [Storybook Parallel Consult] API falló: ${error.message}`);
				return null;
			}),

			// Composition
			extractCompositionFromStorybook(componentId).catch((error) => {
				console.warn(`⚠️ [Storybook Parallel Consult] Composition falló: ${error.message}`);
				return null;
			}),

			// Best Practices
			extractBestPracticesFromStorybook(componentId).catch((error) => {
				console.warn(`⚠️ [Storybook Parallel Consult] Best Practices falló: ${error.message}`);
				return null;
			}),

			// Real World Examples
			extractRealWorldExamplesFromStorybook(componentId).catch((error) => {
				console.warn(`⚠️ [Storybook Parallel Consult] Real World Examples falló: ${error.message}`);
				return null;
			}),

			// Interaction Info (cómo abrir/cerrar)
			extractInteractionInfo(componentId).catch((error) => {
				console.warn(`⚠️ [Storybook Parallel Consult] Interaction Info falló: ${error.message}`);
				return null;
			}),
		]);

		const elapsedTime = Date.now() - startTime;
		console.log(`✅ [Storybook Parallel Consult] Consultas completadas en ${elapsedTime}ms`);
		...
		if (bestPracticesResult.status === 'fulfilled' && bestPracticesResult.value) {
			info.bestPractices = bestPracticesResult.value;
		} else {
			warnings.push('No se pudieron extraer best practices desde Storybook');
		}

		if (realWorldExamplesResult.status === 'fulfilled' && realWorldExamplesResult.value) {
			info.realWorldExamples = realWorldExamplesResult.value;
		} else {
			warnings.push('No se pudieron extraer ejemplos del mundo real desde Storybook');
		}

		if (interactionResult.status === 'fulfilled' && interactionResult.value) {
			info.interactionInfo = interactionResult.value;
		} else {
			warnings.push('No se pudo extraer información de interacción desde Storybook');
		}

		// 4. Validar que tenemos información mínima
		if (!info.exactCode && !info.mcpData && !info.vercelData) {
			errors.push('No se pudo obtener información mínima de Storybook (MCP, Vercel o exactCode)');
		}
		...
		if (!success) {
			console.error(`❌ [Storybook Parallel Consult] Errores encontrados:`, errors);
		}
		...
		console.error(`❌ [Storybook Parallel Consult] Error crítico: ${error.message}`);
		return {
			success: false,
			info: {},
