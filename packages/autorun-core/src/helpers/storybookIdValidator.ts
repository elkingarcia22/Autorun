...
function generateSearchVariations(componentId: string): string[] {
  ...
  variations.push(
    componentId.charAt(0).toUpperCase() + componentId.slice(1).toLowerCase()
  );
...
export async function validateAndCorrectStorybookId(
  ...
  if (component) {
    console.log(
      `✅ [Storybook ID Validator] ID válido (búsqueda exacta): ${componentId}`
    );
    return {
      valid: true,
  ...
  if (component) {
    console.log(
      `✅ [Storybook ID Validator] ID encontrado por nombre: ${component.componentId}`
    );
    return {
      valid: true,
      componentId: component.componentId,
  ...
  console.log(
    `🔍 [Storybook ID Validator] ID no encontrado, generando variaciones...`
  );
  const variations = generateSearchVariations(componentId);
  console.log(
    `🔍 [Storybook ID Validator] ${variations.length} variaciones generadas`
  );
...
export async function verifyStorybookIdExists(
  ...
  try {
    ...
    console.warn(
      `⚠️ [Storybook ID Validator] Error verificando ID: ${componentId}`,
      error
    );
    return false;
  }
...
export async function buildValidatedStorybookUrl(
  ...
  const { getAvailableStoriesForComponent } = await import(
