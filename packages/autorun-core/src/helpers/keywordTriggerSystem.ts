...
export interface KeywordTrigger {
	...
	action: 'activate-step-by-step' | 'verify-checklist' | 'load-guides' | 'block-implementation';
...
export class KeywordTriggerSystem {
	private static readonly TRIGGERS: KeywordTrigger[] = [
		...
		{
			keywords: ['implementar', 'crear', 'agregar'],
			patterns: [/implementar.*button|botón/i, /crear.*button|botón/i, /agregar.*button|botón/i],
		...
		{
			keywords: ['no', 'no hacer', 'no implementar', 'esperar', 'parar'],
			patterns: [
				/no.*implementar/i,
				/no.*crear/i,
				/esperar.*implementar/i,
				/parar.*implementación/i,
			],
			priority: 'high',
	...
	static detectTriggers(userMessage: string): TriggerResult {
		...
		console.log(`🔍 [Keyword Trigger System] Mensaje: ${userMessage.substring(0, 100)}...`);
		...
		for (const trigger of this.TRIGGERS) {
			...
			// Verificar patterns
			const patternMatch = trigger.patterns.some((pattern) => pattern.test(userMessage));
		...
		console.log(`✅ [Keyword Trigger System] ${detectedTriggers.length} trigger(s) detectado(s)`);
		console.log(`✅ [Keyword Trigger System] Acción principal: ${primaryAction}`);
		if (componentName) {
			console.log(`✅ [Keyword Trigger System] Componente detectado: ${componentName}`);
		}
		...
		switch (triggerResult.action) {
			...
			case 'verify-checklist':
				...
				try {
					...
					console.error(`❌ [Keyword Trigger System] Error verificando checklist:`, error);
				}
				break;
			...
			case 'load-guides':
				...
				if (triggerResult.componentName) {
					try {
						...
						const guidesResult = await loadRequiredGuides(triggerResult.componentName);
						return {
							executed: true,
							result: guidesResult,
						};
					} catch (error) {
						console.error(`❌ [Keyword Trigger System] Error cargando guías:`, error);
					}
				}
				break;
 ...
 */
export async function executeKeywordTriggerSystem(userMessage: string): Promise<{
	triggered: boolean;
	componentName?: string;
	shouldActivateFlow: boolean;
	blocked?: boolean;
