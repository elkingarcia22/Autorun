...
export async function verifyBeforeImplementation(
  ...
  console.log(
    `🔍 [Pre-Implementation Verification] Verificando código para: ${componentId}`
  );
  ...
  if (result.valid) {
    console.log(
      `✅ [Pre-Implementation Verification] Todas las verificaciones críticas pasaron`
    );
  } else {
    console.error(
...
async function validateHTMLStructure(
  ...
  if (Math.abs(openTags - closeTags) > 2) {
    warnings.push(
      `Posible desbalance de tags: ${openTags} abiertos, ${closeTags} cerrados`
    );
  }
...
async function validateAgainstSourceCode(
  ...
  try {
    ...
    const componentPrefix = getComponentPrefix(componentId);
    const htmlComponentClasses = htmlClasses.filter((cls) =>
      cls.startsWith(componentPrefix)
    );

    const missingInSource = htmlComponentClasses.filter(
      (cls) => !sourceClasses.includes(cls)
    );

    if (missingInSource.length > 0) {
      warnings.push(
        `Clases no encontradas en código fuente: ${missingInSource.join(', ')}`
      );
    }
  } catch (error: any) {
...
function getRequiredElementsForComponent(componentId: string): Array<{
  ...
  const requiredElements: Record<
    string,
    Array<{ type: 'class' | 'tag'; value: string }>
  > = {
    drawer: [
