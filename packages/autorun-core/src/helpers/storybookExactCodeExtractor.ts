...
export async function extractExactCodeFromStorybook(
	...
	if (!storybookBaseUrl) {
		try {
			...
			throw new Error(`❌ No se pudo obtener URL del Storybook activo. ${error.message}`);
		}
	}
	console.log(
	...
	try {
		...
		console.log(`✅ [Exact Code Extractor] Código extraído: ${codeFromTab.html.length} caracteres`);
...
export async function getSourceCode(componentId: string): Promise<string | null> {
	try {
		...
		const normalizedId = componentId.replace('feedback-', '').replace('data-', '');
		...
		const possiblePaths = [
			...
			`vendor/ubits/packages/addons/${normalizedId}/src/${normalizedId}Provider.js`,
			`packages/addons/${normalizedId}/src/${normalizedId}Provider.ts`,
			// También buscar con el ID original
			`vendor/ubits/packages/components/${componentId}/src/${componentId}Provider.ts`,
			`vendor/ubits/packages/components/${componentId}/src/${componentId}Provider.js`,
		];
