...
export async function searchStorybookIdWithRetry(
  ...
  console.log(
    `🔍 [Storybook ID Search] Buscando ID para: ${componentName} (mapeado: ${mappedId})`
  );
  ...
  try {
    ...
    if (component) {
      console.log(
        `✅ [Storybook ID Search] Encontrado (exacto): ${component.componentId}`
      );
      return {
        found: true,
  ...
  try {
    ...
    if (component) {
      console.log(
        `✅ [Storybook ID Search] Encontrado (por nombre): ${component.componentId}`
      );
      return {
        found: true,
        componentId: component.componentId,
  ...
  try {
    ...
    for (const component of discovery.components) {
      ...
      if (
        ...
        console.log(
          `✅ [Storybook ID Search] Encontrado (index.json): ${component.componentId}`
        );
        return {
          found: true,
          componentId: component.componentId,
    ...
    lastError = error.message;
    console.warn(
      `⚠️ [Storybook ID Search] Error en index.json: ${error.message}`
    );
  }
  ...
  try {
    ...
    if (validation.valid && validation.corrected) {
      console.log(
        `✅ [Storybook ID Search] Encontrado (validación): ${validation.componentId}`
      );
      return {
        found: true,
...
export async function getStorybookIdWithSmartSearch(
  ...
  const searchResult = await searchStorybookIdWithRetry(
    componentName,
    mappedId
  );
