...
import { extractMetadataFromStory, normalizeComponentId } from './storybookMetadataExtractor.js';
...
export async function analyzeComponentInternals(
	...
	const internalComponents = detectInternalComponents(exactCode.html, sourceCode, componentId);
	...
	try {
		...
		console.warn(`   ⚠️ Error extrayendo metadata declarativa: ${error.message}`);
	}
...
function analyzeMainComponent(
	...
	const componentClasses = classes.filter((cls) => cls.startsWith(componentPrefix));
...
function detectInternalComponents(
	...
	for (const known of knownInternals) {
		const component = detectInternalComponentInHTML(html, sourceCode, componentId, known);
		if (component) {
...
function detectInternalComponentInHTML(
	...
	const internalClassPatterns: Record<string, string[]> = {
		...
		'header-complementary': [`${componentPrefix}__header-complementary`, 'ubits-text'],
 ...
 */
function detectAdditionalInternals(html: string, componentId: string): InternalComponent[] {
	...
	for (const match of buttonMatches) {
		...
		if (!additional.some((c) => c.classes.some((cls) => classes.includes(cls)))) {
			additional.push({
 ...
 */
function detectDependencies(internalComponents: InternalComponent[], html: string): string[] {
	const dependencies: string[] = [];

	// Detectar componentes UBITS usados
	const ubitsComponentPattern = /ubits-(button|input|select|checkbox|radio-button|icon)/gi;
...
function detectDependsOnAndInternals(
	...
	if (declarativeMetadata) {
		console.log(`   📋 [Nivel A] Metadata declarativa encontrada en story`);
	...
	idMatches.forEach((match) => {
		...
		if (componentMatch) {
			const componentName = (componentMatch[1] || componentMatch[2]).toLowerCase();
			if (
	...
	foundComponents.forEach((compName) => {
		...
		const internalComponents = ['scroll', 'scrollbar', 'overlay', 'mask', 'progress'];

		if (dependsOnComponents.includes(compName)) {
			if (!dependsOnRequired.includes(compName) && !dependsOnOptional.includes(compName)) {
...
function extractInternalComponentStructure(html: string, classPattern: string): string {
