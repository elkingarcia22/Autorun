...
import { validateAndCorrectStorybookId, buildValidatedStorybookUrl } from './storybookIdValidator';
...
export async function buildValidatedStorybookUrlForComponent(
	...
	try {
		...
		const validation = await validateAndCorrectStorybookId(componentName, componentId);

		if (!validation.valid) {
			const errorMessage = `ID no válido: ${validation.originalId || componentId}`;
			console.error(`❌ [Storybook URL Builder] No se pudo validar ID: ${errorMessage}`);
			// Retornar URL con el ID original como último recurso
			const manager = StorybookManager.getInstance();
			const activeConfig = await manager.getActiveConfig();
			const fallbackUrl = activeConfig
				? await manager.buildStorybookUrl(`?path=/story/${componentId}--${storyName}`)
		...
		console.error(`❌ [Storybook URL Builder] Error construyendo URL: ${error.message}`);
...
export async function buildStorybookUrlWithRetry(
	...
	for (const variation of commonVariations) {
		...
		if (attempt.valid) {
			console.log(`✅ [Storybook URL Builder] URL encontrada con variación: ${variation}`);
			return attempt.url;
		}
	}
