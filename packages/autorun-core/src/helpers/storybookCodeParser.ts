...
export async function parseCodeFromStorybookUrl(
	...
	console.log(`🔍 [Storybook Code Parser] Extrayendo código desde: ${storybookUrl}`);
	...
	try {
		...
		console.log(`✅ [Storybook Code Parser] ${codeBlocks.length} bloques de código extraídos`);
		...
		console.error(`❌ [Storybook Code Parser] Error extrayendo código:`, error.message);
		throw new Error(`No se pudo extraer código desde ${storybookUrl}: ${error.message}`);
	}
}
 ...
 */
function extractDescription(codeBlock: string, html: string): string | undefined {
	// Buscar texto antes del bloque de código
	const beforeMatch = html.match(new RegExp(`([^<]{0,200})${escapeRegex(codeBlock)}`, 'i'));
	if (beforeMatch) {
		const before = beforeMatch[1].trim();
		// Buscar descripciones comunes
		const descMatch = before.match(/(?:description|desc|example|ejemplo)[:：]\s*(.+?)(?:\.|$)/i);
		if (descMatch) {
...
function extractImports(codeBlocks: StorybookCodeBlock[]): string[] {
	...
	for (const block of codeBlocks) {
		...
		const importMatches = block.code.match(/import\s+.*?\s+from\s+["']([^"']+)["']/g);
		if (importMatches) {
 ...
 */
function identifyPrimaryCode(codeBlocks: StorybookCodeBlock[]): string | undefined {
 ...
 */
function deduplicateCodeBlocks(blocks: StorybookCodeBlock[]): StorybookCodeBlock[] {
