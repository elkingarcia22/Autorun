...
export async function validateBeforeImplementation(
  ...
  console.log(
    `🔍 [Pre-Implementation Validator] Validando implementación de: ${componentId}`
  );
  ...
  try {
    ...
    try {
      const { getComponentPropsWithFallback, validateStructureAgainstProps } =
        await import('./mcpWithFallback');
      ...
      if (mcpResult.success && mcpResult.props) {
        ...
        warnings.push(
          `MCP no disponible, usando fallback visual: ${mcpResult.error}`
        );
      } else {
        warnings.push(
          `No se pudieron obtener props desde MCP: ${mcpResult.error}`
        );
      }
    } catch (error: any) {
      ...
      if (exactCode && exactCode.html) {
        ...
        if (!cssValidation.valid) {
          ...
          if (cssValidation.suggestions.length > 0) {
            warnings.push(
              `Sugerencias: ${cssValidation.suggestions.join('; ')}`
            );
          }
        } else {
    ...
    try {
      ...
      if (
        templateContent.includes('tokens.css') ||
        templateContent.includes('--ubits-')
      ) {
    ...
    const allCriticalPassed =
      checklist.storybookConsulted &&
      checklist.codeExtracted &&
      checklist.cssLoaded;
