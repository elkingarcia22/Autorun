  ...
  try {
    ...
    for (const fileName of htmlFiles) {
      ...
      try {
        ...
        console.warn(
          `⚠️ [Template Detector] Error leyendo ${fileName}:`,
          error
        );
      }
    }
    ...
    console.warn(
      `⚠️ [Template Detector] Error leyendo directorio prototypes/:`,
      error
    );
  }
...
export async function detectTemplateToOpen(): Promise<{
  ...
  try {
    const { readWizardState, hasWizardState } = await import(
      './wizardStateDetector'
    );
    if (await hasWizardState()) {
      ...
      if (state && state.url) {
        console.log(
          `📋 [Template Detector] Template detectado desde wizard state: ${state.url}`
        );
        return {
          template: null,
          url: state.url,
  ...
  if (mostRecent) {
    console.log(
      `📋 [Template Detector] Template más reciente detectado: ${mostRecent.fileName}`
    );
    return {
      template: mostRecent,
