...
const COMPONENT_SPECIFIC_GUIDES: Record<string, string[]> = {
	...
	Modal: ['docs/referencia/componentes/modal.md', 'docs/referencia/componentes/feedback-modal.md'],
...
const COMPONENT_STRATEGIES: Record<string, string[]> = {
	Tooltip: ['docs/guias/implementacion/componentes/ESTRATEGIA-TOOLTIP-POPOVER.md'],
	Popover: ['docs/guias/implementacion/componentes/ESTRATEGIA-TOOLTIP-POPOVER.md'],
 ...
 */
export async function loadRequiredGuides(componentName?: string): Promise<GuidesLoadResult> {
	console.log('\n📚 [Guides Loader] ========================================');
	...
	for (const guidePath of GENERAL_GUIDES) {
		...
		if (!guideResult.loaded) {
			...
			result.errors.push(`❌ No se pudo cargar guía general: ${guidePath} - ${guideResult.error}`);
	...
	if (componentName) {
		console.log(`📚 [Guides Loader] Cargando guías específicas para: ${componentName}...`);
		...
		for (const guidePath of specificGuides) {
			...
			if (!guideResult.loaded) {
				...
				console.warn(`  ⚠️ [Guides Loader] Guía específica no encontrada: ${guidePath}`);
			} else {
				console.log(`  ✅ [Guides Loader] Cargada: ${guidePath}`);
			}
		}
		...
		for (const strategyPath of strategies) {
			...
			if (!strategyResult.loaded) {
				console.warn(`  ⚠️ [Guides Loader] Estrategia no encontrada: ${strategyPath}`);
			} else {
	...
	// 4. Resumen
	const totalGuides = result.generalGuides.length + result.componentSpecificGuides.length;
	const loadedGuides =
		result.generalGuides.filter((g) => g.loaded).length +
		result.componentSpecificGuides.filter((g) => g.loaded).length;

	console.log(`📚 [Guides Loader] Resumen: ${loadedGuides}/${totalGuides} guías cargadas`);

	if (result.allLoaded) {
		console.log('✅ [Guides Loader] Todas las guías generales cargadas');
	} else {
		console.warn('⚠️ [Guides Loader] Algunas guías generales no se pudieron cargar');
	}
...
export function verifyGuidesLoaded(guidesResult: GuidesLoadResult): {
	...
	const generalGuidesNotLoaded = guidesResult.generalGuides.filter((g) => !g.loaded);
	...
	const specificGuidesNotLoaded = guidesResult.componentSpecificGuides.filter((g) => !g.loaded);
