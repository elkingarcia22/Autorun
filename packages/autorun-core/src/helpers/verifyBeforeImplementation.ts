...
export async function verifyBeforeImplementation(
  ...
  console.log(
    `🔍 [Verify Before Implementation] Verificando código para: ${componentId}`
  );
  ...
  try {
    const requiredElementsCheck = await validateRequiredElements(
      html,
      componentId
    );
    checks.push({
  ...
  const result: VerificationResult = {
    ...
    errors: criticalFailures.map(
      (c) => `${c.name}: ${c.error || 'Error desconocido'}`
    ),
    warnings: checks
  ...
  if (result.valid) {
    console.log(
      `✅ [Verify Before Implementation] Todas las verificaciones críticas pasaron`
    );
  } else {
    console.error(
...
async function validateHTMLStructure(
  ...
  const hasComponentClasses = new RegExp(`class="[^"]*${componentPrefix}`).test(
    html
  );
  if (!hasComponentClasses) {
    warnings.push(
      `No se encontraron clases del componente (${componentPrefix})`
    );
  }
  ...
  for (const element of requiredElements) {
    if (element.type === 'class') {
      ...
      if (!structureRegex.test(html)) {
        errors.push(
          `Falta estructura requerida: ${element.description || element.value}`
        );
      }
    }
  }
...
async function validateAccessibility(
  ...
  if (
    ...
    buttons.forEach((button, index) => {
      ...
      const hasText = />[^<]+</.test(
        html.substring(html.indexOf(button) + button.length)
      );
...
async function validateAgainstSourceCode(
  ...
  try {
    ...
    const missingClasses = htmlClasses.filter(
      (cls) => !sourceClasses.includes(cls)
    );
    if (missingClasses.length > 0) {
      warnings.push(
        `Clases usadas no encontradas en código fuente: ${missingClasses.join(', ')}`
      );
    }
  } catch (error: any) {
 ...
 */
function extractClassesFromSource(
  sourceCode: string,
  componentId: string
): string[] {
