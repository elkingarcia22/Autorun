...
export class StorybookManager {
	...
	async connectStorybook(
		...
		const connection: StorybookConnection = {
			config: {
				...
				componentMapping: detectedStructure.componentMapping || config.componentMapping,
			},
			connected: true,
		...
		console.log(`✅ [Storybook Manager] Storybook "${config.name}" conectado exitosamente`);
	...
	async disconnectStorybook(storybookId: string): Promise<void> {
		...
		console.log(`✅ [Storybook Manager] Storybook "${connection.config.name}" desconectado`);
	}
	...
	async setActiveStorybook(storybookId: string): Promise<void> {
		...
		console.log(`✅ [Storybook Manager] Storybook activo cambiado a: "${connection.config.name}"`);
	}
	 ...
	 */
	private async checkStorybookAccessibility(url: string, timeout: number = 5000): Promise<boolean> {
		try {
	...
	private async detectStorybookStructure(baseUrl: string): Promise<{
		...
		try {
			...
			console.warn(`⚠️ [Storybook Manager] No se pudo detectar estructura automáticamente:`, error);
		}
	...
	private async loadConnections(): Promise<void> {
		try {
			...
			this.connectionsLoaded = true;
			console.log(`✅ [Storybook Manager] ${this.connections.size} conexión(es) cargada(s)`);
		} catch (error: any) {
			// Si no existe el archivo, empezar con conexiones vacías
			if (error.code !== 'ENOENT') {
				console.warn(`⚠️ [Storybook Manager] Error cargando conexiones:`, error.message);
			} else {
				console.log(
	...
	private async saveConnections(): Promise<void> {
		try {
			...
			console.error(`❌ [Storybook Manager] Error guardando conexiones:`, error);
		}
	}
	 ...
	 */
	async buildStorybookUrl(path: string = '', storybookId?: string): Promise<string> {
		await this.ensureConnectionsLoaded();
		...
		try {
			...
			if (validation.valid) {
				...
				if (validation.corrected && validation.componentId !== mappedId) {
					...
					console.log(`✅ [Storybook Manager] ID válido: ${validation.componentId}`);
				}
			...
			console.warn(`⚠️ [Storybook Manager] Validación falló, intentando con retry...`);
