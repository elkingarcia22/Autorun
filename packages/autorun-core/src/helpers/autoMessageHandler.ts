...
import { executeOnMessageStart, MessageStartResult } from './executeOnMessageStart.js';
...
> {
	...
	console.log(`🚀 [Auto Message Handler] Mensaje: ${userMessage.substring(0, 100)}...`);

	// PASO 1: SIEMPRE ejecutar executeOnMessageStart()
	console.log('🚀 [Auto Message Handler] PASO 1: Ejecutando executeOnMessageStart()...');
	...
	if (result.detected && result.componentName) {
		...
		try {
			...
			console.error(`❌ [Auto Message Handler] Error obteniendo ID de Storybook:`, error);
		}
	}

	// PASO 3: Detectar múltiples componentes en el mensaje
	console.log('🚀 [Auto Message Handler] PASO 3: Detectando múltiples componentes...');
	const additionalComponents = detectAdditionalComponents(userMessage, result.componentName);
	...
	if (componentsToQuery.length > 0) {
		...
		try {
			...
			console.warn(`⚠️ [Auto Message Handler] Error llamando sistema automático de MCP:`, error);
		}
	}
	...
	console.log('🚀 [Auto Message Handler] ========================================\n');
 ...
 */
function detectAdditionalComponents(userMessage: string, alreadyDetected?: string): string[] {
	const detected: string[] = [];
	const lowerMessage = userMessage.toLowerCase();

	// ⚠️ PATRONES COMPLETOS: Todos los componentes de ambos Storybooks (UBITS + Libraries UI)
	// Total: ~80+ componentes detectables
	const componentPatterns: Record<string, RegExp[]> = {
		...
		List: [/\blist\b/i, /\blista\b/i, /(?:implementar|crear|agregar|poner|hacer).*(?:list|lista)/i],
		Pagination: [
			/\bpagination\b/i,
			/\bpaginaci[oó]n\b/i,
