...
export const COMPONENT_STORIES_PATH_MAP: Record<string, string> = {
  ...
  'Navegación/Sub Nav':
    'vendor/ubits/packages/storybook/stories/SubNav.stories.ts',
  'navegacion-sub-nav':
    'vendor/ubits/packages/storybook/stories/SubNav.stories.ts',
  SubNav: 'vendor/ubits/packages/storybook/stories/SubNav.stories.ts',
  'Layout/Sidebar':
    'vendor/ubits/packages/storybook/stories/Sidebar.stories.ts',
  'navegacion-sidebar':
    'vendor/ubits/packages/storybook/stories/Sidebar.stories.ts',
  Sidebar: 'vendor/ubits/packages/storybook/stories/Sidebar.stories.ts',
  'Formularios/Input':
    'vendor/ubits/packages/storybook/stories/Input.stories.ts',
  'formularios-input':
    'vendor/ubits/packages/storybook/stories/Input.stories.ts',
  ...
  'Feedback/Drawer Navigation':
    'vendor/ubits/packages/storybook/stories/Drawer.stories.ts',
  'feedback-drawer-navigation':
    'vendor/ubits/packages/storybook/stories/Drawer.stories.ts',
 ...
 */
async function getComponentTitleFromStoriesFile(
  filePath: string
): Promise<string | null> {
  try {
    ...
    console.error(
      `❌ [Verify Stories] Error leyendo título de ${filePath}:`,
      error
    );
    return null;
  }
...
export async function verifyAvailableStories(
  ...
  try {
    ...
    if (!storyExports || storyExports.length === 0) {
      console.warn(
        `⚠️ [Verify Stories] No se encontraron historias en ${storiesPath}`
      );
...
export async function buildSafeStorybookUrl(
  ...
  const storyExists = storyInfo.availableStories.some(
    (s) => s.name === normalizedStoryName
  );
