...
export async function verifyAvailableStories(
	...
	try {
		...
		if (!storiesPath) {
			console.warn(`⚠️ [Verify Stories] No se encontró ruta para componente: ${componentIdentifier}`);
			return null;
		}
		...
		const componentTitle = await getComponentTitleFromStoriesFile(fullPath) || 
			componentIdentifier;
		...
		const stories: VerifiedStory[] = storyExports.map(match => {
		...
		console.log(`✅ [Verify Stories] Historias encontradas para ${componentIdentifier}:`, 
		...
		console.error(`❌ [Verify Stories] Error verificando historias para ${componentIdentifier}:`, error);
		return null;
	}
}
...
export async function buildSafeStorybookUrl(
	...
	if (!storyInfo) {
		...
		const componentId = COMPONENT_TITLE_TO_ID_MAP[componentIdentifier] || 
			COMPONENT_TITLE_TO_ID_MAP[normalizedIdentifier] ||
			componentIdentifier.toLowerCase().replace(/\s+/g, '-').replace(/\//g, '-');
		const url = `https://ubits-storybook10.vercel.app/?path=/story/${componentId}--default`;
		
		console.warn(`⚠️ [Build Safe URL] No se pudo verificar historias para ${componentIdentifier}. Usando 'default'.`);
 ...
 */
export async function getAvailableStoryNames(
	componentIdentifier: string
): Promise<string[]> {
	const storyInfo = await verifyAvailableStories(componentIdentifier);
