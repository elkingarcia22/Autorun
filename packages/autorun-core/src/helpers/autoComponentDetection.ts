 ...
 */
export async function autoDetectComponent(userMessage: string): Promise<AutoDetectionResult> {
	console.log('\n🔍 [Auto Component Detection] ========================================');
	console.log('🔍 [Auto Component Detection] Iniciando detección automática');
	console.log(`🔍 [Auto Component Detection] Mensaje: ${userMessage.substring(0, 100)}...`);

	// 1. Detección básica con detectComponentFromMessage
	const basicDetection = detectComponentFromMessage(userMessage);
	console.log(`🔍 [Auto Component Detection] Detección básica: ${basicDetection || 'NINGUNO'}`);
	...
	if (componentName === 'Modal' && /\bbot[oó]n\b/i.test(userMessage)) {
		...
		const buttonComponent = proactiveDetection.components.find((c) => c.name === 'Button');
		if (buttonComponent) {
	...
	if (!componentName) {
		console.log('🔍 [Auto Component Detection] No se detectó ningún componente');
		return {
			detected: false,
	...
	try {
		...
		if (!hub) {
			console.warn('⚠️ [Auto Component Detection] AutorunHub no está inicializado');
			return {
				detected: true,
				componentName,
				confidence,
		...
		console.log(`🔍 [Auto Component Detection] Verificando con Pre-Implementation Check...`);
		verification = await (preCheckAddon as any).verifyOnDetection?.(componentName);
		...
		if (verification?.storyBasedPlan) {
			...
			try {
				const { getStoryBasedImplementationPlan } = require('./componentHelpers');
				const componentId = (preCheckAddon as any).getStorybookId?.(componentName);
				if (componentId) {
				...
				console.warn(`⚠️ [Auto Component Detection] No se pudo obtener plan:`, error);
			}
		}
	} catch (error) {
		console.error(`❌ [Auto Component Detection] Error ejecutando verificación:`, error);
	}
