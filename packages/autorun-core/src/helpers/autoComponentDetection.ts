...
export async function autoDetectComponent(
  ...
  try {
    ...
    // ⚠️ CRÍTICO: Si estamos en modo autorun.apply(), pasar skipCheck=true automáticamente
    ...
    const verifyOptions = isAutorunApplyMode ? { skipCheck: true } : undefined;
    console.log(
      `   🔍 [Auto Component Detection] Modo autorun.apply(): ${isAutorunApplyMode}, verifyOptions: ${JSON.stringify(verifyOptions)}`
    );
