...
function mapComponentNameToStorybookURLSync(componentName: string): string {
	...
	return urlMapping[componentName] || `${baseURL}?path=/story/${storybookName}--default`;
}

/**
 * Mapea nombre de componente a archivo de documentación
 */
function mapComponentNameToDocFile(componentName: string): string {
	const mapping: Record<string, string> = {
		DataTable: 'data-data-table',
		Tabs: 'navegacin-tabs',
		Button: 'bsicos-button',
...
export async function autoConsultStorybookMCP(
	...
	try {
		...
		console.log(`💡 [Component Helper] El agente debe usar: mcp_storybook_getComponentsProps(['${storybookName}'])`);
...
export async function getStoryBasedImplementationPlan(
	...
	try {
		const { createStoryBasedImplementationPlan, generateStoryBasedPlanSummary } = await import('./storyBasedImplementation');
		
		console.log(`📚 [Component Helper] Obteniendo plan de implementación basado en historias para: ${componentName}`);
