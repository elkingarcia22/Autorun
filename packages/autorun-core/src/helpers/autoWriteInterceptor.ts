...
export async function autoInterceptWrite(
	...
	const detectedFromMessage = userMessage ? detectComponentFromMessage(userMessage) : null;
	const proactiveDetection = detectComponentsProactively(content + (userMessage || ''));
	...
	if (detectedFromContent) {
		...
		console.log(`✅ [Auto Write Interceptor] Componente detectado desde mensaje: ${componentName}`);
	...
	console.log(`🚨 [Auto Write Interceptor] ⚠️ COMPONENTE DETECTADO: ${componentName}`);
	console.log(`🚨 [Auto Write Interceptor] ⚠️ FORZANDO uso del flujo automático...`);
	...
	try {
		...
		console.log(`📚 [Auto Write Interceptor] Consultando Storybook completo...`);
		...
		console.log(`🔍 [Auto Write Interceptor] Validando estructura antes de escribir...`);
		...
		if (!validationResult.valid) {
			console.error(`❌ [Auto Write Interceptor] Validación de estructura falló:`);
		...
		console.error(`❌ [Auto Write Interceptor] Error crítico: ${error.message}`);
		return {
			shouldIntercept: true,
			componentName,
