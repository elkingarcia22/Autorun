...
export async function startAutorunMCPServer() {
  ...
  // ⚠️ CRÍTICO: Crear servidor con configuración explícita
  // El SDK maneja automáticamente el handshake de inicialización
  const server = new Server(
    ...
    },
    {
      ...
      // ⚠️ CRÍTICO: No agregar handlers personalizados para initialize
  ...
  // ⚠️ NOTA: Este callback se define aquí pero se usa después de conectar el servidor
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    // ⚠️ CRÍTICO: Definir name fuera del try para que esté disponible en el catch
  ...
  // ⚠️ CRÍTICO: Callback cuando la inicialización se completa
  // Esto ayuda a debuggear problemas de handshake y confirma que la conexión es exitosa
  // ⚠️ NOTA: Solo definir una vez, después de crear el servidor pero antes de conectar
  if (!server.oninitialized) {
    server.oninitialized = () => {
      ...
      try {
        ...
        if (clientCaps) {
          console.error('   📊 Capabilities del cliente:', JSON.stringify(clientCaps, null, 2));
        }
        if (clientVersion) {
          console.error('   📊 Versión del cliente:', JSON.stringify(clientVersion, null, 2));
        }
      } catch (error: any) {
        // Ignorar errores al obtener información del cliente
        console.error('   ⚠️ No se pudo obtener información del cliente:', error.message);
      }
    };
  }
  ...
  try {
    ...
    console.error('   ⚠️ Esperando handshake de inicialización del cliente...');
  } catch (error: any) {
...
// ⚠️ CRÍTICO: NO ejecutar automáticamente desde aquí
// autorun-mcp-server.ts siempre llama a startAutorunMCPServer() directamente
// Esta verificación causaba doble ejecución con tsx, por lo que está deshabilitada
// Si necesitas ejecutar el servidor directamente, usa: npm run autorun:mcp-server
// if (import.meta.url === `file://${process.argv[1]}` || import.meta.url.endsWith('autorunMCPServer.ts')) {
