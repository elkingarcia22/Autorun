...
export class FileWatcher {
  ...
  start(onChange: (filePath: string) => void): void {
    ...
    console.log(
      `✅ FileWatcher: Observando ${this.options.watchPaths.length} directorio(s)`
    );
  }
    ...
    try {
      ...
      const watcher = fs.watch(
        ...
        (eventType: string, filename: string | null) => {
          if (!filename) {
            console.log(
              `🔍 FileWatcher: Evento sin filename (eventType: ${eventType})`
            );
            return;
          }
          ...
          if (eventType === 'change' || eventType === 'rename') {
            console.log(
              `📋 FileWatcher: Procesando cambio (${eventType}) en: ${fullPath}`
            );
            this.handleFileChange(fullPath);
          } else {
            console.log(
              `⏭️ FileWatcher: Evento ignorado (tipo: ${eventType}): ${fullPath}`
            );
          }
        }
      );
  ...
  private handleFileChange(filePath: string): void {
    ...
    const timer = setTimeout(() => {
      console.log(
        `⏰ FileWatcher: Timer expirado, verificando archivo: ${filePath}`
      );
      ...
      if (fs.existsSync(filePath)) {
        ...
        if (stats.isFile()) {
          ...
          console.log(
            `📊 FileWatcher: Tamaño del archivo: ${stats.size} bytes`
          );

          if (this.onChangeCallback) {
            console.log(
              `📤 FileWatcher: Llamando onChangeCallback para: ${filePath}`
            );
        ...
        console.log(
          `❌ FileWatcher: Archivo no existe (puede haber sido eliminado): ${filePath}`
        );
      }
    ...
    console.log(
      `⏱️ FileWatcher: Timer creado (${this.options.debounceMs}ms) para: ${filePath}`
    );
  }
  ...
  /**
   * Obtiene las rutas que están siendo observadas
   */
   ...
   */
  isActive(): boolean {
    return this.watchers.size > 0 && this.onChangeCallback !== undefined;
  }
}
