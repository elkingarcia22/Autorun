...
export async function autorunPlan(input: AutorunPlanInput): Promise<AutorunPlanOutput> {
  ...
  try {
    ...
    const steps: AutorunPlanOutput['plan']['steps'] = [
      ...
      {
        step: 4,
        description: 'Validar pre-implementación (5 verificaciones: CSS, estructura, elementos requeridos, accesibilidad, comparación con código fuente)',
        required: true,
        estimatedTime: '2-5s',
      },
      {
      ...
      {
        step: 7,
        description: 'Post-implementación: Prettier (formateo), ESLint (validación), Auto-Reload (recarga), GitHub (commit si está configurado)',
        required: false,
        estimatedTime: '5-15s',
      },
      {
