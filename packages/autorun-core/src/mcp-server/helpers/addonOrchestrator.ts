...
export class AddonOrchestrator {
  ...
  async getHub() {
    if (!this.hub && !this.initialized) {
      try {
        ...
        console.error(`❌ [Addon Orchestrator] Error obteniendo AutorunHub: ${error.message}`);
        ...
        try {
          ...
          if (services && services.canImplement) {
            ...
            if (!canImplement.allowed) {
              ...
              console.error(`      Pasos faltantes: ${canImplement.missingSteps?.join(', ')}`);
              return result;
            }
            ...
            if (services.getOrCreateStoryBasedPlan) {
              try {
                result.plan = await services.getOrCreateStoryBasedPlan(componentName, componentId);
                if (result.plan) {
                  console.log(
                ...
                console.warn(`   ⚠️ No se pudo obtener plan basado en historias: ${error.message}`);
              }
            }
          }
        } catch (error: any) {
          console.error(`   ❌ Error en Pre-Implementation Check: ${error.message}`);
          // No bloquear si hay error, pero registrar
        }
      } else {
        console.log(`   ⚠️ Pre-Implementation Check no está activo, saltando verificación`);
      }
      ...
      if (storybookAddon && storybookAddon.isActive()) {
        try {
          ...
          if (services && services.getStatus) {
            ...
            if (status.running) {
              console.log(`   ✅ Storybook está corriendo: ${status.url || 'N/A'}`);
            } else {
              console.log(`   ⚠️ Storybook no está corriendo (se usará Storybook remoto)`);
            }
          }
        } catch (error: any) {
  ...
  async executePostImplementationPhase(
    ...
    if (filesWritten.length === 0) {
      console.log(`   ⚠️ No hay archivos escritos, saltando fase post-implementación`);
      return result;
    }
    ...
    try {
      ...
      if (prettierAddon && prettierAddon.isActive()) {
        try {
          ...
          if (services && services.format) {
            ...
            console.log(`   ✅ Prettier: ${filesWritten.length} archivo(s) formateado(s)`);
          }
        } catch (error: any) {
      ...
      if (eslintAddon && eslintAddon.isActive()) {
        try {
          ...
          if (services && services.lint) {
            ...
            if (lintResults.fixable && lintResults.fixable > 0 && services.fix) {
            ...
            if (result.eslint.errors > 0) {
              console.warn(`   ⚠️ ESLint: ${result.eslint.errors} error(es) encontrado(s)`);
            }
            if (result.eslint.warnings > 0) {
              console.warn(`   ⚠️ ESLint: ${result.eslint.warnings} advertencia(s) encontrada(s)`);
            }
            ...
            for (const file of filesWritten) {
              if (services.shouldAutoReload(file)) {
                ...
                if (templateUrl && services.reload) {
                  ...
                  console.log(`   ✅ Auto-Reload: Browser recargado en ${templateUrl}`);
      ...
      if (githubAddon && githubAddon.isActive()) {
        try {
          ...
          const addonConfig = hubContext?.config?.autorun?.addons?.config?.github || {};
          const config = {
          ...
          if (config.autoCommit) {
            const services = githubAddon.getServices ? githubAddon.getServices() : null;
            if (services && services.commit) {
              ...
              // Hacer commit
              const commitResult = await services.commit(filesWritten, commitMessage);
              result.github = {
                executed: true,
                committed: true,
                pushed: false,
              ...
              console.log(`   ✅ GitHub: Commit realizado${commitResult?.commitHash ? ` (${commitResult.commitHash.substring(0, 7)})` : ''}`);
      ...
      console.error(`   ❌ Error en fase post-implementación: ${error.message}`);
