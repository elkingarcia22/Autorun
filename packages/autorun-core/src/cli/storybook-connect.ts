				...
				const id =
					idIndex !== -1
						...
						: url
								...
								.replace(/\/.*$/, '')
								.replace(/\./g, '-');
				const bypassToken = bypassTokenIndex !== -1 ? args[bypassTokenIndex + 1] : undefined;
			...
			case 'list': {
				...
				if (connections.length === 0) {
					...
					for (const conn of connections) {
						...
						console.log(`   ${isActive ? '⭐' : '  '} ${conn.config.name} (${conn.config.id})`);
						console.log(`      URL: ${conn.config.url}`);
						console.log(`      Conectado: ${conn.connectedAt?.toLocaleString() || 'N/A'}`);
						if (isActive) {
