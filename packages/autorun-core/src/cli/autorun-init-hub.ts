...
import { ensureAutorunHubInitialized, getAutorunHubStatus, getCurrentHub } from '../AutorunAgent.js';
...
async function main() {
	try {
		...
		if (status.error && !status.initialized) {
			...
			console.warn('   Algunos add-ons pueden no estar disponibles, pero AutorunHub está funcionando');
		}
		...
		if (error.message?.includes('no está configurado') || error.message?.includes('project-config.json')) {
