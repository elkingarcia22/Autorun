...
async function main() {
  try {
    ...
    console.log(
      `   - File Watching: ${status.fileWatching ? '✅ activo' : '❌ inactivo'}`
    );
    ...
    if (status.activeAddons.length > 0) {
      ...
      console.log(
        '   ⚠️ Nota: Algunos add-ons pueden no estar registrados (esto es normal)'
      );
    }
    ...
    if (status.error && !status.initialized) {
      ...
      console.error(
        '   Ejecuta "npm run init" para configurar Autorun primero'
      );
      process.exit(1);
    ...
    if (!status.fileWatching) {
      console.warn(
        '⚠️ FileWatcher no está activo. Algunas funcionalidades pueden no funcionar.'
      );
    }
