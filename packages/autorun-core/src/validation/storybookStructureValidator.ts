...
export async function validateImplementationStructure(
	...
	try {
		...
		const expectedProps = await parsePropsFromComponent(options.componentId, true);
		...
		// 4. Validar props
		const propsValidation = validateProps(options.implementation, expectedProps.props);
		result.propsMatch = propsValidation.match;
		result.differences.push(...propsValidation.differences);

		// 5. Validar CSS
		const cssValidation = validateCSS(options.implementation, expectedStructure.relatedStyles);
		...
		try {
			const bestPractices = await extractBestPracticesFromStorybook(options.componentId);
			if (bestPractices) {
			...
			console.warn(`⚠️ No se pudieron obtener best practices: ${error.message}`);
		}
		...
		console.error(`❌ [Storybook Structure Validator] Error validando:`, error.message);
		result.valid = false;
...
function validateHTMLStructure(
	...
	for (const expectedEl of expectedElements) {
		...
		if (!actualEl) {
			...
			// Comparar clases
			const missingClasses = expectedEl.classes.filter((cls) => !actualEl.classes?.includes(cls));
 ...
 */
function extractPropsFromImplementation(implementation: string): Record<string, string> {
...
function validateAgainstAPI(
		...
		for (const method of api.methods) {
			...
			if (!methodPattern.test(implementation)) {
				...
				if (['success', 'error', 'info', 'warning'].includes(method.name)) {
					warnings.push(`⚠️ Método común "${method.name}" no encontrado en implementación`);
				}
			} else {
	...
	if (api.setup?.required) {
		...
		if (!setupPattern.test(implementation)) {
			warnings.push(`⚠️ Setup requerido no encontrado: ${api.setup.description}`);
		}
	}
