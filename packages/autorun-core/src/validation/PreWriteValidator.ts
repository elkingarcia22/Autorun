...
import { loadRequiredGuides, verifyGuidesLoaded, getGuidesSummary } from '../helpers/guidesLoader';
...
export class PreWriteValidator {
	...
	static async validateBeforeWrite(
		...
		if (componentName) {
			console.log(`🔍 [PreWriteValidator] Cargando guías para: ${componentName}`);
			...
			if (!guidesVerification.valid) {
				...
				console.error(`❌ [PreWriteValidator] Errores al cargar guías:`, guidesVerification.errors);
			}
			...
			console.log(`🔍 [PreWriteValidator] Validando clases CSS para: ${componentName}`);
			try {
				const cssValidation = await validateCSSClassesSimple(content, componentName);
				if (!cssValidation.valid) {
					errors.push(`❌ Clases CSS incorrectas: ${cssValidation.errors.join(', ')}`);
					if (cssValidation.suggestions.length > 0) {
						warnings.push(`💡 Sugerencias de clases: ${cssValidation.suggestions.join('; ')}`);
					}
				} else {
					console.log(`✅ [PreWriteValidator] Clases CSS validadas correctamente`);
				}
			} catch (error: any) {
		...
		const operationsResult = await this.verifyCommonOperations(content, filePath);
		console.log(`🔍 [PreWriteValidator] Resultado detección de operaciones:`, operationsResult);
	...
	private static async verifyChecklist(componentName: string): Promise<ValidationResult> {
		console.log(`  📋 [verifyChecklist] Verificando checklist para: ${componentName}`);
		...
		if (!preCheckAddon) {
			console.log(`  ⚠️  [verifyChecklist] Pre-Implementation Check add-on no está disponible`);
		...
		console.log(`  ✅ [verifyChecklist] Pre-Implementation Check add-on encontrado`);

		try {
			console.log(`  🔍 [verifyChecklist] Llamando canImplement(${componentName})...`);
			...
			if (!check.allowed) {
				console.log(`  ❌ [verifyChecklist] Checklist incompleto: ${check.reason}`);
				return {
					valid: false,
					errors: [
	...
	private static async verifyCommonOperations(
		...
		try {
			...
			const detectedOperations = OperationDetector.detectOperations(content, filePath);
			...
			for (const operation of detectedOperations) {
				...
				const guideWasRead = OperationDetector.checkGuideWasRead(operation.requiredGuide);
