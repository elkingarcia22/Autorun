...
import { shouldAutoReload, getTemplateUrlFromPath } from '../helpers/autoReloadHelper';
...
export function resetMessageStartState() {
	...
	console.log('🔄 [Tool Interceptor] Estado de messageStart reseteado para nuevo mensaje');
}
...
export async function interceptedWrite(
	...
	try {
		...
		if (!guardResult.allowed) {
			console.error(`❌ [Tool Interceptor] guardWrite() BLOQUEÓ write(): ${guardResult.reason}`);
			console.error(
		...
		console.warn(`⚠️ [Tool Interceptor] Continuando sin guardWrite() (puede haber errores)`);
	}
	...
	if (!messageStartExecuted && context?.userMessage) {
		...
		try {
			...
			console.log(`✅ [Tool Interceptor] Bloqueado: ${messageStartResult.blocked ? 'SÍ' : 'NO'}`);

			if (messageStartResult.blocked) {
				console.error(
					`❌ [Tool Interceptor] IMPLEMENTACIÓN BLOQUEADA: ${messageStartResult.reason}`,
				);
				throw new Error(`❌ IMPLEMENTACIÓN BLOQUEADA: ${messageStartResult.reason}`);
			}
			...
			if (messageStartResult.mcpMessages && messageStartResult.mcpMessages.length > 1) {
				console.log(
	...
	if (context?.componentName) {
		console.log(`🔍 [Tool Interceptor] Validando fase actual para: ${context.componentName}`);

		// Obtener paso activo actual
		const activeStepResult = await ActiveStepGuide.getCurrentStep(context.componentName);

		if (activeStepResult.blocked && activeStepResult.currentStep) {
			const step = activeStepResult.currentStep;
			console.error(`❌ [Tool Interceptor] BLOQUEADO: Debes completar el paso activo primero`);
			console.error(`❌ [Tool Interceptor] Paso requerido: ${step.description}`);
			console.error(
				`❌ [Tool Interceptor] Acciones requeridas: ${step.requiredActions.join(', ')}`,
			);

			if (step.storybookUrl) {
				console.log(`📚 [Tool Interceptor] URL de Storybook: ${step.storybookUrl}`);
			}

			if (step.guides && step.guides.length > 0) {
				console.log(`📖 [Tool Interceptor] Guías requeridas: ${step.guides.join(', ')}`);
			}
		...
		const currentPhase = await PhaseValidator.getNextRequiredPhase(context.componentName);
		if (currentPhase) {
			...
			if (!phaseValidation.valid) {
				console.error(`❌ [Tool Interceptor] Orden de fases inválido: ${phaseValidation.reason}`);
				throw new Error(`❌ IMPLEMENTACIÓN BLOQUEADA: ${phaseValidation.reason}`);
			}
		}
	}

	// ⭐ NUEVO: Validar implementación común ANTES del flujo automático
	const implementationValidation = ComponentImplementationValidator.validateImplementation(
		contents,
		context?.componentName,
	);

	if (!implementationValidation.valid) {
		console.error('❌ [Tool Interceptor] Errores de implementación detectados:');
	...
	if (context?.componentName) {
		try {
			...
			const componentId = await mapAndValidateComponentNameToStorybookId(context.componentName);

			console.log(`🔍 [Tool Interceptor] Validando estructura antes de escribir: ${componentId}`);
			...
			console.warn(`⚠️ [Tool Interceptor] Error en validación de estructura: ${error.message}`);
			console.warn(`⚠️ [Tool Interceptor] Continuando sin validación (puede haber errores)`);
		}
	}

	// Ejecutar flujo automático ANTES de escribir
	const flow = await autoImplementationFlow(filePath, contents, undefined, context);
	...
	if (!flow.canWrite) {
		...
		if (flow.storybookUrl) {
			console.log(`📚 [Tool Interceptor] URL de Storybook: ${flow.storybookUrl}`);
			console.log('📚 [Tool Interceptor] El agente debe navegar automáticamente a Storybook');
			console.log(
		...
		if (flow.plan) {
			...
			console.log(`📋 [Tool Interceptor] Total de historias: ${flow.plan.totalSteps || 'N/A'}`);
		}
	...
	try {
		...
		console.error(`❌ [Tool Interceptor] Error escribiendo archivo: ${error.message}`);
		throw new Error(`❌ Error escribiendo archivo: ${error.message}`);
	}

	console.log('✅ [Tool Interceptor] write() completado');
	console.log('✅ [Tool Interceptor] NO necesitas llamar write() después - ya está escrito');
	console.log('🛡️ [Tool Interceptor] ========================================\n');
...
export async function interceptedSearchReplace(
	...
	if (!messageStartExecuted && context?.userMessage) {
		...
		try {
			...
			console.error(`❌ [Tool Interceptor] Error ejecutando handleUserMessage(): ${error.message}`);
			// Continuar de todas formas, pero registrar el error
		}
	}
...
export async function interceptAutoReload(filePath: string): Promise<void> {
	...
	console.log('🔄 [Auto-Reload Interceptor] Mensaje [AUTORUN_AUTO_RELOAD] detectado');
	...
	try {
		...
		console.log(`🔄 [Auto-Reload Interceptor] URL del template: ${templateUrl}`);
	} catch (error) {
		console.error('❌ [Auto-Reload Interceptor] Error:', error);
	}

	console.log('🔄 [Auto-Reload Interceptor] ========================================\n');
}
...
export async function interceptStorybookMCP(
	...
	console.log('📚 [Storybook MCP Interceptor] Mensaje [AUTORUN_STORYBOOK_MCP] detectado');
	console.log(`📚 [Storybook MCP Interceptor] Componente: ${componentName} (ID: ${componentId})`);

	console.log('📚 [Storybook MCP Interceptor] El agente debe ejecutar:');
	console.log(`   mcp_storybook_getComponentsProps(['${componentId}'])`);

	console.log('📚 [Storybook MCP Interceptor] ========================================\n');
}
