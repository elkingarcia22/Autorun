...
export interface WatermarkMeta {
  ...
  deps: string[];
  hash: string;
  // ✅ MEJORA 5: Tokens esperados del contrato (opcional)
  tokens?: string[];
  // ✅ MEJORA 5: ID de Storybook para auditoría (opcional)
  storybookId?: string;
}
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (currentBlock && /<!--\s*\/AUTORUN\s*-->/.test(line)) {
      try {
        ...
        console.warn(
          `Error parseando watermark en línea ${currentBlock.startLine}: ${error}`
        );
      }
