...
export class MCPDetector {
  ...
  private static async isRunningInCursor(): Promise<boolean> {
    try {
      ...
      const hasCursorEnv = !!(
        process.env.CURSOR_VERSION || process.env.CURSOR_AGENT
      );
      if (hasCursorEnv) {
        return true;
      }
   ...
   */
  private static async checkServerConfigured(
    serviceName: string
  ): Promise<boolean> {
    try {
      ...
      const configPaths = [
        ...
        process.env.HOME
          ...
          : null,
        process.env.HOME
          ? path.join(process.env.HOME, '.mcp', 'mcp.json')
          : null,
        // Variable de entorno
        process.env.MCP_CONFIG_PATH
          ? path.join(process.env.MCP_CONFIG_PATH, 'mcp.json')
          : null,
