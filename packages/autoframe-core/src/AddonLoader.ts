...
export class AddonLoader {
  ...
  async load(addonPath: string): Promise<IAddon> {
    ...
    const modulePath = path.isAbsolute(mainPath)
      ? mainPath
      : path.join(resolvedPath, mainPath);
    ...
    const AddonClass = addonModule.default || addonModule[manifest.export] || addonModule[Object.keys(addonModule)[0]];
  ...
  private inferAddonType(packageName: string): 'component' | 'functional' | 'design' | 'testing' {
    ...
    if (name.includes('github') || name.includes('clarity') || name.includes('vercel') || name.includes('functional')) {
      return 'functional';
    }
