 ...
 * con el Autoframe Hub usando el ComponentAddonAdapter.
 */

import { AutoframeHub } from '../AutoframeHub';
 ...
 * 
 * @param hub Instancia del AutoframeHub
...
export async function loadAndRegisterLegacyComponent(
  ...
  if (!isLegacyComponent(legacyAddon)) {
    throw new Error(`El componente en ${resolvedPath} no implementa la interfaz legacy ComponentAddon`);
  }
