...
export class MCPPromptInteractive {
  ...
  async promptForMCP(options: MCPPromptOptions): Promise<boolean> {
    ...
    this.showMCPInfo(serviceDisplayName, mcpInfo);

    // Preguntar al usuario
    return new Promise((resolve) => {
      this.rl.question('\n¿Deseas instalar y configurar MCP para mejorar la experiencia? (S/N/I): ', async (answer) => {
        ...
        } else if (normalized === 'I' || normalized === 'INFO' || normalized === 'INSTRUCCIONES') {
   ...
   */
  private showMCPInfo(serviceDisplayName: string, mcpInfo: MCPServerInfo): void {
    console.log('\n' + '='.repeat(70));
