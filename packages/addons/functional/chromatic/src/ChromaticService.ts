...
export class ChromaticService {
  ...
  async run(options?: {
    ...
    if (!this.isChromaticInstalled()) {
      console.log(
        '📦 Chromatic no está instalado. Instalando automáticamente...'
      );
      try {
        ...
        return {
          success: false,
          error:
            'Chromatic no está instalado. Ejecuta: npm install --save-dev chromatic',
        };
      }
    }
    ...
    try {
      ...
      if (
        options?.onlyChanged !== undefined
          ? options.onlyChanged
          : this.config.onlyChanged
      ) {
      ...
      try {
        ...
        const changesDetected =
          output.includes('visual changes') ||
          output.includes('changes detected');
  ...
  private async installChromatic(): Promise<void> {
    ...
    try {
      ...
      console.warn(
        '⚠️  No se pudo instalar Chromatic automáticamente. Ejecuta manualmente: npm install --save-dev chromatic'
      );
      throw error;
    }
  }
