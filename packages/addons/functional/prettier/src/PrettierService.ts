...
export class PrettierService {
  ...
  private async installPrettier(): Promise<void> {
    ...
    try {
      ...
      console.warn(
        '⚠️  No se pudo instalar Prettier automáticamente. Ejecuta manualmente: npm install --save-dev prettier'
      );
      throw error;
    }
  }
  ...
  private async ensurePrettierConfig(): Promise<void> {
    ...
    console.log(
      `✅ Configuración de Prettier generada en: ${prettierConfigPath}`
    );
  }
  ...
  async format(
    ...
    if (!this.initialized && !this.isPrettierInstalled()) {
      console.log(
        '📦 Prettier no está instalado. Instalando automáticamente...'
      );
      try {
        ...
        throw new Error(
          'Prettier no está instalado. Ejecuta: npm install --save-dev prettier'
        );
      }
    }
