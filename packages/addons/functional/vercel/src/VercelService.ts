...
export class VercelService {
   ...
   */
  async createProject(projectName: string, options?: {
    framework?: string;
    buildCommand?: string;
    outputDirectory?: string;
    installCommand?: string;
  ...
  private async deployWithFiles(
    ...
    const response = await this.apiRequest('POST', `/v13/deployments`, {
      name: projectId,
      files: fileEntries,
  ...
  async addDomain(projectId: string, domain: string, gitBranch?: string): Promise<VercelDomain> {
    return await this.apiRequest('POST', `/v9/projects/${projectId}/domains`, {
      name: domain,
      gitBranch
    }, {
      teamId: this.config.teamId
  ...
  async removeDomain(projectId: string, domain: string): Promise<void> {
    await this.apiRequest('DELETE', `/v9/projects/${this.config.projectName}/domains/${domain}`, undefined, {
      teamId: this.config.teamId
    });
  }
