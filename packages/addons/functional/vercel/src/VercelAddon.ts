...
export class VercelAddon implements IFunctionalAddon {
	...
	private async offerMCPIntegration(): Promise<void> {
		try {
			...
			if (mcpInfo.configured) {
				console.log(
					'✅ Vercel Add-on: MCP detectado y configurado. Usando MCP para mejor experiencia.',
				);
				this.useMCP = true;
				return;
			}
	...
	async initialize(context: AutorunContext): Promise<void> {
		...
		const addonConfig = context.config.autorun?.addons?.config?.vercel || {};
		this.config = {
		...
		if (!this.config.token) {
			...
			console.warn(
				'   Configura VERCEL_TOKEN en variables de entorno o en la configuración del proyecto.',
			);
			return;
		}
