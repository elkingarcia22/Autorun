...
export class StorybookService {
  ...
  private async installStorybook(): Promise<void> {
    ...
    try {
      ...
      console.warn(
        '⚠️  No se pudo instalar Storybook automáticamente. Ejecuta manualmente: npm install --save-dev @storybook/react @storybook/addon-essentials'
      );
      throw error;
    }
  }
  ...
  async start(): Promise<StorybookProcess> {
    ...
    if (!this.isStorybookInstalled()) {
      ...
      console.log(
        '📦 Storybook no está instalado. Instalando automáticamente...'
      );
      try {
        ...
        throw new Error(
          'Storybook no está instalado. Ejecuta: npm install --save-dev @storybook/react @storybook/addon-essentials'
        );
      }
    }
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        this.process = spawn(
          'npx',
          ['storybook', 'dev', '-p', port.toString(), '--host', host],
          {
            cwd: this.projectPath,
            stdio: 'pipe',
            shell: true,
  ...
  async generateStory(
    ...
    const componentPath =
      options?.componentPath || `components/${componentName}`;
