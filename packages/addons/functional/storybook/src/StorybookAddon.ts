...
export class StorybookAddon implements IFunctionalAddon {
  ...
  async onFileChange(filePath: string): Promise<void> {
    ...
    // ⚠️ CORRECCIÓN: Verificar que el add-on existe antes de intentar obtener el servicio
    if (this.context) {
      try {
        const clarityAddon = this.context.hub?.getAddon?.('clarity');
        if (clarityAddon) {
        ...
        // Ignorar si Clarity no está disponible (es opcional)
      }
    }
  }
