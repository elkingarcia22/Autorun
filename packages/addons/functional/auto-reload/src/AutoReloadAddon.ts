...
export class AutoReloadAddon implements IFunctionalAddon {
  ...
  readonly description =
    'Recarga automática de página y logs automáticos cuando hay errores';
  ...
  async initialize(context: AutorunContext): Promise<void> {
    ...
    console.log('🔄 [AutoReload Add-on] Inicializando...');
    ...
    // Suscribirse a eventos fileChange del hub
    if (context.hub) {
      console.log(
        '🔄 [AutoReload Add-on] Suscribiéndose a eventos fileChange...'
      );
      // El hub emitirá eventos 'fileChange' y los add-ons los recibirán vía onFileChange
      console.log('✅ [AutoReload Add-on] Suscrito a eventos fileChange');
    }

    console.log('✅ [AutoReload Add-on] Inicializado correctamente');
    console.log('🔄 [AutoReload Add-on] Browser MCP se obtendrá dinámicamente');
    console.log(
      '🔄 [AutoReload Add-on] ========================================\n'
    );
  }
  ...
  async onFileChange(filePath: string, content?: string): Promise<void> {
    ...
    console.log(
      `🔄 [AutoReload Add-on] onFileChange llamado para: ${filePath}`
    );
    console.log(`🔄 [AutoReload Add-on] Add-on activo: ${this.active}`);

    if (!this.active) {
      console.log(
        '⚠️ [AutoReload Add-on] Add-on no está activo, ignorando cambio'
      );
      console.log(
        '🔄 [AutoReload Add-on] ========================================\n'
      );
    ...
    const isInPrototypes = filePath.includes('prototypes/');
    const isHtmlJsCss =
      filePath.endsWith('.html') ||
      filePath.endsWith('.js') ||
      filePath.endsWith('.css');

    console.log(`🔍 [AutoReload Add-on] Verificando archivo:`);
    console.log(`   - Incluye 'prototypes/': ${isInPrototypes}`);
    console.log(`   - Es HTML/JS/CSS: ${isHtmlJsCss}`);

    if (!isInPrototypes || !isHtmlJsCss) {
      console.log(
        `⏭️ [AutoReload Add-on] Archivo ignorado (no es HTML/JS/CSS en prototypes/): ${filePath}`
      );
      console.log(
        '🔄 [AutoReload Add-on] ========================================\n'
      );
    ...
    console.log(
      `✅ [AutoReload Add-on] Archivo válido para auto-reload: ${filePath}`
    );
    ...
    if (filePath.startsWith('prototypes/')) {
      ...
      console.log(
        `1. Llamar MCP tool: autorun.verify({ targetFiles: "diff", options: { strict: true } })`
      );
      console.log(
        `2. Si verification.valid === false → NO recargar (fail-closed)`
      );
      console.log(
        `3. Si verification.valid === true → Continuar con recarga normal`
      );
      console.log(
  ...
  async requestReload(filePath?: string): Promise<void> {
    if (!this.active) {
      console.log(
        '⚠️ AutoReload: Add-on no está activo, ignorando solicitud de recarga'
      );
      return;
    }
