...
export class AutoReloadAddon implements IFunctionalAddon {
	...
	async onFileChange(filePath: string, content?: string): Promise<void> {
		...
		if (now - this.lastReloadTime < this.RELOAD_COOLDOWN) {
			console.log(`⏭️ AutoReload: Cooldown activo, ignorando cambio (${now - this.lastReloadTime}ms desde última recarga)`);
			return;
		}
		...
		console.log(`💡 AutoReload: El agente de Cursor debe interceptar el mensaje [AUTORUN_AUTO_RELOAD] y recargar la página`);
		console.log(`💡 AutoReload: Usar: mcp_cursor-ide-browser_browser_snapshot() → obtener URL → mcp_cursor-ide-browser_browser_navigate({ url })`);
	...
	private async reloadPage(): Promise<void> {
		...
		console.log('💡 El agente debe usar: mcp_cursor-ide-browser_browser_navigate({ url: currentUrl })');
		...
		if (browserMCP) {
			...
			console.log('💡 El agente de Cursor debe interceptar esta llamada y usar herramientas MCP directamente');
		}
	}
	...
	async requestReload(filePath?: string): Promise<void> {
		...
		const now = Date.now();
		if (now - this.lastReloadTime < this.RELOAD_COOLDOWN) {
			console.log(`⏭️ AutoReload: Cooldown activo, ignorando solicitud (${now - this.lastReloadTime}ms desde última recarga)`);
			return;
		}

		console.log(`🔄 AutoReload: Solicitud de recarga recibida${filePath ? ` para: ${filePath}` : ''}`);
		console.log('💡 AutoReload: El agente debe usar las herramientas MCP del Browser para recargar');
		console.log('💡 AutoReload: Usar: mcp_cursor-ide-browser_browser_snapshot() → obtener URL → mcp_cursor-ide-browser_browser_navigate({ url })');
