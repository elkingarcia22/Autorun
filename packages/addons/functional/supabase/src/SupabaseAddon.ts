...
export class SupabaseAddon implements IFunctionalAddon {
  ...
  async initialize(context: AutoframeContext): Promise<void> {
    ...
    const addonConfig = context.config.autoframe?.addons?.config?.supabase || {};
    this.config = {
    ...
    if (!this.config.url || !this.config.anonKey) {
      console.warn('⚠️  Supabase Add-on: URL o anonKey no configurados. Configura SUPABASE_URL y SUPABASE_ANON_KEY.');
      return;
    }
