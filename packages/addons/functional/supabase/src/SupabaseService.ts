...
export class SupabaseService {
   ...
   */
  async signIn(email: string, password: string): Promise<{ user: SupabaseUser | null; session: SupabaseSession | null; error: any }> {
    if (!this.client) {
      throw new Error('Supabase client no está disponible. Instala @supabase/supabase-js');
    }
   ...
   */
  async uploadFile(bucket: string, path: string, file: File | Blob): Promise<{ path: string; error: any }> {
    if (!this.client) {
      throw new Error('Supabase client no está disponible');
    }
