...
export class LighthouseService {
  ...
  async audit(url: string, options?: Partial<LighthouseConfig>): Promise<LighthouseResult> {
    ...
    try {
      ...
      if (config.output === 'json') {
        ...
        const jsonCommand = this.buildLighthouseCommand(url, { ...config, output: 'json' }, jsonFile);
  ...
  private buildLighthouseCommand(
    ...
    if (config.throttling) {
      ...
      if (config.throttling.cpuSlowdownMultiplier) {
        parts.push(`--throttling-cpu-slowdown-multiplier=${config.throttling.cpuSlowdownMultiplier}`);
      }
    }
   ...
   */
  async auditMultiple(urls: string[], options?: Partial<LighthouseConfig>): Promise<LighthouseResult[]> {
  ...
  compareResults(before: LighthouseResult, after: LighthouseResult): {
    ...
    const improved = performanceDiff > 0 || accessibilityDiff > 0 || bestPracticesDiff > 0 || seoDiff > 0;
