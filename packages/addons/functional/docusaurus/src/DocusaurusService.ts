...
export class DocusaurusService {
  ...
  async initialize(): Promise<void> {
    ...
    if (!this.isDocusaurusInstalled()) {
      console.warn('⚠️  Docusaurus no está instalado. Ejecuta: npm install --save-dev @docusaurus/core @docusaurus/preset-classic');
      return;
    }
  ...
  async start(): Promise<DocusaurusProcess> {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        // Iniciar Docusaurus
        this.process = spawn('npx', ['docusaurus', 'start', '--port', port.toString(), '--host', host], {
...
`;
  ...
  async generateConfig(): Promise<void> {
    const configPath = path.join(this.projectPath, this.config.configFile || 'docusaurus.config.js');
