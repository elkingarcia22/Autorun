...
export class PreImplementationCheckAddon implements IFunctionalAddon {
	...
	readonly description = 'Verifica automáticamente que se sigan todos los pasos obligatorios antes de implementar componentes';
	...
	static detectComponentFromMessage(message: string): string[] {
		...
		try {
			const { detectComponentsProactively } = require('@autorun/core/helpers/proactiveDetection');
	...
	async getContextualChecklist(
		...
		try {
			const { getContextualChecklist } = require('@autorun/core/helpers/proactiveDetection');
	...
	async suggestNextStep(
		...
		try {
			const { suggestNextStep } = require('@autorun/core/helpers/proactiveDetection');
	...
	async verifyOnDetection(componentName: string): Promise<{
		...
		try {
			const { getStoryBasedImplementationPlan } = await import('@autorun/core/helpers/componentHelpers');
			...
			const planResult = await getStoryBasedImplementationPlan(componentName, componentId);
			...
			console.warn(`⚠️ [Pre-Implementation Check] No se pudo obtener plan basado en historias:`, error);
...
💡 Para completar el checklist automáticamente:
   ...
   - Verificar nuevamente antes de implementar
		...
		if (storyBasedPlan) {
			...
			console.log(`📋 Implementar ${storyBasedPlan.totalSteps} historias, UNA a la vez.\n`);
		}
	...
	async initialize(context: AutorunContext): Promise<void> {
		...
		const addonConfig = context.config.autorun?.addons?.config?.['pre-implementation-check'] || {};
		const enabled = addonConfig.enabled !== false;

		if (!enabled) {
			console.log('⚠️ Pre-Implementation Check Add-on: Deshabilitado en configuración');
			return;
		}
	...
	async canImplement(componentName: string): Promise<{
		...
		const checklist = this.checklists.get(componentName) || this.createEmptyChecklist(componentName);
		...
		const attempt: ImplementationAttempt = {
			...
			checklist,
			blocked: !allowed,
			reason: allowed ? undefined : `Faltan pasos obligatorios: ${missingSteps.join(', ')}`,
		};
		...
		if (!allowed) {
			...
			try {
				const { generateContextualErrorMessage } = require('@autorun/core/helpers/errorMessages');
				const errorMessage = await generateContextualErrorMessage('checklist-incomplete', {
					componentName,
					missingSteps: check.missingSteps,
					checklist: check.checklist,
...
💡 Para completar el checklist automáticamente:
   ...
   - Verificar nuevamente antes de implementar
	...
	async markStepCompleted(
		...
		console.log(`✅ Pre-Implementation Check: Paso "${step}" completado para ${componentName}`);
	}
	...
	async consultStorybookMCP(componentName: string): Promise<any> {
		...
		console.log(`🔍 Pre-Implementation Check: Intentando consultar Storybook MCP para '${componentName}' (ID: ${storybookId})...`);
		...
		try {
			...
			console.log(`💡 Pre-Implementation Check: El agente debe interceptar el mensaje [AUTORUN_STORYBOOK_MCP] y ejecutar:`);
			...
			console.warn(`⚠️ Pre-Implementation Check: Error consultando Storybook MCP:`, error);
			return null;
		}
	}
	...
	async loadComponentDocumentation(componentName: string): Promise<any> {
		...
		const docPath = path.join(process.cwd(), 'docs/referencia/componentes', docFileName);

		console.log(`📚 Pre-Implementation Check: Cargando documentación para '${componentName}' desde: ${docPath}`);
		...
		try {
			...
			console.log(`✅ Pre-Implementation Check: Documentación cargada para '${componentName}'`);
			...
			if (error.code === 'ENOENT') {
				console.warn(`⚠️ Pre-Implementation Check: Documentación no encontrada para '${componentName}' en: ${docPath}`);
			} else {
				console.warn(`⚠️ Pre-Implementation Check: Error cargando documentación:`, error.message);
			}
			return null;
		}
	}
	...
	private parseDocumentation(docContent: string, componentName: string): any {
		...
		const propsMatch = docContent.match(/##\s+Props?\s*\n([\s\S]*?)(?=\n##|\n###|$)/i);
		if (propsMatch) {
			// Intentar extraer lista de props
			const propsSection = propsMatch[1];
			const propLines = propsSection.split('\n').filter(line => 
				line.trim().startsWith('-') || line.trim().startsWith('*') || line.includes('|')
			);
			doc.props = propLines;
		}
		...
		if (codeBlockMatches) {
			doc.examples = codeBlockMatches.map(block => block.replace(/```[\w]*\n?/g, '').trim());
		}
	...
	async onFileChange(filePath: string, content?: string): Promise<void> {
		if (!this.active) {
			console.log('⚠️ Pre-Implementation Check: Add-on no está activo, ignorando fileChange');
			return;
		}

		console.log(`🔍 Pre-Implementation Check: onFileChange llamado para: ${filePath}`);

		// Solo analizar archivos HTML en prototypes/
		if (!filePath.includes('prototypes/') || !filePath.endsWith('.html')) {
			console.log(`🔍 Pre-Implementation Check: Archivo ignorado (no es HTML en prototypes/): ${filePath}`);
			return;
		}
		...
		this.storyBasedPlans.set(componentName, storyBasedPlan);
				...
				if (storybookData) {
					...
					console.log(`✅ Pre-Implementation Check: Storybook MCP consultado automáticamente para '${componentName}'`);
				} else {
					console.log(`💡 Pre-Implementation Check: Para consultar Storybook MCP automáticamente, el agente debe usar:`);
					console.log(`   mcp_storybook_getComponentsProps(['${this.getStorybookId(componentName)}'])`);
					console.log(`   Esto se implementará cuando las herramientas MCP estén disponibles en Node.js`);
				}
				...
				const documentation = await this.loadComponentDocumentation(componentName);
				if (documentation) {
					console.log(`✅ Pre-Implementation Check: Documentación cargada para '${componentName}'`);
					console.log(`   - Props encontradas: ${documentation.props.length}`);
					console.log(`   - Ejemplos encontrados: ${documentation.examples.length}`);
					console.log(`   - Tokens encontrados: ${documentation.tokens.length}`);
					...
					console.log(`⚠️ Pre-Implementation Check: No se pudo cargar documentación para '${componentName}'`);
					console.log(`   Verifica que existe: docs/referencia/componentes/${this.getDocFileName(componentName)}`);
				}
				
				// ⭐ NUEVO: Verificar implementación automáticamente
				if (documentation || storybookData) {
					await this.verifyImplementation(componentName, content, documentation, storybookData);
				}
				...
				if (!checkResult.allowed) {
					...
					console.warn(`📋 Pasos faltantes: ${checkResult.missingSteps.join(', ')}`);
					
					if (storyBasedPlan) {
						console.warn(`\n📚 Plan basado en historias disponible: ${storyBasedPlan.totalSteps} historias`);
						console.warn(`   Implementar UNA historia a la vez siguiendo el plan.\n`);
					}
					
					console.warn(`💡 Completa el checklist antes de implementar:\n`);
					console.warn(`   1. Consultar Storybook en Vercel: https://ubits-storybook10.vercel.app/`);
					console.warn(`   2. Consultar Storybook MCP: mcp_storybook_getComponentsProps('${componentName}')`);
					console.warn(`   3. Consultar documentación: docs/referencia/componentes/`);
					...
					if (componentName === 'DataTable') {
						...
						console.warn(`   Considera usar implementación por pasos para asegurar calidad.\n`);
					}
					...
					if (componentName === 'DataTable') {
						...
						console.log(`   ⚠️ IMPORTANTE: DataTable es un componente complejo.`);
						console.log(`   📋 RECOMENDACIÓN: Usar implementación por pasos para asegurar calidad.`);
						console.log(`   📚 Ver guía: docs/guias/implementacion/GUIA-IMPLEMENTACION-POR-PASOS.md\n`);
	...
	async verifyImplementation(
		...
		if (documentation && documentation.tokens) {
			...
			if (invalidTokens.length > 0) {
				suggestions.push(`💡 Tokens no documentados encontrados: ${invalidTokens.join(', ')}`);
			}
		}
		...
		if (componentName === 'Tabs') {
			...
			const marginTopInlinePattern = /(?:tabsContainer|container|tabs-container)[^}]*style[^}]*margin-top[^}]*16/i;
			// Detectar margin-top en CSS para contenedor de tabs
			const marginTopCSSPattern = /#[a-z-]*tabs-container[^}]*\{[^}]*margin-top[^}]*\}/i;
			// Detectar margin-top en style.cssText
			const marginTopStylePattern = /\.style\.cssText\s*=\s*['"][^'"]*margin-top[^'"]*16/i;

			if (marginTopInlinePattern.test(code) || marginTopCSSPattern.test(code) || marginTopStylePattern.test(code)) {
				issues.push(`🚨 ERROR CRÍTICO #55: Se detectó margin-top en el contenedor de tabs`);
				issues.push(`   ❌ INCORRECTO: margin-top debe venir del gap del contenedor padre (.main-content)`);
				issues.push(`   ✅ CORRECTO: Ajustar gap del .main-content, NO agregar margin-top al contenedor de tabs`);
				issues.push(`   📚 Ver: docs/guias/referencia/GUIA-ERRORES-COMUNES-UBITS.md - Error #55`);
			}
		}
		...
		if (issues.length === 0 && suggestions.length === 0) {
			console.log(`✅ Pre-Implementation Check: Implementación de '${componentName}' verificada correctamente`);
		}
	 ...
	 */
	async getOrCreateStoryBasedPlan(componentName: string, componentId?: string): Promise<any> {
		...
		try {
			const { getStoryBasedImplementationPlan } = await import('@autorun/core/helpers/componentHelpers');
			const componentIdToUse = componentId || this.getStorybookId(componentName);
			const planResult = await getStoryBasedImplementationPlan(componentName, componentIdToUse);
			...
			console.error(`❌ Error obteniendo plan basado en historias para ${componentName}:`, error);
			return null;
		}
	}
	...
	getServices(): Record<string, (...args: any[]) => any> {
		return {
			...
			clearChecklist: (componentName: string) => this.clearChecklist(componentName),
			getStoryBasedPlan: (componentName: string) => this.getStoryBasedPlan(componentName), // ⭐ NUEVO
			getOrCreateStoryBasedPlan: (componentName: string, componentId?: string) => 
				this.getOrCreateStoryBasedPlan(componentName, componentId), // ⭐ NUEVO
			verifyOnDetection: (componentName: string) => this.verifyOnDetection(componentName), // ⭐ NUEVO
			getContextualChecklist: (componentName: string, context?: any) => 
				this.getContextualChecklist(componentName, context), // ⭐ NUEVO
			suggestNextStep: (componentName: string, completedSteps: string[], context?: any) => 
