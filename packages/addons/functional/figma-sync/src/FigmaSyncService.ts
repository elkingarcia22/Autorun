...
export class FigmaSyncService {
  ...
  async initialize(): Promise<void> {
    ...
    if (this.config.projectTokensPath && !existsSync(this.config.projectTokensPath)) {
      console.warn(`⚠️  Ruta de tokens del proyecto no encontrada: ${this.config.projectTokensPath}`);
    }
  ...
  private async loadFigmaTokens(): Promise<Record<string, any>> {
    if (!this.config.figmaTokensPath) {
      throw new Error('Ruta de tokens de Figma no configurada');
    }
      ...
      for (const [key, value] of Object.entries(obj)) {
        ...
        if (value && typeof value === 'object' && !Array.isArray(value)) {
          if ('$value' in value) {
            ...
            if (typeof finalValue === 'string' && (finalValue.startsWith('#') || finalValue.match(/^\d+$/))) {
  ...
  private mapProjectToFigmaKey(projectKey: string): string | null {
    ...
    const reverseMapping: Record<string, string> = {
      'light.brand.ubits-accent-brand': 'figma.light.color.accent.brand',
      'dark.brand.ubits-accent-brand': 'figma.dark.color.accent.brand',
    };
...
${comparison.different.map(d => `- **${d.figmaKey}** → **${d.projectKey}**
  - Figma: \`${d.figmaValue}\`
  - Proyecto: \`${d.projectValue}\``).join('\n')}
...
`;
  ...
  getStatus(): {
    ...
    return {
      ...
      figmaTokensExists: this.config.figmaTokensPath ? existsSync(this.config.figmaTokensPath) : false,
      projectTokensExists: this.config.projectTokensPath ? existsSync(this.config.projectTokensPath) : false
    };
  }
