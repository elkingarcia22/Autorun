...
export function renderMask(options: MaskOptions): string {
	...
	const classes = ['autorun-mask-overlay', className].filter(Boolean).join(' ');
...
export function createMask(options: MaskOptions): {
	...
	if (closeOnOverlayClick) {
		overlay.addEventListener('click', (e) => {
			...
			if (
				target.classList.contains('autorun-mask-layer') ||
				target.classList.contains('autorun-mask-overlay')
			) {
				closeMask();
			}
		});
