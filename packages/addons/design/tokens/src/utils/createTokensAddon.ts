  ...
  function flatten(obj: Record<string, any>, prefix = ''): void {
    for (const [key, value] of Object.entries(obj)) {
      ...
      if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
        ...
        // Asegurar que tenga el prefijo --autoframe-
        const cssVar = fullKey.startsWith('--autoframe-') 
          ? fullKey 
          : `--autoframe-${fullKey}`;
 ...
 */
class CustomTokensAddon extends AutoframeTokensAddon {
  ...
  async initialize(context: any): Promise<void> {
    ...
    try {
      ...
      if (typeof document !== 'undefined') {
        ...
        styleElement.id = `autoframe-tokens-${this.name.replace(/[^a-z0-9]/gi, '-')}`;
...
export async function applyTokensFromStorybook(
  ...
  if (replaceExisting && typeof document !== 'undefined') {
    const existingTokens = document.querySelectorAll('style[id^="autoframe-tokens"], link[href*="tokens.css"]');
    existingTokens.forEach(el => {
      // No remover si es el link estático principal (por seguridad)
      if (el.tagName === 'LINK' && (el as HTMLLinkElement).id !== 'autoframe-tokens-static') {
        el.remove();
      }
    });
  }
  ...
  if (typeof window !== 'undefined' && (window as any).Autoframe?.Tokens) {
    (window as any).Autoframe.Tokens.currentAddon = addon;
  }
}
