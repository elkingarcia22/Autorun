/**
 * TokensAddon - Add-on de tokens Autoframe genéricos
 * 
 * Este add-on carga los tokens Autoframe genéricos como CSS variables.
 * Mantiene compatibilidad con el sistema actual.
 */

import type { TokensAddon, AppContext } from './types/TokensAddon';

export class AutoframeTokensAddon implements TokensAddon {
  name = '@autoframe/tokens';
  version = '1.0.0';
  ...
  private readonly requiredTokens: string[] = [
    // Button tokens
    '--autoframe-button-primary-bg-default',
    '--autoframe-button-primary-hover',
    '--autoframe-button-primary-pressed',
    '--autoframe-btn-primary-fg',
    '--autoframe-btn-secondary-bg-default',
    '--autoframe-btn-secondary-fg-default',
    '--autoframe-btn-secondary-border',
    '--autoframe-btn-tertiary-fg-default',
    ...
    '--autoframe-bg-3',
    '--autoframe-bg-active',
    '--autoframe-bg-disabled-button',
    
    // Foreground tokens
    '--autoframe-fg-1-high',
    '--autoframe-fg-1-medium',
    '--autoframe-fg-1-low',
    '--autoframe-fg-on-disabled-button',
    
    // Border tokens
    '--autoframe-border-1',
    '--autoframe-border-2',
    '--autoframe-border-disabled-button',
    
    // Accent tokens
    '--autoframe-accent-brand-static-inverted',
    '--autoframe-accent-success',
    '--autoframe-accent-error',
  ...
  private hasStaticTokensLoaded(): boolean {
    ...
    for (const style of styles) {
      if (style.textContent && style.textContent.includes('--autoframe-')) {
        return true;
      }
    }
  ...
  private async loadFallbackTokens(): Promise<void> {
    ...
    fallbackLink.id = 'autoframe-tokens-fallback';
  ...
  async initialize(context: AppContext): Promise<void> {
    ...
    try {
      ...
      if (this.hasStaticTokensLoaded()) {
        console.log('✅ Tokens Autoframe ya cargados estáticamente (modo compatibilidad)');
      ...
      try {
        ...
        if (!validation.isValid) {
          ...
          console.log('✅ TokensAddon Autoframe inicializado y cargado - Todos los tokens válidos');
        }
      } catch (loadError) {
  ...
  private async loadTokensCSS(): Promise<void> {
    ...
    try {
      ...
      this.styleElement.id = 'autoframe-tokens-addon';
      ...
      this.linkElement.id = 'autoframe-tokens-addon-link';
  ...
  getTokenList(): string[] {
    ...
    if (this.tokensCSS) {
      const matches = this.tokensCSS.matchAll(/--autoframe-[^:;]+/g);
