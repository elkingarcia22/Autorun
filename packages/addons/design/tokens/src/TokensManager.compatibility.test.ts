...
describe('TokensManager - Compatibilidad', () => {
  ...
  test('debe permitir usar tokens en componentes sin cambios', () => {
    ...
    testElement.style.background = 'var(--autoframe-button-primary-bg-default)';
    testElement.style.color = 'var(--autoframe-fg-1-high)';
  ...
  test('debe funcionar sin inicializar (modo pasivo)', () => {
    // Inyectar tokens
    const style = document.createElement('style');
    style.textContent = `:root { --autoframe-bg-1: var(--autoframe-bg-1); }`;
    document.head.appendChild(style);
    ...
    testEl.style.background = 'var(--autoframe-bg-1)';
...
describe('TokensManager - Integración con Componentes', () => {
  ...
  test('componentes deben poder usar tokens después de inicializar', async () => {
    ...
    style.textContent = `
      :root {
        --autoframe-button-primary-bg-default: var(--autoframe-accent-brand-static-inverted);
        --autoframe-btn-primary-fg: var(--autoframe-fg-1-high);
        --autoframe-bg-1: var(--autoframe-bg-1);
      }
    `;
    document.head.appendChild(style);
    ...
    button.className = 'autoframe-button';
    button.style.background = 'var(--autoframe-button-primary-bg-default)';
    button.style.color = 'var(--autoframe-btn-primary-fg)';
  ...
  test('debe validar tokens requeridos por componentes', async () => {
    ...
    style.textContent = `
      :root {
        ...
        --autoframe-fg-1-high: var(--autoframe-fg-1-high);
        --autoframe-fg-1-medium: var(--autoframe-fg-1-medium);
        --autoframe-border-1: var(--autoframe-border-1);
        --autoframe-accent-brand-static-inverted: var(--autoframe-accent-brand-static-inverted);
        --autoframe-spacing-2: var(--autoframe-spacing-2);
        --autoframe-spacing-3: var(--autoframe-spacing-3);
      }
    `;
    document.head.appendChild(style);
