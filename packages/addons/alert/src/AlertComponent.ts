/**
 * Web Component: autorun-alert
 * Componente de alert AUTORUN como Web Component nativo
 */

import type { AlertOptions, AlertType } from './types/AlertOptions';
import { renderAlert } from './AlertProvider';

export class AUTORUNAlert extends HTMLElement {
	...
	private updateOptions() {
		this.options = {
			...
			duration: this.getAttribute('duration')
				? parseInt(this.getAttribute('duration') || '0', 10)
				: 0,
	...
	private attachEventListeners() {
		const closeButton = this.querySelector('.autorun-alert__close');
	...
	public close() {
		...
		this.classList.add('autorun-alert--closing');
	...
	public updateMessage(newMessage: string) {
		...
		const textElement = this.querySelector('.autorun-alert__text');
		if (textElement) {
