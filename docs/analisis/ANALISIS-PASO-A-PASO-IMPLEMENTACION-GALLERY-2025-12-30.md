...
### **Lo que debe hacer automáticamente:**
...
**Proceso:**
1. `handleUserMessage("implementar gallery")` se ejecuta automáticamente
2. Detecta "gallery" en el mensaje
3. Mapea "gallery" → "layout-gallery" (storybookId)
4. Retorna información del componente
...
- ✅ StorybookId: "layout-gallery"
- ✅ API esperada: "window.UBITS.Gallery.create" (desde componentAvailabilityDetector)
...
**Código Extraído (ejemplo desde Gallery.stories.ts):**
```javascript
...
## 🔍 PASO 7: Comparación: Código Hardcodeado vs Automático

### **Código Hardcodeado Actual (línea ~1459):**
```javascript
// ❌ FUNCIÓN HARDCODEADA
function createGallery() {
  ...
  // Verificar que window.UBITS.Gallery esté disponible
  if (!window.UBITS || !window.UBITS.Gallery || !window.UBITS.Gallery.create) {
    console.warn('⚠️ [Gallery] window.UBITS.Gallery no está disponible, esperando...');
    setTimeout(createGallery, 500);
    return;
  }
  ...
  try {
    ...
    console.log('✅ [Gallery] Gallery creada exitosamente usando window.UBITS.Gallery.create()');
...
function createGallery() {
  ...
  try {
    // ✅ Código extraído de Storybook (NO hardcodeado)
    const galleryElement = window.UBITS.Gallery.create({
...
**Diferencias Clave:**
- ✅ **Hardcodeado:** Items, props, callbacks hardcodeados en el template
- ✅ **Automático:** Items, props, callbacks desde Storybook
- ✅ **Hardcodeado:** Función definida manualmente en el template
- ✅ **Automático:** Función generada automáticamente desde snippet
- ✅ **Hardcodeado:** Llamada `setTimeout()` hardcodeada
- ✅ **Automático:** Llamada `setTimeout()` generada automáticamente
...
3. **✅ NO hay función hardcodeada `createGallery()`:**
   - ❌ NO debe existir función `createGallery()` hardcodeada en el template (línea ~1459)
   - ✅ Solo debe existir la función generada automáticamente

4. **✅ NO hay llamada hardcodeada:**
   - ❌ NO debe existir `setTimeout(createGallery, 500)` hardcodeado (línea ~1538)
...
3. **✅ NO hay hardcode:**
   - NO hay función hardcodeada (línea ~1459 eliminada)
   - NO hay llamada hardcodeada (línea ~1538 eliminada)
...
**Ahora voy a ejecutar la implementación y analizar el resultado paso a paso.**
