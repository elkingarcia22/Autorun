...
async function testPreWriteValidator() {
	...
	const testFilePath = path.join(
		PROJECT_ROOT,
		'prototypes',
		'canvas-administrador-encuestas-2025-12-10.html',
	);
	...
	try {
		...
		testContent =
			'<ubits-tabs><ubits-tab-list><ubits-tab>Test</ubits-tab></ubits-tab-list></ubits-tabs>';
	}

	// Extraer una porción del contenido que contiene componentes
	const contentSnippet =
		testContent.substring(
	...
	const result1 = await PreWriteValidator.validateBeforeWrite(testFilePath, contentSnippet, {
		componentName: 'Tabs',
