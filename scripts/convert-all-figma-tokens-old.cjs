...
function resolveReferenceImproved(
	ref,
	tokenMap,
	pColors,
	allTokensMap = {},
	...
	if (ref.startsWith('{') && ref.endsWith('}')) {
		...
		if (tokenMap[key]) {
			const resolved = resolveReferenceImproved(
				tokenMap[key].value,
				tokenMap,
				pColors,
				allTokensMap,
...
function processFile(
	...
	try {
		...
		for (const [key, token] of Object.entries(tokenMap)) {
			...
			const resolvedValue = resolveReferenceImproved(
				token.value,
				tokenMap,
				pColors,
				allTokensMap,
			...
			if (resolvedValue) {
				...
				if (
					...
					(cleanKey.startsWith('color.light.') || cleanKey.startsWith('color.dark.'))
				) {
...
function processAllFiles() {
	...
	if (fs.existsSync(modifiersDir)) {
		const files = fs
			.readdirSync(modifiersDir)
			.filter((f) => f.endsWith('.json') && f !== 'Normal.json');
		files.forEach((file) => {
