...
function convertToVercelDirect(content) {
	...
	converted = converted.replace(/href="\/vercel-proxy\/([^"]+)"/g, (match, path) => {
		return `href="${vercelUrl(path)}"`;
	});

	// Convertir src="/vercel-proxy/..." a URLs directas
	converted = converted.replace(/src="\/vercel-proxy\/([^"]+)"/g, (match, path) => {
	...
	converted = converted.replace(/src="\/vendor\/ubits\/packages\/([^"]+)"/g, (match, path) => {
...
async function processFile(filePath) {
	try {
		...
		// Contar conversiones
		const hrefMatches = (converted.match(/href="https:\/\/ubits-storybook10\.vercel\.app/g) || [])
			.length;
		const srcMatches = (converted.match(/src="https:\/\/ubits-storybook10\.vercel\.app/g) || [])
			.length;
...
async function main() {
	...
	if (filePath) {
		...
		console.log(
			'💡 Para convertir un archivo específico: node scripts/convert-to-vercel-direct.js <ruta-al-archivo>',
		);
		console.log('');
