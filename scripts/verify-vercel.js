...
function fetchFromVercel(path, followRedirects = true, cookieHeader = '') {
  return new Promise((resolve, reject) => {
    ...
    const req = https.request(options, (res) => {
      ...
      if (followRedirects && (res.statusCode === 307 || res.statusCode === 302 || res.statusCode === 301)) {
...
async function runTests() {
  ...
  try {
    ...
    if (result.statusCode === 200) {
      ...
      try {
        ...
        console.log(`   📊 Storybook tiene ${json.stories ? Object.keys(json.stories).length : 0} stories`);
      } catch (e) {
  ...
  console.log('');
  console.log('💡 El sistema intentará usar Vercel primero, y si falla usará vendor/ubits/ como fallback.');
}
