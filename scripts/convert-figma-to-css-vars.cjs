...
function resolveReference(ref, tokenMap, pColors, visited = new Set(), depth = 0, originalKey = null) {
  if (depth > 50) {
    return null;
  }
  ...
  if (ref.startsWith('{') && ref.endsWith('}')) {
    ...
    if (tokenValue && tokenValue !== ref) {
      const resolved = resolveReference(tokenValue, tokenMap, pColors, visited, depth + 1, originalKey || key);
      visited.delete(key);
      return resolved;
    }
...
function extractAndResolveFigmaTokens() {
  ...
  if (fs.existsSync(btnToneDir)) {
    ...
    files.forEach(file => {
      ...
      totalResolved += processTokenFile(filePath, pColors, allTokens, 'light', `btn-tone.${baseName}`);
    });
  }
  ...
  if (fs.existsSync(buttonToneDir)) {
    ...
    files.forEach(file => {
      ...
      totalResolved += processTokenFile(filePath, pColors, allTokens, 'light', `button-tone.${baseName}`);
    });
  }
...
function generateJSON(tokens) {
  const output = {
    ...
    metadata: {
      description: 'Tokens de Figma convertidos preservando estructura, nomenclatura y agrupaciones originales',
